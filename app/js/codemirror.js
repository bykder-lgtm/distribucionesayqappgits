(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var Transaction = /** @class */ (function () {
        ...
        Transaction.prototype.replaceSelection = function (text) {
            ...
            return this.forEachRange(function (range) {
                var change = new Change(range.from, range.to, content);
                _this.change(change);
    ...
    var DOMPos = /** @class */ (function () {
        ...
        DOMPos.before = function (dom, precise) { return new DOMPos(dom.parentNode, domIndex(dom), precise); };
        DOMPos.after = function (dom, precise) { return new DOMPos(dom.parentNode, domIndex(dom) + 1, precise); };
        return DOMPos;
    }());
    var none$1 = [];
    ...
    var TextView = /** @class */ (function (_super) {
        ...
        TextView.prototype.domAtPos = function (pos) { return new DOMPos(this.textDOM, pos); };
    ...
    var CompositionView = /** @class */ (function (_super) {
        ...
        CompositionView.prototype.domAtPos = function (pos) { return new DOMPos(this.widget.value.text, pos); };
    ...
    var none$4 = [];
    function maybeNone(array) { return array.length ? array : none$4; }
    ...
    var RangeSet = /** @class */ (function () {
        ...
        RangeSet.prototype.updateInner = function (added, filter, filterFrom, filterTo, offset, length) {
            ...
            for (var i = 0; i < this.children.length; i++) {
                ...
                if (localRanges || filter && filterFrom <= endPos && filterTo >= pos)
                    newChild = newChild.updateInner(localRanges || none$4, filter, filterFrom, filterTo, pos, newChild.length);
                if (newChild != child)
        ...
        RangeSet.empty = new RangeSet(0, 0, none$4, none$4);
        return RangeSet;
    }());
    ...
    function collapseSet(children, local, add, start, offset, length) {
        ...
        if (mustSort)
            local.sort(byPos);
        return new RangeSet(length, local.length, local, none$4);
    }
        ...
        for (var i = start; i < ranges.length;) {
            ...
            if (add.length) {
                if (add.length == ranges.length)
                    children.push(new RangeSet(endPos - pos, add.length, add.map(function (r) { return r.move(-pos); }), none$4));
                else
    ...
    function rebalanceChildren(local, children, childSize) {
        var _loop_1 = function (i, off) {
            ...
            if (child.size == 0 && (i > 0 || children.length == 1)) {
                ...
                children.splice(i, 2, new RangeSet(child.length + next.length, child.size + next.size, child.local.concat(next.local.map(function (d) { return d.move(child.length); })), none$4));
            }
            else {
                ...
                if (joinTo > i + 1) {
                    var joined = new RangeSet(length_1, size, none$4, children.slice(i, joinTo));
                    var joinedLocals = [];
    ...
    var Decoration = /** @class */ (function (_super) {
        ...
        Decoration.prototype.mapSimple = function (mapping, from, to) {
            var newFrom = mapping.mapPos(from, this.startSide, MapMode.TrackDel);
            if (from == to && this.startSide == this.endSide)
                return newFrom < 0 ? null : new Range(newFrom, newFrom, this);
            var newTo = mapping.mapPos(to, this.endSide, MapMode.TrackDel);
            if (newFrom < 0) {
                if (newTo < 0)
                    return null;
    ...
    var PointDecoration = /** @class */ (function (_super) {
        ...
        PointDecoration.prototype.map = function (mapping, from, to) {
            ...
            if (this.block) {
                var type = this.type;
                var newFrom = type == 2 /* WidgetAfter */ ? mapping.mapPos(from, 1, MapMode.TrackAfter) : mapping.mapPos(from, -1, MapMode.TrackBefore);
                var newTo = type == 3 /* WidgetRange */ ? mapping.mapPos(to, 1, MapMode.TrackAfter) : newFrom;
    ...
    var ContentBuilder = /** @class */ (function () {
        ...
        ContentBuilder.prototype.posCovered = function () {
            ...
            return !last.breakAfter && !(last instanceof BlockWidgetView && last.type == 1 /* WidgetBefore */);
        };
    ...
    var clickAddsSelectionRange = ViewExtension.defineBehavior();
    var dragMovesSelection = ViewExtension.defineBehavior();
    ...
    var ViewUpdate = /** @class */ (function () {
        ...
        // @internal
        metadata) {
            if (transactions === void 0) { transactions = none$5; }
            if (metadata === void 0) { metadata = none$5; }
            this.view = view;
            this.transactions = transactions;
            this.metadata = metadata;
            this.state = transactions.length ? transactions[transactions.length - 1].apply() : view.state;
            this.changes = transactions.reduce(function (chs, tr) { return chs.appendSet(tr.changes); }, ChangeSet.empty);
    ...
    var DocView = /** @class */ (function (_super) {
        ...
        function DocView(view, onDOMChange) {
            var _this = _super.call(this) || this;
            _this.view = view;
            _this.viewports = none$6;
            _this.compositionDeco = Decoration.none;
            _this.gapDeco = Decoration.none;
        ...
        DocView.prototype.init = function (state) {
            ...
            this.heightMap = HeightMap.empty().applyChanges(none$6, Text.empty, this.heightOracle.setDoc(state.doc), changedRanges);
            this.children = [new LineView];
            this.children[0].setParent(this);
            this.viewports = this.decorations = none$6;
            this.minWidth = 0;
        ...
        DocView.prototype.update = function (update, scrollIntoView) {
            ...
            var changedRanges = update ? update.changes.changedRanges() : none$6;
            ...
            this.heightMap = this.heightMap.applyChanges(none$6, prevDoc, this.heightOracle.setDoc(state.doc), changedRanges);
            ...
            if (browser.chrome && !this.compositionDeco.size && update && update.changes.changes.some(function (ch) { return ch.text.length > 1; }))
        ...
        DocView.prototype.updateChildren = function (changes, viewports, oldLength) {
            ...
            var gapChanges = findChangedRanges(this.gapDeco, gapDeco, changes, oldLength);
            ...
            for (var i = changes.length - 1;; i--) {
                ...
                var _a = ContentBuilder.build(this.state.doc, fromB, toB, allDeco), content = _a.content, breakAtStart = _a.breakAtStart;
        ...
        DocView.prototype.computeUpdate = function (state, update, initializing, contentChanges, viewportBias, scrollIntoView) {
            for (var i = 0;; i++) {
                ...
                var decorations = this.view.getEffect(ViewField.decorationEffect).concat(this.compositionDeco);
                ...
                var _a = decoChanges(update ? contentChanges : none$6, decorations, this.decorations, prevState.doc.length), content = _a.content, height = _a.height;
                ...
                // for inserted ranges.
                var heightChanges = extendWithRanges(none$6, height);
                if (update)
            ...
            this.view.withUpdating(function () {
                ...
                for (var i = 0;; i++) {
                    ...
                    if (covered && !_this.heightOracle.heightChanged)
                        break;
                    if (!update)
                        update = new ViewUpdate(_this.view);
                    if (i > 10)
                        throw new Error("Layout failed to converge");
                    var contentChanges = covered ? none$6 : _this.computeUpdate(_this.state, null, false, none$6, scrollBias, -1);
    ...
    function decoChanges(diff, decorations, oldDecorations, oldLength) {
        ...
        for (var i = decorations.length - 1; i >= 0; i--) {
            ...
            if (deco.size == 0 && oldDeco.size == 0)
                continue;
            var newRanges = findChangedRanges(oldDeco, deco, diff, oldLength);
    ...
    function computeCompositionDeco(view, changes) {
        ...
        var newFrom = ChangedRange.mapPos(from, 1, changes), newTo = Math.max(newFrom, ChangedRange.mapPos(to, -1, changes));
    ...
    var CompositionWidget = /** @class */ (function (_super) {
        ...
        Object.defineProperty(CompositionWidget.prototype, "customView", {
            get: function () { return CompositionView; },
            enumerable: true,
            configurable: true
        });
    ...
    var compositionEndSlot = Slot.define();
        ...
        if (change) {
            ...
            if (view.inputState.lastSelectionTime > Date.now() - 50) {
                if (view.inputState.lastSelectionOrigin == "keyboard")
                    ...
                    tr.addMeta(Transaction.userEvent(view.inputState.lastSelectionOrigin));
            }
            view.dispatch(tr);
            return true;
        }
    ...
    var EditorView = /** @class */ (function () {
        ...
        EditorView.prototype.setState = function (state, extensions) {
            ...
            this.withUpdating(function () {
                ...
                StyleModule.mount(_this.root, styles);
                for (var _i = 0, _a = _this.behavior.get(styleModule); _i < _a.length; _i++) {
                    var s = _a[_i];
                    StyleModule.mount(_this.root, s);
                }
        ...
        EditorView.prototype.update = function (transactions, metadata) {
            ...
            this.withUpdating(function () {
                var update = transactions.length > 0 || metadata.length > 0 ? new ViewUpdate(_this, transactions, metadata) : null;
    ...
    var GutterMarker = /** @class */ (function (_super) {
        ...
        GutterMarker.create = function (pos, value) {
            return new Range(pos, pos, new this(value));
        };
    ...
    var defaults = {
        ...
        elementClass: "",
        initialMarkers: function () { return RangeSet.empty; },
    ...
    function gutter(config) {
        var conf = fillConfig(config, defaults);
        return ViewExtension.all(viewPlugin(function (view) { return new GutterView(view, conf); }), styleModule(styles$1));
    }
    var GutterView = /** @class */ (function () {
        function GutterView(view, config) {
            ...
            this.spacer = null;
            this.dom = document.createElement("div");
            this.dom.className = "codemirror-gutter " + config.class + " " + styles$1.gutter;
        ...
        GutterElement.prototype.update = function (height, above, markers, eltClass) {
            ...
            if (this.markers != markers) {
                ...
                for (var ch = void 0; ch = this.dom.lastChild;)
                    ch.remove();
                var cls = "codemirror-gutter-element " + styles$1.gutterElement;
                if (eltClass)
                    cls += " " + eltClass;
    ...
    var lineNumberMarkers = Slot.define();
    var lineNumbers = ViewExtension.unique(function (configs) {
        var config = combineConfig(configs, {
            fixed: true,
            formatNumber: String
        });
        ...
        return gutter({
            class: "codemirror-line-numbers " + styles$1.lineNumberGutter,
            fixed: config.fixed,
            elementClass: styles$1.lineNumberGutterElement,
            updateMarkers: function (markers, update) {
                var slot = update.getMeta(lineNumberMarkers);
                if (slot)
    ...
    function moveSelection(view, dir, granularity) {
        ...
        if (transaction.selection.eq(view.state.selection))
            return false;
        if (granularity == "line")
            transaction.addMeta(Transaction.preserveGoalColumn(true));
    ...
    function doMatchBrackets(state, config) {
        ...
        for (var _i = 0, _a = state.selection.ranges; _i < _a.length; _i++) {
            ...
            if (match.to)
                decorations.push(Decoration.mark(match.to, match.to + 1, { class: style }));
        }
    ...
    var defaultStyles = new StyleModule({
        matching: { color: "#0b0" },
            ...
            for (var pos = from, cursor = doc.iterRange(from, to), m = void 0; !cursor.next().done;) {
                if (!cursor.lineBreak) {
                    while (m = SPECIALS.exec(cursor.value)) {
                        ...
                        target.push(Decoration.replace(pos + m.index, pos + m.index + m[0].length, { widget: widget }));
                    }
                }
    ...
    var StateCache = /** @class */ (function () {
        ...
        StateCache.prototype.apply = function (transaction) {
            ...
            for (var _i = 0, _a = this.states; _i < _a.length; _i++) {
                var cached = _a[_i];
                var mapped = transaction.changes.mapPos(cached.pos, -1, MapMode.TrackDel);
                if (mapped > 0)
    ...
    */
    ...
    function css(config, parserConfig) {
    ...
    function xml(editorConf, config_) {
    ...
    exports.legacyModes = legacyModes;
