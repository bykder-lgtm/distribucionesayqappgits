...
if ($res) {
    while ($row = mysqli_fetch_assoc($res)) {
        $docs = [];
        if (!empty($row['url_documentacion_rut_aliado'])) $docs[] = ['nombre' => 'RUT', 'url' => $row['url_documentacion_rut_aliado']];
        if (!empty($row['url_documentacion_camaracomercio_aliado'])) $docs[] = ['nombre' => 'Cámara de Comercio', 'url' => $row['url_documentacion_camaracomercio_aliado']];
        if (!empty($row['url_documentacion_cedula_aliado'])) $docs[] = ['nombre' => 'Cédula', 'url' => $row['url_documentacion_cedula_aliado']];
        $aliados[] = ['cod_administrador' => $row['cod_administrador'], 'nombres_apellidos_tercero' => $row['nombres_apellidos_tercero'] ?: ($row['nombres'] . ' ' . $row['apellidos']), 'fecha' => date('d/m/Y', strtotime($row['fecha'])), 'documentos' => $docs];
    }
}
