...
<a class="btn btn-primary" href="../admin/reporte_venta_factura_electronica_fechas.php"><h6>Reporte Ventas Por Factura Electronica Rango de Fechas</h6></a>
<a class="btn btn-secondary" href="../admin/reporte_venta_factura_electronica_fechas_marcada_descagado_archivo_plano.php?cod_administrador=0&cod_tercero=0&cod_dependencia=0&cod_tipo_pago=0&nombre_tipo_compra=0&fecha_ymd_venta_producto_ini=2010-01-01&fecha_ymd_venta_producto_fin=2050-01-01&cod_tipo_forma_pago=0&nombre_tipo_factura=ELECTRONICA"><h6>Reporte Ventas Por Factura Electronica Marcadas como Descargadas Por Archivo palno</h6></a>
</div>
<div class="row-fluid">
...
<table class="table table-striped">
    <tr>
        ...
        <th style="text-align:center;">TOTAL BASE IVA</th>
        <th style="text-align:center;">TOTAL IVA</th>
        <th style="text-align:center;">NUMEROS DE FACTURAS</th>
        <th style="text-align:center;">ULTIMA FACTURA EN <?php echo $nombre_operador_factura_electronica ?></th>
        <th style="text-align:center;">GENERAR</th>
    </tr>
    <tr>
        <th style="text-align:center;"><input type="checkbox" id="seleccionar-todos" name="cod_info_factura_venta[]"></th>
        <th style="text-align:center;" id="total_venta">0</th>
        <th style="text-align:center;" id="total_base_iva">0</th>
        <th style="text-align:center;" id="total_iva">0</th>
        <th style="text-align:center;" id="total_reg">0</th>
        <th style="text-align:center;"><input class="input-block-level" name="cod_ultima_factura" type="number" value="0" min="0" style="width: 140px;" required/></th>
        <td style="text-align:center;"><input type="submit" name="REGENERAR_CODIGOS_DE_FACTURA_SELECIONADOS" value="Regenerar Codigos de Facturas Para los Selecionados" title="Regenerar Facturas" /></td>
    </tr>
</table>
...
<thead>
    <tr>
        ...
        <th style="text-align:center;"><a href="#">Dian</a></th>
        <th style="text-align:center;"><a href="#">Dataico</a></th>
        <th style="text-align:center;"><a href="#">Enviar</a></th>
        <?php } ?>
        ...
        <?php if ($cod_estado_enviar_factura_venta_electronica_dian_api == '1') { ?>
        <th style="text-align:center;"><a href="#">Nota Credito (Devolucion)</a></th>
        <?php } ?>
        <th style="text-align:center"><a href="#">Check</a></th>
...
while ($datos_total_tipo_factura = mysqli_fetch_assoc($consulta_total_tipo_factura)) {
    ...
    $nombre_tercero                = trim($datos_tercero['nombre1_tercero'].' '.$datos_tercero['nombre2_tercero'].' '.$datos_tercero['apellido1_tercero'].' '.$datos_tercero['apellido2_tercero'].' | '.$cod_tercero_db);
...
?>
    <tr>
        ...
        <?php if ($cod_estado_enviar_factura_venta_electronica_dian_api == '1') { ?>
        <td style="text-align:center;" id="estadodian<?php echo $cod_info_factura_venta ?>"><img src="<?php echo $imagen_estado_factura_dian?>" class="img-polaroid" alt=""></td>
        <td style="text-align:center;" id="estadodataico<?php echo $cod_info_factura_venta ?>"><img src="<?php echo $imagen_estado_factura_dataico?>" class="img-polaroid" alt=""></td>
        ...
        <td style="text-align:left"><?php echo $nombre_tercero?> <a href="../admin/edit_tercero.php?cod_tercero=<?php echo $cod_tercero_db?>&pagina=<?php echo $pagina?>" target='_blank'>(<?php echo $identificacion_tercero?>)</a></td>
        ...
        <?php if ($cod_estado_enviar_factura_venta_electronica_dian_api == '1') { 
            if ($nombre_estado_factura_dataico_dian == 'ANULADA') { ?>
                <td style="text-align:center;" id="apidian_nota_credito<?php echo $cod_info_factura_venta ?>" data="<?php echo $cod_info_factura_venta ?>"><?php echo $nombre_estado_factura_dataico_dian?></td>
            <?php } else { ?>
                <td style="text-align:center;" id="apidian_nota_credito<?php echo $cod_info_factura_venta ?>" data="<?php echo $cod_info_factura_venta ?>"><a class="EnviarFacturaNotaCreditoDevolucionDianDataico" style="cursor:pointer;"><img src="../imagenes/btn_nota_credito_devolucion_grand.png" class="img-polaroid" alt=""></a></td>
            <?php } ?>
        <?php } ?>
        <td style="text-align:center;"><div id="listado"><input name="cod_info_factura_venta[]" class="cod_estado_check_factura_electronica" id="cod_estado_check_factura_electronica__<?php echo $cod_info_factura_venta ?>" type="checkbox" value="<?php echo $cod_info_factura_venta ?>" <?php if($cod_estado_check_factura_electronica=='1'){ echo "checked"; } ?>></div></td>
...
<table class="table table-striped">
<tr>
<td style="text-align:center;"><input type="submit" name="GENERAR_ARCHIVO_PLANO_DE_FACTURA_ELECTRONICA" value="GENERAR ARCHIVO PLANO DE FACTURA ELECTRONICA" title="GENERAR ARCHIVO PLANO DE FACTURA ELECTRONICA"/></td>
</tr>
...
$(document).ready(function() {
    ...
    $('.EnviarFacturaNotaCreditoDevolucionDianDataico').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "../admin/enviar_datos_devolucion_nota_credito_dian_json.php",
            data: datos_url_ajax,
            //dataType: 'json',
            beforeSend: function(objeto){
...
<script>
  $(function(){
    $('#seleccionar-todos').change(function() {
      ...
      $('#listado_superior > input[type=checkbox]').prop('checked', $(this).is(':checked'));
        ...
        var valor = cod_estado_check_factura_electronica;
...
<script>
$('input:checkbox[name="cod_estado_check_factura_electronica"]').change(function(){ 
    var valor = $(this).val();
    ...
    if (campo == 'cod_estado_check_factura_electronica') { if( $('#cod_estado_check_factura_electronica__'+ids).prop('checked') ) { valor = 1; $('#cod_estado_check_factura_electronica__'+ids).prop('checked',true); } else { valor = 0; $('#cod_estado_check_factura_electronica__'+ids).prop('checked',false); } }
});
</script>
