...
$sql_consulta = "SELECT tbl15_grupo_area.nombre_grupo_area, tbl15_grupo_area_cargo.nombre_grupo_area_cargo, tbl15_grupo_area_cargo.cod_grupo_area_cargo
FROM tbl15_grupo_area RIGHT JOIN (tbl15_historia_clinica LEFT JOIN tbl15_grupo_area_cargo ON tbl15_historia_clinica.cod_grupo_area_cargo = tbl15_grupo_area_cargo.cod_grupo_area_cargo) 
ON tbl15_grupo_area.cod_grupo_area = tbl15_grupo_area_cargo.cod_grupo_area GROUP BY tbl15_historia_clinica.cod_grupo_area_cargo";
...
$sql_cargo = "SELECT tbl15_grupo_area.nombre_grupo_area, tbl15_grupo_area_cargo.nombre_grupo_area_cargo, tbl15_grupo_area_cargo.cod_grupo_area_cargo
...
WHERE (((tbl15_grupo_area_cargo.cod_grupo_area_cargo)='$cod_grupo_area_cargo'))";
...
tbl15_historia_clinica.cod_administrador, tbl15_historia_clinica.cod_cliente, tbl15_historia_clinica.cod_factura, tbl15_historia_clinica.costo_motivo_consulta, tbl15_historia_clinica.cod_grupo_area_cargo,
...
WHERE ((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) ORDER BY tbl15_historia_clinica.fecha_time DESC";
$resultado_hist_clinica = mysqli_query($conectar, $sql_hist_clinica) or die(mysqli_error($conectar));
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_citas = "SELECT COUNT(cod_historia_clinica) AS conteo_historia_clinica, tbl15_historia_clinica.cod_grupo_area_cargo 
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo')";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_atendido = "SELECT COUNT(cod_historia_clinica) AS conteo_historia_clinica, tbl15_historia_clinica.cod_grupo_area_cargo 
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1)";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_no_atendido = "SELECT COUNT(cod_historia_clinica) AS conteo_historia_clinica, tbl15_historia_clinica.cod_grupo_area_cargo 
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo') AND ((tbl15_historia_clinica.cod_estado_facturacion)=0)";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_atendido_hombre = "SELECT Count(tbl15_cliente.nombre_sexo) AS conteo_hombre, tbl15_historia_clinica.cod_grupo_area_cargo, tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
GROUP BY tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
HAVING (((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) AND ((tbl15_cliente.nombre_sexo)='M'))";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_atendido_mujer = "SELECT Count(tbl15_cliente.nombre_sexo) AS conteo_mujer, tbl15_historia_clinica.cod_grupo_area_cargo, tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
GROUP BY tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
HAVING (((tbl15_historia_clinica.cod_grupo_area_cargo)='$cod_grupo_area_cargo') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) AND ((tbl15_cliente.nombre_sexo)='F'))";
