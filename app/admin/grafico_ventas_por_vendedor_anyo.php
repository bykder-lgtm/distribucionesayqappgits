...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto, anyo FROM tbl15_venta_producto WHERE vendedor = '$vendedor' GROUP BY anyo ORDER BY anyo ASC";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto FROM tbl15_venta_producto WHERE vendedor = '$vendedor' GROUP BY anyo ORDER BY anyo ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysql_fetch_array($result)) {
