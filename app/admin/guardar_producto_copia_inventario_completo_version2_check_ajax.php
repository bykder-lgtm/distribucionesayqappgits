...
// ------------------------------------------------------------------------------------------------- //
if (($tipo_ajax=='tbl15_producto_copia_inventario') && ($campo=='cod_estado_check')) {
	$cod_producto_copia_inventario          = intval($_POST['id']);
	$und_producto_nuevo                     = addslashes($_POST['valor']);

	$sql_productos_cargados = "SELECT cod_estado, cod_estado_check FROM tbl15_producto_copia_inventario WHERE (cod_producto_copia_inventario = '$cod_producto_copia_inventario')";
	...
	$cod_estado                             = $info_productos_cargados['cod_estado'];
	$cod_estado_check                       = $info_productos_cargados['cod_estado_check'];

	if ($cod_estado_check == '1') { $cod_estado_check = 0; $cod_estado = 0; } else { $cod_estado_check = 1; $cod_estado = 1; }

	$data_sql = ("UPDATE tbl15_producto_copia_inventario SET cod_estado = '$cod_estado', cod_estado_check = '$cod_estado_check', fecha_actualizacion = '$fecha_actualizacion' 
    ...
    $sql_estado = "SELECT * FROM tbl15_estado WHERE cod_estado = '$cod_estado_check'";
