...
if (isset($_REQUEST['cod_info_factura_venta'])) {
	...
	//-------------------------------------------------------------------------------------------------------------------//
	//-------------------------------------------------------------------------------------------------------------------//
	$mostrar_datos_sql = "SELECT fecha_anyo, cod_factura, cod_tipo_forma_pago, descuento_ptj, cod_tercero, cod_tipo_pago, observacion, cod_resolucion_facturacion, observacion_tercero, reteica_ptj, retefuente_ptj
	...
	$observacion_tercero                     = $matriz_consulta['observacion_tercero'];
	$cod_resolucion_facturacion              = $matriz_consulta['cod_resolucion_facturacion'];
	$reteica_ptj                             = $matriz_consulta['reteica_ptj'];
	$retefuente_ptj                          = $matriz_consulta['retefuente_ptj'];
	$fecha_anyo                              = date("d/m/Y", strtotime($matriz_consulta['fecha_anyo']));

	//if ($retefuente_ptj == '0.00') { $retefuente_ptj = ''; } else { $retefuente_ptj = $retefuente_ptj; }
	//if ($reteica_ptj == '0.00') { $reteica_ptj = ''; } else { $reteica_ptj = $reteica_ptj; }
	//-------------------------------------------------------------------------------------------------------------------//
	//-------------------------------------------------------------------------------------------------------------------//
	...
	$nombres_apellidos_completos             = trim($nombre1_tercero.' '.$nombre2_tercero.' '.$apellido1_tercero.' '.$apellido2_tercero);
	
	if ($apellidos_tercero == '') { $apellidos_tercero = $nombres_tercero; } else { $apellidos_tercero = $apellidos_tercero; }
	//-------------------------------------------------------------------------------------------------------------------//
	//-------------------------------------------------------------------------------------------------------------------//
	...
	if ($observacion_tercero == '') { $observacion_tercero = "SIN OBSERVACIONES"; } else { $observacion_tercero = $observacion_tercero; }
	if ($observacion == '') { $observacion = "SIN OBSERVACIONES"; } else { $observacion = $observacion; }
	//-------------------------------------------------------------------------------------------------------------------//
	if ($nombre_tipo_cliente == 'PERSONA_NATURAL') { 
		$nombre_empresa_juridica = ""; 
		$nombres_tercero = $nombres_tercero; 
		$apellidos_tercero = $apellidos_tercero; 
	} else { 
		$nombre_empresa_juridica = $nombres_apellidos_completos; 
		$nombres_tercero = ''; 
		$apellidos_tercero = ''; 
	}
	//-------------------------------------------------------------------------------------------------------------------//
...
//-------------------------------------------------------------------------------------------------------------------//
	...
	$datos_json .= '				"company_name": "' .$nombre_empresa_juridica. '", ';
	...
	precio_ipc, cod_tipo_pago, cod_tipo_forma_pago, nombre_tipo_moneda 
	...
	$datos_json .= '			"notes": [';
	$datos_json .= '"' .$observacion.'"';
	$datos_json .= '			 ], ';

	$datos_json .= '			"retentions": [';
		if ($reteica_ptj <> '0') {
	$datos_json .= ' 				{ ';
	$datos_json .= '   	 				"tax_category": "RET_ICA", ';
	$datos_json .= '   		 			"tax_rate": ' .$reteica_ptj. '';
	$datos_json .= ' 				}, ';
	}
	if ($retefuente_ptj <> '0') {
	$datos_json .= ' 				{ ';
	$datos_json .= '   	 				"tax_category": "RET_FUENTE", ';
	$datos_json .= '   		 			"tax_rate": ' .$retefuente_ptj. '';
	$datos_json .= ' 				} ';
	}
	$datos_json .= ' 			] ';
	$datos_json .= '		} ';
