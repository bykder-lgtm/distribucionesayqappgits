...
if (isset($_GET["cod_tercero"])) {
	...
	$sql_cuenta_cobrar = "SELECT cod_cuentas_cobrar, cod_factura FROM tbl15_cuentas_cobrar WHERE (cod_tercero = '$cod_tercero') AND (cod_estado_archivado = '0')";
	$consulta_cuenta_cobrar = mysqli_query($conectar, $sql_cuenta_cobrar) or die(mysqli_error($conectar));
	$total_datos = mysqli_num_rows($consulta_cuenta_cobrar);
	while ($datos_cuenta_cobrar = mysqli_fetch_assoc($consulta_cuenta_cobrar)) {
	...
	$actualizar_sql1 = sprintf("UPDATE tbl15_cuentas_cobrar SET abonado = '$abonado', subtotal = '$subtotal' WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar')");
