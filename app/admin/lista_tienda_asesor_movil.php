...
<!-- jQuery CDN (Requerido para Select2 y Ajax) -->
...
$subquery_aliados_asesor = "SELECT cod_administrador FROM tbl15_administrador WHERE cod_seguridad = '23' AND cod_asesor = '$cod_administrador' AND cod_estado != '0'";

$view = isset($_GET['view']) ? $_GET['view'] : 'normal'; // normal o rapida

// Base query for counting total stores (for pagination)
$sql_conteo_base = "FROM tbl15_tienda t ";
$sql_conteo_where = "WHERE t.cod_estado != '0' ";

if ($view == 'rapida') {
    $sql_conteo_where .= "AND t.cod_administrador = '$cod_administrador' AND (t.cod_aliado_estrategico = '0' OR t.cod_aliado_estrategico IS NULL) ";
} else {
    $sql_conteo_where .= "AND t.cod_aliado_estrategico IN ($subquery_aliados_asesor) ";
}

if (!empty($busqueda)) { $sql_conteo_where .= " AND (t.nombre_tienda LIKE '%$busqueda%' OR t.identificacion_tercero LIKE '%$busqueda%' OR t.nombre1_tercero LIKE '%$busqueda%' OR t.cod_tienda LIKE '$busqueda')"; }
if ($cod_departamento_filtro > 0) { $sql_conteo_where .= " AND t.cod_departamento = '$cod_departamento_filtro'"; }
if ($cod_municipio_filtro > 0) { $sql_conteo_where .= " AND t.cod_municipio = '$cod_municipio_filtro'"; }
if (!empty($barrio_filtro)) { $sql_conteo_where .= " AND t.barrio_tercero LIKE '%$barrio_filtro%'"; }
if ($has_gps_filtro === 'si') { $sql_conteo_where .= " AND t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != ''"; }
else if ($has_gps_filtro === 'no') { $sql_conteo_where .= " AND (t.ubicacion_gps_tienda IS NULL OR t.ubicacion_gps_tienda = '')"; }

$sql_conteo = "SELECT COUNT(*) as total " . $sql_conteo_base . $sql_conteo_where;
$res_conteo = mysqli_query($conectar, $sql_conteo);
$total_registros = ($res_conteo) ? mysqli_fetch_assoc($res_conteo)['total'] : 0;
$total_paginas = ceil($total_registros / $registros_por_pagina);

// Main query for stores
$sql_tiendas_select = "SELECT t.*, d.nombre_departamento, m.nombre_municipio, ta.nombre_tipo_aliado, ta.color_tipo_aliado, (SELECT COUNT(*) FROM tbl15_info_factura_venta WHERE cod_tienda = t.cod_tienda AND nombre_estado_factura = 'ABIERTA') as creditos_activos ";
...
$sql_tiendas_from .= "LEFT JOIN tbl15_tipo_aliado ta ON t.cod_tipo_aliado = ta.cod_tipo_aliado ";
$sql_tiendas_where = $sql_conteo_where; // Reuse the WHERE clause from the count query

if ($view == 'rapida') {
    $sql_tiendas_select .= ", 'Tienda Rápida' as nombre_aliado ";
} else {
    $sql_tiendas_select .= ", a.nombres_apellidos_tercero as nombre_aliado ";
    $sql_tiendas_from .= "LEFT JOIN tbl15_administrador a ON t.cod_aliado_estrategico = a.cod_administrador ";
}

$sql_tiendas = $sql_tiendas_select . $sql_tiendas_from . $sql_tiendas_where;
...
// Totales para las pestañas (these should also respect filters if applied, but for simplicity, keeping them as original for now)
// Total Normal
$sql_total_normal_base = "SELECT COUNT(*) as total FROM tbl15_tienda t LEFT JOIN tbl15_administrador a ON t.cod_aliado_estrategico = a.cod_administrador WHERE t.cod_aliado_estrategico IN ($subquery_aliados_asesor) AND t.cod_estado != '0'";
$sql_total_normal_filtered = $sql_total_normal_base;
if (!empty($busqueda)) { $sql_total_normal_filtered .= " AND (t.nombre_tienda LIKE '%$busqueda%' OR t.identificacion_tercero LIKE '%$busqueda%' OR t.nombre1_tercero LIKE '%$busqueda%' OR t.cod_tienda LIKE '$busqueda')"; }
if ($cod_departamento_filtro > 0) { $sql_total_normal_filtered .= " AND t.cod_departamento = '$cod_departamento_filtro'"; }
if ($cod_municipio_filtro > 0) { $sql_total_normal_filtered .= " AND t.cod_municipio = '$cod_municipio_filtro'"; }
if (!empty($barrio_filtro)) { $sql_total_normal_filtered .= " AND t.barrio_tercero LIKE '%$barrio_filtro%'"; }
if ($has_gps_filtro === 'si') { $sql_total_normal_filtered .= " AND t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != ''"; }
else if ($has_gps_filtro === 'no') { $sql_total_normal_filtered .= " AND (t.ubicacion_gps_tienda IS NULL OR t.ubicacion_gps_tienda = '')"; }
$res_total_normal = mysqli_query($conectar, $sql_total_normal_filtered);
$count_normal = 0;
if ($res_total_normal) { $datos_total_normal = mysqli_fetch_assoc($res_total_normal); $count_normal = $datos_total_normal['total']; }

// Total Rápida
$sql_total_rapida_base = "SELECT COUNT(*) as total FROM tbl15_tienda t WHERE t.cod_administrador = '$cod_administrador' AND (t.cod_aliado_estrategico = '0' OR t.cod_aliado_estrategico IS NULL) AND t.cod_estado != '0'";
$sql_total_rapida_filtered = $sql_total_rapida_base;
if (!empty($busqueda)) { $sql_total_rapida_filtered .= " AND (t.nombre_tienda LIKE '%$busqueda%' OR t.identificacion_tercero LIKE '%$busqueda%' OR t.nombre1_tercero LIKE '%$busqueda%' OR t.cod_tienda LIKE '$busqueda')"; }
if ($cod_departamento_filtro > 0) { $sql_total_rapida_filtered .= " AND t.cod_departamento = '$cod_departamento_filtro'"; }
if ($cod_municipio_filtro > 0) { $sql_total_rapida_filtered .= " AND t.cod_municipio = '$cod_municipio_filtro'"; }
if (!empty($barrio_filtro)) { $sql_total_rapida_filtered .= " AND t.barrio_tercero LIKE '%$barrio_filtro%'"; }
if ($has_gps_filtro === 'si') { $sql_total_rapida_filtered .= " AND t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != ''"; }
else if ($has_gps_filtro === 'no') { $sql_total_rapida_filtered .= " AND (t.ubicacion_gps_tienda IS NULL OR t.ubicacion_gps_tienda = '')"; }
$res_total_rapida = mysqli_query($conectar, $sql_total_rapida_filtered);
$count_rapida = 0;
if ($res_total_rapida) { $datos_total_rapida = mysqli_fetch_assoc($res_total_rapida); $count_rapida = $datos_total_rapida['total']; }

// Contar tiendas con firma (based on current view and filters)
$sql_con_firma_base = "SELECT COUNT(*) as total FROM tbl15_tienda t WHERE t.url_firma_electronica IS NOT NULL AND t.url_firma_electronica != '' AND t.cod_estado != '0'";
if ($view == 'rapida') {
    $sql_con_firma_base .= " AND t.cod_administrador = '$cod_administrador' AND (t.cod_aliado_estrategico = '0' OR t.cod_aliado_estrategico IS NULL)";
} else {
    $sql_con_firma_base .= " AND t.cod_aliado_estrategico IN ($subquery_aliados_asesor)";
}
$sql_con_firma_filtered = $sql_con_firma_base;
if (!empty($busqueda)) { $sql_con_firma_filtered .= " AND (t.nombre_tienda LIKE '%$busqueda%' OR t.identificacion_tercero LIKE '%$busqueda%' OR t.nombre1_tercero LIKE '%$busqueda%' OR t.cod_tienda LIKE '$busqueda')"; }
if ($cod_departamento_filtro > 0) { $sql_con_firma_filtered .= " AND t.cod_departamento = '$cod_departamento_filtro'"; }
if ($cod_municipio_filtro > 0) { $sql_con_firma_filtered .= " AND t.cod_municipio = '$cod_municipio_filtro'"; }
if (!empty($barrio_filtro)) { $sql_con_firma_filtered .= " AND t.barrio_tercero LIKE '%$barrio_filtro%'"; }
if ($has_gps_filtro === 'si') { $sql_con_firma_filtered .= " AND t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != ''"; }
else if ($has_gps_filtro === 'no') { $sql_con_firma_filtered .= " AND (t.ubicacion_gps_tienda IS NULL OR t.ubicacion_gps_tienda = '')"; }
$resultado_con_firma = mysqli_query($conectar, $sql_con_firma_filtered);
$tiendas_con_firma = 0;
...
// Contar tiendas con GPS (based on current view and filters)
$sql_con_gps_base = "SELECT COUNT(*) as total FROM tbl15_tienda t WHERE t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != '' AND t.cod_estado != '0'";
if ($view == 'rapida') {
    $sql_con_gps_base .= " AND t.cod_administrador = '$cod_administrador' AND (t.cod_aliado_estrategico = '0' OR t.cod_aliado_estrategico IS NULL)";
} else {
    $sql_con_gps_base .= " AND t.cod_aliado_estrategico IN ($subquery_aliados_asesor)";
}
$sql_con_gps_filtered = $sql_con_gps_base;
if (!empty($busqueda)) { $sql_con_gps_filtered .= " AND (t.nombre_tienda LIKE '%$busqueda%' OR t.identificacion_tercero LIKE '%$busqueda%' OR t.nombre1_tercero LIKE '%$busqueda%' OR t.cod_tienda LIKE '$busqueda')"; }
if ($cod_departamento_filtro > 0) { $sql_con_gps_filtered .= " AND t.cod_departamento = '$cod_departamento_filtro'"; }
if ($cod_municipio_filtro > 0) { $sql_con_gps_filtered .= " AND t.cod_municipio = '$cod_municipio_filtro'"; }
if (!empty($barrio_filtro)) { $sql_con_gps_filtered .= " AND t.barrio_tercero LIKE '%$barrio_filtro%'"; }
if ($has_gps_filtro === 'si') { $sql_con_gps_filtered .= " AND t.ubicacion_gps_tienda IS NOT NULL AND t.ubicacion_gps_tienda != ''"; }
else if ($has_gps_filtro === 'no') { $sql_con_gps_filtered .= " AND (t.ubicacion_gps_tienda IS NULL OR t.ubicacion_gps_tienda = '')"; }
$resultado_con_gps = mysqli_query($conectar, $sql_con_gps_filtered);
$tiendas_con_gps = 0;
...
$sql_estados_aliado_lista_base = "SELECT ta.nombre_tipo_aliado, ta.color_tipo_aliado, COUNT(t.cod_tienda) as cantidad 
FROM tbl15_tienda t 
LEFT JOIN tbl15_tipo_aliado ta ON t.cod_tipo_aliado = ta.cod_tipo_aliado 
WHERE t.cod_estado != '0'";

if ($view == 'rapida') {
    $sql_estados_aliado_lista_base .= " AND t.cod_administrador = '$cod_administrador' AND (t.cod_aliado_estrategico = '0' OR t.cod_aliado_estrategico IS NULL)";
} else {
    $sql_estados_aliado_lista_base .= " AND t.cod_aliado_estrategico IN ($subquery_aliados_asesor)";
}
$sql_estados_aliado_lista = $sql_estados_aliado_lista_base;
...
if ($resultado_estados_lista) {
    while ($row = mysqli_fetch_assoc($resultado_estados_lista)) {
        ...
        $color = !empty($row['color_tipo_aliado']) ? $row['color_tipo_aliado'] : '#10b981';
...
$sql_aliados = "SELECT cod_administrador, cedula, nombres, apellidos, nombres_apellidos_tercero, comision_ptj FROM tbl15_administrador WHERE (cod_seguridad = '23' AND cod_asesor = '$cod_administrador' AND cod_estado != '0') ORDER BY nombres_apellidos_tercero ASC";
...
-->
    ...
    <div class="animate-in delay-1" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.05) 0%, rgba(16, 185, 129, 0.02) 100%); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 16px; padding: 1rem; margin-bottom: 1.5rem;">
        <div style="display: flex; align-items: center; gap: 0.5rem; color: #10b981; font-size: 0.95rem; font-weight: 700; margin-bottom: 1rem;">
...
function applyAdvancedFilters() {
    ...
    let url = 'lista_tienda_asesor_movil.php?busqueda=' + encodeURIComponent(busqueda);
