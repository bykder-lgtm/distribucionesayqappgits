...
$salida_carrito_compra_ajax .= '								<h3><a href="#">'.$nombre_concepto_multi_virtual.' DE VENTA</a> ['.$cod_base_caja.'] - ID: '.$cod_info_factura_venta.'</h3>';
...
while ($datos_producto = mysqli_fetch_assoc($consulta_producto)) {
	...
	$salida_carrito_compra_ajax .= '														<div class="media mb-2 border-bottom" id="elim'.$cod_venta_producto_temporal.'">';
	$salida_carrito_compra_ajax .= '															<div class="media-body"><a class="eliminar" data="'.$cod_venta_producto_temporal.'" id="cod_venta_producto_temporal'.$cod_venta_producto_temporal.'"><i id="eliminar_load'.$cod_venta_producto_temporal.'" class="fas fa-times"></i></a><span class="mx-2">|</span><a href="#">'.$nombre_producto.'</a><span class="mx-2">|</span>'.$fecha_hora_venta_producto.'';
...
$salida_carrito_compra_ajax .= '											</div>';
																			...
																			if ($total_datos <> '0') {
$salida_carrito_compra_ajax .= '											<div id="mostrar_si_existe_registro">';
$salida_carrito_compra_ajax .= '											<div class="col-md-12 col-lg-12">';
...
$salida_carrito_compra_ajax .= '													<div class="d-flex gr-total">';
$salida_carrito_compra_ajax .= '														<h5>Recibido</h5>';
$salida_carrito_compra_ajax .= '														<div class="ml-auto h5"><input type="text" name="vlr_cancelado_number" id="vlr_cancelado_number" value="" class="form-control" style="width: 150px;" required></div>';
$salida_carrito_compra_ajax .= '														<input type="hidden" name="vlr_cancelado" id="vlr_cancelado" value="">';
$salida_carrito_compra_ajax .= '													</div>';
...
//************************************************************************************************************************************//
$sql_producto = "SELECT cod_venta_producto_temporal, nombre_producto, und_venta, precio_venta_producto, 
total_venta_producto, url_img_min_producto, url_img_orig_producto FROM tbl15_venta_producto_temporal WHERE (cuenta = '$cuenta') AND (cod_caja_virtual = '$cod_caja_virtual') 
ORDER BY cod_venta_producto_temporal DESC";
$consulta_producto = mysqli_query($conectar, $sql_producto) or die(mysqli_error($conectar));
$total_datos = mysqli_num_rows($consulta_producto);
while ($datos_producto = mysqli_fetch_assoc($consulta_producto)) {
...
$codigoHTML_menu .= '            <a href="../admin/carrito_compra_temporal_version_tactil.php" class="btn btn-default hvr-hover btn-cart">VER CARRITO</a>';
...
$datos_array = array(
	...
	'salida_info_actualizada_carrito_compra_ajax' => $salida_carrito_compra_ajax, 
