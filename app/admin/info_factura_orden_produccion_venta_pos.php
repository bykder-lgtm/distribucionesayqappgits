...
<?php
$suma_temporal = "SELECT Sum(total_venta_producto) As total_venta, Sum(total_costo_producto) As total_compra FROM tbl15_orden_produccion_venta_producto 
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            //let id = this.id;
            $.post("guardar_info_factura_y_orden_produccion_venta_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:id }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
-->
   ...
   <td style="text-align:center;"><input name="vlr_cancelado" id="vlr_cancelado" type="number" value="<?php echo $vlr_cancelado ?>" style="width: 100px;" required/></td>
    ...
    <td style="text-align:center;"><a href="../admin/orden_produccion_venta_productos_opcion_imprimir.php?cod_info_orden_produccion_factura_venta=<?php echo $cod_info_orden_produccion_factura_venta?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/guardar.png" class="img-polaroid" alt=""></a></td>
    <td style="text-align:center;"></td>

<?php 
$sql_temporal = "SELECT cod_orden_produccion_venta_producto FROM tbl15_orden_produccion_venta_producto 
WHERE (cod_info_orden_produccion_factura_venta = '$cod_info_orden_produccion_factura_venta')";
...
<input type="hidden" name="cod_orden_produccion_venta_producto[]" value="<?php echo $datos_temporal['cod_orden_produccion_venta_producto']; ?>" size="4">
<?php } ?>  
<?php $pagina ='facturacion_orden_produccion_venta_producto_manual_pos.php'; ?>
