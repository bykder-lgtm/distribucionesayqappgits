...
<a class="btn btn-primary" href="#"><h6>Lista de Paciente Atendidos Por Empresa Y Fecha</h6></a>
</div>
<div class="row-fluid">
...
<td><a href="#">FECHA:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
<select name="fecha_ymd" class="selectpicker" data-show-subtext="true" data-live-search="true" required>
<?php if (isset($fecha_ymd)) { echo "<option value='' >Selecione</option>";
} else { echo  "<option value='' selected >Selecione</option>"; }
$sql_consulta = ("SELECT fecha_ymd FROM tbl15_historia_clinica GROUP BY fecha_ymd ORDER BY fecha_ymd DESC");
$resultado = mysqli_query($conectar, $sql_consulta);
while ($contenedor = mysqli_fetch_assoc($resultado)) {
if(isset($fecha_ymd) and $fecha_ymd == $contenedor['fecha_ymd']) {
$seleccionado = "selected"; } else { $seleccionado = ""; }
...
WHERE ((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) ORDER BY tbl15_historia_clinica.fecha_time DESC";
...
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd')";
...
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1)";
...
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) 
ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
WHERE ((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd') AND ((tbl15_historia_clinica.cod_estado_facturacion)=0)";
...
GROUP BY tbl15_historia_clinica.fecha_ymd, tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
HAVING (((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) AND ((tbl15_cliente.nombre_sexo)='M'))";
...
FROM tbl15_empresa RIGHT JOIN (tbl15_cliente RIGHT JOIN (tbl15_administrador RIGHT JOIN tbl15_historia_clinica ON tbl15_administrador.cod_administrador = tbl15_historia_clinica.cod_administrador) ON tbl15_cliente.cod_cliente = tbl15_historia_clinica.cod_cliente) ON tbl15_empresa.nombre_empresa = tbl15_historia_clinica.nombre_empresa
GROUP BY tbl15_historia_clinica.fecha_ymd, tbl15_historia_clinica.nombre_empresa, tbl15_historia_clinica.cod_estado_facturacion, tbl15_cliente.nombre_sexo
HAVING (((tbl15_historia_clinica.nombre_empresa)='$nombre_empresa') AND (tbl15_historia_clinica.fecha_ymd = '$fecha_ymd') AND ((tbl15_historia_clinica.cod_estado_facturacion)=1) AND ((tbl15_cliente.nombre_sexo)='F'))";
...
<br>
<td align="center">FECHA: <?php echo $fecha_ymd ?></td>
</form> 
