...
$tipo_soporte                                       = "MOVIMIENTO_CONTABLE";
/* ----------------------------------------------------------------------------------------------------------/ */
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formulario_de_actualizacion")) {
	...
	if ($nombre_nota_observacion == '') { $nombre_nota_observacion = 'SOPORTES MOVIMIENTO CONTABLE'; } else { $nombre_nota_observacion = $nombre_nota_observacion; }
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($url_img1 <> '') { 
		...
		$nombre_normal2                                     = $tipo_soporte.'_'.$fecha_ymdHis.'_'.$cod_movimiento_contable.'_'.$cod_tercero.'_'.$cod_administrador.'.'.$formato_img2;
...
//-------------------------------------- -----------------------------------------------------------------//
//-------------------------------------- -----------------------------------------------------------------//
	$actualizar_sql1 = "UPDATE tbl15_movimiento_contable SET url_img_orig_producto = '$url_img_orig_producto' WHERE (cod_movimiento_contable = '$cod_movimiento_contable')";
	$resultado_actualizacion1 = mysqli_query($conectar, $actualizar_sql1) or die(mysqli_error($conectar));

	$agreg = "INSERT INTO tbl15_nota_observacion (nombre_nota_observacion, fecha_ymd, fecha_hora, cuenta, cod_administrador, fecha_creacion, cod_tipo_nota_observacion, cod_movimiento_contable, url_img_orig_producto, url_img_min_producto) 
	VALUES ('$nombre_nota_observacion', '$fecha_ymd', '$fecha_hora', '$cuenta_actual', '$cod_administrador', '$fecha_creacion', '$cod_tipo_nota_observacion', '$cod_movimiento_contable', '$url_img_orig_producto', '$url_img_min_producto')";
