...
$mostrar_datos_sql = "SELECT * FROM tbl15_producto_bodega WHERE ((nombre_producto LIKE '%$buscar%') OR (cod_producto_barra LIKE '$buscar%')) ORDER BY nombre_producto ASC";
$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
$total_resultados = mysqli_num_rows($consulta);
} else {
$mostrar_datos_sql = "SELECT * FROM tbl15_producto_bodega WHERE ((nombre_producto LIKE '%$buscar%') OR (cod_producto_barra LIKE '$buscar%')) AND (nombre_tipo_producto='VACUNA') ORDER BY nombre_producto ASC";
$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
$total_resultados = mysqli_num_rows($consulta);  
}
...
<td align="Left"><a href="../admin/reg_compra_temporal_producto_reg.php?cod_producto_barra=<?php echo $cod_producto_barra?>&nombre_tipo_cargue_factura=<?php echo $nombre_tipo_cargue_factura?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&pagina=<?php echo $pagina?>" tabindex=3><?php echo $nombre_producto ?></a></td>
