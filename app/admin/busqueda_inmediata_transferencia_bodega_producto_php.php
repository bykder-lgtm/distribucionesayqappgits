...
//----------------------------------------------------------------------------------------------------------------//
$sql_infos_empresas = "SELECT cod_estado_marca_global, cod_estado_busqueda_venta_manual_resultado_unico_redirect_global, cod_estado_cod_barra2_global, cod_estado_venta_prod_en_cero_global FROM tbl15_info_empresa WHERE cod_info_empresa = '1'";
...
if($buscar <> NULL) {
	...
	if ($buscar_por == 'nombre_producto') {
		$mostrar_datos_sql = "SELECT cod_producto, cod_producto_barra, cod_producto_barra2, nombre_producto, und_producto, precio_compra_producto, precio_venta_producto, nombre_tipo_producto, cod_marca 
...
while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
	...
	if ($cod_estado_venta_prod_en_cero_global == '1') { if ($und_producto > 0) { $condcional_para_habilitar_enlace_venta = 'SI'; } else { $condcional_para_habilitar_enlace_venta = 'NO'; } } else { $condcional_para_habilitar_enlace_venta = 'SI'; }
?>
	<tr>
		...
		<td style="text-align:left;"><a href="../admin/reg_transferencia_bodega_temporal_producto_reg.php?cod_producto_barra=<?php echo $cod_producto_barra?>&buscar_por=<?php echo $buscar_por?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&pagina=<?php echo $pagina?>" tabindex=3><?php echo $nombre_producto ?></a></td>
		<?php } else { ?>
