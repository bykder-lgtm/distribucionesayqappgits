<?php
$sql_info_factura_venta = "SELECT * FROM tbl15_info_factura_venta WHERE (nombre_estado_factura = 'ABIERTA') AND (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
$consulta_info_factura_venta = mysqli_query($conectar, $sql_info_factura_venta);
$total_datos_info_factura_venta = mysqli_num_rows($consulta_info_factura_venta);

if ($total_datos_info_factura_venta <> 0)  { 
...
<?php } ?>
<!-- //////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
<?php if ($cod_estado_tipo_servicio_global == '1') { ?>
<script language="javascript">
$(document).ready(function(){
    $("#cod_tipo_servicio").on('change', function () {
        var valor = $(this).val();
        var campo = "cod_tipo_servicio";
        var tipo_ajax = "tbl15_info_factura_venta";
        var pagina_local = "<?php echo $pagina_local_redirect_tipo_pago;?>";
...
<?php if ($tipo_dispositivo == 'PC') { ?>
    <table class="table table-striped" border="1" cellspacing="0" cellpadding="20">
      <tr>
        ...
        <?php if ($cod_estado_renta_alquiler_global == '1') { ?><th style="text-align:center;">ES <?php echo $nombre_tipo_plan_alquiler_defect_global; ?>?</th><?php } ?>
      ...
      <tr>
       ...
       <?php } ?>
        ...
        <?php if ($cod_estado_renta_alquiler_global == '1') { ?>
        <td style="text-align:center;">
            <select name="cod_estado_alquiler_renta" id="cod_estado_alquiler_renta" class="selectpicker" data-show-subtext="true" data-live-search="true" style="width: 50px;" tabindex="1">
        ...
        <td style="text-align:center;">
            ...
            <br>
            <b>TIPO SERV:</b>
            <select name="cod_tipo_servicio" id="cod_tipo_servicio" class="selectpicker" data-show-subtext="true" data-live-search="true" style="width: 130px;" tabindex="1">
                <?php if (isset($cod_tipo_servicio)) { echo "<option value='' selected >ESCOGER</option>"; } else { echo "<option value='' selected >ESCOGER</option>"; }
                $consulta2_sql = "SELECT cod_tipo_servicio, nombre_tipo_servicio FROM tbl15_tipo_servicio WHERE (cod_estado = '1') ORDER BY cod_tipo_servicio ASC";
                $consulta2 = mysqli_query($conectar, $consulta2_sql);
                while ($datos2 = mysqli_fetch_assoc($consulta2)) {
                if(isset($cod_tipo_servicio) AND $cod_tipo_servicio == $datos2['cod_tipo_servicio']) {
                $seleccionado = "selected"; } else { $seleccionado = ""; }
                $codigo = $datos2['cod_tipo_servicio'];
                $nombre = $datos2['nombre_tipo_servicio'];
                echo "<option value='".$codigo."' $seleccionado >".$nombre."</option>"; } ?>
            </select>
            <?php } ?>
            <br>
...
-->
        ...
        <td style="text-align:center;">
            ...
            <?php if ($cod_estado_retefuente_global == '1') { ?>
                ...
                <br>
                <?php echo number_format($total_retefuente, 0, ",", "."); ?>
            <?php } ?>
            <?php if ($cod_estado_reteica_global == '1') { ?>
                <br>
    ...
    <table class="table table-condensed" border="" cellspacing="0" cellpadding="0">
      ...
      <tr>
       ...
       <td style="text-align:center;"><input name="cod_base_caja" id="cod_base_caja" type="number" value="<?php echo $cod_base_caja ?>" style="width: 40px;" tabindex="1" required/></td>
        ...
        <?php if ($cod_seguridad==1) { ?>
           <td style="text-align:center;">
            <input name="fecha_anyo" id="fecha_anyo" type="date" value="<?php echo $fecha_anyo ?>" style="width: 105px;" tabindex="1" required/>
            <?php if ($cod_estado_fecha_entrega_venta_global == '1') { ?><br><b>FECHA ENTREGA:</b> <input name="fecha_entrega" id="fecha_entrega" type="date" value="<?php echo $fecha_entrega ?>" style="width: 105px;" tabindex="1" /><?php } ?>
            </td>
        <?php } else { ?>
           <td style="text-align:center;">
    ...
    <table <table class="table table-condensed" border="" cellspacing="0" cellpadding="0">
      ...
      <tr>
       ...
       <td style="text-align:center;"><?php echo $cod_info_factura_venta ?></td>
        ...
        <input type="hidden" name="verificacion_envio" value="1" size="15">
        <input type="hidden" name="cod_estado_vacuna" value="0" size="15">
        <input type="hidden" name="modo_venta_por_defecto" value="<?php echo $modo_venta_por_defecto?>" size="15">
        </tr>
    </table>
<?php } ?>
</form>
