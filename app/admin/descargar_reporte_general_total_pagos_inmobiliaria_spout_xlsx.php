...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_TOTAL_PAGOS_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_TOTAL_PAGOS";
...
//-----------------------------------------------------------------------------------------------------//
// Headers
	$writer->addRow(array('TOTAL VALOR PAGADO A PROPIETARIOS', 'TOTAL VALOR PAGADO POR ARRENDATARIOS'));
// Then a foreach
/*
...
*/
		$writer->addRow(array($total_valor_pagado_a_propietario, $total_recibido_pago_inquilino));
	//}
	$writer->close();
}