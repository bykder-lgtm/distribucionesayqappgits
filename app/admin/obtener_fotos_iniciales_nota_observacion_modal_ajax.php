...
if ($cod_info_factura_venta === '') {
    echo '<div class="col-12">Error: cod_info_factura_venta no especificado</div>';
    exit;
}
$html = '';
$sql_nota_observacion = "SELECT * FROM tbl15_nota_observacion WHERE (cod_info_factura_venta = '$cod_info_factura_venta') AND (codigo_estado_revision <> '2') ORDER BY cod_posicion ASC";
...
while ($matriz_nota_observacion = mysqli_fetch_assoc($consulta_nota_observacion)) {
    ...
    if ($cod_estado_obligatorio == '1') { 
        ...
        $container_style = "border: 2px solid #ffc107; border-radius: 8px; padding: 1.5rem; background-color: #fffbf0; border-left: 4px solid #376F00;";
        $titulo_style = "font-weight: 600; color: #376F00; margin-bottom: 1rem; text-align: center;";
    } else { 
        $imagen_obligatoria = ''; 
    ...
    $html .= "        <h6 style=\"{$titulo_style}\">{$nombre_nota_observacion} {$imagen_obligatoria}</h6>\n";
