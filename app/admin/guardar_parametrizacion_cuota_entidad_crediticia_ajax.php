...
if (isset($_POST['accion']) && $_POST['accion'] == 'eliminar') {
    ...
    $sql_delete = "DELETE FROM tbl15_parametrizacion_entidad_crediticia_cuota WHERE cod_entidad_crediticia = '$cod_entidad_crediticia'";
...
try {
    // Insert or Update cuotas individually
    foreach ($cuotas as $cuota) {
        ...
        $sql_check = "SELECT cod_parametrizacion_entidad_crediticia_cuota FROM tbl15_parametrizacion_entidad_crediticia_cuota WHERE cod_entidad_crediticia = '$cod_entidad_crediticia' AND cuota = '$num_cuota'";
        ...
        if (!mysqli_query($conectar, $sql_save)) { throw new Exception('Error al guardar cuota ' . $num_cuota . ': ' . mysqli_error($conectar)); }
    }
    // Commit transaction
    mysqli_query($conectar, "COMMIT");
    echo json_encode(['success' => true, 'message' => 'Parametrización actualizada correctamente. ' . count($cuotas) . ' cuotas procesadas.']);
} catch (Exception $e) {
    // Rollback on error
