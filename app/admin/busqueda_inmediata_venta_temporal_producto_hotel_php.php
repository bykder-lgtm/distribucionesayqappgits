...
if($buscar <> NULL) {
	if ($buscar_por == 'nombre_producto') {
		...
		FROM tbl15_producto WHERE ((nombre_producto LIKE '$buscar%') AND (nombre_tipo_producto = 'PRODUCTO')) ORDER BY nombre_producto ASC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'cod_producto_barra') {
		$mostrar_datos_sql = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_compra_producto, precio_venta_producto, nombre_tipo_producto, cod_marca 
		FROM tbl15_producto WHERE ((cod_producto_barra LIKE '$buscar') AND (nombre_tipo_producto = 'PRODUCTO'))  ORDER BY nombre_producto ASC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'cod_producto_barra_nombre_producto') {
		$mostrar_datos_sql = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_compra_producto, precio_venta_producto, nombre_tipo_producto, cod_marca 
		FROM tbl15_producto WHERE ((nombre_producto LIKE '$buscar%') OR (cod_producto_barra LIKE '$buscar') AND (nombre_tipo_producto = 'PRODUCTO')) ORDER BY nombre_producto ASC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} else {
		$mostrar_datos_sql = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_compra_producto, precio_venta_producto, nombre_tipo_producto, cod_marca 
                ...
                <table class="table table-dark">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <th style="text-align:left;"><a href="../admin/reg_venta_temporal_producto_hotel_reg.php?cod_producto_barra=<?php echo $cod_producto_barra?>&cod_info_factura_venta=<?php echo $cod_info_factura_venta?>&buscar_por=<?php echo $buscar_por?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&cuenta=<?php echo $cuenta?>&cod_caja_virtual=<?php echo $cod_caja_virtual?>&pagina=<?php echo $pagina?>" onclick="deshabilitar_url_doble_clic(this)"><?php echo $nombre_producto ?></a></th>
