...
$incren_sup = 0;
$sql_gasto_inmueble_scrip = "SELECT cod_gasto_inmueble_detalle_venta_temporal FROM tbl15_gasto_inmueble_detalle_venta_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') 
ORDER BY cod_gasto_inmueble_detalle_venta_temporal DESC";
...
$datos_factura = "SELECT cod_gasto_inmueble_detalle_venta_temporal FROM tbl15_gasto_inmueble_detalle_venta_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
$consulta = mysqli_query($conectar, $datos_factura);
$total_datos = mysqli_num_rows($consulta);

$datos_data_info_gasto_inmueble_detalle = "SELECT * FROM tbl15_info_gasto_inmueble_detalle_venta WHERE (nombre_estado_factura = 'ABIERTA') AND (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
xmlhttp.open("POST","../admin/busqueda_inmediata_venta_temporal_gasto_inmueble_detalle_php.php",true);
...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody><tr>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/lista_gasto_inmueble_detalle_temporal_virtual.php?pagina=<?php echo $pagina ?>">LISTA DE GASTOS PENDIENTES | </a></strong></td>
        <?php if ($cod_seguridad==1) { ?>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/reporte_gasto_inmueble_venta_relacionado_deducido.php">LISTA DE GASTOS DEDUCIDOS A PROPIETARIOS</a></strong></td>
        <?php } else { ?>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/reporte_gasto_inmueble_venta_relacionado_deducido.php">LISTA DE GASTOS DEDUCIDOS A PROPIETARIOS</a></strong></td>
        <?php } ?>
    </tr></tbody>
...
<?php } ?>
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_factura_venta_gasto_inmueble_detalle.php'); ?>
<!-- ***************************************************************************************************************************** -->
...
$sql_gasto_inmueble_detalle_venta_temporal = "SELECT * FROM tbl15_gasto_inmueble_detalle_venta_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') 
ORDER BY cod_gasto_inmueble_detalle_venta_temporal DESC";
$consulta_gasto_inmueble_detalle_venta_temporal = mysqli_query($conectar, $sql_gasto_inmueble_detalle_venta_temporal);
while ($datos_gasto_inmueble_detalle_venta_temporal = mysqli_fetch_assoc($consulta_gasto_inmueble_detalle_venta_temporal)) {

$cod_gasto_inmueble_detalle_venta_temporal       = $datos_gasto_inmueble_detalle_venta_temporal['cod_gasto_inmueble_detalle_venta_temporal'];
$und_venta                                       = $datos_gasto_inmueble_detalle_venta_temporal['und_venta'];
$cod_gasto_inmueble                              = $datos_gasto_inmueble_detalle_venta_temporal['cod_gasto_inmueble'];
$nombre_gasto_inmueble_detalle                   = $datos_gasto_inmueble_detalle_venta_temporal['nombre_gasto_inmueble_detalle'];
$descripcion_gasto_inmueble_detalle              = $datos_gasto_inmueble_detalle_venta_temporal['descripcion_gasto_inmueble_detalle'];
$precio_venta_producto                           = $datos_gasto_inmueble_detalle_venta_temporal['precio_venta_producto'];
$precio_compra_producto                          = $datos_gasto_inmueble_detalle_venta_temporal['precio_compra_producto'];
$fecha_gasto_inmueble_detalle                    = $datos_gasto_inmueble_detalle_venta_temporal['fecha_gasto_inmueble_detalle'];
$cod_tipo_estado_incluido                        = $datos_gasto_inmueble_detalle_venta_temporal['cod_tipo_estado_incluido'];
$cod_tipo_forma_pago                             = $datos_gasto_inmueble_detalle_venta_temporal['cod_tipo_forma_pago'];
$url_img_orig_producto                           = $datos_gasto_inmueble_detalle_venta_temporal['url_img_orig_producto'];
...
?>
	<tr id="tr<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>">
		<td style="text-align:center;"></td>
		<?php if ($cod_estado_eliminar_caja_mesa_virtual == '1') { ?><td  style="text-align:center;"><a href="../admin/eliminar.php?llave=<?php echo $cod_gasto_inmueble_detalle_venta_temporal?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&cuenta=<?php echo $cuenta ?>&cod_caja_virtual=<?php echo $cod_caja_virtual ?>&cod_cuentas_cobrar_alerta=<?php echo $cod_cuentas_cobrar_alerta ?>&cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar ?>&cod_factura=<?php echo $cod_factura ?>&numero_alerta=<?php echo $numero_alerta ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>&palabra=<?php echo $palabra ?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/eliminar.png" class="img-polaroid" alt=""></a></td><?php } ?>
        ...
        <td style="text-align:center;"><textarea name="descripcion_gasto_inmueble_detalle" id="descripcion_gasto_inmueble_detalle<?php echo $incre_inf;?>" class="<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>" rows="1" cols="100" style="font-size:24px; width: 500px;"><?php echo $descripcion_gasto_inmueble_detalle;?></textarea></td>
        <td style="text-align:center;"><input name="precio_compra_producto" type="text" id="precio_compra_producto<?php echo $incre_inf;?>" class="<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>" value="<?php echo $precio_compra_producto;?>" style="font-size:24px; width: 130px;" /></td>
        <td style="text-align:center;"><input name="precio_venta_producto" type="text" id="precio_venta_producto<?php echo $incre_inf;?>" class="<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>" value="<?php echo $precio_venta_producto;?>" style="font-size:24px; width: 130px;" /></td>
        <td style="text-align:center;"><input name="fecha_gasto_inmueble_detalle" type="date" id="fecha_gasto_inmueble_detalle<?php echo $incre_inf;?>" class="<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>" value="<?php echo $fecha_gasto_inmueble_detalle;?>" style="font-size:24px; width: 180px;" /></td>
        <!--<td style="text-align:center"><input type="file" name="url_img1" id="fecha_gasto_inmueble_detalle<?php echo $incre_inf;?>" class="<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>"></td></td>-->
        <td style="text-align: center;"><a href="../admin/gasto_inmueble_detalle_venta_temporal_soporte_archivo_adjunto.php?cod_gasto_inmueble_detalle_venta_temporal=<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>&cod_info_gasto_inmueble_detalle_venta=<?php echo $cod_info_gasto_inmueble_detalle_venta;?>&cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar;?>&cod_cuentas_cobrar_alerta=<?php echo $cod_cuentas_cobrar_alerta;?>&cod_caja_virtual=<?php echo $cod_caja_virtual;?>&cuenta=<?php echo $cuenta_actual;?>&pagina=<?php echo $pagina_local;?>"><img src=../imagenes/adjuntar_archivo.png alt="Adjuntar"><?php echo $existe_archivo_cargado;?></a></td>
        ...
        <td style="text-align:center;"><a href="../admin/edit_tipo_estado_incluido_reg.php?cod_gasto_inmueble_detalle_venta_temporal=<?php echo $cod_gasto_inmueble_detalle_venta_temporal?>&cod_tipo_estado_incluido=<?php echo $cod_tipo_estado_incluido ?>&cod_info_gasto_inmueble_detalle_venta=<?php echo $cod_info_gasto_inmueble_detalle_venta ?>&cuenta=<?php echo $cuenta_actual ?>&cod_caja_virtual=<?php echo $cod_caja_virtual ?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&cod_cuentas_cobrar_alerta=<?php echo $cod_cuentas_cobrar_alerta ?>&cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar ?>&cod_factura=<?php echo $cod_factura ?>&numero_alerta=<?php echo $numero_alerta ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>&palabra=<?php echo $palabra ?>&pagina=<?php echo $pagina_local ?>"><?php echo $nombre_tipo_estado_incluido;?></a></td>
        <td style="text-align:center;"></td>
	</tr id="tr<?php echo $cod_gasto_inmueble_detalle_venta_temporal;?>">
<?php } ?>
</tbody>
...
$(document).ready(function() {
    ...
    $('.eliminar').click(function(){
        ...
        var dataString = 'llave='+cod_gasto_inmueble_detalle_venta_temporal+'&'+'tab='+'<?php echo $tab ?>'+'&'+'campo='+'<?php echo $campo ?>'+'&'+'tipo='+'<?php echo $tipo ?>';
        ...
        $.ajax({
            ...
            success: function() {           
                $('#eliminar-ok').empty();
                $('#eliminar-ok').append('<div align="center" class="correcto">Se ha eliminado correctamente el codigo = '+cod_gasto_inmueble_detalle_venta_temporal+'.</div>').fadeIn("slow");
                $('#'+parent).fadeOut("slow");
                $('#cod_gasto_inmueble_detalle_venta_temporal'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#cod_producto_barra_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#nombre_cliente_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#nombre_producto_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#und_venta_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#precio_venta_producto_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#mensaje_alerta_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#total_venta_producto_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#nombre_tipo_unidad_medida_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
                $('#nombre_frec_duracion_'+cod_gasto_inmueble_detalle_venta_temporal).fadeOut("slow");
...
$(document).ready(function() {
    ...
    $("input[name='pesar_producto_input']").change(function(){
        ...
        var foco = '';
        var pagina = '';
        var cod_gasto_inmueble_detalle_venta_temporal = id;
