...
$(function () {
    $('#container_grafico_estadistico').highcharts({
        ...
        xAxis: [{
            categories: [
<?php
$sql = "SELECT fecha, fecha_mes_venta_producto FROM tbl15_venta_producto GROUP BY fecha_mes_venta_producto ORDER BY fecha AS";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto-total_compra_producto) AS total_venta_producto, fecha_mes_venta_producto, fecha_ymd_venta_producto 
FROM tbl15_venta_producto GROUP BY fecha_mes_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
...
?>
            ...
            data: [
<?php
$sql = "SELECT SUM(costo) AS costo, fecha_dmy FROM tbl15_egreso GROUP BY fecha_dmy ORDER BY fecha_dmy ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
