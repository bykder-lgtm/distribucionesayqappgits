...
if (($campo == 'und_unidades') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, cajas_sobre, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc, precio_compra_producto_ant_desc 
	...
	$precio_costo_producto                                      = $info_temporal['precio_costo_producto'];
	...
	$precio_costo_con_descuento_dto1                           = $precio_costo_producto - ($precio_costo_producto * ($dto1/100));
	$precio_costo_con_descuento_dto2                           = $precio_costo_con_descuento_dto1 - ($precio_costo_con_descuento_dto1 * ($dto2/100));
	$precio_costo_con_descuentos                               = $precio_costo_con_descuento_dto2;
	$total_costo_producto                                      = $precio_costo_con_descuentos * $und_compra;
	$precio_compra_producto                                    = $precio_costo_con_descuento_dto2 + ($precio_costo_con_descuento_dto2 * ($iva_ptj/100));
	...
	if ($iva_ptj == '0') { $total_compra_gravada_producto = 0; $total_compra_no_gravada_producto = $und_compra * $precio_compra_producto; } else { $total_compra_gravada_producto = $und_compra * $precio_compra_producto; $total_compra_no_gravada_producto = 0; }

	$precio_compra_producto_desc_unit                           = $precio_costo_con_descuentos;
	$iva_unit                                                   = $precio_costo_con_descuentos * ($iva_ptj/100);
	$iva_total                                                  = $iva_unit * $und_compra;
	$precio_compra_producto_unit_mas_iva                        = $precio_costo_con_descuentos + ($precio_costo_con_descuentos * ($iva_ptj/100));
	$total_precio_compra_producto_unit_mas_iva                  = $precio_compra_producto_unit_mas_iva * $und_compra;
	$desc_unit                                                  = $precio_costo_producto - $precio_costo_con_descuentos;
	$desc_total                                                 = $desc_unit * $und_compra;
	$total_dto                                                  = ($desc_total);
	$descuento                                                  = ($desc_unit);
	$precio_compra_producto_ant_desc                            = $precio_costo_producto;
	...
	total_compra_producto_ant_desc = '$total_compra_producto_ant_desc', 
	precio_compra_producto_desc_unit = '$precio_compra_producto_desc_unit', desc_unit = '$desc_unit', desc_total = '$desc_total', iva_unit = '$iva_unit', iva_total = '$iva_total', 
	precio_compra_producto_unit_mas_iva = '$precio_compra_producto_unit_mas_iva', total_precio_compra_producto_unit_mas_iva = '$total_precio_compra_producto_unit_mas_iva', 
	total_compra_gravada_producto = '$total_compra_gravada_producto', total_compra_no_gravada_producto = '$total_compra_no_gravada_producto' 
	WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { $afectado = "SI"; } else { $afectado = "NO"; }
	// -------------------------------------------------------------------------------------------------------------------------------//
	$datos_info_temporal_compra_gravada = "SELECT SUM(total_compra_producto_ant_desc) AS total_compra_gravada_factura FROM tbl15_compra_producto_temporal WHERE (cod_info_factura_compra = '$cod_info_factura_compra') AND (iva_ptj <> '0')";
	$consulta_info_temporal_compra_gravada = mysqli_query($conectar, $datos_info_temporal_compra_gravada) or die(mysqli_error($conectar));
	$info_temporal_compra_gravada = mysqli_fetch_assoc($consulta_info_temporal_compra_gravada);

	$total_compra_gravada_factura                               = $info_temporal_compra_gravada['total_compra_gravada_factura'];

	$datos_info_temporal_compra_no_gravada = "SELECT SUM(total_compra_producto) AS total_compra_no_gravada_factura FROM tbl15_compra_producto_temporal WHERE (cod_info_factura_compra = '$cod_info_factura_compra') AND (iva_ptj = '0')";
	$consulta_info_temporal_compra_no_gravada = mysqli_query($conectar, $datos_info_temporal_compra_no_gravada) or die(mysqli_error($conectar));
	$info_temporal_compra_no_gravada = mysqli_fetch_assoc($consulta_info_temporal_compra_no_gravada);

	$total_compra_no_gravada_factura                            = $info_temporal_compra_no_gravada['total_compra_no_gravada_factura'];

	$datos_info_temporal_compra_desc_total = "SELECT SUM(desc_total) AS desc_total, SUM(total_precio_compra_producto_unit_mas_iva) AS total_compra_factura, SUM(iva_total) AS iva_total FROM tbl15_compra_producto_temporal 
	WHERE (cod_info_factura_compra = '$cod_info_factura_compra')";
	$consulta_info_temporal_compra_desc_total = mysqli_query($conectar, $datos_info_temporal_compra_desc_total) or die(mysqli_error($conectar));
	$info_temporal_compra_desc_total = mysqli_fetch_assoc($consulta_info_temporal_compra_desc_total);

	$total_descuento_factura                                    = $info_temporal_compra_desc_total['desc_total'];
	$total_compra_factura                                       = $info_temporal_compra_desc_total['total_compra_factura'];
	$iva_total                                                  = $info_temporal_compra_desc_total['iva_total'];
	$subtotal_factura                                           = ($total_compra_gravada_factura + $total_compra_no_gravada_factura) - $total_descuento_factura;
	$total_iva_factura                                          = $iva_total;
	$total_rete_iva_factura                                     = 0;
	$total_impoconsumo_factura                                  = 0;
	// -------------------------------------------------------------------------------------------------------------------------------//
	$suma_factura = "SELECT SUM(total_compra_producto) AS total_compra_producto, SUM(valor_iva * und_compra) AS valor_iva, SUM(precio_costo_producto * und_compra) AS total_precio_costo,
	...
	$total_factura_compra_retefuente                            = ($total - ($total_rete_fuente + $total_ret_ica));
	$total_factura_compra                                       = $total; 
	$total_descuento                                            = round($total_descuento_factura, 2);
	...
	$datos_totales_temporal = "SELECT SUM(total_compra_gravada_producto) AS total_compra_gravada_factura, SUM(total_compra_no_gravada_producto) AS total_compra_no_gravada_factura, SUM(desc_total) AS total_descuento_factura, 
	SUM(iva_total) AS total_iva_factura, SUM(total_precio_compra_producto_unit_mas_iva) AS total_compra_factura	FROM tbl15_compra_producto_temporal WHERE (cod_info_factura_compra = '$cod_info_factura_compra')";
	$consulta_totales_temporal = mysqli_query($conectar, $datos_totales_temporal) or die(mysqli_error($conectar));
	$totales_temporal = mysqli_fetch_assoc($consulta_totales_temporal);

	$total_compra_gravada_factura                               = $totales_temporal['total_compra_gravada_factura'];
	$total_compra_no_gravada_factura                            = $totales_temporal['total_compra_no_gravada_factura'];
	$total_descuento_factura                                    = $totales_temporal['total_descuento_factura'];
	$subtotal_factura                                           = ($total_compra_gravada_factura + $total_compra_no_gravada_factura) - $total_descuento_factura;
	$total_iva_factura                                          = $totales_temporal['total_iva_factura'];
	$total_compra_factura                                       = $totales_temporal['total_compra_factura'];
	...
	total_compra_gravada_factura = '$total_compra_gravada_factura', total_compra_no_gravada_factura = '$total_compra_no_gravada_factura', total_descuento_factura = '$total_descuento_factura', 
	subtotal_factura = '$subtotal_factura', total_iva_factura = '$total_iva_factura', total_compra_factura = '$total_compra_factura'
	WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	...
	$respuesta_ajax['precio_compra_producto']                   = round($precio_compra_producto, 2);
	$respuesta_ajax['subtotal']                                 = round($subtotal, 2);
	$respuesta_ajax['total_precio_ipc']                         = round($total_precio_ipc, 2);
	$respuesta_ajax['valor_iva']                                = round($valor_iva, 2);
	$respuesta_ajax['total']                                    = round($total, 2);
	$respuesta_ajax['valor_neto']                               = round($valor_neto, 2);
	$respuesta_ajax['total_rete_fuente']                        = round($total_rete_fuente, 2);
	$respuesta_ajax['total_ret_ica']                            = round($total_ret_ica, 2);
	$respuesta_ajax['total_factura_compra_retefuente']          = round($total_factura_compra_retefuente, 2);
	$respuesta_ajax['total_factura_compra']                     = round($total_factura_compra, 2);
	$respuesta_ajax['total_descuento']                          = round($total_descuento, 2);
	$respuesta_ajax['total_compra_imp']                         = round($total_compra_imp, 2);
	$respuesta_ajax['total_compra_producto']                    = round($total_compra_producto, 2);
	$respuesta_ajax['precio_costo_producto']                    = round($precio_costo_producto, 2);
	$respuesta_ajax['total_costo_producto']                     = round($total_costo_producto, 2);
	$respuesta_ajax['precio_ipc_total']                         = round($precio_ipc_total, 2);
	$respuesta_ajax['total_compra_producto_ant_desc']           = round($total_compra_producto_ant_desc, 2);
	$respuesta_ajax['und_compra']                               = $und_compra;
	$respuesta_ajax['incre']                                    = $incre;
	$respuesta_ajax['ganancia_ptj']                             = $ganancia_ptj;
...
if (($campo == 'und_caja') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc, precio_compra_producto_ant_desc
	FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
	$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
	$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
if (($campo == 'und_compra') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc, precio_compra_producto_ant_desc, cod_producto_barra
	FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
	$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
	$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
	...
	$datos_info_producto = "SELECT und_producto AS und_producto_antiguo,  precio_compra_producto AS precio_compra_producto_antiguo FROM tbl15_producto WHERE (cod_producto_barra = '$cod_producto_barra')";
	...
	$und_producto_antiguo                                       = ($info_producto['und_producto_antiguo']);
	$precio_compra_producto_antiguo                             = ($info_producto['precio_compra_producto_antiguo']);
	$total_precio_compra_producto_antiguo                       = ($und_producto_antiguo * $precio_compra_producto_antiguo);
	$und_producto_nuevo                                         = ($und_compra);
	...
	$precio_compra_producto_promedio                            = ($total_precio_compra_producto_antiguo + $total_precio_compra_producto_nuevo) / ($und_producto_antiguo + $und_producto_nuevo);

	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET precio_compra_producto_promedio = '$precio_compra_producto_promedio' WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	...
	$respuesta_ajax['precio_ipc_total']                         = round($precio_ipc_total, 2);
	$respuesta_ajax['total_compra_producto_ant_desc']           = round($total_compra_producto_ant_desc, 2);
	$respuesta_ajax['total_compra_gravada_factura']             = round($total_compra_gravada_factura, 2);
	$respuesta_ajax['total_compra_no_gravada_factura']          = round($total_compra_no_gravada_factura, 2);
	$respuesta_ajax['und_compra']                               = $und_compra;
	$respuesta_ajax['incre']                                    = $incre;
	$respuesta_ajax['ganancia_ptj']                             = $ganancia_ptj;
	$respuesta_ajax['mensaje']                                  = 'Datos actualizados correctamente.';
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'precio_costo_producto') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$precio_costo_producto                                      = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal                               = intval($_REQUEST['id']);
	$precio_compra_producto_promedio                            = 0;
	$precio_venta_producto_promedio                             = 0;

	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc, cod_producto_barra
	FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
	$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
	$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
	...
	$datos_info_temporal_compra_desc_total = "SELECT SUM(desc_total) AS desc_total, SUM(total_precio_compra_producto_unit_mas_iva) AS total_compra_factura, SUM(iva_total) AS iva_total 
	FROM tbl15_compra_producto_temporal WHERE (cod_info_factura_compra = '$cod_info_factura_compra')";
	$consulta_info_temporal_compra_desc_total = mysqli_query($conectar, $datos_info_temporal_compra_desc_total) or die(mysqli_error($conectar));
	$info_temporal_compra_desc_total = mysqli_fetch_assoc($consulta_info_temporal_compra_desc_total);
	...
	total_factura_compra_retefuente = '$total_factura_compra_retefuente', total_factura_compra = '$total_factura_compra', total_descuento = '$total_descuento', total_compra_imp = '$total_compra_imp', 
	total_compra_gravada_factura = '$total_compra_gravada_factura', total_compra_no_gravada_factura = '$total_compra_no_gravada_factura', total_descuento_factura = '$total_descuento_factura', 
	subtotal_factura = '$subtotal_factura', total_iva_factura = '$total_iva_factura', total_compra_factura = '$total_compra_factura' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	...
	$respuesta_ajax['incre']                                    = $incre;
	$respuesta_ajax['ganancia_ptj']                             = $ganancia_ptj;
	$respuesta_ajax['precio_compra_producto_promedio']          = round($precio_compra_producto_promedio, 2);
	$respuesta_ajax['precio_venta_producto_promedio']           = round($precio_venta_producto_promedio, 2);
	$respuesta_ajax['total_compra_gravada_factura']             = round($total_compra_gravada_factura, 2);
	$respuesta_ajax['total_compra_no_gravada_factura']          = round($total_compra_no_gravada_factura, 2);
	$respuesta_ajax['mensaje']                                  = 'Datos actualizados correctamente.';
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'precio_costo_producto_por_caja') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$precio_costo_producto_por_caja                             = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal                               = intval($_REQUEST['id']);
	$precio_compra_producto_promedio                            = 0;
	$precio_venta_producto_promedio                             = 0;
	...
	$precio_costo_producto                                      = ($precio_costo_producto_por_caja / $und_unidades);
	$precio_compra_producto_nuevo                               = $precio_costo_producto;
	...
	precio_compra_producto_desc_unit = '$precio_compra_producto_desc_unit', desc_unit = '$desc_unit', desc_total = '$desc_total', iva_unit = '$iva_unit', iva_total = '$iva_total', 
	precio_compra_producto_unit_mas_iva = '$precio_compra_producto_unit_mas_iva', total_precio_compra_producto_unit_mas_iva = '$total_precio_compra_producto_unit_mas_iva', 
	total_compra_gravada_producto = '$total_compra_gravada_producto', total_compra_no_gravada_producto = '$total_compra_no_gravada_producto' , precio_costo_producto_por_caja = '$precio_costo_producto_por_caja' 
	WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
if (($campo == 'precio_compra_producto') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$precio_compra_producto_desc_unit_dto1                      = ($precio_compra_producto * ($dto1/100));
	$precio_compra_producto_desc_unit_dto2                      = $precio_compra_producto_desc_unit_dto1 * ($dto2/100);
	$desc_unit                                                  = $precio_compra_producto_desc_unit_dto1 + $precio_compra_producto_desc_unit_dto2;
	$precio_compra_producto_desc_unit                           = $precio_compra_producto - $desc_unit;
	$desc_total                                                 = $desc_unit * $und_compra;
	$iva_unit                                                   = $precio_compra_producto_desc_unit * ($iva_ptj/100);
	$iva_total                                                  = $iva_unit * $und_compra;
	$precio_compra_producto_unit_mas_iva                        = $precio_compra_producto_desc_unit + ($precio_compra_producto_desc_unit * ($iva_ptj/100));
	$total_compra_producto                                      = ($precio_compra_producto_unit_mas_iva * $und_compra) + $precio_ipc_total;
	$total_precio_compra_producto_unit_mas_iva                  = $precio_compra_producto_unit_mas_iva * $und_compra;
	if ($iva_ptj == '0') { $total_compra_gravada_producto = 0; $total_compra_no_gravada_producto = $und_compra * $precio_compra_producto; } else { $total_compra_gravada_producto = $und_compra * $precio_compra_producto; $total_compra_no_gravada_producto = 0; }
...
if (($campo == 'iva_ptj') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET iva_ptj = '$iva_ptj', precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_compra_producto_ant_desc = '$precio_compra_producto_ant_desc', 
	total_compra_producto_ant_desc = '$total_compra_producto_ant_desc', descuento = '$descuento', total_dto = '$total_dto', ganancia_ptj = '$ganancia_ptj', 
	precio_compra_producto_desc_unit = '$precio_compra_producto_desc_unit', desc_unit = '$desc_unit', desc_total = '$desc_total', iva_unit = '$iva_unit', iva_total = '$iva_total', 
	precio_compra_producto_unit_mas_iva = '$precio_compra_producto_unit_mas_iva', total_precio_compra_producto_unit_mas_iva = '$total_precio_compra_producto_unit_mas_iva', 
...
if (($campo == 'dto1') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET dto1 = '$dto1', precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_compra_producto_ant_desc = '$precio_compra_producto_ant_desc', 
	total_compra_producto_ant_desc = '$total_compra_producto_ant_desc', descuento = '$descuento', total_dto = '$total_dto', ganancia_ptj = '$ganancia_ptj', 
	precio_compra_producto_desc_unit = '$precio_compra_producto_desc_unit', desc_unit = '$desc_unit', desc_total = '$desc_total', iva_unit = '$iva_unit', iva_total = '$iva_total', 
	precio_compra_producto_unit_mas_iva = '$precio_compra_producto_unit_mas_iva', total_precio_compra_producto_unit_mas_iva = '$total_precio_compra_producto_unit_mas_iva', 
...
if (($campo == 'dto2') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET dto2 = '$dto2', precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_compra_producto_ant_desc = '$precio_compra_producto_ant_desc', 
	total_compra_producto_ant_desc = '$total_compra_producto_ant_desc', descuento = '$descuento', total_dto = '$total_dto', ganancia_ptj = '$ganancia_ptj', 
	precio_compra_producto_desc_unit = '$precio_compra_producto_desc_unit', desc_unit = '$desc_unit', desc_total = '$desc_total', iva_unit = '$iva_unit', iva_total = '$iva_total', 
	precio_compra_producto_unit_mas_iva = '$precio_compra_producto_unit_mas_iva', total_precio_compra_producto_unit_mas_iva = '$total_precio_compra_producto_unit_mas_iva', 
...
if (($campo == 'precio_venta_producto') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$sql_factura_compra_temp = "SELECT precio_compra_producto, cod_producto_barra FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
	...
	$cod_producto_barra                                = $datos_factura_compra_temp['cod_producto_barra'];
	$campo_incre                                       = addslashes($_REQUEST['campo_incre']);
	$incre_explode                                     = explode($campo, $campo_incre);
	$incre                                             = $incre_explode[1];
	$ganancia_ptj                                      = intval((($precio_venta_producto - $precio_compra_producto) / $precio_compra_producto) * 100);

	if ($cod_estado_promediar_precio_compra_y_venta_cargar_factura_global == '1') {

		$datos_info_temporal = "SELECT AVG(precio_venta_producto) AS precio_venta_producto_promedio FROM tbl15_factura_compra_producto WHERE (cod_producto_barra = '$cod_producto_barra') ORDER BY fecha_seg_venta_producto DESC LIMIT 0, 3";
		$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
		$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);

		$precio_venta_producto_promedio                     = $info_temporal['precio_venta_producto_promedio'];

		$data_sql = ("UPDATE tbl15_compra_producto_temporal SET precio_venta_producto_promedio = '$precio_venta_producto_promedio' WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
		$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	}
...
if (($campo == 'ganancia_ptj') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$datos_info_temporal = "SELECT precio_compra_producto, cod_producto_barra, precio_compra_producto_promedio FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
	$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
	$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);

	$precio_compra_producto                 = $info_temporal['precio_compra_producto'];
	$cod_producto_barra                     = $info_temporal['cod_producto_barra'];
	$precio_compra_producto_promedio        = $info_temporal['precio_compra_producto_promedio'];
	$precio_venta_producto                  = $precio_compra_producto + ($precio_compra_producto * ($ganancia_ptj/100));
	$precio_venta_producto_promedio         = $precio_compra_producto_promedio + ($precio_compra_producto_promedio * ($ganancia_ptj/100));

	if ($cod_estado_promediar_precio_compra_y_venta_cargar_factura_global == '1') {

		$precio_venta_producto                  = $precio_venta_producto_promedio;

		$data_sql = ("UPDATE tbl15_compra_producto_temporal SET ganancia_ptj = '$ganancia_ptj', precio_venta_producto = '$precio_venta_producto', precio_venta_producto_promedio = '$precio_venta_producto_promedio' 
		WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
		$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	} else {
	...
	$respuesta_ajax['incre']                           = $incre;
	$respuesta_ajax['precio_venta_producto_promedio']  = $precio_venta_producto_promedio;
	$respuesta_ajax['precio_compra_producto_promedio'] = $precio_compra_producto_promedio;
	$respuesta_ajax['mensaje']                         = 'Datos actualizados correctamente.';
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'precio_venta_producto_por_caja') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$precio_venta_producto_por_caja           = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal             = intval($_REQUEST['id']);
	$precio_compra_producto_promedio          = 0;
	$precio_venta_producto_promedio           = 0;
	...
	$precio_venta_producto2                = ($precio_venta_producto_por_caja / $und_unidades);
	$campo_incre                           = addslashes($_REQUEST['campo_incre']);
	$incre_explode                         = explode($campo, $campo_incre);
	$incre                                 = $incre_explode[1];
	$ganancia_ptj                          = intval((($precio_venta_producto - $precio_compra_producto) / $precio_compra_producto) * 100);
	...
	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET precio_venta_producto_por_caja = '$precio_venta_producto_por_caja', precio_venta_producto = '$precio_venta_producto', 
	precio_venta_producto2 = '$precio_venta_producto2', ganancia_ptj = '$ganancia_ptj' WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc  
FROM tbl15_compra_producto_temporal WHERE (cod_compra_producto_temporal = '$cod_compra_producto_temporal')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
