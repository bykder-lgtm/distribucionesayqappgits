...
if (($campo=='cod_info_factura_venta') && ($tipo_ajax=='tbl15_info_factura_venta_notificacion_chatbot_telegram')) {
	...
	if (isset($_REQUEST['btn_origen'])) { $btn_origen = addslashes($_REQUEST['btn_origen']); } else{ $btn_origen = ''; }
	...
	$sql_administrador = "SELECT telefono, telefono1_tercero FROM tbl15_administrador WHERE (cod_administrador = '$cod_administrador_revisor')";
	$consulta_administrador = mysqli_query($conectar, $sql_administrador);
	$matriz_administrador = mysqli_fetch_assoc($consulta_administrador);

    $telefono_revisor                                    = $matriz_administrador['telefono'];
	...
	$datos_array['telefono_revisor'] = "".$telefono_revisor;
	$datos_array['telefono1_tercero'] = "".$telefono1_tercero;
	$datos_array['cod_estado_factura'] = "".$cod_estado_factura;
	$datos_array['btn_origen'] = "".$btn_origen;
