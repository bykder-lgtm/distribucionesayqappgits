...
function procesarImagen($file_key, $directorio_orig) {
    ...
    if (move_uploaded_file($_FILES[$file_key]['tmp_name'], $ruta_orig)) { 
        return array('orig' => $ruta_orig, 'min' => $ruta_orig); 
    }
    return array('orig' => null, 'min' => null);
}
// Construir SQL UPDATE con campos correctos de la tabla tbl15_tienda
$sql_update = "UPDATE tbl15_tienda SET nombre_tienda = UPPER('$nombre_tienda'), identificacion_tercero = '$identificacion_tercero', nombre1_tercero = UPPER('$nombre_tienda'), 
telefono1_tercero = '$telefono1_tercero', direccion_tercero = UPPER('$direccion_tercero'), correo_tercero = '$correo_tercero', cod_aliado_estrategico = '$cod_aliado_estrategico', 
