...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_PAGO_DETALLADO_A_ARRENDATARIOS_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_PAGO_DETALLADO_A_ARRENDATARIOS";
...
//-----------------------------------------------------------------------------------------------------//
// Headers
	$writer->addRow(array('CONTRATO', 'ARRENDATARIO | INMUEBLE', 'VALOR RECIBIDO', 'INTERESES', 'INGRESO JURIDICA (ARRIENDO)', 'FECHA REG PAGO', 'ID'));
// Then a foreach
    ...
    while ($datos_total_tipo_factura = mysqli_fetch_assoc($consulta_total_tipo_factura)) {
		...
		$writer->addRow(array($cod_factura, $nombre_tercero.' | '.$nombre_producto, $total_recibido, $monto_cuota_interes, $ingreso_gasto_juridica, date("d-m-Y", strtotime($fecha_pago_reg)), $cod_cuentas_cobrar_alerta));
	}
	$writer->addRow(array('','TOTAL' , $total_recibido_pago_inquilino, $total_ingreso_por_interes_inquilino, $total_otros_ingresos_juridica_arriendo));
