...
$modo_venta_por_defecto             = addslashes($_REQUEST['modo_venta_por_defecto']);
...
if ($cod_estado_ordenar_por_fecha_modificacion_caja_mesa_global == '1') { $ordenar_consulta = 'ORDER BY fecha_modificacion DESC'; } else { $ordenar_consulta = 'ORDER BY cod_prioridad DESC'; }
...
if ($cod_seguridad == '1') {
	if ($cod_estado_revisado_venta_temporal_global == '1') {
		$tabla_caja_mesa .= '		<th style="text-align:center;">.......</th>';
	}
}

if ($cod_estado_aviso_alerta_timbre_vendedor_pedido_cocina_global == '1') {
	$tabla_caja_mesa .= '		<th style="text-align:center;">....</th>';
}
...
if ($cod_estado_habilitar_total_venta_caja_mesa_virtual == '0') {
	$tabla_caja_mesa .= '		<th style="text-align:center;">TOTAL VENTA</th>';
}

if ($cod_estado_tipo_metodo_envio_global == '1') {
	$tabla_caja_mesa .= '		<th style="text-align:center;">ENVIO</th>';
}

$tabla_caja_mesa .= '		<th style="text-align:center;">CLIENTE</th>';
$tabla_caja_mesa .= '		<th style="text-align:center;">FECHA | HORA</th>';
$tabla_caja_mesa .= '		<th style="text-align:center;">APP</th>';

if ($cod_estado_posicion_mapa_gps_pedidos_info_venta_global == '1') {
	$tabla_caja_mesa .= '		<th style="text-align:center;">MAPA</th>';
}

$tabla_caja_mesa .= '		<th style="text-align:center;">TVV</th>';
$tabla_caja_mesa .= '		<th style="text-align:center;">ID</th>';

if ($cod_estado_eliminar_caja_mesa_virtual == '1') {
	$tabla_caja_mesa .= '		<th style="text-align:center;">ELIM</th>';
}
...
if (($cod_seguridad == '1') || ($cod_estado_facturacion_venta_acceso_facturas_otros_user == '1')) {
	...
	cod_tipo_servicio, descripcion_tipo_forma_pago
...
while ($datos = mysqli_fetch_assoc($consulta)) {
    ...
    $cod_tipo_servicio                             = $datos['cod_tipo_servicio'];
	...
	$sql_datos_venta_temp = "SELECT cod_venta_producto_temporal, cod_producto_barra, und_venta, nombre_producto, cod_estado_revisado, comentario_producto FROM tbl15_venta_producto_temporal 
	WHERE (cod_info_factura_venta = '$cod_info_factura_venta') ORDER BY cod_venta_producto_temporal DESC $limite_mostrar_registro";
	...
	while ($datos_venta_temp = mysqli_fetch_assoc($consulta_datos_venta_temp)) {
		...
		if ($cod_seguridad == '1') {
			if ($cod_estado_revisado_venta_temporal_global == '1') {
				$tabla_caja_mesa .= '		<td style="text-align:center;">'.$img_estado_revisado.'</td>';
			}
		}

		if ($cod_estado_aviso_alerta_timbre_vendedor_pedido_cocina_global == '1') {
		    if ($cod_estado_revisado_notificacion_vendedor == '1') {
		        $url_desactivar_timbre_aviso = '../admin/desactivar_timbre_aviso_reg.php?cod_info_factura_venta='.$cod_info_factura_venta.'&pagina='.$pagina;
		...
		$tabla_caja_mesa .= '		<td style="text-align:center;">'.$mostrar_objeto_desactivar_timbre_aviso.'</td>';
		}

		$tabla_caja_mesa .= '		<td style="text-align:center;"><h3>'.$cod_base_caja.'</h3></td>';


		$tabla_caja_mesa .= '		<td style="text-align:left;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$cuenta_usuario.'</a></td>';
		$tabla_caja_mesa .= '		<td style="text-align:left;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$nombre_producto_concat.$observacion.'</a></td>';
		$tabla_caja_mesa .= '		<td style="text-align:left;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$nombres_domiciliario.'</a></td>';
		$tabla_caja_mesa .= '		<td style="text-align:left;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$observacion_tercero.'</a></td>';

		if ($cod_estado_prioridad_caja_mesa_global == '1') {
			$tabla_caja_mesa .= '<td style="text-align:center"><input type="number" onFocus="Focus(this.id, this.value)" onBlur="Blur(this.id, this.value, "cod_prioridad", '.$cod_info_factura_venta.')" id="'.$cod_info_factura_venta.'" value="'.$cod_prioridad.'" class="input-block-level" style="width: 60px;"></td>';
		}

		if ($cod_estado_habilitar_total_venta_caja_mesa_virtual == '0') {
			$tabla_caja_mesa .= '		<td style="text-align:center;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.number_format($total_venta_producto_ciclo, 0, ",", ".").'</a></td>';
		}

		if ($cod_estado_tipo_metodo_envio_global == '1') {
			$tabla_caja_mesa .= '		<td style="text-align:center;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$nombre_tipo_metodo_envio.'</a></td>';
		}

		$tabla_caja_mesa .= '		<td style="text-align:left;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$cliente.$nombre_cliente_visitante.'</a></td>';
		$tabla_caja_mesa .= '		<td style="text-align:center;"><a href="../admin/entrar_sesion_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina_redirect.'">'.$fecha_anyo.' | '.$fecha_hora.'</a></td>';
		$tabla_caja_mesa .= '		<td style="text-align:center;">'.$nombre_tipo_aplicacion.'</td>';

		if ($cod_estado_posicion_mapa_gps_pedidos_info_venta_global == '1') {
			$tabla_caja_mesa .= '		<td style="text-align:center"><a href="#" class="" title="Ver" onclick="obtener_datos_mapa_modal('.$cod_info_factura_venta.');" data-toggle="modal" data-target=".abrir_modal_mapa_visita"><img src="../imagenes/btn_mapa.png" class="img-polaroid" alt=""></a></td>';
		}

		$tabla_caja_mesa .= '		<td style="text-align:center;">'.$descripcion_tipo_forma_pago.'</td>';
		$tabla_caja_mesa .= '		<td style="text-align:center;">'.$cod_info_factura_venta.'</td>';


		if ($cod_estado_eliminar_caja_mesa_virtual == '1') {
			$tabla_caja_mesa .= '		<td style="text-align:center;"><a href="../admin/eliminar_caja_virtual.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&cod_base_caja='.$cod_base_caja.'&pagina='.$pagina.'"><img src=../imagenes/eliminar.png></td>';
		}
