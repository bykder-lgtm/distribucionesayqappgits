<?php 
$nombre_pagina          = "Resultados de SimulaciÃ³n";
$cod_seguridad_pag      = "1";
...
if (isset($_REQUEST['nombre_tipo_origen_simulacion'])) { 
    ...
    // Para vendedor: obtener el cod_aliado_estrategico desde su perfil
    $sql_aliado_vendedor = "SELECT cod_aliado_estrategico FROM tbl15_administrador WHERE cod_administrador = '$cod_administrador'";
    $consulta_aliado_vendedor = mysqli_query($conectar, $sql_aliado_vendedor);
    $datos_aliado_vendedor = mysqli_fetch_assoc($consulta_aliado_vendedor);
    $cod_aliado_estrategico = $datos_aliado_vendedor['cod_aliado_estrategico'];
    //---------------------------------------------------------------------------------------------------------------------------------//
    // Determinar origen de la simulaciÃ³n
    //---------------------------------------------------------------------------------------------------------------------------------//
    if ($nombre_tipo_origen_simulacion == 'TIENDA_VIRTUAL' || $nombre_tipo_origen_simulacion == 'TIENDA_VIRTUAL_MOVIL') {
        ...
        // Solo aplicar interÃ©s si es precio de contado (cod_tipo_simulacion_credito == 1)
        if ($cod_tipo_simulacion_credito == 1 || $cod_tipo_simulacion_credito == 0) {
            ...
            // Si es precio a crÃ©dito, el valor ya incluye el interÃ©s
        ...
        // Para simulaciÃ³n libre, obtener el cod_aliado_estrategico desde la sesiÃ³n del usuario
        // Si el usuario es un aliado (cod_seguridad == 23), usar su cod_administrador
        /*
        if ($cod_seguridad == '23') {
            $cod_aliado_estrategico                                   = $cod_administrador;
        } else {
            // Si no es aliado, intentar obtener desde el parÃ¡metro o usar valor por defecto
            if (isset($_REQUEST['cod_aliado_estrategico'])) {
                $cod_aliado_estrategico                               = intval($_REQUEST['cod_aliado_estrategico']);
            } else {
                // Si no hay parÃ¡metro, obtener del administrador en sesiÃ³n
                $cod_aliado_estrategico                               = $cod_administrador;
            }
        }
        */
    ...
    } elseif ($cod_seguridad == '2') { //VENDEDOR - usa config del aliado
        $nombre_compo_interes_ptj = 'aliado_estrategico_interes_ptj';
        $nombre_compo_aval_ptj = 'aliado_estrategico_aval_ptj';
    } else { //DEFAULT
        $nombre_compo_interes_ptj = 'aliado_estrategico_interes_ptj';
...
$(document).ready(function(){
    ...
    <!-- Header de Resultados -->
        <!--
        <div class="resultado-header-simulador"><h1><i class="fa fa-check-circle"></i> Resultados de tu SimulaciÃ³n</h1><p>Elige la mejor opciÃ³n de financiamiento</p></div>
        -->
        <div class="info-producto-card-simulador acordeon-simulador">
            <div class="acordeon-header" onclick="toggleAcordeon(this)">
                <h3><i class="fa fa-info-circle"></i> InformaciÃ³n del CrÃ©dito</h3>
                <div class="info-item-simulador">
                    <i class="fa fa-credit-card"></i>
                    <span class="label">Valor a crÃ©dito: </span>
            ...
            <div class="acordeon-content" style="display: none;">
                ...
                <div class="info-item-simulador">
                    ...
                    <span class="label">Tipo de SimulaciÃ³n:</span>
        ...
        <!-- LÃ­neas de CrÃ©dito -->
        <?php
        // Consulta con JOIN para obtener las lÃ­neas de crÃ©dito parametrizadas para este aliado
        ...
        <?php
            while ($datos_parametrizacion_entidad_crediticia_aliado = mysqli_fetch_assoc($consulta_parametrizacion_entidad_crediticia_aliado)) {
                ...
                // Usar el interÃ©s de la tabla de parametrizaciÃ³n (especÃ­fico del aliado)
                //$interes_ptj_aliado                                               = $datos_parametrizacion_entidad_crediticia_aliado['interes_ptj'];
                ...
                //---------------------------------------------------------------------------------------------------------------------------------//
                // Usar el interÃ©s de la parametrizaciÃ³n del aliado (si existe), sino usar el de la entidad
                ...
                //---------------------------------------------------------------------------------------------------------------------------------//
                if ($cod_tipo_simulacion_credito == 1 || $cod_tipo_simulacion_credito == 0) { // OPCIÃ“N 1: Calcular crÃ©dito CON intereses (partir del valor de contado)
                ...
                } else { // OPCIÃ“N 2: Calcular crÃ©dito con PRECIO A CRÃ‰DITO (el valor ingresado ya incluye intereses)
                    ...
                    $cuota_credito                                                 = $total_pagar / $numero_cuotas;
                    $text_label_valor                                              = 'Valor a CrÃ©dito';
                }
        ?>
        <div class="entidad-card-simulador">
            ...
            <!-- Valor del CrÃ©dito -->
            <div class="valor-credito-simulador">ðŸ’° $<?php echo number_format($total_pagar, 0, ",", ".") ?></div>
            ...
            <form name="formulario_<?php echo $cod_entidad_crediticia_db ?>" method="GET" action="../admin/reg_siscredito_tercero_cliente_simulador_por_cuotas_max_entidad_crediticia.php">
                ...
                <div class="cuotas-selector-simulador">
                    <label for="cuotas_<?php echo $cod_entidad_crediticia_db ?>"><i class="fa fa-calendar-alt"></i> NÃºmero de cuotas: </label>
                    <?php if ($cod_estado_aplicar_cuota_max == '1') { 
                        // Para Bancolombia (cod_entidad_crediticia = 1), cuotas fijas segÃºn meses_max_entidad_crediticia
                        $cuotas_fijas_bancolombia = $meses_max_entidad_crediticia;
                        $numero_cuotas = $cuotas_fijas_bancolombia; // Forzar nÃºmero de cuotas
                    ?>
...
<?php } else { ?>
    <main class="container py-5">
        <div class="no-resultados-simulador">
            ...
            <h3>Error en la simulaciÃ³n</h3>
            <p>No se recibieron los datos necesarios para calcular el crÃ©dito.</p>
...
?>
<!-- Modal de Solicitud de CrÃ©dito -->
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content modal-solicitud-credito">
            <div class="modal-header-solicitud">
                <h5 class="modal-title-solicitud" id="modalSolicitudLabel"><i class="fa fa-file-text"></i> Solicitud de CrÃ©dito</h5>
                <button type="button" class="close-modal-solicitud" data-bs-dismiss="modal" aria-label="Cerrar">
                    <span style="font-size: 1.5rem; line-height: 1; font-weight: 300;">Ã—</span>
                </button>
            </div>
            <div class="modal-body-solicitud">
                <form id="formSolicitudCredito" method="POST">

                    <!-- Resumen del CrÃ©dito y Producto/CategorÃ­a -->
                    <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); padding: 1.5rem; border-radius: 12px; margin-bottom: 1.5rem;">
                        <!-- Resumen del CrÃ©dito -->
                        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
                            <div style="text-align: center;">
                                <div style="color: rgba(255, 255, 255, 0.9); font-size: 0.75rem; font-weight: 600; text-transform: uppercase; margin-bottom: 0.25rem;">TOTAL CRÃ‰DITO:</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-0">
                                <label style="color: white; font-size: 0.85rem; font-weight: 600; margin-bottom: 0.5rem; display: flex; align-items: center;"><i class="fa fa-tags" style="margin-right: 0.5rem;"></i>CategorÃ­a *</label>
                                <select name="cod_categoria" id="cod_categoria" class="form-control-solicitud" required style="background: white !important; border: 2px solid rgba(255, 255, 255, 0.5); color: #2d3748 !important; font-size: 0.95rem; font-weight: 600; text-shadow: none;">
                                    <option value="" style="background: white; color: #2d3748;">-- Selecciona una categorÃ­a --</option>
                                    ...
                                    while ($datos2 = mysqli_fetch_assoc($consulta2)) {
                                        ...
                                        echo "<option value='".$codigo."' $seleccionado style='background: #d97706; color: white;'>".$nombre."</option>"; } ?>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control-solicitud" name="identificacion_tercero" id="modal_credito_identificacion_tercero" placeholder="NÃºmero de identificaciÃ³n" required>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="tel" class="form-control-solicitud" name="telefono1_tercero" id="modal_credito_telefono1_tercero" placeholder="NÃºmero de celular" required>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="label-form-solicitud"><i class="fa fa-map-marker-alt"></i> DirecciÃ³n *</label>
                            <input type="text" class="form-control-solicitud" name="direccion_tercero" id="modal_credito_direccion_tercero" placeholder="DirecciÃ³n de residencia" required>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            <div class="form-check" style="display: flex; align-items: start; padding: 1rem; background: #f7fafc; border-radius: 8px; border: 2px solid #e2e8f0;">
                                ...
                                <label class="form-check-label" for="modal_credito_acepta_tratamiento_datos" style="cursor: pointer; font-size: 0.9rem; color: #2d3748; line-height: 1.5;">
                                    <i class="fa fa-shield-alt" style="color: #f59e0b; margin-right: 0.5rem;"></i>
                                    <strong>Acepto el tratamiento de datos personales *</strong>
                                    <small style="display: block; color: #718096; margin-top: 0.25rem;">Autorizo el tratamiento de mis datos personales de acuerdo con la polÃ­tica de privacidad.</small>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            <div class="form-check" style="display: flex; align-items: start; padding: 1rem; background: #f7fafc; border-radius: 8px; border: 2px solid #e2e8f0;">
                                ...
                                <label class="form-check-label" for="modal_credito_acepta_terminos_condiciones" style="cursor: pointer; font-size: 0.9rem; color: #2d3748; line-height: 1.5;">
                                    <i class="fa fa-file-contract" style="color: #f59e0b; margin-right: 0.5rem;"></i>
                                    <strong>Acepto los tÃ©rminos y condiciones *</strong>
                                    <small style="display: block; color: #718096; margin-top: 0.25rem;">He leÃ­do y acepto los tÃ©rminos y condiciones del servicio de crÃ©dito.</small>
                                </label>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function(){
    ...
    $('form[name^="formulario_"]').submit(function(e){
        ...
        if(!select_cod_meses_credito.val()){
            e.preventDefault();
            alert('Por favor selecciona el nÃºmero de cuotas');
            return false;
        }
    });
});
...
$(document).ready(function() {
    ...
    // Abrir modal al hacer clic en el botÃ³n de solicitar
    $(document).on('click', '.btn-abrir-modal-solicitud', function(e) {
        e.preventDefault();
        // Obtener los datos del botÃ³n
        ...
        // Obtener el nÃºmero de cuotas - primero del input hidden, luego del select
        ...
        if (!codMesesCredito) { alert('Por favor selecciona el nÃºmero de cuotas antes de solicitar el crÃ©dito'); return false; }
        ...
        // Extraer solo el valor numÃ©rico de la cuota (quitar $, puntos y espacios)
        ...
        // Rellenar el resumen del crÃ©dito en el modal
        ...
        $('#cuotaCreditoHidden').val(cuotaValor); // Usar el valor numÃ©rico limpio
...
$(document).ready(function() {
    ...
    $('#formSolicitudCredito').on('submit', function(e) {
        ...
        // Obtener el formulario especÃ­fico del modal
        ...
        if (!documento || documento === '' || documento.length === 0) { errorMessage += '- Ingrese el nÃºmero de documento\n'; isValid = false; }
        if (!nombre1 || nombre1 === '') { errorMessage += '- Ingrese el primer nombre\n'; isValid = false; }
        if (!apellido1 || apellido1 === '') { errorMessage += '- Ingrese el primer apellido\n'; isValid = false; }
        if (!celular) { errorMessage += '- Ingrese el nÃºmero de celular\n'; isValid = false; } else if (celular.length < 10) { errorMessage += '- El celular debe tener al menos 10 dÃ­gitos\n'; isValid = false; }
        
        if (!correo) {
            errorMessage += '- Ingrese el correo electrÃ³nico\n'; isValid = false; } else {
        ...
        if (!direccion) { errorMessage += '- Ingrese la direcciÃ³n\n'; isValid = false; }
        if (!acepta_tratamiento_datos) { errorMessage += '- Debe aceptar el tratamiento de datos personales\n'; isValid = false; }
        if (!acepta_terminos_condiciones) { errorMessage += '- Debe aceptar los tÃ©rminos y condiciones\n'; isValid = false; }
        if (!isValid) { alert('Por favor complete los siguientes campos:\n\n' + errorMessage); return false; }
        // Si todo estÃ¡ correcto, mostrar mensaje de procesamiento y enviar por AJAX
        ...
        $.ajax({
            type: "POST",
            url: "../admin/reg_simulador_por_cuotas_vendedor_movil_ajax_reg.php",
            data: formData,
            beforeSend: function() {
                //console.log("Enviando solicitud de crÃ©dito...");
            },
...
<style>
    ...
    /* AnimaciÃ³n para mensajes de verificaciÃ³n */
    @keyframes slideDown {
        from {
            opacity: 0;
    ...
    /* Estilos para inputs y selects en fondo pÃºrpura */
