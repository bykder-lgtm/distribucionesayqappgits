...
$(function () {
    $('#container_grafico_estadistico').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto, cod_productos, nombre_productos FROM tbl15_venta_producto GROUP BY cod_productos ORDER BY total_venta_producto ASC LIMIT 0,100";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto FROM tbl15_venta_producto GROUP BY cod_productos ORDER BY total_venta_producto ASC LIMIT 0,100";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
