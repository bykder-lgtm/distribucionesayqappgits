...
<table class="table table-striped" cellspacing="0" cellpadding="20">
  <tr>
    ...
    <td style="text-align:left;"><input class="input-block-level" name="fecha_operacion_ini" type="date" value="<?php echo $fecha_operacion_ini ?>" required/></td>
  </tr>
  <tr>
    <td style="text-align:right;">FECHA FIN: </td>
    <td style="text-align:left;"><input class="input-block-level" name="fecha_operacion_fin" type="date" value="<?php echo $fecha_operacion_fin ?>" required/></td>
  </tr>
  <tr>
...
if (isset($_GET['fecha_operacion_ini'])) {
  ...
  $sql_cliente = "SELECT cod_venta_producto, cod_producto, cod_producto_barra, cod_info_factura_venta, cod_factura, nombre_producto, 
  und_venta, precio_compra_producto, total_compra_producto, precio_venta_producto, total_venta_producto, nombre_tipo_producto, nombre_tipo_unidad_medida, fecha_seg_venta_producto, 
  nombre_tipo_presentacion, und_producto_inv, fecha_ymd_venta_producto, fecha_hora_venta_producto, cod_administrador, comentario_producto, cuenta, und_producto, cod_tercero
  FROM tbl15_venta_producto WHERE (fecha_ymd_venta_producto BETWEEN '$fecha_operacion_ini' AND '$fecha_operacion_fin') $filtro_cod_producto_barra ORDER BY cod_venta_producto ASC";
  ...
  while ($info_cliente = mysqli_fetch_assoc($resultado_cliente)) {
    ...
    $cod_tercero_db                                     = $info_cliente['cod_tercero'];

    $sql_tercero = "SELECT nombre1_tercero FROM tbl15_tercero WHERE cod_tercero = '$cod_tercero_db'";
  ...
  <?php if ($total_reg <> '0') { ?>
    ...
    foreach ($datos_vectores as $clave=>$value) {
      ...
      <tr>
        ...
        <td style="text-align:center"><?php echo $id_operacion?></td>
      </tr>
    <?php } ?>
    </tbody>
    </table>
