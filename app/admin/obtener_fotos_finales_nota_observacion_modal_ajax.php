...
$html = '';
$sql_nota_observacion = "SELECT * FROM tbl15_nota_observacion WHERE (cod_info_factura_venta = '$cod_info_factura_venta') AND (cod_estado_obligatorio2 = '1' OR url_img_orig_producto = '') AND (codigo_estado_revision <> '2') ORDER BY cod_posicion ASC";
...
while ($matriz_nota_observacion = mysqli_fetch_assoc($consulta_nota_observacion)) {
    ...
    $nombre_tipo_recorte_foto                       = $matriz_nota_observacion['nombre_tipo_recorte_foto'];
    ...
    if ($cod_estado_obligatorio == '1') { 
        ...
        $container_style = "border: 2px solid #ffc107; border-radius: 8px; padding: 1.5rem; background-color: #fffbf0; border-left: 4px solid #10162D;";
        $titulo_style = "font-weight: 600; color: #10162D; margin-bottom: 1rem; text-align: center;";
    } else { 
        $imagen_obligatoria = ''; 
    ...
    if ($cod_estado_obligatorio2 == '1') { 
        $imagen_obligatoria2 = '*'; 
        // Estilo sutil para imágenes obligatorias
        $container_style2 = "border: 2px solid #ffc107; border-radius: 8px; padding: 1.5rem; background-color: #fffbf0; border-left: 4px solid #10162D;";
        $titulo_style2 = "font-weight: 600; color: #10162D; margin-bottom: 1rem; text-align: center;";
    } else { 
        $imagen_obligatoria2 = ''; 
        // Estilo normal para imágenes opcionales
    ...
    // Establecer el tipo de recorte (por defecto CEDULA si está vacío)
    $tipo_recorte = !empty($nombre_tipo_recorte_foto) ? $nombre_tipo_recorte_foto : 'CEDULA';
    ...
    if (!empty($url_img_min_producto)) {
        ...
        $btn_camara_html = "<button type=\"button\" id=\"btn_activar_camara_{$cod_nota_observacion}\" class=\"btn_activar_camara btn flex-grow-1\" data-tipo-recorte=\"{$tipo_recorte}\"><i class=\"fa fa-camera\" style=\"margin-right: 8px;\"></i>Cámara</button>";
