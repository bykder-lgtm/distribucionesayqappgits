...
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
$calcular_datos_cuenta_pagar = "SELECT identificacion_tercero, nombre_tipo_identificacion, nombre1_tercero, apellido1_tercero, total_monto_deuda_cuenta_pagar, total_subtotal_cuenta_pagar, total_abonado_cuenta_pagar 
...
$nombre_tipo_identificacion                        = $datos_cuenta_pagar['nombre_tipo_identificacion'];
...
if ($tamano_papel_impresora == '58') {
	...
	$printer->setJustification(Printer::JUSTIFY_LEFT);
	$printer->setEmphasis(false);
	$printer->text(str_pad($nombre_tipo_identificacion." PROVEEDOR: ".$identificacion_tercero, 20));
	...
	$printer->text("ABONOS Y CUENTAS CUENTA POR PAGAR (PROVEEDOR)"."\n");
	...
	$printer->setJustification(Printer::JUSTIFY_LEFT);
	$printer->setEmphasis(false);
	$printer->text(str_pad($nombre_tipo_identificacion." PROVEEDOR: ".$identificacion_tercero, 25));
	$printer->text(str_pad("NOMBRE PROVEEDOR: ".utf8_decode($nombre_cliente), 25));
	$printer->text("\n");
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	$printer->text("FACTURAS EN CREDITO PROVEEDOR"."\n");
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	$printer->setJustification(Printer::JUSTIFY_CENTER);
	$printer->text("<<=========================================================>>");
	...
	tbl15_cuentas_pagar.mensaje, tbl15_cuentas_pagar.fecha_pago, tbl15_cuentas_pagar.fecha, tbl15_cuentas_pagar.vendedor, tbl15_cuentas_pagar.cod_info_factura_compra
	...
	while ($datos_cuenta_pagar = mysqli_fetch_assoc($consulta_datos_cuenta_pagar)) {
		...
		$printer->text(str_pad($cod_cuentas_pagar, 10));
		$printer->text("\n");
	}
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	while ($datos = mysqli_fetch_assoc($consulta)) {
		...
		$printer->text(str_pad($cod_cuentas_pagar_abonos, 10));
		$printer->text("\n");
	}
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
