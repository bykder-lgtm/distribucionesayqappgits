...
$cod_info_factura_transferencia_bodega             = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET cod_historia_clinica = '$cod_historia_clinica' WHERE cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega'");
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET fecha_ymd_venta_producto = '$fecha_ymd_venta_producto', fecha_mes_venta_producto = '$fecha_mes_venta_producto', 
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_tipo_inventario') && ($tipo_ajax=='tbl15_info_factura_transferencia_bodega')) {
$cod_tipo_inventario                = intval($_REQUEST['valor']);
$cod_info_factura_transferencia_bodega             = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia_bodega SET cod_tipo_inventario = '$cod_tipo_inventario' WHERE cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_tipo_producto_consumo') && ($tipo_ajax=='tbl15_info_factura_transferencia_bodega')) {
$cod_tipo_producto_consumo               = addslashes($_REQUEST['valor']);
$cod_info_factura_transferencia_bodega             = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia_bodega SET cod_tipo_producto_consumo = '$cod_tipo_producto_consumo' WHERE cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET cod_administrador = '$cod_administrador' WHERE cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_tipo_moneda') && ($tipo_ajax=='tbl15_info_factura_transferencia_bodega')) {
...
if (($campo=='und_venta') && ($tipo_ajax=='tbl15_transferencia_bodega_producto_temporal')) {
    ...
    $datos_info_temporal = "SELECT precio_compra_producto, precio_venta_producto FROM tbl15_transferencia_bodega_producto_temporal WHERE (cod_transferencia_bodega_producto_temporal = '$cod_transferencia_bodega_producto_temporal')";
    ...
    $data_sql = ("UPDATE tbl15_transferencia_bodega_producto_temporal SET und_venta = '$und_venta', total_compra_producto = '$total_compra_producto', total_venta_producto = '$total_venta_producto', 
...
if (($campo=='cod_historia_clinica') && ($tipo_ajax=='tbl15_info_factura_transferencia_bodega')) {
$cod_historia_clinica         = intval($_REQUEST['valor']);
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_producto_barra') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$cod_producto_barra           = addslashes($_REQUEST['valor']);
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET cod_producto_barra = '$cod_producto_barra' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_producto') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$nombre_producto              = addslashes(strtoupper($_REQUEST['valor']));
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET nombre_producto = '$nombre_producto' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_venta') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$und_venta                    = addslashes($_REQUEST['valor']);
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);
...
cod_tipo_pago, cod_info_factura_transferencia_bodega, cod_factura, precio_costo_producto, precio_venta_producto, und_venta, iva_ptj, fecha_ymd_venta_producto, fecha_hora_venta_producto, cuenta 
FROM tbl15_transferencia_bodega_producto WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'";
$resultado_datos = mysqli_query($conectar, $sql_datos);
$info_datos = mysqli_fetch_assoc($resultado_datos);
...
$cod_info_factura_transferencia_bodega         = $info_datos['cod_info_factura_transferencia_bodega'];
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET und_venta = '$und_venta', total_costo_producto = '$total_costo_producto', total_compra_producto = '$total_compra_producto' , total_venta_producto = '$total_venta_producto'
WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$agregar_regis = sprintf("UPDATE tbl15_info_factura_transferencia_bodega SET total_precio_compra = '$total_precio_compra', total_precio_venta = '$total_precio_venta', 
total_datos_data = '$total_datos_data' WHERE cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega'");
...
if ($cod_tipo_pago == '2') {
$sql_total_deuda_credito = "SELECT SUM(total_venta_producto) AS monto_deuda FROM tbl15_transferencia_bodega_producto WHERE (cod_info_factura_transferencia_bodega = '$cod_info_factura_transferencia_bodega')";
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$precio_venta_producto        = intval($_REQUEST['valor']);
$cod_venta_producto           = intval($_REQUEST['id']);

$datos_info_temporal = "SELECT und_venta FROM tbl15_transferencia_bodega_producto WHERE (cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET precio_venta_producto = '$precio_venta_producto', total_venta_producto = '$total_venta_producto'
WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_audiometria') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$und_audiometria              = intval($_REQUEST['valor']);
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);

$datos_info_temporal = "SELECT precio_venta_producto FROM tbl15_transferencia_bodega_producto WHERE (cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET und_audiometria = '$und_audiometria', total_venta_audiometria = '$total_venta_audiometria',  
total_venta_producto = '$total_venta_producto' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_optometria') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET und_optometria = '$und_optometria', total_venta_optometria = '$total_venta_optometria',  
total_venta_producto = '$total_venta_producto' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_electrocardiograma') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
...
$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET und_electrocardiograma = '$und_electrocardiograma', total_venta_electrocardiograma = '$total_venta_electrocardiograma',  
total_venta_producto = '$total_venta_producto' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_cliente') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$nombre_cliente               = addslashes(strtoupper($_REQUEST['valor']));
$cod_transferencia_bodega_producto           = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET nombre_cliente = '$nombre_cliente' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='fecha_alerta') && ($tipo_ajax=='tbl15_transferencia_bodega_producto')) {
$fecha_alerta                = addslashes(($_REQUEST['valor']));
$cod_transferencia_bodega_producto_temporal = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_transferencia_bodega_producto SET fecha_alerta = '$fecha_alerta' WHERE cod_transferencia_bodega_producto = '$cod_transferencia_bodega_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
