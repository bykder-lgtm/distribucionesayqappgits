...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_GASTOS_POR_REPARACIONES_ARRENDATARIOS_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_GASTOS_POR_REPARACIONES_ARRENDATARIOS";
...
//-----------------------------------------------------------------------------------------------------//
// Headers
	$writer->addRow(array('CONTRATO', 'ARRENDATARIO | INMUEBLE', 'CONCEPTO', 'DESCRIPCION', 'COSTO FINAL (P.VENTA)', 'COSTO ADMINISTRACION DE REPARACION (P.COMPRA)', 'GANANCIA', 'FECHA', 'ID'));
// Then a foreach
	...
	while ($datos_gasto_inmueble_inquilino_venta = mysqli_fetch_assoc($consulta_gasto_inmueble_inquilino_venta)) {
		...
		$writer->addRow(array($cod_factura, $nombre_tercero.' | '.$nombre_producto, $nombre_gasto_inmueble_detalle, $descripcion_gasto_inmueble_detalle, $precio_venta_producto, $precio_compra_producto, $total_ganancia, date("d-m-Y", strtotime($fecha_gasto_inmueble_detalle)), $cod_info_gasto_inmueble_inquilino_venta));
	}
	$writer->addRow(array('', '', '', 'TOTAL', $total_precio_venta_producto, $total_precio_compra_producto, $total_ingreso_por_reparaciones));
