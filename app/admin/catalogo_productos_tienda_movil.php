...
<body>
<?php
if (isset($_GET['cod_tienda'])) { $cod_tienda = intval( $_GET['cod_tienda']); } else { $cod_tienda = ''; }
if (isset($_GET['buscador'])) { $buscador_get = addslashes($_GET['buscador']); } else { $buscador_get = ''; }

// Obtener el interés predeterminado
$sql_entidad_crediticia_predeterminada = "SELECT nombre_tienda FROM tbl15_tienda WHERE cod_tienda = '$cod_tienda'";
...
$nombre_tienda = $matriz_entidad_crediticia['nombre_tienda'];
?>

<?php include_once("../admin/01_modulo_encabezado_superior_movil.php"); ?>
...
<?php
$contador = 0;
if (isset($_GET['cod_tienda']) && $_GET['cod_tienda'] != '') { $cod_tienda = intval($_GET['cod_tienda']); }
...
$sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    ...
    cod_categoria, cod_estado, cod_tienda FROM tbl15_producto WHERE (cod_tienda = '$cod_tienda' AND cod_estado = '1')";
