...
//-------------------------------------- -----------------------------------------------------------------//
$sql_calcular_cod_abono_global = "SELECT MAX(cod_abono_global) AS cod_abono_global FROM tbl15_cuentas_pagar_abonos";
...
WHERE (tbl15_cuentas_pagar.cod_tercero='$cod_tercero') AND (tbl15_cuentas_pagar.subtotal > 0) ORDER BY tbl15_cuentas_pagar.subtotal ASC LIMIT 0,1";
...
$subtotal_db               = $datos_cuenta_pagar['subtotal'];
$cod_factura_db            = $datos_cuenta_pagar['cod_factura'];

if ($abonado_global >= $subtotal_db) {
$smtr_val                  = 0;
$smtr_abonar               = 0;
//-------------------------------------- -----------------------------------------------------------------//
foreach($_POST["cod_factura"] as $key => $cod_factura) {

$sql_cuenta_pagar = "SELECT monto_deuda, abonado, subtotal, cod_tercero FROM tbl15_cuentas_pagar WHERE (cod_factura='$cod_factura')";
$consulta_datos_cuenta_pagar = mysqli_query($conectar, $sql_cuenta_pagar) or die(mysqli_error($conectar));
$datos_cuenta_pagar = mysqli_fetch_assoc($consulta_datos_cuenta_pagar);

$monto_deuda                   = $datos_cuenta_pagar['monto_deuda'];
$abonado                       = $datos_cuenta_pagar['abonado'];
$abonar                        = $datos_cuenta_pagar['subtotal'];
...
$agreg_reg = "INSERT INTO tbl15_cuentas_pagar_abonos (cod_tercero, cod_factura, abonado, cuenta, fecha_pago, fecha_anyo, 
fecha_mes, anyo, fecha_invert, hora, mensaje, cod_abono_global, cod_administrador, cod_tipo_forma_pago) 
VALUES ('$cod_tercero', '$cod_factura', '$abonar', '$cuenta_actual', '$fecha_pago', '$fecha_anyo', 
'$fecha_mes', '$anyo', '$fecha_invert', '$hora', '$mensaje', '$cod_abono_global', '$cod_administrador', '$cod_tipo_forma_pago')";
$resultado_tbl15_cuentas_pagar_abonos = mysqli_query($conectar, $agreg_reg) or die(mysqli_error($conectar));
//-------------------------------------- -----------------------------------------------------------------//
...
$sql_tot_abono_fact = "SELECT SUM(abonado) AS abonado FROM tbl15_cuentas_pagar_abonos WHERE cod_factura = '$cod_factura'";
...
?>
<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/cuentas_pagar_abono_global_opcion_imprimir.php?cod_abono_global=<?php echo $cod_abono_global ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>">
<?php
}
// FIN CONDICIONAL ($abonado_global >= $subtotal_db)
} else {
$sql_autoincremento_tbl15_cuentas_pagar_abonos = "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$base_datos' AND TABLE_NAME = 'tbl15_cuentas_pagar_abonos'";
$exec_autoincremento_tbl15_cuentas_pagar_abonos = mysqli_query($conectar, $sql_autoincremento_tbl15_cuentas_pagar_abonos) or die(mysqli_error($conectar));
$datos_autoincremento_tbl15_cuentas_pagar_abonos = mysqli_fetch_assoc($exec_autoincremento_tbl15_cuentas_pagar_abonos);
$cod_cuentas_pagar_abonos = $datos_autoincremento_tbl15_cuentas_pagar_abonos['AUTO_INCREMENT'];
//-------------------------------------- -----------------------------------------------------------------//
//-------------------------------------- REGISTRAR DATOS --------------------------------------//
$agregar_reg_tbl15_cuentas_pagar_abonos = "INSERT INTO tbl15_cuentas_pagar_abonos (cod_tercero, cod_factura, abonado, cuenta, fecha_pago, fecha_anyo, 
fecha_mes, anyo, fecha_invert, hora, mensaje, cod_administrador, cod_tipo_forma_pago) 
VALUES ('$cod_tercero', '$cod_factura_db', '$abonado_global', '$cuenta_actual', '$fecha_pago', '$fecha_anyo', 
'$fecha_mes', '$anyo', '$fecha_invert', '$hora', '$mensaje', '$cod_administrador', '$cod_tipo_forma_pago')";
$resultado_tbl15_cuentas_pagar_abonos = mysqli_query($conectar, $agregar_reg_tbl15_cuentas_pagar_abonos) or die(mysqli_error($conectar));
//-------------------------------------- REGISTRAR DATOS --------------------------------------//
$sql_cuenta_pagar_factura = "SELECT monto_deuda FROM tbl15_cuentas_pagar WHERE cod_factura = '$cod_factura_db'";
$consulta_cuenta_pagar_factura = mysqli_query($conectar, $sql_cuenta_pagar_factura) or die(mysqli_error($conectar));
$dato_cuenta_pagar_factura = mysqli_fetch_assoc($consulta_cuenta_pagar_factura);

$sql_total_abono_factura = "SELECT SUM(abonado) AS abonado FROM tbl15_cuentas_pagar_abonos WHERE cod_factura = '$cod_factura_db'";
...
$actualizar_sql1 = sprintf("UPDATE tbl15_cuentas_pagar SET abonado = '$abonado_total', subtotal = '$subtotal' WHERE cod_factura = '$cod_factura_db'");
$resultado_actualizacion1 = mysqli_query($conectar, $actualizar_sql1) or die(mysqli_error($conectar));
//-------------------------------------- -----------------------------------------------------------------//
?>
<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/cuentas_pagar_opcion_imprimir.php?cod_cuentas_pagar_abonos=<?php echo $cod_cuentas_pagar_abonos ?>&cod_factura=<?php echo $cod_factura_db ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>">
<?php } ?>
<?php } ?>