...
$sql_cliente = "SELECT * FROM tbl15_info_factura_venta WHERE (fecha_entrega < '$fecha_hoy') AND (nombre_estado_factura = 'CERRADA') AND (cod_tipo_pago = '2') AND (subtotal <> '0') ORDER BY fecha_entrega ASC";
...
$dias_seg                                = (strtotime($fecha_hoy)-strtotime($fecha_entrega))/86400;
$dias_abs                                = abs($dias_seg); 
$dias                                    = floor($dias_abs);
if ($dias_seg > 0) { $mensaje_dias_cumple = "CADUCO HACE ".$dias." DIAS"; } elseif ($dias_seg < 0) { $mensaje_dias_cumple = "CADUCO HACE ".$dias." DIAS"; } else { $mensaje_dias_cumple = "CADUCA HOY"; }
