...
$data_sql = ("UPDATE tbl15_subproducto SET cod_tercero = '$cod_tercero' WHERE cod_info_factura_subproducto = '$cod_info_factura_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_producto_barra') && ($tipo_ajax=='tbl15_subproducto')) {
$cod_producto_barra           = addslashes($_POST['valor']);
$cod_subproducto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET cod_producto_barra = '$cod_producto_barra' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_producto') && ($tipo_ajax=='tbl15_subproducto')) {
$nombre_producto              = addslashes(strtoupper($_POST['valor']));
$cod_subproducto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET nombre_producto = '$nombre_producto' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='comentario_producto') && ($tipo_ajax=='tbl15_subproducto')) {
$comentario_producto           = addslashes($_POST['valor']);
$cod_subproducto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET comentario_producto = '$comentario_producto' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_frec_duracion') && ($tipo_ajax=='tbl15_subproducto')) {
$nombre_frec_duracion         = addslashes(strtoupper($_POST['valor']));
$cod_subproducto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET nombre_frec_duracion = '$nombre_frec_duracion' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_venta') && ($tipo_ajax=='tbl15_subproducto')) {
$und_venta                    = addslashes($_POST['valor']);
$cod_subproducto  = intval($_POST['id']);

$datos_info_temporal = "SELECT precio_costo_producto, precio_venta_producto FROM tbl15_subproducto WHERE (cod_subproducto = '$cod_subproducto')";
...
$data_sql = ("UPDATE tbl15_subproducto SET und_venta = '$und_venta', total_costo_producto = '$total_costo_producto', total_venta_producto = '$total_venta_producto', 
cod_estado_permitir_venta = '$cod_estado_permitir_venta' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto') && ($tipo_ajax=='tbl15_subproducto')) {
$precio_venta_producto        = intval($_POST['valor']);
$cod_subproducto  = intval($_POST['id']);

$datos_info_temporal = "SELECT und_venta, precio_costo_producto FROM tbl15_subproducto WHERE (cod_subproducto = '$cod_subproducto')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
$data_sql = ("UPDATE tbl15_subproducto SET precio_venta_producto = '$precio_venta_producto', total_venta_producto = '$total_venta_producto',
cod_estado_permitir_venta = '$cod_estado_permitir_venta' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_cliente') && ($tipo_ajax=='tbl15_subproducto')) {
$nombre_cliente              = addslashes(strtoupper($_POST['valor']));
$cod_subproducto = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET nombre_cliente = '$nombre_cliente' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_tipo_cobrar') && ($tipo_ajax=='tbl15_subproducto')) {
$cod_tipo_cobrar             = addslashes(($_POST['valor']));
$id                          = addslashes($_POST['id']);
$frag                        = explode("__", $id);
$cod_subproducto = $frag[1];

$data_sql = ("UPDATE tbl15_subproducto SET cod_tipo_cobrar = '$cod_tipo_cobrar' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='fecha_alerta') && ($tipo_ajax=='tbl15_subproducto')) {
$fecha_alerta                = addslashes(($_POST['valor']));
$cod_subproducto = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_subproducto SET fecha_alerta = '$fecha_alerta' WHERE cod_subproducto = '$cod_subproducto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if ( mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='identificacion_tercero') && ($tipo_ajax=='tbl15_subproducto')) {
