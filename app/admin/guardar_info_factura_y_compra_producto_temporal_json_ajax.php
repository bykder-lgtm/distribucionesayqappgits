...
// ------------------------------------------------------------------------------------------------- //
$sql_infos_empresas = "SELECT cod_estado_promediar_precio_compra_y_venta_cargar_factura_global, cod_estado_modulo_puc_global, cod_estado_movimiento_contable_cuenta_personal_global FROM tbl15_info_empresa WHERE cod_info_empresa = '1'";
...
$fecha_pago                             = addslashes($_REQUEST['valor']);
$cod_info_factura_compra                = intval($_REQUEST['id']);
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'cod_puc') && ($tipo_ajax == 'tbl15_info_factura_compra')) {
	$cod_puc                             = intval($_REQUEST['valor']);
	$cod_info_factura_compra             = intval($_REQUEST['id']);

	$data_sql = ("UPDATE tbl15_info_factura_compra SET cod_puc = '$cod_puc' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'cod_movimiento_contable_cuenta_personal') && ($tipo_ajax == 'tbl15_info_factura_compra')) {
$cod_movimiento_contable_cuenta_personal         = intval($_REQUEST['valor']);
$cod_info_factura_compra             = intval($_REQUEST['id']);

$data_sql = ("UPDATE tbl15_info_factura_compra SET cod_movimiento_contable_cuenta_personal = '$cod_movimiento_contable_cuenta_personal' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'cod_sino_crear_mov_contable') && ($tipo_ajax == 'tbl15_info_factura_compra')) {
	$cod_sino_crear_mov_contable         = intval($_REQUEST['valor']);
	$cod_info_factura_compra             = intval($_REQUEST['id']);

	$data_sql = ("UPDATE tbl15_info_factura_compra SET cod_sino_crear_mov_contable = '$cod_sino_crear_mov_contable' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'cod_resolucion_facturacion') && ($tipo_ajax == 'tbl15_info_factura_compra')) {
	$cod_resolucion_facturacion         = addslashes($_REQUEST['valor']);
	$cod_info_factura_compra             = intval($_REQUEST['id']);
	...
	if ($nombre_tipo_resolucion_facturacion == 'DOCUMENTO_SOPORTE') { $nombre_tipo_cargue_factura = 'FACTURA_COMPRA_DOC_SOPORTE'; } else { $nombre_tipo_cargue_factura = 'FACTURA_COMPRA_NORMAL'; }

	$data_sql = ("UPDATE tbl15_info_factura_compra SET cod_resolucion_facturacion = '$cod_resolucion_facturacion', nombre_tipo_cargue_factura = '$nombre_tipo_cargue_factura', 
	nombre_tipo_factura = '$nombre_tipo_factura' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
if (($campo == 'und_unidades') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$und_unidades                           = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal           = intval($_REQUEST['id']);

	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, precio_venta_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, 
	precio_ipc, precio_compra_producto_ant_desc, cod_producto_barra
	...
	$precio_compra_producto_nuevo           = $info_temporal['precio_compra_producto'];
	...
	$datos_info_registros_antiguos = "SELECT precio_compra_producto AS precio_compra_producto_antiguo, und_producto AS und_producto_antiguo FROM tbl15_producto WHERE (cod_producto_barra = '$cod_producto_barra')";
	$consulta_info_registros_antiguos = mysqli_query($conectar, $datos_info_registros_antiguos) or die(mysqli_error($conectar));
	$info_registros_antiguos = mysqli_fetch_assoc($consulta_info_registros_antiguos);

	$precio_compra_producto_antiguo                   = ($info_registros_antiguos['precio_compra_producto_antiguo']);
	$und_producto_antiguo                             = ($info_registros_antiguos['und_producto_antiguo']);
...
if (($campo == 'und_caja') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$und_caja                               = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal           = intval($_REQUEST['id']);
...
if (($campo == 'precio_compra_producto') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$precio_compra_producto_nuevo           = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal           = intval($_REQUEST['id']);
	...
	$respuesta_ajax['incre']                           = $incre;
	$respuesta_ajax['ganancia_ptj']                    = $ganancia_ptj;
	$respuesta_ajax['precio_compra_producto_promedio'] = number_format($precio_compra_producto_promedio, 0, ",", ".");
...
// ------------------------------------------------------------------------------------------------- //
if (($campo == 'precio_compra_producto_por_caja') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	$precio_compra_producto_por_caja        = addslashes($_REQUEST['valor']);
	$cod_compra_producto_temporal           = intval($_REQUEST['id']);
	$precio_compra_producto_promedio        = 0;
	$precio_venta_producto_promedio         = 0;
	...
	$data_sql = ("UPDATE tbl15_compra_producto_temporal SET precio_compra_producto_por_caja = '$precio_compra_producto_por_caja', precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_compra_producto_ant_desc = '$precio_compra_producto_ant_desc', 
	total_compra_producto_ant_desc = '$total_compra_producto_ant_desc', descuento = '$descuento', total_dto = '$total_dto', ganancia_ptj = '$ganancia_ptj' 
	WHERE cod_compra_producto_temporal = '$cod_compra_producto_temporal'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
if (($campo == 'dto1') && ($tipo_ajax == 'tbl15_compra_producto_temporal')) {
	...
	$total                                  = $suma['total_compra_producto'];
	$total_precio_costo                     = $suma['total_precio_costo'];
	//$valor_iva                              = round($suma['valor_iva'], 2);
	$subtotal                               = $suma['subtotal'];
	$total_sin_iva                          = $subtotal;
	$total_con_iva                          = $suma['con_iva'];
	$valor_iva                              = $total_con_iva - $total_sin_iva;
	$total_precio_ipc                       = $suma['total_precio_ipc'];
	$valor_neto                             = $total;
	$total_rete_fuente                      = $subtotal * ($nombre_rete_fuente_ptj/100);
	...
	$total_factura_compra_retefuente        = ($total - ($total_rete_fuente + $total_ret_ica));
	$total_factura_compra                   = $total; 
	$total_descuento                        = $suma['total_dto'];
	$total_compra_imp                       = $total_con_iva;
	$base_valor_iva_ant_desc                = $suma['base_valor_iva_ant_desc'];
	$total_compra_producto_ant_desc         = $suma['total_compra_producto_ant_desc'];
