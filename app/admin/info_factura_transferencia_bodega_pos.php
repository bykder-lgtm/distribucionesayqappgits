...
<?php
$suma_temporal = "SELECT Sum(total_venta_producto) As total_venta, Sum(total_compra_producto) As total_compra FROM tbl15_transferencia_bodega_producto_temporal 
...
$datos_data_info_factura = "SELECT * FROM tbl15_info_factura_transferencia_bodega WHERE (nombre_estado_factura = 'ABIERTA') AND (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
$consulta_data_info_factura = mysqli_query($conectar, $datos_data_info_factura);
$data_info_factura = mysqli_fetch_assoc($consulta_data_info_factura);
$factura_ocupada = mysqli_num_rows($consulta_data_info_factura);

$cod_info_factura_transferencia_bodega      = $data_info_factura['cod_info_factura_transferencia_bodega'];
...
$(document).ready(function(){
    $("#fecha_anyo").on('change', function () {
            ...
            $.post("guardar_info_factura_transferencia_bodega_temporal_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_factura_transferencia_bodega; ?> }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            //let id = this.id;
            $.post("guardar_info_factura_transferencia_bodega_temporal_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:id }, function(data){
...
<form method="post" name="formulario" action="../admin/transferencia_bodega_producto_reg.php">
<table class="table table-striped" border="1" cellspacing="0" cellpadding="20">
  <tr>
    ...
    <th style="text-align:center;">TRANSFERIR A</th>
    <?php } ?>
  ...
  <tr>
    <td style="text-align:center;"></td>
   <td style="text-align:center;"><?php echo $cod_info_factura_transferencia_bodega ?></td>
...
-->
    ...
    <?php if ($cod_estado_empresa_transferencia_directa_global == '1') { ?>
    <td style="text-align:center;">
        <select name="cod_empresa_transferencia_directa" id="cod_empresa_transferencia_directa" class="selectpicker" data-show-subtext="true" data-live-search="true" style="width: 120px;" required>
            <?php if (isset($cod_empresa_transferencia_directa)) { echo "<option value='' >...</option>"; } else { echo "<option value='' selected ></option>"; }
            $consulta2_sql = "SELECT cod_empresa_transferencia_directa, nombre_empresa_transferencia_directa FROM tbl15_empresa_transferencia_directa WHERE (cod_estado = '1') ORDER BY cod_empresa_transferencia_directa ASC";
            $consulta2 = mysqli_query($conectar, $consulta2_sql);
            while ($datos2 = mysqli_fetch_assoc($consulta2)) {
            if(isset($cod_empresa_transferencia_directa) AND $cod_empresa_transferencia_directa == $datos2['cod_empresa_transferencia_directa']) {
            $seleccionado = "selected"; } else { $seleccionado = ""; }
...
<?php 
$sql_temporal = "SELECT cod_transferencia_bodega_producto_temporal FROM tbl15_transferencia_bodega_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
<input type="hidden" name="cod_transferencia_bodega_producto_temporal[]" value="<?php echo $datos_temporal['cod_transferencia_bodega_producto_temporal']; ?>" size="4">
<?php } ?>

<?php $pagina ='facturacion_transferencia_temporal_producto_manual_pos.php'; ?>
<input type="hidden" name="cod_info_factura_transferencia_bodega" value="<?php echo $cod_info_factura_transferencia_bodega ?>" size="10">
