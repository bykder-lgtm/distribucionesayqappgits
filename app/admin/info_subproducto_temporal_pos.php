...
<?php
$datos_data_info_factura = "SELECT * FROM tbl15_info_factura_subproducto WHERE (cod_info_factura_subproducto = '$cod_info_factura_subproducto')";
...
$(document).ready(function(){
    $("#fecha_anyo").on('change', function () {
            ...
            $.post("guardar_info_factura_y_subproducto_temporal_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_factura_subproducto; ?> }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            var tipo_ajax = "tbl15_subproducto_temporal";
            var id = $(this).attr("class");
            //let id = this.id;
            $.post("guardar_info_factura_y_subproducto_temporal_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:id }, function(data){
...
<form method="post" name="formulario" action="../admin/subproducto_producto_reg.php">
<table class="table table-striped" border="1" cellspacing="0" cellpadding="20">
  <tr>
    <th style="text-align:center;"></th>
    <th style="text-align:center;">ID</th>
    <th style="text-align:center;">NOMBRE PRODUCTO PRINCIPAL</th>
    <?php if ($existe_temporal <> 0) { ?><th style="text-align:center;">GUARDAR</th><?php } ?>
    <th style="text-align:center;"></th>
  </tr>
  <tr>
   ...
   <td style="text-align:center;"><?php echo $cod_info_factura_subproducto ?></td>
    <td style="text-align:left; width:300px">
        <select name="cod_producto_barra_madre" id="cod_producto_barra_madre" class="chosen-select" data-show-subtext="true" data-live-search="true" style="width: 400px;" required>
            <?php if (isset($cod_producto_barra_madre)) { echo "<option value='' >...</option>"; } else { echo  "<option value='' selected ></option>"; }
            $consulta2_sql = "SELECT cod_producto_barra, nombre_producto, cod_producto FROM tbl15_producto ORDER BY nombre_producto ASC";
   ...
   <input name="cod_tercero" id="cod_tercero" type="hidden" value="<?php echo $cod_tercero ?>" />
    <?php if ($existe_temporal <> 0) { ?><td style="text-align:center;"><input type="image" src="../imagenes/guardar.png" tabindex=3 name="vender" value="Guardar" /></td><?php } ?> 
   <td style="text-align:center;"></td>

<?php 
$sql_temporal = "SELECT cod_subproducto_temporal FROM tbl15_subproducto_temporal WHERE (cod_info_factura_subproducto = '$cod_info_factura_subproducto')";
...
<input type="hidden" name="cod_subproducto_temporal[]" value="<?php echo $datos_temporal['cod_subproducto_temporal']; ?>" size="4">
<?php } ?>

<?php $pagina ='facturacion_subproducto_temporal_producto_manual_pos.php'; ?>
