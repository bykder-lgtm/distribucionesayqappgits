...
$cod_info_factura_transferencia             = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_cotizacion_compra_producto SET cod_historia_clinica = '$cod_historia_clinica' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_tipo_cargue_factura') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$nombre_tipo_cargue_factura             = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET nombre_tipo_cargue_factura = '$nombre_tipo_cargue_factura' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_factura') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$cod_factura                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET cod_factura = '$cod_factura' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_rete_fuente_ptj') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$nombre_rete_fuente_ptj                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$sql_info_cotizacion_compra = "SELECT nombre_rete_fuente_ptj, ret_ica_ptj FROM tbl15_info_factura_transferencia WHERE (cod_info_factura_transferencia = '$cod_info_factura_transferencia')";
...
$data_sql = ("UPDATE tbl15_info_factura_transferencia SET nombre_rete_fuente_ptj = '$nombre_rete_fuente_ptj', total_rete_fuente = '$total_rete_fuente', 
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='ret_ica_ptj') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
...
$data_sql = ("UPDATE tbl15_info_factura_transferencia SET ret_ica_ptj = '$ret_ica_ptj', total_ret_ica = '$total_ret_ica',
total_factura_compra_retefuente = '$total_factura_compra_retefuente'
WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_compra') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_compra                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_compra = '$total_compra' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='subtotal') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$subtotal                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET subtotal = '$subtotal' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='valor_iva') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$valor_iva                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET valor_iva = '$valor_iva' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_descuento') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_descuento                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_descuento = '$total_descuento' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_precio_ipc') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_precio_ipc                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_precio_ipc = '$total_precio_ipc' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_compra_imp') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_compra_imp                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_compra_imp = '$total_compra_imp' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_rete_fuente') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_rete_fuente                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_rete_fuente = '$total_rete_fuente' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_ret_ica') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_ret_ica                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_ret_ica = '$total_ret_ica' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='total_factura_compra_retefuente') && ($tipo_ajax=='tbl15_info_factura_transferencia')) {
$total_factura_compra_retefuente                 = addslashes($_POST['valor']);
$cod_info_factura_transferencia     = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_info_factura_transferencia SET total_factura_compra_retefuente = '$total_factura_compra_retefuente' WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$cod_transferencia_producto_temporal  = intval($_POST['id']);
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_unidades') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$und_unidades                    = intval($_POST['valor']);
$cod_transferencia_producto_temporal  = intval($_POST['id']);

$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, dto1, dto2, iva_ptj, cod_info_factura_transferencia, precio_ipc 
FROM tbl15_transferencia_producto_temporal WHERE (cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal')";
...
$cod_info_factura_transferencia     = $info_temporal['cod_info_factura_transferencia'];
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET und_unidades = '$und_unidades', und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_info_factura_transferencia SET subtotal = '$subtotal', total_precio_ipc = '$total_precio_ipc',
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_caja') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET und_caja = '$und_caja', und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_compra') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_compra_producto') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='iva_ptj') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET iva_ptj = '$iva_ptj', valor_iva = '$valor_iva', precio_costo_producto = '$precio_costo_producto', 
total_costo_producto = '$total_costo_producto', total_compra_producto = '$total_compra_producto' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='dto1') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET dto1 = '$dto1', descuento = '$descuento', total_dto = '$total_dto', precio_costo_producto = '$precio_costo_producto', 
total_costo_producto = '$total_costo_producto' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='dto2') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET dto2 = '$dto2', descuento = '$descuento', total_dto = '$total_dto', precio_costo_producto = '$precio_costo_producto', 
total_costo_producto = '$total_costo_producto' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_info_factura_transferencia SET subtotal = '$subtotal', precio_ipc_total = '$precio_ipc_total',
valor_iva = '$valor_iva', total = '$total', valor_neto = '$valor_neto', total_rete_fuente = '$total_rete_fuente', total_ret_ica = '$total_ret_ica', 
total_factura_compra_retefuente = '$total_factura_compra_retefuente', total_factura_compra = '$total_factura_compra', total_descuento = '$total_descuento', total_compra_imp = '$total_compra_imp' 
WHERE cod_info_factura_transferencia = '$cod_info_factura_transferencia'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_ipc') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_ipc = '$precio_ipc', precio_ipc_total = '$precio_ipc_total', total_compra_producto = '$total_compra_producto' 
WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='fecha_vencimiento') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$fecha_vencimiento            = addslashes(($_POST['valor']));
$cod_transferencia_producto_temporal = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET fecha_vencimiento = '$fecha_vencimiento' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_venta_producto = '$precio_venta_producto' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto2') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$precio_venta_producto2                    = intval($_POST['valor']);
$cod_transferencia_producto_temporal  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_venta_producto2 = '$precio_venta_producto2' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto3') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$precio_venta_producto3                    = intval($_POST['valor']);
$cod_transferencia_producto_temporal  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_venta_producto3 = '$precio_venta_producto3' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto4') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$precio_venta_producto4                    = intval($_POST['valor']);
$cod_transferencia_producto_temporal  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_venta_producto4 = '$precio_venta_producto4' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto5') && ($tipo_ajax=='tbl15_transferencia_producto_temporal')) {
$precio_venta_producto5                    = intval($_POST['valor']);
$cod_transferencia_producto_temporal  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_transferencia_producto_temporal SET precio_venta_producto5 = '$precio_venta_producto5' WHERE cod_transferencia_producto_temporal = '$cod_transferencia_producto_temporal'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
