...
function enviarRespuesta($success, $message, $data = []) { echo json_encode(['success' => $success, 'message' => $message, 'data' => $data ]); exit; }

// Evitar ejecuciones duplicadas - verificar si ya se procesó esta solicitud
$request_id = 'aprobar_venta_' . (isset($_POST['cod_info_factura_venta']) ? $_POST['cod_info_factura_venta'] : '');
if (isset($_SESSION[$request_id]) && (time() - $_SESSION[$request_id]) < 5) { enviarRespuesta(false, 'Solicitud duplicada detectada. Por favor espere.'); }
$_SESSION[$request_id] = time();

// Validar que la solicitud sea POST
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { enviarRespuesta(false, 'Método de solicitud no permitido'); }
// Validar sesión
if (!isset($_SESSION['cod_administrador']) || empty($_SESSION['cod_administrador'])) { enviarRespuesta(false, 'Sesión no válida. Por favor inicie sesión nuevamente'); }
// Validar y obtener parámetros POST
if (!isset($_POST['cod_info_factura_venta']) || empty($_POST['cod_info_factura_venta'])) { enviarRespuesta(false, 'No se recibió el código de factura'); }
if (!isset($_POST['pagina']) || empty($_POST['pagina'])) { enviarRespuesta(false, 'No se recibió la página de origen'); }

try {
    // Sanitizar y validar datos de entrada
    $cod_info_factura_venta                   = intval($_POST['cod_info_factura_venta']);
    $cuenta                                   = mysqli_real_escape_string($conectar, trim($_POST['cuenta']));
    $cod_caja_virtual                         = intval($_POST['cod_caja_virtual']);
    $modo_venta_por_defecto                   = intval($_POST['modo_venta_por_defecto']);
    $pagina                                   = mysqli_real_escape_string($conectar, trim($_POST['pagina']));
    // Validar que cod_info_factura_venta sea válido
    if ($cod_info_factura_venta <= 0) { enviarRespuesta(false, 'Código de factura no válido'); }
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	$resultado_regis = mysqli_query($conectar, $agregar_regis);
    
    if (!$resultado_regis) {
        throw new Exception('Error al actualizar el estado de la factura: ' . mysqli_error($conectar));
    }
    //---------------------------------------------------------------------------------------------------------------------------------//
    // Generar URL de la factura
    $url_factura = "../admin/venta_productos_opcion_imprimir_siscredito_visitante_intern.php?cod_info_factura_venta=" . $cod_info_factura_venta . "&pagina=" . urlencode($pagina);
    
    // Limpiar el flag de solicitud procesada antes de enviar respuesta exitosa
    unset($_SESSION[$request_id]);
    
    // Enviar respuesta exitosa con la URL de la factura
    $nombres_apellidos_cliente = isset($nombres_apellidos) ? $nombres_apellidos : '';
    enviarRespuesta(true, 'Venta aprobada exitosamente', [
        'cod_info_factura_venta' => $cod_info_factura_venta,
        'cod_factura' => $cod_factura,
        'url_factura' => $url_factura,
        'nombre_cliente' => $nombres_apellidos_cliente,
        'monto_total' => number_format($monto_deuda, 0, ',', '.'),
        'nombre_estado_factura' => $nombre_estado_factura
    ]);

} catch (Exception $e) {
    // Limpiar el flag de solicitud procesada en caso de error
    if (isset($request_id)) {
        unset($_SESSION[$request_id]);
    }
    // En caso de error, hacer rollback si es necesario y enviar respuesta de error
    enviarRespuesta(false, 'Error al aprobar la venta: ' . $e->getMessage());
}
?>