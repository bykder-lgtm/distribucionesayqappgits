...
<table class="table table-striped" cellspacing="0" cellpadding="20">
  <tr>
    <th style="text-align:center;">MAS VENDIDOS POR TOTAL DE VENTA</th>
  </tr>
</table>
...
?>
    ...
    $(function () {
        $('#container_grafico_estadistico').highcharts({
            ...
            title: {
                text: 'LOS <?php echo $total_reg_mostrar ?> PRODUCTOS MAS VENDIDOS POR TOTAL DE VENTA DEL <?php echo date("d-m-Y", strtotime($fecha_ymd_venta_producto_ini)) ?> AL <?php echo date("d-m-Y", strtotime($fecha_ymd_venta_producto_fin)) ?>'
            },
            subtitle: {
    ...
    FROM tbl15_venta_producto WHERE (fecha_ymd_venta_producto BETWEEN '$fecha_ymd_venta_producto_ini' AND '$fecha_ymd_venta_producto_fin') GROUP BY nombre_producto ORDER BY 3 DESC LIMIT 0, $total_reg_mostrar";
    ...
    ?>
            ...
            series: [{
                name: 'PRODUCTOS MAS VENDIDOS POR TOTAL DE VENTA',
                data: [
    <?php
    $sql = "SELECT nombre_producto, SUM(und_venta) AS total_und_venta, SUM(total_venta_producto) AS total_venta_producto 
    FROM tbl15_venta_producto WHERE (fecha_ymd_venta_producto BETWEEN '$fecha_ymd_venta_producto_ini' AND '$fecha_ymd_venta_producto_fin') GROUP BY nombre_producto ORDER BY 3 DESC LIMIT 0, $total_reg_mostrar";
    $result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
    while ($registros = mysqli_fetch_array($result)) {
        $total_venta_producto = ($registros["total_venta_producto"]);
    ?>
