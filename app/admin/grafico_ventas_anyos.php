...
<a class="btn btn-primary" href="#"><h6>Reporte Estadistico Ventas Por Año</h6></a>
</div>
<div class="row-fluid">
...
$(function () {
    $('#container_grafico_estadistico').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT fecha_anyo_venta_producto FROM tbl15_venta_producto GROUP BY fecha_anyo_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
?>
'<?php echo $registros["fecha_anyo_venta_producto"] ?>',
<?php
}
?>
            ]
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto FROM tbl15_venta_producto GROUP BY fecha_anyo_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
