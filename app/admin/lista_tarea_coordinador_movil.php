...
$cod_seguridad_pag      = "23";
$pagina_local           = $_SERVER['PHP_SELF'];

// ****************************** MODULO DE SESION ******************************
include_once("../admin/01_admin_modulo_inicio_sesion_adm_coordinador.php");
include_once("../admin/01_admin_modulo_info_empresa_adm_coordinador.php");
// ******************************************************************************

// Consultar mis tareas (Coordinador) conectadas a los estados de la nueva tabla
$sql_tareas = "
SELECT 
    t.*, 
...
$sql_externos = "SELECT cod_administrador, nombres_apellidos_tercero FROM tbl15_administrador WHERE cod_estado != '0' AND cod_coordinador = '$cod_administrador' ORDER BY nombres_apellidos_tercero ASC";
...
<style>
/* Estilos adaptados heredados del Kanban del Líder (Variante Visual Azul Púrpura) */
:root {
    --theme-color: #6366f1; /* Indigo para coordinador */
...
.column-badge { background: rgba(var(--theme-color-rgb), 0.2); padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.8rem; color: #818cf8; }
...
.task-title { font-size: 1rem; font-weight: 700; margin-bottom: 0.5rem; color: #818cf8; }
...
.fab-btn {
    position: fixed; bottom: 90px; right: 20px; width: 60px; height: 60px; background: linear-gradient(135deg, #6366f1 0%, #4338ca 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(99, 102, 241, 0.4); font-size: 1.5rem; z-index: 1000; cursor: pointer; border: none; color: white;
}
/* Tabs for separation */
...
<div style="padding: 1.5rem 1rem 0 1rem;">
    <h1 style="font-size: 1.5rem; margin:0;"><i class="fa-solid fa-code-branch" style="color:var(--theme-color);"></i> Board Coordinador</h1>
...
function moverTarea(cod, actual) {
    Swal.fire({
        title: 'Mover Tarea', html: 'Módulo de actualización en preparación', background: '#1a1f2e', color: '#fff', confirmButtonColor: '#6366f1'
    });
}
</script>
</body>
