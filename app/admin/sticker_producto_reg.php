...
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
$cod_info_factura_sticker      = intval($_POST['cod_info_factura_sticker']);
...
$sql_maxima_factura = "SELECT Max(cod_factura) AS cod_factura FROM tbl15_info_factura_sticker WHERE (nombre_estado_factura = 'CERRADA')";
...
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
$sql_info_imp_factura = "SELECT * FROM tbl15_info_factura_sticker WHERE cod_info_factura_sticker = '$cod_info_factura_sticker'";
...
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
$sql_total_sticker_producto_temporal = "SELECT SUM(und_venta * precio_venta_producto) AS total_precio_venta, SUM(und_venta * precio_costo_producto) AS total_precio_compra
FROM tbl15_sticker_producto_temporal WHERE (cod_info_factura_sticker = '$cod_info_factura_sticker')";
$consulta_total_sticker_producto_temporal = mysqli_query($conectar, $sql_total_sticker_producto_temporal) or die(mysqli_error($conectar));
$datos_total_sticker_producto_temporal = mysqli_fetch_assoc($consulta_total_sticker_producto_temporal);

$total_precio_compra               = $datos_total_sticker_producto_temporal['total_precio_compra'];
$total_precio_venta                = $datos_total_sticker_producto_temporal['total_precio_venta'];
...
if ($cod_info_factura_sticker) {

for ($i=0; $i < $total_datos; $i++) {

$cod_sticker_producto_temporal       = $_POST['cod_sticker_producto_temporal'][$i];

$sql_mconsulta = "SELECT * FROM tbl15_sticker_producto_temporal WHERE (cod_sticker_producto_temporal = '$cod_sticker_producto_temporal')";
...
$agregar_reg_venta_producto = "INSERT INTO tbl15_sticker_producto (cod_info_factura_sticker, cod_factura, cod_producto, cod_producto_barra, 
...
VALUES ('$cod_info_factura_sticker', '$cod_factura', '$cod_producto', '$cod_producto_barra', 
'$nombre_producto', '$und_venta', '$precio_compra_producto', '$total_compra_producto', '$precio_costo_producto', '$total_costo_producto', '$precio_venta_producto', 
'$total_venta_producto', '$nombre_tipo_producto', '$und_producto', '$fecha_ymd_venta_producto', '$fecha_hora_venta_producto',
'$nombre_tipo_unidad_medida', '$posologia_cantidad', '$posologia_peso', '$nombre_tipo_presentacion',
'$fecha_mes_venta_producto', '$fecha_anyo_venta_producto', '$fecha_seg_venta_producto', '$cuenta', '$cod_administrador', 
...
//----------------------------------------------------------------------- ---------------------------------------------------------//
//----------------------------------------------------------------------- ---------------------------------------------------------//
$borrar_sql = sprintf("DELETE FROM tbl15_sticker_producto_temporal WHERE (cod_info_factura_sticker = '$cod_info_factura_sticker')");
$Result1 = mysqli_query($conectar, $borrar_sql) or die(mysqli_error($conectar));
//----------------------------------------------------------------------- ---------------------------------------------------------//
//----------------------------------------------------------------------- ---------------------------------------------------------//   
$agregar_regis = sprintf("UPDATE tbl15_info_factura_sticker SET cod_estado_factura = '$cod_estado_factura', cod_factura = '$cod_factura', fecha_anyo = '$fecha_anyo', 
...
nombre_maquina = '$nombre_maquina', tiempo_ejecucion = '$tiempo_ejecucion', cod_resolucion_facturacion = '$cod_resolucion_facturacion', observacion = '$observacion'
...
$url_redir = "../admin/sticker_tiketeadora_productos_opcion_imprimir_js.php?cod_info_factura_sticker=".$cod_info_factura_sticker."&cod_tipo_pago=".$cod_tipo_pago."&pagina=".$pagina;
