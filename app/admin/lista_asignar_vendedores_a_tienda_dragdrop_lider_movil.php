...
// Consulta de Vendedores activos que pertenecen al líder actual
// cod_seguridad = '2' (Vendedor)
$sql_vendedores = "SELECT cod_administrador, nombres, apellidos, nombres_apellidos_tercero, cod_tienda FROM tbl15_administrador WHERE cod_seguridad = '2' AND cod_estado != '0' AND cod_lider = '$cod_administrador' ORDER BY nombres_apellidos_tercero ASC";
$res_vendedores = mysqli_query($conectar, $sql_vendedores);

$vendedores_por_tienda = [];
$vendedores_no_asignados = [];

while($vendedor = mysqli_fetch_assoc($res_vendedores)) {
    $iniciales = '';
    if (!empty($vendedor['nombres'])) { $iniciales = strtoupper(substr($vendedor['nombres'], 0, 1)); }
    if (!empty($vendedor['apellidos'])) { $iniciales .= strtoupper(substr($vendedor['apellidos'], 0, 1)); }
    if (empty($iniciales)) { $iniciales = 'VD'; }
    $vendedor['iniciales'] = $iniciales;

    $tienda_id = $vendedor['cod_tienda'];
    if(empty($tienda_id) || $tienda_id == 0 || $tienda_id == '') {
        $vendedores_no_asignados[] = $vendedor;
    } else {
        if(!isset($vendedores_por_tienda[$tienda_id])) { $vendedores_por_tienda[$tienda_id] = []; }
        $vendedores_por_tienda[$tienda_id][] = $vendedor;
    }
}
...
<main class="page-container">
    <div class="page-header animate-in">
        <h1><i class="fa-solid fa-arrows-up-down-left-right"></i> Asignación Interactiva de Vendedores a Tiendas</h1>
        <p>Arrastra y suelta a los vendedores para asignarlos a una tienda determinada.</p>
    </div>

    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;" class="animate-in">
        <h3 style="color: #ef4444; font-size: 1.3rem; margin: 0;"><i class="fa-solid fa-user-xmark"></i> Vendedores Sin Tienda Asignada</h3>
        <span class="badge-count" id="count-0" style="background: rgba(239, 68, 68, 0.2); color: #ef4444; font-weight: bold; font-size: 1rem; padding: 0.3rem 1rem;"><?php echo count($vendedores_no_asignados); ?></span>
    </div>
    
    <div class="franja-sin-asignar animate-in">
        <div class="zona-drop zona-drop-horizontal" data-id="0" id="lista-0">
            <?php foreach($vendedores_no_asignados as $vd): ?>
            <div class="item-dragg" data-user="<?php echo $vd['cod_administrador']; ?>">
                <div class="item-avatar" style="background: #ef4444;"><?php echo $vd['iniciales']; ?></div>
                <div class="item-info">
                    <span class="item-name">
                        <?php echo utf8_encode($vd['nombres_apellidos_tercero']); ?>
                    </span>
                    <span class="item-role">ID: <?php echo $vd['cod_administrador']; ?></span>
                </div>
                <i class="fa-solid fa-grip-vertical" style="color: rgba(255,255,255,0.3);"></i>
            </div>
            <?php endforeach; ?>
            
            <div class="empty-msg" style="width: 100%; text-align: center; color: rgba(255,255,255,0.4); font-style: italic; padding: 1rem; <?php echo (count($vendedores_no_asignados) > 0) ? 'display:none;' : ''; ?>">Todos los vendedores tienen tienda. Arrastra aquí para quitar asignación y dejarlos sueltos.</div>
        </div>
    </div>

    <div style="margin-bottom: 1.5rem; display: flex; flex-direction: column; gap: 0.5rem;" class="animate-in delay-1">
        <div>
            <h3 style="color: #10b981; font-size: 1.3rem; margin: 0;"><i class="fa-solid fa-store"></i> Tiendas</h3>
            <p style="color: rgba(255,255,255,0.5); font-size: 0.9rem; margin-bottom: 0.5rem;">Arrastra aquí a los vendedores para asignarlos al equipo de una tienda.</p>
        </div>
        <div style="width: 100%; max-width: 400px; position: relative;">
            ...
            <input type="text" id="buscador-principal-asesores" placeholder="Buscar tienda por nombre..." style="width: 100%; padding: 0.8rem 1rem 0.8rem 2.5rem; border-radius: 10px; border: 1px solid rgba(16, 185, 129, 0.3); background: rgba(0,0,0,0.2); color: white; outline: none; font-size: 0.95rem;">
        </div>
    </div>

    <div class="franja-coordinadores animate-in delay-1">
        <?php while($tienda = mysqli_fetch_assoc($res_tiendas_lista)): 
            $td_id = $tienda['cod_tienda'];
            $vendedores_esta_lista = isset($vendedores_por_tienda[$td_id]) ? $vendedores_por_tienda[$td_id] : [];
        ?>
        <div class="card-lista">
            <div class="card-header-lista header-asesor">
                ...
                <span class="badge-count" id="count-<?php echo $td_id; ?>" style="background: rgba(16, 185, 129, 0.2); color: #10b981; font-weight: bold;"><?php echo count($vendedores_esta_lista); ?></span>
            </div>
            <div style="padding: 0.5rem 1rem; border-bottom: 1px solid rgba(139, 92, 246, 0.2); background: rgba(0,0,0,0.1);">
                <input type="text" class="buscador-asesor" data-target="lista-<?php echo $td_id; ?>" placeholder="Buscar vendedor asignado..." style="width: 100%; padding: 0.5rem; border-radius: 6px; border: 1px solid rgba(139, 92, 246, 0.3); background: rgba(255,255,255,0.05); color: white; outline: none; font-size: 0.85rem;">
            </div>
            <div class="zona-drop" data-id="<?php echo $td_id; ?>" id="lista-<?php echo $td_id; ?>">
                <?php foreach($vendedores_esta_lista as $vd): ?>
                <div class="item-dragg" data-user="<?php echo $vd['cod_administrador']; ?>">
                    <div class="item-avatar"><?php echo $vd['iniciales']; ?></div>
                    <div class="item-info">
                        <span class="item-name">
                            <?php echo utf8_encode($vd['nombres_apellidos_tercero']); ?>
                        </span>
        ...
        zonasDrop.forEach(zona => {
            new Sortable(zona, {
                ...
                onEnd: function (evt) {
                    ...
                    if (fromList !== toList) {
                        let id_vendedor = itemEl.getAttribute('data-user');
                        let id_tienda_nueva = toList.getAttribute('data-id'); // 0 si es No Asignados
                        
                        // Actualizar contadores
                        document.getElementById('count-' + fromList.getAttribute('data-id')).innerText = fromList.querySelectorAll('.item-dragg').length;
                        document.getElementById('count-' + id_tienda_nueva).innerText = toList.querySelectorAll('.item-dragg').length;
                        ...
                        $.ajax({
                            url: 'procesar_asignar_vendedores_a_tienda_dragdrop_lider_ajax.php', type: 'POST', data: { accion: 'asignar_vendedor_tienda', id_vendedor: id_vendedor, id_tienda_nueva: id_tienda_nueva }, dataType: 'json',
