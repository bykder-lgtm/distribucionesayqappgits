...
if($action == 'ajax') {
    ...
    $pagina = isset($_REQUEST['pagina']) ? mysqli_real_escape_string($conectar,(strip_tags($_REQUEST['pagina'], ENT_QUOTES))) : '';
}
...
if($action == 'ajax') {
    ...
    $sWhere = " WHERE (tbl15_info_factura_venta.cod_vendedor = '$cod_administrador') AND (tbl15_info_factura_venta.url_img_orig_producto != '' AND tbl15_info_factura_venta.url_img_orig_producto IS NOT NULL)";
    ...
    if (!$consulta_cantidad_registros) { $cantidad_registros = 0; } else { $datos_cantidad_registros = mysqli_fetch_array($consulta_cantidad_registros); $cantidad_registros = $datos_cantidad_registros['cantidad_registros']; }
...
?>
        <div class="card-comprobante" onclick="abrirModalComprobantePago('<?php echo $cod_info_factura_venta; ?>', '<?php echo $url_img_orig_producto; ?>')">
            <div class="comprobante-icono"><i class="fa fa-file-image-o"></i></div>
            ...
            <div class="comprobante-ver-btn"><i class="fa fa-eye"></i></div>
        </div>
<?php
        }
    } else {
...
?>
<style>
.card-comprobante { background: linear-gradient(135deg, #1a1d3a 0%, #0a0e27 100%); border-radius: 12px; padding: 0.7rem 1rem; margin-bottom: 0.5rem; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2); border: 1px solid rgba(147, 51, 234, 0.15); display: flex; align-items: center; gap: 0.75rem; cursor: pointer; transition: all 0.25s ease; }
.card-comprobante:hover { border-color: rgba(147, 51, 234, 0.5); box-shadow: 0 4px 15px rgba(147, 51, 234, 0.2); transform: translateX(3px); }
.comprobante-icono { flex-shrink: 0; width: 40px; height: 40px; border-radius: 10px; background: rgba(147, 51, 234, 0.15); display: flex; align-items: center; justify-content: center; }
.comprobante-icono i { font-size: 1.2rem; color: #9333ea; }
.comprobante-info { flex: 1; min-width: 0; overflow: hidden; }
.comprobante-info h6 { color: #fff; font-weight: 600; font-size: 0.85rem; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.comprobante-info p { color: #718096; font-size: 0.75rem; margin: 0.15rem 0 0 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.comprobante-meta { margin-right: 0.5rem; }
.comprobante-monto { color: #9333ea; font-weight: 700; }
.comprobante-badge { flex-shrink: 0; color: white; padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.65rem; font-weight: 600; white-space: nowrap; }
.comprobante-ver-btn { flex-shrink: 0; width: 34px; height: 34px; border-radius: 8px; background: linear-gradient(135deg, #9333ea 0%, #f97316 100%); display: flex; align-items: center; justify-content: center; transition: transform 0.2s ease; }
.comprobante-ver-btn i { color: white; font-size: 0.85rem; }
.card-comprobante:hover .comprobante-ver-btn { transform: scale(1.1); }
</style>
