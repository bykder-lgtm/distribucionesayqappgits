...
<a class="btn btn-primary" href="#"><h6>Reporte Fecha de Entrega</h6></a>
</div>
<div class="row-fluid">
...
$fecha_impr                              = date("Ymd");
$hora_impr                               = date("His");
$fecha_entrega_ini                       = date("Y-m-d");

if (isset($_GET['dias_alerta_entrega_venta'])) {
$dias_alerta_entrega_venta               = addslashes($_GET['dias_alerta_entrega_venta']);
$fecha_entrega_fin                       = date('Y-m-d', strtotime($fecha_entrega_ini.'+'.$dias_alerta_entrega_venta.' day'));
}
?>
...
<tr>
<th style="text-align:center;"><a href="../admin/reporte_fecha_entrega_venta_caducados.php">Reporte Fecha de Entrega Caducada</a></th>
<th style="text-align:center;"><a href="../admin/reporte_fecha_entrega_venta_a_caducar.php">Reporte Fecha de Entrega a Caducar</a></th>
</tr>
</table>

<table class="table table-striped">
  <tr>
    <th style="text-align:center;">DIAS PARA LA ENTREGA: <input class="" name="dias_alerta_entrega_venta" type="number" value="<?php echo $dias_alerta_entrega_venta ?>" required/></th>
  </tr>
</table>
<div class="actions">
...
<th style="text-align:center;"><a href="#">Feha Entrega</a></th>
...
$sql_cliente = "SELECT * FROM tbl15_info_factura_venta WHERE (fecha_entrega BETWEEN '$fecha_entrega_ini' AND '$fecha_entrega_fin') AND (cod_tipo_pago = '2') AND (subtotal <> '0') ORDER BY fecha_entrega ASC";
...
$fecha_cumpleanos_alerta                 = date('Y-m-d', strtotime($fecha_entrega_ini.'+'.$dias_alerta_entrega_venta.' day'));
$dias_seg                                = (strtotime($fecha_entrega)-strtotime($fecha_actual))/86400;
$dias_abs                                = abs($dias_seg); 
$dias                                    = floor($dias_abs);
if ($dias_seg > 0) { $mensaje_dias_cumple = "CADUCA DENTRO DE ".$dias." DIAS"; } elseif ($dias_seg < 0) { $mensaje_dias_cumple = "CUMPLIO HACE ".$dias." DIAS"; } else { $mensaje_dias_cumple = "CADUCA HOY"; }
