...
*/
		...
		while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
			...
			$sql_data = "INSERT INTO tbl15_nota_observacion (cod_cuentas_cobrar, nombre_nota_observacion, cuenta, cod_administrador, cod_tercero, cod_tipo_nota_observacion, 
			fecha_creacion, fecha_ymd, fecha_hora, codigo_estado_revision) 
			VALUES ('$cod_cuentas_cobrar', '$nombre_nota_observacion', '$cuenta', '$cod_administrador', '$cod_tercero', '$cod_tipo_nota_observacion', 
			'$fecha_creacion', '$fecha_ymd', '$fecha_hora', '$codigo_estado_revision')";
			$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	    }
    } else {
		...
		while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
			...
			$sql_data = "INSERT INTO tbl15_nota_observacion (cod_cuentas_cobrar, nombre_nota_observacion, cuenta, cod_administrador, cod_tercero, 
			cod_tipo_nota_observacion, fecha_creacion, codigo_estado_revision) 
			VALUES ('$cod_cuentas_cobrar', '$nombre_nota_observacion', '$cuenta', '$cod_administrador', '$cod_tercero', 
			'$cod_tipo_nota_observacion', '$fecha_creacion', '$codigo_estado_revision')";
			$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	    }
    }
	...
	//---------------------------------------------------------------------------------------------------------------------------------//
	$url_redir = $pagina_redirect."?cod_tercero_codifcryp=".$cod_tercero_codifcryp;
