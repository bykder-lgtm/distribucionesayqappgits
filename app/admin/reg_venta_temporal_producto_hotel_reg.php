...
//---------------------------------------------------------------------------------------------------------------------------------------------//
	...
	$pagina = addslashes($_GET['pagina'])."?&foco=".$foco."&cod_info_factura_venta=".$cod_info_factura_venta."&buscar_por=".$buscar_por."&cuenta=".$cuenta."&cod_caja_virtual=".$cod_caja_virtual.$condicional_url_categoria."&pagina=facturacion_venta_temporal_producto_manual_pos.php";
	...
	$sql_info = "SELECT * FROM tbl15_info_factura_venta WHERE (nombre_estado_factura = '$nombre_estado_factura') AND (cod_info_factura_venta = '$cod_info_factura_venta')";
...
//---------------------------------------------------------------------FACTURA NUEVA INICIO----------------------------------------------------------------//
		if ($factura_abierta == '0') {
			...
			cod_estado_habitacion_hotel, cod_tipo_habitacion_hotel, fecha_ymd_parqueo_ini, fecha_ymd_parqueo_fin, cod_estado_tipo_hotel_parqueo) 
...
//---------------------------------------------------------------------FACTURA ABIERTA INICIO----------------------------------------------------------------//
		...
		$sql_info_factura = "SELECT cod_info_factura_venta, cod_tercero FROM tbl15_info_factura_venta WHERE (nombre_estado_factura = '$nombre_estado_factura') AND (cod_info_factura_venta = '$cod_info_factura_venta')";
		...
		$sql_venta_producto_temporal_repetido = "SELECT und_venta, precio_venta_producto FROM tbl15_venta_producto_temporal WHERE (cod_info_factura_venta = '$cod_info_factura_venta') AND (cod_producto_barra = '$cod_producto_barra')";
		...
		if (($existe_venta_producto_temporal_repetido <> '0') && ($cod_estado_sumar_producto_repetido_venta_temporal_global == '1')) {
			...
			$sql_data = sprintf("UPDATE tbl15_venta_producto_temporal SET und_venta = '$und_venta_aument', total_venta_producto = '$total_venta_producto_aument' WHERE (cod_info_factura_venta = '$cod_info_factura_venta') AND (cod_producto_barra = '$cod_producto_barra')");
			$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));	
		} else {
