...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['accion'])) {
    if ($_POST['accion'] == 'asignar_coordinadores_lider_masivo') {
        ...
        if (!is_array($ids_coordinadores) || count($ids_coordinadores) == 0) { echo json_encode(['status' => 'error', 'message' => 'No se han seleccionado coordinadores.']); exit; }
        ...
        if (mysqli_query($conectar, $sql_update)) { echo json_encode(['status' => 'success', 'message' => count($clean_ids) . ' coordinadores han sido asignados exitosamente.']); } else { echo json_encode(['status' => 'error', 'message' => 'Error al actualizar base de datos: ' . mysqli_error($conectar)]); } exit;
    }
}
