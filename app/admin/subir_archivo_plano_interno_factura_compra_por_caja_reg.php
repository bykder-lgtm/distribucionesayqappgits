...
if (isset($_POST['cod_tipo_origen_factura_compra'])) {
	...
	do {
		if ($datos_archivo_plano_intern) {
			...
			$cod_producto_barra_get              = trim($datos_archivo_plano_intern[1]);
			...
			$precio_compra_producto_caj             = trim($datos_archivo_plano_intern[8]);
			$precio_costo_producto_caj              = trim($datos_archivo_plano_intern[8]);
			$nombre_proveedor                       = $precio_compra_producto_caj;
			$und_caja                               = $und_compra;
			$und_unidades                           = intval($datos_producto['cajas_sobre']);
			//---------------------------------------------------------------------------------------------------------------------------------------------//
			//---------------------------------------------------------------------------------------------------------------------------------------------//
			if ($cod_producto_barra == '') { $cod_producto_barra = $cod_producto_barra_get; } else { $cod_producto_barra = $cod_producto_barra; }
			if ($und_caja == '0') { $und_caja = '1'; } else { $und_caja = $und_compra; }
			if ($und_unidades == '0') { $und_unidades = '1'; } else { $und_unidades = $datos_producto['cajas_sobre']; }
			if($iva_ptj == '0') { $dto1 = 0; $dto2 = 0; } else { $dto1 = 0; $dto2 = 0; }
			//---------------------------------------------------------------------------------------------------------------------------------------------//
			if ($und_unidades == '1') {
				$und_compra                             = $und_unidades * $und_caja;
				$unidades_total                         = $und_compra;
				$precio_compra_producto_unidad          = $precio_compra_producto_caj;
				$precio_compra_producto_ant_desc        = $precio_compra_producto_unidad;
				$total_compra_producto_ant_desc         = $precio_compra_producto_ant_desc * $unidades_total;
				$precio_compra_producto_base_iva        = $precio_compra_producto_ant_desc;
				$precio_compra_producto_dto1            = $precio_compra_producto_base_iva;
				$precio_compra_producto_dto2            = $precio_compra_producto_dto1;
				...
				$precio_compra_producto                 = ($precio_compra_producto_caj / $und_unidades);
				$calc_dto1                              = $precio_compra_producto_base_iva;
				$precio_compra_producto_dto1            = $precio_compra_producto_base_iva - $calc_dto1;
				$calc_dto2                              = $precio_compra_producto_dto1;
				$precio_compra_producto_dto2            = $precio_compra_producto_dto1 - $calc_dto2;
				$precio_compra_dto_iva                  = $precio_compra_producto_dto2;
...
//---------------------------------------------------------------------------------------------------------------------------------------------//
			if ($existe_producto == '1') {
				...
				$nombre_producto0                 = trim($datos_archivo_plano_intern[2]);
			...
			cajas_sobre, und_sobre, cod_dependencia, dto1, dto2, und_producto, precio_compra_producto_ant_desc, total_compra_producto_ant_desc) 
