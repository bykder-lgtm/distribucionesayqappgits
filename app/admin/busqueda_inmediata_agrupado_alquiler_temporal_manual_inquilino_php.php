...
if ($buscar_por == 'documento_nombre_inquilino') {
	$mostrar_datos_sql = "SELECT * FROM tbl15_tercero WHERE ((nombre1_tercero LIKE '%$buscar%') OR (identificacion_tercero LIKE '$buscar')) AND ((nombre_tipo_tercero = '$nombre_tipo_tercero') OR (nombre_tipo_tercero = 'AMBOS')) AND (cod_estado = '1') ORDER BY nombre1_tercero ASC";
	$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
	$total_resultados = mysqli_num_rows($consulta);
} elseif ($buscar_por == 'cod_producto_barra') {
	$mostrar_datos_sql = "SELECT * FROM tbl15_tercero WHERE ((identificacion_tercero LIKE '$buscar')) AND (cod_estado = '1') ORDER BY nombre1_tercero ASC";
	$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
	$total_resultados = mysqli_num_rows($consulta);
} elseif ($buscar_por == 'cod_producto_barra_nombre_producto') {
	$mostrar_datos_sql = "SELECT * FROM tbl15_tercero WHERE (nombre1_tercero LIKE '$buscar%') OR (identificacion_tercero LIKE '$buscar') AND (cod_estado = '1') ORDER BY nombre1_tercero ASC";
	$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
	$total_resultados = mysqli_num_rows($consulta);
} else {
	$mostrar_datos_sql = "SELECT * FROM tbl15_tercero WHERE ((nombre1_tercero LIKE '%$buscar%') OR (identificacion_tercero LIKE '$buscar')) AND ((nombre_tipo_tercero = '$nombre_tipo_tercero') OR (nombre_tipo_tercero = 'AMBOS')) AND (cod_estado = '1') ORDER BY nombre1_tercero ASC";
...
?>
	...
	<td style="text-align:left;"><a href="../admin/facturacion_agrupado_alquiler_temporal_manual.php?cod_tercero=<?php echo $cod_tercero?>&buscar_por=<?php echo $buscar_por?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&cuenta=<?php echo $cuenta?>&cod_caja_virtual=<?php echo $cod_caja_virtual?>&pagina=<?php echo $pagina?>"><?php echo $nombre1_tercero ?></a></td>
	<td style="text-align:left;"><?php echo $direccion_tercero; ?></td>
	<td style="text-align:left;"><?php echo $telefono1_tercero; ?></td>
	<td style="text-align:left;"><?php echo $correo_tercero; ?></td>
