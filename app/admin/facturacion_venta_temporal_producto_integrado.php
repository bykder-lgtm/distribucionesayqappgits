...
function hacer_busqueda() {
    ...
    xmlhttp.open("POST","../admin/busqueda_inmediata_venta_temporal_producto_integrado_php.php",true);
...
<?php if ($modo_venta_por_defecto == 'manual') { ?>
    <table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
        <tbody>
        	<tr>
                <?php if ($cod_estado_venta_barras==1) { ?><td bgcolor="#fff" align="center"><a href="../admin/facturacion_venta_temporal_producto_integrado.php?modo_venta_por_defecto=barras"><strong>Venta Barras</strong></a></td><?php } ?>
                ...
                <strong>Venta Manual: [<?php echo $cod_base_caja ?>]<input type="text" id="busqueda" name="busqueda" onkeyup="hacer_busqueda()" style="height:40" placeholder="Buscar"/><input type="hidden" id="pagina" name="pagina" value="<?php echo $pagina_local ?>"/></strong><div id="logo_cargador"></div>
                </td>
        	</tr>
        </tbody>
    </table>
<?php } else { ?>
    <form method="GET" name="formulario" action="../admin/reg_venta_temporal_producto_reg.php">
    <table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
        <tbody>
            <tr>
                <?php if ($cod_estado_venta_manual==1) { ?><td bgcolor="#fff" align="center"><a href="../admin/facturacion_venta_temporal_producto_integrado.php?modo_venta_por_defecto=manual"><strong>Venta Manual</strong></a></td><?php } ?>
                <td bgcolor="#fff" align="center"><strong>Buscar por:</strong>
                    <select class="form-control" name="buscar_por" id="buscar_por" onchange="hacer_busqueda()" style="width: 180px;">
                        <?php if (isset($buscar_por)) { echo ""; } else { echo  "<option value='' selected >Selecione</option>"; }
                        $consulta2_sql = ("SELECT cod_buscar_por, nombre_buscar_por, titulo_buscar_por FROM tbl15_buscar_por WHERE (cod_estado = '1') ORDER BY cod_buscar_por ASC");
                        $consulta2 = mysqli_query($conectar, $consulta2_sql);
                        while ($datos2 = mysqli_fetch_assoc($consulta2)) {
                        if(isset($buscar_por) and $buscar_por == $datos2['nombre_buscar_por']) {
                        $seleccionado = "selected"; } else { $seleccionado = ""; }
                    ...
                    [<?php echo $cod_base_caja ?>]<input name="cod_producto_barra" id="busqueda" style="height:26" placeholder="C&oacute;digo del producto" required autofocus/><input type="submit" style="font-size:15px" name="buscador" tabindex=3 value="Vender Producto" />
                </td>
            </tr>
                ...
                <input type="hidden" name="modo_venta_por_defecto" value="<?php echo $modo_venta_por_defecto; ?>">
...
<?php } ?>
    ...
    <table class="table table-striped">
        ...
        $smtr++; ?>
            <th style="text-align:center; width:50px;"><a href="<?php echo $pagina_local ?>?cod_categoria=<?php echo $cod_categoria?>&cuenta=<?php echo $cuenta_actual?>&cod_caja_virtual=<?php echo $cod_caja_virtual?>"><img src="../imagenes/categoria_producto.png"><br><span style="font-family:verdana,geneva,sans-serif; font-size:12px"><?php echo $nombre_categoria; ?></span></a></th>
        <?php } ?>
        </tr>
    </table>
<?php if ($cod_estado_btn_categoria_desplegable_global == 1) { ?>
...
<?php } ?>
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_factura_venta_temporal_integral.php'); ?>
<!-- ***************************************************************************************************************************** -->
...
?>
    <tr id="tr<?php echo $cod_venta_producto_temporal;?>">
        ...
        <?php if (($nombre_tipo_und_caja_sobre=='CAJA') || ($nombre_tipo_und_caja_sobre=='SOBRE')) { ?>
            <td style="text-align:center;" id="und_caja_sobre_<?php echo $incre;?>"><input name="und_caja_sobre" type="number" id="und_caja_sobre<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" onChange="calc_total_venta();" value="<?php echo $und_caja_sobre;?>" step="any" lang="en" min=0 <?php echo $max_und_venta;?> oninput="validity.valid||(value='');" style="width: 70px;" lang="en" min=0 <?php echo $max_und_venta;?> oninput="validity.valid||(value='');"/></td>
