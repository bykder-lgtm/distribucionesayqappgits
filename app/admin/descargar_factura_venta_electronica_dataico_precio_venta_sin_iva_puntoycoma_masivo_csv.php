...
if (isset($_POST["GENERAR_ARCHIVO_PLANO_DE_FACTURA_ELECTRONICA"])) {
	...
	foreach($_POST["cod_info_factura_venta"] as $key => $cod_info_factura_venta) {
		...
		$sql_data = sprintf("UPDATE tbl15_venta_producto SET cod_estado_check_factura_electronica = '$cod_estado_check_factura_electronica' WHERE cod_info_factura_venta = '$cod_info_factura_venta'");
		$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));

		$sql_data = sprintf("UPDATE tbl15_info_factura_venta SET cod_estado_check_factura_electronica = '$cod_estado_check_factura_electronica' WHERE cod_info_factura_venta = '$cod_info_factura_venta'");
		$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
		...
		tbl15_venta_producto.cod_venta_producto, tbl15_venta_producto.cod_producto_barra, tbl15_venta_producto.cod_factura, tbl15_venta_producto.nombre_producto, tbl15_venta_producto.und_venta, 
		...
		while ($datos = mysqli_fetch_assoc($consulta)) {
			...
			$nombres_tercero                 = $nombre1_tercero.' '.$nombre2_tercero;
			$apellidos_tercero               = $apellido1_tercero.' '.$apellido2_tercero;
			$nombre_cliente_concat           = $nombre1_tercero.' '.$nombre2_tercero.' '.$apellido1_tercero.' '.$apellido2_tercero;
			...
			$salida .=''.$nombre_pais.';';
			$salida .=''.$nombre_cliente_concat.';';
			$salida .=''.$nombres_tercero.';';
