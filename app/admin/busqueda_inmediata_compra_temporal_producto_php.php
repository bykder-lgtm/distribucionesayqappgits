...
if($buscar <> NULL) {
	if ($buscar_por == 'nombre_producto') {
		...
		$mostrar_datos_sql = "SELECT * FROM tbl15_producto WHERE (cod_producto_barra LIKE '$buscar') $condicional_barra2 ORDER BY nombre_producto ASC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'cod_producto_barra_nombre_producto') {
		$mostrar_datos_sql = "SELECT * FROM tbl15_producto WHERE (nombre_producto LIKE '$buscar%') OR (cod_producto_barra LIKE '$buscar') $condicional_barra2 ORDER BY nombre_producto ASC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
...
<?php if ($cod_estado_cod_barra2_global == '1') { ?><td style="text-align:center;"><?php echo $cod_producto_barra2; ?></td><?php } ?>
	<td align="Left"><a href="../admin/reg_compra_temporal_producto_reg.php?cod_producto_barra=<?php echo $cod_producto_barra?>&buscar_por=<?php echo $buscar_por?>&nombre_tipo_cargue_factura=<?php echo $nombre_tipo_cargue_factura?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&pagina=<?php echo $pagina?>" tabindex=3><?php echo $nombre_producto ?></a></td>
