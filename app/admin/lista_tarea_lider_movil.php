...
// ****************************** MODULO DE SESION ******************************
include_once("../admin/01_admin_modulo_inicio_sesion_adm_lider.php");
include_once("../admin/01_admin_modulo_info_empresa_adm_lider.php");
// ******************************************************************************

// Consultar mis tareas (Líder) conectadas a los estados de la nueva tabla con uniones para nombres
$sql_tareas = "
SELECT 
    t.*, 
...
// Obtener administradores externos para asignación
$sql_externos = "SELECT cod_administrador, nombres_apellidos_tercero FROM tbl15_administrador WHERE cod_estado != '0' AND cod_lider = '$cod_administrador' ORDER BY nombres_apellidos_tercero ASC";
...
<style>
:root {
    --theme-color: #f59e0b; /* Scrum Amber theme */
...
.kanban-column {
    min-width: 85vw; /* For mobile swiping */
    max-width: 85vw;
...
@media (min-width: 768px) {
    .kanban-column { min-width: 320px; max-width: 350px; scroll-snap-align: start; }
}
...
.column-badge { background: rgba(var(--theme-color-rgb), 0.2); padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.8rem; }
...
.task-title { font-size: 1rem; font-weight: 700; margin-bottom: 0.5rem; color: #facc15; }
...
<div style="padding: 1.5rem 1rem 0 1rem;">
    <h1 style="font-size: 1.5rem; margin:0;"><i class="fa-solid fa-list-check" style="color:var(--theme-color);"></i> Tablero Scrum</h1>
    <p style="color:rgba(255,255,255,0.6); font-size:0.85rem; margin-top:5px;">Gestiona tus tareas de forma ágil</p>
</div>
            ...
            <div class="column-body">
                ...
                <div class="task-card filter-<?php echo $filtro_clase; ?>" onclick="verTarea(<?php echo $t['cod_tarea']; ?>)">
                    ...
                    <div class="task-footer">
                        ...
                        <button class="btn-sm btn-outline-theme" onclick="event.stopPropagation(); moverTarea(<?php echo $t['cod_tarea']; ?>,'<?php echo $t['nombre_estado_tarea']; ?>')">Mover <i class="fa-solid fa-person-walking-arrow-right"></i></button>
                    </div>
                </div>
                <?php } } ?>
            </div>
...
function abrirModalNuevaTarea() { document.getElementById('modalNuevaTarea').classList.add('show'); }

function cerrarModalNuevaTarea() { document.getElementById('modalNuevaTarea').classList.remove('show'); }
...
function moverTarea(cod_tarea, estado_actual) {
    ...
    Swal.fire({
        title: 'Actualizar Estado', html: `<select id="swal-nuevo-estado" class="swal2-select" style="width:100%;">${optionsHtml}</select>`, background: '#1a1f2e', color: '#fff', showCancelButton: true, confirmButtonText: 'Actualizar',
        preConfirm: () => { return document.getElementById('swal-nuevo-estado').value; }
    }).then((result) => {
...
function verTarea(cod_tarea) {
    // Para ver los detalles. Expandible
    console.log("Visualizando " + cod_tarea);
}
</script>
</body>
</html>
