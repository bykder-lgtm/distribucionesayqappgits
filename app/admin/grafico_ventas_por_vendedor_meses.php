...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto, fecha_mes_venta_producto FROM tbl15_venta_producto WHERE vendedor = '$vendedor' GROUP BY fecha_mes_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto) AS total_venta_producto FROM tbl15_venta_producto WHERE vendedor = '$vendedor' GROUP BY fecha_mes_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysql_fetch_array($result)) {
