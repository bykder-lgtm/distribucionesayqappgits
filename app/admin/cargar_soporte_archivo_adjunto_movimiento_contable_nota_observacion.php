...
$datos_data_info_factura = "SELECT * FROM tbl15_movimiento_contable WHERE (cod_movimiento_contable = '$cod_movimiento_contable')";
$consulta_data_info_factura = mysqli_query($conectar, $datos_data_info_factura);
$data_info_factura = mysqli_fetch_assoc($consulta_data_info_factura);
$factura_ocupada = mysqli_num_rows($consulta_data_info_factura);

$cod_movimiento_contable                               = $data_info_factura['cod_movimiento_contable'];
...
$descripcion_movimiento                                = $data_info_factura['descripcion_movimiento'];
...
$pagina_redirect                                       = $pagina.'?cod_movimiento_contable='.$cod_movimiento_contable.'&pagina='.$pagina;
?>
...
<form name="formulario_de_actualizacion" method="post" enctype="multipart/form-data" action="../admin/cargar_soporte_archivo_adjunto_movimiento_contable_nota_observacion_reg.php">
<fieldset>
<table border="1" class="table table-responsive">
    <thead>
        ...
        <tr>
            ...
            <td style="text-align:center;"><input class="input-block-level" name="nombre_nota_observacion" type="text" value="<?php echo $descripcion_movimiento ?>" placeholder="" /></td>

            <td style="text-align:center">
                <select name="cod_tipo_nota_observacion" id="cod_tipo_nota_observacion" class="form-control">
                <?php $sql_consulta="SELECT * FROM tbl15_tipo_nota_observacion WHERE (cod_tipo_nota_observacion = '10')";
...
<tbody>
<?php
$mostrar_datos_sql = "SELECT * FROM tbl15_nota_observacion WHERE (cod_movimiento_contable = '$cod_movimiento_contable') ORDER BY cod_nota_observacion DESC";
...
while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
    ...
    $cod_cuentas_cobrar_abonos                      = $matriz_consulta['cod_cuentas_cobrar_abonos'];
    $cod_cuentas_pagar_abonos                       = $matriz_consulta['cod_cuentas_pagar_abonos'];
    ...
    if (($cod_tipo_nota_observacion == 0) && ($url_img_orig_producto <> '')) { //NOTAS Y OBSERVACIONES
        ...
        $url_redirect_recurso = '../admin/aaaaaaa.php'.'?cod_movimiento_contable='.$cod_movimiento_contable;
        $titulo_celda = "";
    } elseif (($cod_tipo_nota_observacion == 11) && ($url_img_orig_producto <> '')) { //SOPORTES ABONO CUENTA COBRAR
        $url_redirect_recurso = '../admin/aaaaaaa.php'.'?cod_cuentas_cobrar_abonos='.$cod_cuentas_cobrar_abonos;
        $titulo_celda = "";
    } elseif (($cod_tipo_nota_observacion == 12) && ($url_img_orig_producto <> '')) { //SOPORTES ABONO CUENTA PAGAR
        $url_redirect_recurso = '../admin/aaaaaaa.php'.'?cod_cuentas_pagar_abonos='.$cod_cuentas_pagar_abonos;
        $titulo_celda = "";
    } else {
        $url_redirect_recurso = '../admin/aaaaaaa.php'.'?cod_nota_observacion='.$cod_nota_observacion;
    }
...
<tr id="elim<?php echo $cod_nota_observacion;?>">
    <td style="text-align:center" id="elim<?php echo $cod_nota_observacion;?>"><a href="../admin/soporte_principal_factura_venta_reg.php?cod_nota_observacion=<?php echo $cod_nota_observacion?>&cod_movimiento_contable=<?php echo $cod_movimiento_contable ?>&pagina=<?php echo $pagina_local ?>"><img src="<?php echo $url_img_active ?>" class="img-polaroid" alt=""></a></td>
