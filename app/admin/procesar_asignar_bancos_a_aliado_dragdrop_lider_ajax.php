...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['accion'])) {
    ...
    if ($_POST['accion'] == 'asignar_cuenta_aliado') {
        ...
        if ($id_cuenta > 0) {
            ...
            $sql_update = "UPDATE tbl15_banco_cuenta SET cod_aliado_estrategico = $query_aliado WHERE cod_banco_cuenta = '$id_cuenta'";
            if (mysqli_query($conectar, $sql_update)) { echo json_encode(['status' => 'success', 'message' => 'La cuenta bancaria ha sido asignada correctamente.']); } else { echo json_encode(['status' => 'error', 'message' => 'Error al actualizar base de datos: ' . mysqli_error($conectar)]); }
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Identificador de cuenta no válido.']);
        }
        exit;
    }
