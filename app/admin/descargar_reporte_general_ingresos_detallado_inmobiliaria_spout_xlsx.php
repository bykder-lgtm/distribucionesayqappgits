...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_INGRESOS_DETALLADO_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_INGRESOS_DETALLADO";
...
// Then a foreach
	...
	while ($info_ingreso = mysqli_fetch_assoc($resultado_ingreso)) {
		...
		$writer->addRow(array($nombre_concepto_movimiento_caja, $costo, $comentario, $nombre_dependencia, $nombre_tipo_forma_pago, date("d-m-Y", strtotime($fecha_dmy))));
	}
	$writer->addRow(array('TOTAL', $total_otros_ingresos,));
