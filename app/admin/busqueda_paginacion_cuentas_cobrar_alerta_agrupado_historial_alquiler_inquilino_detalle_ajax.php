...
<th style="text-align: center;">MES_|_PRECIO_|_ALQUILER_|_FECHA_LIMITE_PAGO_|_ESTADO_PAGO</th>
</tr>
...
tbl15_cuentas_cobrar_alerta.cod_cuentas_cobrar, tbl15_cuentas_cobrar_alerta.vendedor, tbl15_cuentas_cobrar_alerta.cod_factura
...
WHERE (tbl15_cuentas_cobrar_alerta.cod_estado_archivado = '0') AND $filtro_consulta_estado_busqueda_ajax_rel $filtro_consulta_estado_hoy_rel $filtro_consulta_estado_pago_rel $filtro_consulta_buscar_por_rel 
GROUP BY tbl15_cuentas_cobrar_alerta.cod_factura ORDER BY tbl15_tercero.nombre1_tercero ASC";
...
$sql_mostrar_todo = "SELECT * FROM tbl15_cuentas_cobrar_alerta WHERE (cod_factura = '$cod_factura') AND (cod_estado_archivado = '0') AND (fecha_mes <= '$fecha_mes') AND (cod_estado <> '1') ORDER BY fecha_pago ASC";
$consulta_mostrar_todo = mysqli_query($conectar, $sql_mostrar_todo);
while ($datos_mostrar_todo = mysqli_fetch_assoc($consulta_mostrar_todo)) {

$cod_cuentas_cobrar_alerta      = $datos_mostrar_todo['cod_cuentas_cobrar_alerta'];
$cod_cuentas_cobrar             = $datos_mostrar_todo['cod_cuentas_cobrar'];
$numero_alerta                  = $datos_mostrar_todo['numero_alerta'];
$cod_factura                    = $datos_mostrar_todo['cod_factura'];
$monto_deuda                    = $datos_mostrar_todo['monto_deuda'];
$abonado                        = $datos_mostrar_todo['abonado'];
$subtotal                       = $datos_mostrar_todo['subtotal'];
$mensaje                        = $datos_mostrar_todo['mensaje'];
$fecha_pago                     = $datos_mostrar_todo['fecha_pago'];
$vendedor                       = $datos_mostrar_todo['vendedor'];
$monto_cuota                    = $datos_mostrar_todo['monto_cuota'];
$cod_estado                     = $datos_mostrar_todo['cod_estado'];
$fecha_pago_reg                 = $datos_mostrar_todo['fecha_pago_reg'];
$hora_pago_reg                  = $datos_mostrar_todo['hora_pago_reg'];
$cod_producto                   = $datos_mostrar_todo['cod_producto'];
$cod_producto_barra             = $datos_mostrar_todo['cod_producto_barra'];
$nombre_producto                = $datos_mostrar_todo['nombre_producto'];
$cod_cuentas_cobrar_abonos      = $datos_mostrar_todo['cod_cuentas_cobrar_abonos'];
...
$cuentas_cobrar_viejas_concat  .= "".str_replace(' ',"&nbsp;",str_pad('('.$nombre_letra_tabla_mes.' - '.$nombre_anyo_mes.')',20,' ',STR_PAD_RIGHT)).' | '.str_replace(' ',"&nbsp;",str_pad(number_format($monto_cuota, 0, ",", "."),8,' ',STR_PAD_RIGHT)).' | '.str_replace(' ',"&nbsp;",str_pad($fecha_pago,11,' ',STR_PAD_RIGHT)).' | '.str_replace(' ',"&nbsp;",str_pad($nombre_estado,10,' ',STR_PAD_RIGHT)).'<br>';
}
?>
<tr>
<?php if ($cod_seguridad== '3') { ?>
...
<?php } ?>
<td style="text-align: center;"><a href="../admin/cuentas_cobrar_agrupado_detalle_factura_alquiler.php?cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar;?>&cod_tercero=<?php echo $cod_tercero;?>&cod_factura=<?php echo $cod_factura;?>&pagina=<?php echo $pagina;?>&cod_estado_hoy=<?php echo $cod_estado_hoy;?>&cod_estado_pago=<?php echo $cod_estado_pago;?>&buscar_por=<?php echo $buscar_por;?>&busqueda_ajax=<?php echo $busqueda_ajax;?>&action=<?php echo $action;?>&tabla=<?php echo $tabla;?>&page=<?php echo $page;?>"><img src=../imagenes/ver.png alt="Abonar"></a></td>
...
<td style="text-align: left;"><font size='3'><?php echo $cuentas_cobrar_viejas_concat;?></font></td>
</tr>
<?php } ?>
</table>