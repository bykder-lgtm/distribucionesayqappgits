...
if (isset($_POST["cod_banner_slider"])) {
	...
	if ($url_img1 <> '') { 
		...
		$nombre_normal2                  = 'SLIDER_PERSONAJE_'.$fecha_ymdHis.'_'.$cod_banner_slider.'_ori'.'.'.$formato_orig2;
		$url_personaje_banner_slider      = $ruta_foto_orig.$nombre_normal2;

		copy($_FILES['url_img1']['tmp_name'], $url_personaje_banner_slider);
		/* ----------------------------------------------------------------------------------------------------------/ */
		...
		if ($imagen_foto_miniatura->uploaded) {
			...
			$imagen_foto_miniatura->file_new_name_body          = 'SLIDER_PERSONAJE_'.$fecha_ymdHis.'_'.$cod_banner_slider.'_min'; // agregamos un nuevo nombre
			$imagen_foto_miniatura->process($ruta_foto_miniatura);

			$nombre_miniatura = 'SLIDER_PERSONAJE_'.$fecha_ymdHis.'_'.$cod_banner_slider.'_min'.'.'.$formato;
			$url_personaje_banner_slider_min = $ruta_foto_miniatura.$nombre_miniatura;

			$sql_data = sprintf("UPDATE tbl15_banner_slider SET url_personaje_banner_slider = '$url_personaje_banner_slider', url_personaje_banner_slider_min = '$url_personaje_banner_slider_min' WHERE cod_banner_slider = '$cod_banner_slider'");
