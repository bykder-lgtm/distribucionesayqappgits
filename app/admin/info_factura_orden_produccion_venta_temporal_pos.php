...
<?php
$suma_temporal = "SELECT Sum(total_venta_producto) As total_venta, Sum(total_costo_producto) As total_compra FROM tbl15_orden_produccion_venta_producto_temporal 
...
$datos_data_info_factura = "SELECT * FROM tbl15_info_orden_produccion_factura_venta WHERE (nombre_estado_factura = 'ABIERTA') AND (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
$(document).ready(function(){
    $("#nombre_comentario").on('change', function () {
            ...
            var tipo_ajax = "tbl15_info_orden_produccion_factura_venta";
            $.post("guardar_info_factura_y_orden_produccion_venta_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_orden_produccion_factura_venta; ?> }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            var tipo_ajax = "tbl15_orden_produccion_venta_producto_temporal";
...
<form method="post" name="formulario" action="../admin/orden_produccion_venta_producto_reg.php">
...
<?php 
$sql_temporal = "SELECT cod_orden_produccion_venta_producto_temporal FROM tbl15_orden_produccion_venta_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
<input type="hidden" name="cod_orden_produccion_venta_producto_temporal[]" value="<?php echo $datos_temporal['cod_orden_produccion_venta_producto_temporal']; ?>" size="4">
<?php } ?>  
<?php $pagina ='facturacion_orden_produccion_venta_temporal_producto_manual_pos.php'; ?>
<input type="hidden" name="cod_info_orden_produccion_factura_venta" value="<?php echo $cod_info_orden_produccion_factura_venta ?>" size="10">
