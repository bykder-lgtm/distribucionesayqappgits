...
if (($campo=='precio_venta_producto') && ($tipo_ajax=='tbl15_gasto_inmueble_detalle_venta_temporal')) {
	...
	$sql_productos = "SELECT SUM(precio_venta_producto) AS precio_venta_producto FROM tbl15_gasto_inmueble_detalle_venta_temporal WHERE (cod_factura = '$cod_factura') $filtro_incluido";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
...
if (($campo=='precio_compra_producto') && ($tipo_ajax=='tbl15_gasto_inmueble_detalle_venta_temporal')) {
	...
	$sql_productos = "SELECT SUM(precio_compra_producto) AS precio_compra_producto FROM tbl15_gasto_inmueble_detalle_venta_temporal WHERE (cod_factura = '$cod_factura') $filtro_incluido";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
