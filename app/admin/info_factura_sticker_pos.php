...
<?php
$suma_temporal = "SELECT Sum(total_venta_producto) As total_venta, Sum(total_costo_producto) As total_compra FROM tbl15_sticker_producto 
...
$datos_data_info_factura = "SELECT * FROM tbl15_info_factura_sticker WHERE (cod_info_factura_sticker = '$cod_info_factura_sticker')";
...
$(document).ready(function(){
    $("#fecha_anyo").on('change', function () {
            ...
            $.post("guardar_info_factura_y_sticker_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_factura_sticker; ?> }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            var tipo_ajax = "tbl15_sticker_producto";
            var id = $(this).attr("class");
            //let id = this.id;
            $.post("guardar_info_factura_y_sticker_producto_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:id }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
<form method="post" name="formulario" action="../admin/sticker_tiketeadora_productos_opcion_imprimir_js.php?cod_info_factura_sticker=<?php echo $cod_info_factura_sticker ?>&pagina=0">
...
<?php 
$sql_temporal = "SELECT cod_sticker_producto FROM tbl15_sticker_producto WHERE (cod_info_factura_sticker = '$cod_info_factura_sticker')";
...
<input type="hidden" name="cod_sticker_producto[]" value="<?php echo $datos_temporal['cod_sticker_producto']; ?>" size="4">
<?php } ?>
