...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formulario_de_actualizacion")) {
	...
	$nombre_nota_observacion                            = 'SOPORTES FACTURA DE VENTA | '.addslashes($_POST['nombre_nota_observacion']);

	$pagina_redirect                                    = $pagina.'?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&pagina='.$pagina;
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($url_img1 <> '') { 
		...
		$nombre_normal2                                     = $tipo_soporte.'_'.$fecha_ymdHis.'_'.$cod_info_factura_venta.'_'.$cod_tercero.'_'.$cod_administrador.'.'.$formato_img2;
...
//-------------------------------------- -----------------------------------------------------------------//
//-------------------------------------- -----------------------------------------------------------------//
	$actualizar_sql1 = "UPDATE tbl15_info_factura_venta SET url_img_orig_producto = '$url_img_orig_producto' WHERE (cod_info_factura_venta = '$cod_info_factura_venta')";
	$resultado_actualizacion1 = mysqli_query($conectar, $actualizar_sql1) or die(mysqli_error($conectar));

	$agreg = "INSERT INTO tbl15_nota_observacion (nombre_nota_observacion, fecha_ymd, fecha_hora, cuenta, cod_administrador, fecha_creacion, cod_tipo_nota_observacion, cod_info_factura_venta, url_img_orig_producto, url_img_min_producto) 
	VALUES ('$nombre_nota_observacion', '$fecha_ymd', '$fecha_hora', '$cuenta_actual', '$cod_administrador', '$fecha_creacion', '$cod_tipo_nota_observacion', '$cod_info_factura_venta', '$url_img_orig_producto', '$url_img_min_producto')";
