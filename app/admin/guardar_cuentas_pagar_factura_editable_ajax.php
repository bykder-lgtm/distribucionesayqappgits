...
if ($campo == 'monto_deuda') {
	$sql_edit_cuenta_cob = "UPDATE tbl15_cuentas_pagar SET $campo = '$valor_intro' WHERE (cod_cuentas_pagar = '$cod_cuentas_pagar')";
	$consulta_edit_cuenta_cob = mysqli_query($conectar, $sql_edit_cuenta_cob) or die(mysqli_error($conectar));
//----------------------------------------------------------------------------------------------------//
//----------------------------------------------------------------------------------------------------//
	$sql_cuenta_pagar_factura = "SELECT monto_deuda, cod_tercero, cod_factura, cod_info_factura_compra FROM tbl15_cuentas_pagar WHERE (cod_cuentas_pagar = '$cod_cuentas_pagar')";
	...
	$cod_tercero                 = $dato_cuenta_pagar_factura['cod_tercero'];
	$cod_factura                 = $dato_cuenta_pagar_factura['cod_factura'];
	$cod_info_factura_compra     = $dato_cuenta_pagar_factura['cod_info_factura_compra'];
//----------------------------------------------------------------------------------------------------//
//----------------------------------------------------------------------------------------------------//
	$sql_datos_cuenta_pagar = "SELECT SUM(monto_deuda) AS total_monto_deuda_cuenta_pagar FROM tbl15_cuentas_pagar WHERE (cod_tercero = '$cod_tercero')";
...
//----------------------------------------------------------------------------------------------------//
//----------------------------------------------------------------------------------------------------//
	$sql_datos_cuentas_pagar = "SELECT monto_deuda, abonado FROM tbl15_cuentas_pagar WHERE (cod_cuentas_pagar = '$cod_cuentas_pagar')";
	...
	$monto_deuda                           = $datos_cuentas_pagar['monto_deuda'];
	$abonado                               = $datos_cuentas_pagar['abonado'];
	$total_pendiente                       = $monto_deuda - $abonado;

	$sql_edit_cuenta_cob = "UPDATE tbl15_cuentas_pagar SET subtotal = '$total_pendiente' WHERE (cod_cuentas_pagar = '$cod_cuentas_pagar')";
	$consulta_edit_cuenta_cob = mysqli_query($conectar, $sql_edit_cuenta_cob) or die(mysqli_error($conectar));
//----------------------------------------------------------------------------------------------------//
//----------------------------------------------------------------------------------------------------//
