...
$fecha_ymd_venta_producto = $registros["fecha_ymd_venta_producto"];
$fecha_anyo = date("d/m/Y", $fecha_ymd_venta_producto);
?>
'<?php echo $fecha_anyo ?>',
<?php
}
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto-total_compra_producto) AS total_venta_producto FROM tbl15_venta_producto GROUP BY fecha_ymd_venta_producto ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
...
?>
            ...
            data: [
<?php
$sql = "SELECT SUM(costo) AS costo FROM tbl15_egreso GROUP BY fecha_dmy ORDER BY fecha_dmy ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
