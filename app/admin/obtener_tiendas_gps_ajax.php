...
// Construir subquery de aliados según el rol
// cod_seguridad: 21 (Lider), 22 (Coordinador), 2 (Asesor)
// Aliados tienen cod_seguridad = 23
...
if ($cod_seguridad == '21') { // LIDER
    ...
    // Si no es ninguno de los roles anteriores, devolvemos vacío o restringido
    // Para administradores (rol 1 u otros), quizá mostrar todo? 
    // El requerimiento dice Lideres, Coordinadores y Asesores.
}
...
// 2. Obtener solo tiendas CON GPS
$sql_gps = "SELECT 
                t.cod_tienda, 
                t.nombre_tienda, 
                t.nombre1_tercero as dueno,
                t.direccion_tienda,
                t.telefono_tienda,
                t.ubicacion_gps_tienda,
            ...
            WHERE t.cod_aliado_estrategico IN ($subquery_aliados) 
            AND t.ubicacion_gps_tienda IS NOT NULL 
            AND t.ubicacion_gps_tienda != ''
            AND t.cod_estado != '0'";
...
while ($row = mysqli_fetch_assoc($res_gps)) {
    ...
    if (count($coords) == 2) {
        $tiendas[] = [
            ...
            'dueno' => $row['dueno'],
            'aliado' => $row['nombre_aliado'],
            'direccion' => $row['direccion_tienda'],
            'telefono' => $row['telefono_tienda'],
            'lat' => trim($coords[0]),
            'lng' => trim($coords[1])
        ];
    }
