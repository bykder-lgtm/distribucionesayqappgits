...
$cod_administrador_tercero    = $cod_administrador;
...
if (isset($_REQUEST['cod_producto_barra'])) {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------//
	$sql_zona1 = "SELECT nombre1_tercero, telefono1_tercero, correo_tercero FROM tbl15_administrador WHERE (cod_administrador = '$cod_administrador')";
	$consulta_zona1 = mysqli_query($conectar, $sql_zona1);
	$datos_zona1 = mysqli_fetch_assoc($consulta_zona1);

	$nombre1_tercero                    = $datos_zona1['nombre1_tercero'];
	$telefono1_tercero                  = $datos_zona1['telefono1_tercero'];
	$correo_tercero                     = $datos_zona1['correo_tercero'];
	//---------------------------------------------------------------------------------------------------------------------------------------------//
	...
	if ($factura_abierta == '0') {
		...
		nombre1_tercero, telefono1_tercero, correo_tercero, cod_administrador_tercero) 
		...
		precio_venta_producto_orig, und_producto, cod_base_caja, cod_opcion_descontable_inv, url_img_orig_producto, url_img_min_producto, cod_administrador_tercero) 
	...
	//---------------------------------------------------------------------FACTURA ABIERTA INICIO----------------------------------------------------------------//
		...
		cod_opcion_descontable_inv, url_img_orig_producto, url_img_min_producto, cod_administrador_tercero) 
		VALUES ('$cod_info_factura_venta_carrito_compra', '$cod_tercero', '$cod_producto', '$cod_producto_barra', '$nombre_producto', '$und_venta', 
		'$precio_compra_producto', '$total_compra_producto', '$precio_costo_producto', '$total_costo_producto', 
		'$precio_venta_producto', '$total_venta_producto', '$posologia_cantidad', '$posologia_peso', '$nombre_tipo_producto', '$nombre_tipo_unidad_medida', '$nombre_tipo_presentacion', 
		'$nombre_via_administracion', '$nombre_frec_duracion', '$fecha_ymd_venta_producto', '$fecha_mes_venta_producto', '$fecha_anyo_venta_producto', '$fecha_seg_venta_producto', '$cuenta', 
	...
	$codigoHTML_menu .= '            <a href="../admin/carrito_compra_temporal_visitante_intern_confirmdirect.php" class="btn btn-default hvr-hover btn-cart">VER CARRITO</a>';
