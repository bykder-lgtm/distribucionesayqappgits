...
<table class="table table-striped">
	<tr>
		<th style="text-align: center;"><h4>Listado Cartera Pagada</h4></th>
	</tr>
</table>
...
FROM tbl15_cuentas_cobrar WHERE ((tbl15_cuentas_cobrar.cod_estado_pago = '1') OR (tbl15_cuentas_cobrar.subtotal <= '0')) ORDER BY tbl15_cuentas_cobrar.cod_cuentas_cobrar DESC";
...
while ($datos_cuenta_pagar = mysqli_fetch_assoc($consulta_datos_cuenta_pagar)) {
	...
	if ($codigo_tipo_modulo_cuenta_cobrar_defect_global == '1') {
		...
		$url_redirect = "../admin/cuentas_cobrar_detalle_factura_todo_archivado_y_no_archivado.php?cod_tercero=".$cod_tercero."&cod_cuentas_cobrar=".$cod_cuentas_cobrar.'&cod_estado_pago='.$cod_estado_pago."&pagina=".$pagina;
	}
