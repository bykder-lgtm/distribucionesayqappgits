    ...
    if ($directorio_min) {
        ...
        if ($dimensiones) {
            ...
            if ($imagen_orig) {
                ...
                switch ($tipo_imagen) { case 'image/png': imagepng($imagen_nueva, $ruta_min); break; case 'image/gif': imagegif($imagen_nueva, $ruta_min); break; default: imagejpeg($imagen_nueva, $ruta_min, 85); }
...
if (isset($_POST['identificacion_tercero']) && !empty($_POST['identificacion_tercero'])) {
    ...
    // Aceptar ambos nombres de campo (compatibilidad con diferentes formularios)
	...
	//---------------------------------------------------------------------------------------------------------------------------------//
    // Verificar si ya existe una tienda con esa identificación
	$sql_dato_tienda = "SELECT cod_tienda FROM tbl15_tienda WHERE identificacion_tercero = '".($identificacion_tercero)."'";
	$consultar_dato_tienda = mysqli_query($conectar, $sql_dato_tienda) or die(mysqli_error($conectar));
	$existe_dato_tienda = mysqli_num_rows($consultar_dato_tienda);
	//---------------------------------------------------------------------------------------------------------------------------------//
    if($existe_dato_tienda > 0) {
        // Ya existe una tienda con esa identificación
        ...
        nombre_plataforma_ecommerce, nombre_sistema_contable, cod_banco_cuenta, ubicacion_gps_tienda, garantia_tienda,
        ...
        if ($exec_data && mysqli_affected_rows($conectar) > 0) {
            $cod_tienda_insertado = mysqli_insert_id($conectar);
            echo json_encode(['success' => true, 'afectado' => 'SI', 'cod_tienda' => $cod_tienda_insertado, 'nombre_tienda' => $nombre_tienda, 'correo_tercero' => $correo_tercero, 'telefono1_tercero' => $telefono1_tercero, 'mensaje' => 'Tienda registrada correctamente']);
        } else {
            echo json_encode(['success' => false, 'mensaje' => 'Error al registrar la tienda: ' . mysqli_error($conectar)]);
        }
    }
} else {
    echo json_encode(['success' => false, 'mensaje' => 'Datos incompletos. El NIT/Documento es requerido.']);
}
?>