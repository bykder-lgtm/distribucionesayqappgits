...
include_once("../admin/01_modulo_diseno_superior_visitante_extnosesion.php");
...
if (isset($_POST['nombre_origen_formulario'])) {
  ...
  $obtener_cedula = "SELECT cod_producto_barra, nombre_producto FROM tbl15_producto WHERE cod_producto = '".($cod_producto)."'";
  ...
  $sql_autoincremento_cotizar_producton = "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$base_datos' AND TABLE_NAME = 'tbl15_cotizar_producto'";
  ...
  $sql = "INSERT INTO tbl15_cotizar_producto (cod_producto, cod_producto_barra, nombre_producto, asunto, nombre, correo, 
  ...
  VALUE (\"$cod_producto\",\"$cod_producto_barra\",\"$nombre_producto\",\"$asunto\",\"$nombre\",\"$correo\",
  \"$telefono\",\"$telefono_whatsapp\",\"$comentario\",\"$nombre_origen_formulario\",\"$nombre_estado\",\"$ip\",\"$nombre_pais\",\"$region\",\"$ciudad\",
  \"$longitud\",\"$latitud\",\"$fecha_ymd_his\",\"$fecha_hora\",\"$fecha_ymd\",\"$fecha_mes\",\"$fecha_anyo\",\"$fecha_time\")";
  $insertar_datos = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
  ...
  $nombre_contacto1_limp                    = str_replace(" ", "_", $nombre);
  $motivo_consulta_limp                     = str_replace(" ", "_", $nombre);
  $nombre_empresa_limp                      = str_replace(" ", "_", $nombre);
  $nombres_limp                             = str_replace(" ", "_", $nombre);
  $cod_historia_clinica_strpad              = "";
  ...
  <thead>
    ...
    <tr>
      <th style='text-align:center;'><span style='color: #000; font-size: 12px; line-height: 16.8px; font-family: Lato, sans-serif;'>Producto: ".$cod_producto_barra.' - '.$nombre_producto."</span></th>
    </tr>
    <tr>
  ...
  if(!$mail->send()) { //INICIO SI EL CORREO NO SE ENVIA PORQUE HAY ERRORES
    ...
    $url_redir = "../admin/formulario_cotizacion_opcion_imprimir_visitante_extnosesion.php?cod_cotizar_producto=".$cod_cotizar_producto."&pagina=".$pagina;
?>
