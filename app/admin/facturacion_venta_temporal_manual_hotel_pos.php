...
xmlhttp.open("POST","../admin/busqueda_inmediata_venta_temporal_hotel_php.php",true);
...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody>
    	<tr>
            ...
            $consulta2_sql = ("SELECT cod_buscar_por, nombre_buscar_por, titulo_buscar_por FROM tbl15_buscar_por WHERE ((cod_tipo_busqueda = '1') AND (cod_estado = '1')) ORDER BY cod_buscar_por ASC");
            ...
            <strong>Venta Manual:<!--<a href="../admin/facturacion_venta_temporal_producto_manual_pos_completa.php">%%</a>--> [<?php echo $cod_base_caja ?>]<input type="text" id="busqueda" name="busqueda" onkeyup="hacer_busqueda()" style="height:40" placeholder="Buscar"/><input type="hidden" id="pagina" name="pagina" value="<?php echo $pagina_local ?>"/></strong><div id="logo_cargador"></div>
            </td>
    	</tr>
    </tbody>
</table>
...
?>
    ...
    <div class="row">
        ...
        <?php
        $smtr = 0;
        $mostrar_datos_sql = "SELECT cod_producto, cod_producto_barra, nombre_producto, cod_categoria, precio_venta_producto, url_img_orig_producto, url_img_min_producto
        FROM tbl15_producto WHERE (cod_categoria = '$cod_categoria') AND (nombre_tipo_producto <> 'SUBPRODUCTO') AND  (cod_estado_habitacion_hotel = '0') ORDER BY nombre_producto DESC";
        $consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
        while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
        ...
        ?>
            <div class="col-lg-12 col-sm-6 col-md-6 col-lg-4 col-xl-4">
                <td style="text-align:center; height:60px"><a href="../admin/reg_venta_temporal_producto_reg.php?cod_producto_barra=<?php echo $cod_producto_barra?>&buscar_por=<?php echo $buscar_por?>&nombre_tipo_moneda=<?php echo $nombre_tipo_moneda?>&nombre_tipo_factura=<?php echo $nombre_tipo_factura?>&foco=<?php echo $foco?>&cod_estado_vacuna=<?php echo $cod_estado_vacuna?>&cuenta=<?php echo $cuenta_actual?>&cod_caja_virtual=<?php echo $cod_caja_virtual?>&cod_categoria=<?php echo $cod_categoria?>&pagina=<?php echo $pagina?>"><img src="<?php echo $url_img_min_producto?>" class="img-responsive imgesp d-none d-md-block" style="height:200%;"><br><?php echo $nombre_producto.'<br>'.number_format($precio_venta_producto, 0, ",", "."); ?></a></td>
            </div>
        <?php } ?>
            </tr>
        </table>
...
<?php } ?>
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_factura_venta_temporal_hotel_pos.php'); ?>
<!-- ***************************************************************************************************************************** -->
...
<thead>
	<tr>
        ...
        <?php if ($cod_estado_origen_produccion == '1') { ?><th style="text-align:center;">ORIGEN PROD</th><?php } ?>
...
?>
	<tr id="tr<?php echo $cod_venta_producto_temporal;?>">
		...
		<?php if ($cod_estado_eliminar_caja_mesa_virtual == '1') { ?><td  style="text-align:center;"><a href="../admin/eliminar.php?llave=<?php echo $cod_venta_producto_temporal?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&cuenta=<?php echo $cuenta ?>&cod_caja_virtual=<?php echo $cod_caja_virtual ?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/eliminar.png" class="img-polaroid" alt=""></a></td><?php } ?>
        ...
        <?php if ($cod_estado_bascula_balanza_cod_barras_pesar_producto_global == '1') { ?><td style="text-align:center;"><button type="button" id="btn_pesar_producto<?php echo $incre;?>" class="btn_pesar_producto">PESAR</button><input name="pesar_producto_input" type="text" id="pesar_producto_input<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="" step="any" style="width: 80px;" /></td><?php } ?>

        <?php if ($cod_estado_habilitar_hora_venta_temporal_global == '1') { ?><td style="text-align:center;"><?php echo $hora_cargue;?></td><?php } ?>

		<?php if ($cod_estado_comentario_venta_global == '1') { ?><td style="text-align:center;"><input name="comentario_producto" type="text" id="comentario_producto<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $comentario_producto;?>" style="width: 150px;" /></td><?php } ?>

        <?php if ($cod_estado_origen_produccion == '1') { ?>
        <td style="text-align:center">
            <select name="cod_origen_produccion" id="cod_origen_produccion<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" data-show-subtext="true" data-live-search="true" style="width: 140px;" required>
		...
		<td style="text-align:center;" id="fecha_ymd_parqueo_ini_<?php echo $incre;?>"><input name="fecha_ymd_parqueo_ini" type="date" id="fecha_ymd_parqueo_ini<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $fecha_ymd_parqueo_ini;?>" style="width: 120px;" />-<input name="fecha_hora_parqueo_ini" type="time" id="fecha_hora_parqueo_ini<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $fecha_hora_parqueo_ini;?>" style="width: 90px;" /></td>
		<td style="text-align:center;" id="fecha_ymd_parqueo_fin_<?php echo $incre;?>"><input name="fecha_ymd_parqueo_fin" type="date" id="fecha_ymd_parqueo_fin<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $fecha_ymd_parqueo_fin;?>" style="width: 120px;" />-<input name="fecha_hora_parqueo_fin" type="time" id="fecha_hora_parqueo_fin<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $fecha_hora_parqueo_fin;?>" style="width: 90px;" /></td>
		<?php } else { ?>
        ...
        <?php if ($cod_estado_peso_producto_global == '1') { ?>
        <td style="text-align:center;" id="peso_producto_<?php echo $incre;?>"><input name="peso_producto" type="number" id="peso_producto<?php echo $incre;?>" class="<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $peso_producto;?>" step="any" min=0 style="width: 70px;" /></td>
        <?php } ?>
