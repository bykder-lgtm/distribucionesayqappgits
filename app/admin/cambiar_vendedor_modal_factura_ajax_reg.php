...
try {
    ...
    $cod_vendedor = mysqli_real_escape_string($conectar, $_POST['cod_vendedor']);
    $cod_estado_factura = '';

    // Validar que se recibieron los datos necesarios
    if (empty($cod_info_factura_venta) || empty($cod_vendedor)) { throw new Exception('Faltan datos requeridos'); }
    // Actualizar el vendedor en la factura
    $sql_actualizar = "UPDATE tbl15_info_factura_venta SET cod_vendedor = ? WHERE cod_info_factura_venta = ?";
    ...
    mysqli_stmt_bind_param($stmt, "ii", $cod_vendedor, $cod_info_factura_venta);
    ...
    if ($filas_afectadas > 0) { 
        echo json_encode(array('success' => true, 'message' => 'Vendedor actualizado correctamente', 'cod_vendedor' => $cod_vendedor, 'cod_banco_cuenta' => $cod_banco_cuenta, 'cod_estado_factura' => $cod_estado_factura)); 
    } else { 
        echo json_encode(array('success' => true, 'message' => 'No se realizaron cambios', 'cod_vendedor' => $cod_vendedor, 'cod_banco_cuenta' => $cod_banco_cuenta, 'cod_estado_factura' => $cod_estado_factura)); 
    }
