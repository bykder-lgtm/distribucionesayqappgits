...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_num_rows($res_check) > 0) { echo json_encode(['status' => 'error', 'message' => 'El asesor ya se encuentra registrado (cédula o correo duplicado).']); exit; }
    // Obtener datos del lider (cod_lider)
    ...
    $cod_asesor                                                         = $cod_administrador_incre;
    $cuenta                                                             = $identificacion.'-'.$cod_administrador_incre;
	//---------------------------------------------------------------------------------------------------------------------------------//
    $sql_coord = "SELECT cod_lider FROM tbl15_administrador WHERE cod_administrador = '$cod_administrador_actual'";
    $res_coord = mysqli_query($conectar, $sql_coord);
    $row_coord = mysqli_fetch_assoc($res_coord);
    $cod_lider                                                          = $row_coord['cod_lider'] ? $row_coord['cod_lider'] : 0;
    ...
    telefono1_tercero, correo_tercero, nombre_tipo_cliente, nombre_tipo_regimen, nombre_tipo_impuesto, cod_lider, cod_lider, cod_asesor, cod_estado_activacion_usuario, 
