...
if($buscar <> NULL) {
	if ($buscar_por == 'nombre1_tercero') {
		$mostrar_datos_sql = "SELECT * FROM tbl15_info_factura_compra INNER JOIN tbl15_tercero ON tbl15_info_factura_compra.cod_tercero = tbl15_tercero.cod_tercero 
        WHERE (tbl15_info_factura_compra.nombre_estado_factura = 'CERRADA') AND (tbl15_tercero.nombre1_tercero LIKE '%$buscar%') AND (tbl15_info_factura_compra.nombre_estado_factura_dataico_dian <> 'ANULADA') 
        ORDER BY cod_info_factura_compra DESC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'identificacion_tercero') {
		$mostrar_datos_sql = "SELECT * FROM tbl15_info_factura_compra INNER JOIN tbl15_tercero ON tbl15_info_factura_compra.cod_tercero = tbl15_tercero.cod_tercero 
        WHERE (tbl15_info_factura_compra.nombre_estado_factura = 'CERRADA') AND (tbl15_tercero.identificacion_tercero LIKE '$buscar%') AND (tbl15_info_factura_compra.nombre_estado_factura_dataico_dian <> 'ANULADA') 
        ORDER BY cod_info_factura_compra DESC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'cod_factura') {
		$mostrar_datos_sql = "SELECT * FROM tbl15_info_factura_compra INNER JOIN tbl15_tercero ON tbl15_info_factura_compra.cod_tercero = tbl15_tercero.cod_tercero 
        WHERE (tbl15_info_factura_compra.nombre_estado_factura = 'CERRADA') AND (tbl15_info_factura_compra.cod_factura LIKE '$buscar') AND (tbl15_info_factura_compra.nombre_estado_factura_dataico_dian <> 'ANULADA') 
        ORDER BY cod_info_factura_compra DESC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} elseif ($buscar_por == 'fecha_ymd_venta_producto') {
		$mostrar_datos_sql = "SELECT * FROM tbl15_info_factura_compra INNER JOIN tbl15_tercero ON tbl15_info_factura_compra.cod_tercero = tbl15_tercero.cod_tercero 
        WHERE (tbl15_info_factura_compra.nombre_estado_factura = 'CERRADA') AND (tbl15_info_factura_compra.fecha_anyo LIKE '$buscar') AND (tbl15_info_factura_compra.nombre_estado_factura_dataico_dian <> 'ANULADA') 
        ORDER BY cod_info_factura_compra DESC";
		$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
		$total_resultados = mysqli_num_rows($consulta);
	} else {
...
while ($datos_total_tipo_factura = mysqli_fetch_assoc($consulta)) {
    ...
    $total_factura_compra_retefuente                    = $datos_total_tipo_factura['total_factura_compra_retefuente'];
    ...
    $sql_datos_venta_temp = "SELECT cod_factura_compra_producto, cod_producto_barra, und_compra, nombre_producto 
    FROM tbl15_factura_compra_producto WHERE (cod_info_factura_compra = '$cod_info_factura_compra') ORDER BY cod_factura_compra_producto DESC $limite_mostrar_registro";
    ...
    while ($datos_venta_temp = mysqli_fetch_assoc($consulta_datos_venta_temp)) {
        ...
        $und_venta_con                       = $datos_venta_temp['und_compra'];

        $nombre_producto_concat .= "".intval($und_venta_con)." | ".$nombre_producto_con.'<br>';
    }
    ?>
	<tr class="even pointer">
        <td style="text-align:center"><a href="../admin/reg_buscar_factura_compra_nota_debito_temporal_reg.php?cod_info_factura_compra=<?php echo $cod_info_factura_compra ?>"><?php echo $nombre_tipo_factura?></a></td>
        <td style="text-align:center"><a href="../admin/reg_buscar_factura_compra_nota_debito_temporal_reg.php?cod_info_factura_compra=<?php echo $cod_info_factura_compra ?>"><?php echo ($prefijo_resolucion_facturacion.'|'.$cod_factura)?></a></td>
        <td style="text-align:left"><a href="../admin/reg_buscar_factura_compra_nota_debito_temporal_reg.php?cod_info_factura_compra=<?php echo $cod_info_factura_compra ?>"><?php echo $nombre_tercero?></a></td>
