...
if ((isset($_POST["insersion"])) && ($_POST["insersion"] == "formulario_de_insersion")) {
	...
	if (isset($_POST['observacion_archivador']) <> '') { $observacion_archivador = addslashes($_POST['observacion_archivador']); } else { $observacion_archivador = ''; }
...
/* ----------------------------------------------------------------------------------------------------------/ */
	$limpiar_nombre_paciente                            = str_replace(' ','_', funcion_eliminar_acentos(trim(substr($nombre_paciente, 0, 30))));
	$limpiar_nombre_medico_tratante                     = str_replace(' ','_', funcion_eliminar_acentos(trim(substr($nombre_medico_tratante, 0, 30))));
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($url_img1 <> '') { 
		...
		$nombre_normal2                                     = $limpiar_nombre_paciente.'__'.$limpiar_nombre_tipo_archivo_abrev.'__'.$limpiar_cod_historia_clinica.'__'.$cod_archivador.'__'.$cod_archivo_adjunto.'__'.$cod_tipo_carpeta.'__'.$fecha_ymdHis.'.'.$formato_img2;
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if (move_uploaded_file($_FILES['url_img1']['tmp_name'], $url_img_orig_producto)) {
		$sql_archivador = "INSERT INTO tbl15_archivador (cod_historia_clinica, nombre_paciente, nombre_medico_tratante, 
		...
		VALUES ('$cod_historia_clinica', UPPER('$nombre_paciente'), UPPER('$nombre_medico_tratante'), 
		'$observacion_archivador', '$cod_factura', '$cod_tipo_ambito', '$cod_tipo_estante', '$cod_tipo_cubiculo', '$cod_tipo_carpeta', '$cod_tabla_retencion_documental', '$cod_entidad_origen_archivo', '$cod_tipo_archivo', 
		'$cod_tipo_nota_observacion', '$nombre_original_archivo', '$cantidad_folios', '$nombre_tipo_extencion_archivo', '$numero_historia_clinica', '$fecha_creacion', '$peso_archivo',
 	...
 	$pagina_redirect                                    = addslashes($_POST['pagina']).'?cod_archivador='.$cod_archivador.'&cod_archivo_adjunto='.$cod_archivo_adjunto.'&resultado_subir_archivo='.$resultado_subir_archivo;
/* ----------------------------------------------------------------------------------------------------------/ */
?>
