...
$(function () {
    $('#container_grafico_estadistico').highcharts({
        ...
        xAxis: [{
            categories: [
<?php
$mostrar_datos_sql = "SELECT fecha_mes FROM tbl15_movimiento_contable WHERE (nombre_estado_factura = 'CERRADA') GROUP BY fecha_mes ORDER BY fecha_ymd ASC";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$mostrar_datos_sql = "SELECT SUM(total_costo_movimiento_contable) AS total_ingreso FROM tbl15_movimiento_contable 
WHERE (nombre_estado_factura = 'CERRADA' AND nombre_tipo_documento = 'RECIBO DE CAJA') GROUP BY fecha_mes ORDER BY fecha_ymd ASC";
$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
$total_resultados = mysqli_num_rows($consulta);
while ($datos = mysqli_fetch_assoc($consulta)) { 
...
?>
            ...
            data: [
<?php
$mostrar_datos_sql = "SELECT SUM(total_costo_movimiento_contable) AS total_egreso FROM tbl15_movimiento_contable 
WHERE (nombre_estado_factura = 'CERRADA' AND nombre_tipo_documento = 'COMPROBANTE DE EGRESO') GROUP BY fecha_mes ORDER BY fecha_ymd ASC";
$consulta = mysqli_query($conectar, $mostrar_datos_sql) or die(mysqli_error($conectar));
$total_resultados = mysqli_num_rows($consulta);
while ($datos = mysqli_fetch_assoc($consulta)) { 
