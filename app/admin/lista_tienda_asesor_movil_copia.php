...
$(document).on('submit', '#formRegistroProducto', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                Swal.fire({ icon: 'success', title: '¡Producto Registrado!', text: response.message || 'El producto se registró correctamente.', background: '#1a1f2e', color: 'white', confirmButtonColor: '#10b981', customClass: { container: 'swal-high-zindex' } }).then(function() {
                    document.getElementById('formRegistroProducto').reset();
                    var previewImg = document.getElementById('preview_producto_img');
                    if (previewImg) { previewImg.style.display = 'none'; previewImg.src = ''; }
                });
