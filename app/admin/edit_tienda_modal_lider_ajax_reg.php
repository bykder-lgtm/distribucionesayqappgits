...
$nombre1_tercero                    = isset($_POST['nombre_representante']) ? mysqli_real_escape_string($conectar, $_POST['nombre_representante']) : '';
$identificacion_representante       = isset($_POST['identificacion_representante']) ? mysqli_real_escape_string($conectar, $_POST['identificacion_representante']) : '';
...
if (mysqli_query($conectar, $sql_update)) {
    echo json_encode(['success' => true, 'message' => 'Tienda actualizada correctamente', 'nombre_tienda' => $nombre_tienda]);
} else {
    echo json_encode(['success' => false, 'message' => 'Error al actualizar: ' . mysqli_error($conectar)]);
}
?>
