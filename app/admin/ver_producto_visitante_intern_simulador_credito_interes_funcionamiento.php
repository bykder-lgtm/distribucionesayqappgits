            ...
            <div class="row">
                <div class="col-xl-3 col-lg-3 col-sm-12 col-xs-12 sidebar-shop-left">
                    <div class="product-categori">
                        ...
                        <?php include_once("../admin/modulo_categoria_producto_filtro_visitante_intern.php"); ?>
                        <div style="text-align:center"><a href="https://wa.me/?text=¡Mira%20esto!%20<?php echo $pagina_compartir_catalogo ?>" target="_blank">Compartir en WhatsApp<img src="../imagenes/btn_red_social_whatsapp.jpg" class="img-polaroid" alt=""></a></div>
...
if (isset($_GET['cod_categoria'])) {
    ...
    cod_categoria, cod_estado, cod_tienda FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') AND (cod_categoria = '$cod_categoria') ORDER BY nombre_producto DESC";
}
elseif (isset($_GET['buscador'])) { 
    ...
    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado, cod_tienda FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') AND (nombre_producto LIKE '%$buscador_get%') ORDER BY nombre_producto DESC";
}
elseif (isset($_GET['nombre_marca_codifcryp'])) { 
    ...
    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado, cod_tienda FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') AND (nombre_marca LIKE '$nombre_marca') ORDER BY nombre_producto DESC";
}
else { 
    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado, cod_tienda FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') ORDER BY nombre_producto DESC";
}
...
while ($datos_producto = mysqli_fetch_assoc($consulta_producto)) {
    ...
    //$precio_venta_producto_mas_comision_funcionamiento            = $precio_venta_producto + ($precio_venta_producto * ($comision_funcionamiento_interes_propio_empresa_ptj / 100));
                                        ...
                                        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-4 col-xl-3">
                                            <div class="products-single fix">
                                                <div class="box-img-hover">
                                                    ...
                                                    <img src="<?php echo $url_img_min_producto ?>" class="img-thumbnail" alt="Image">
                                                    <div class="mask-icon">
                                                        <ul>
                                                        <!--    
                                                        ...
                                                        -->
                                                            ...
                                                            <!--<form action="../admin/formulario_cotizacion_reg.php" method="post" id="contactForm"><button class="btn hvr-hover" id="submit" type="submit">Cotizar Credito</button><input type="hidden" class="form-control" name="cod_producto_codifcryp" value="<?php echo $cod_producto_codifcryp ?>"></form>-->
                                                            <a href="../admin/simulador_credito_producto_visitante_intern_interes_funcionamiento.php?cod_producto_codifcryp=<?php echo $cod_producto_codifcryp ?>" class="cart_modif">Cotizar Credito</a>
                                                ...
                                                <div class="why-text">
                                                    ...
                                                    <h5> Precio: $<?php echo number_format($precio_venta_producto_mas_comision_funcionamiento, 0, ",", ".") ?></h5>
                                                    <h6> Contado desde: $<?php echo number_format($precio_venta_producto, 0, ",", ".") ?></h6>
                                                    <h6><?php echo $nombre_tienda ?></h5>
                                                </div>
                                            </div>
                                        </div>
<?php } ?>
...
if (isset($_GET['cod_categoria'])) { 
    $cod_categoria_get = intval($_GET['cod_categoria']); 
    //$cod_categoria_codif          = DAXCODIFCRYPTOR::descriptardax($cod_categoria_get);
    //$cod_categoria                = DAXCODIFCRYPTOR::descodiftextodax($cod_categoria_codif);

    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') AND (cod_categoria = '$cod_categoria_get') ORDER BY nombre_producto DESC";
}
elseif (isset($_GET['buscador'])) { 
    $buscador_get = addslashes($_GET['buscador']); 
    ...
    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') AND (nombre_producto LIKE '%$buscador_get%') ORDER BY nombre_producto DESC";
}
else { 
    $sql_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, und_producto, precio_venta_producto, descripcion_producto, 
    precio_venta_producto2, url_img_min_producto, url_img_orig_producto, nombre_promocion, nombre_promocion_ing, 
    cod_categoria, cod_estado FROM tbl15_producto WHERE (nombre_estado = 'HABILITADO') AND (cod_tienda = '$cod_tienda') ORDER BY nombre_producto DESC";
}
$consulta_producto = mysqli_query($conectar, $sql_producto) or die(mysqli_error($conectar));
while ($datos_producto = mysqli_fetch_assoc($consulta_producto)) {
...
$(document).ready(function() {
    ...
    $('.hvr-hover').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "../admin/reg_venta_temporal_producto_visitante_intern_ajax_reg.php",
            data: datos_url_ajax,
