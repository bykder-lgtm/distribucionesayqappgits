...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formulario_de_actualizacion")) {
	...
	$cod_cuentas_pagar_abonos                                 = intval($_POST['cod_cuentas_pagar_abonos']);
...
//-------------------------------------- -----------------------------------------------------------------//
    $sql = "SELECT abonado FROM tbl15_cuentas_pagar_abonos WHERE (cod_cuentas_pagar_abonos = '$cod_cuentas_pagar_abonos')";
...
//-------------------------------------- -----------------------------------------------------------------//
	$sql_datos_cuentas_pagar = "SELECT cod_factura FROM tbl15_cuentas_pagar WHERE (cod_cuentas_pagar = '$cod_cuentas_pagar')";
	$consulta_datos_cuentas_pagar = mysqli_query($conectar, $sql_datos_cuentas_pagar);
	$datos_cuentas_pagar = mysqli_fetch_assoc($consulta_datos_cuentas_pagar);

	$cod_factura                                              = $datos_cuentas_pagar['cod_factura'];
//-------------------------------------- -----------------------------------------------------------------//
...
//-------------------------------------- -----------------------------------------------------------------//
	...
	WHERE (cod_cuentas_pagar_abonos = '$cod_cuentas_pagar_abonos')";
...
//-------------------------------------- -----------------------------------------------------------------//
	$actualizar_cuentas_pagar_abonos = "UPDATE tbl15_cuentas_pagar_abonos SET cod_tipo_forma_pago = '$cod_tipo_forma_pago' WHERE (cod_cuentas_pagar_abonos = '$cod_cuentas_pagar_abonos')";
	$resultado_cuentas_pagar_abonos = mysqli_query($conectar, $actualizar_cuentas_pagar_abonos) or die(mysqli_error($conectar));
?>
	<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/cuentas_pagar_abonos.php?cod_tercero=<?php echo $cod_tercero ?>&cod_cuentas_pagar_abonos=<?php echo $cod_cuentas_pagar_abonos ?>&cod_cuentas_pagar=<?php echo $cod_cuentas_pagar ?>&cod_factura=<?php echo $cod_factura ?>&cliente=<?php echo $cliente ?>&pagina=<?php echo $pagina ?>">
<?php } ?>