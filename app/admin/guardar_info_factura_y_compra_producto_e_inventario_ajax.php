...
$cod_info_factura_compra             = intval($_POST['id']);
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET fecha_ymd_venta_producto = '$fecha_ymd_venta_producto', 
fecha_mes_venta_producto = '$fecha_mes_venta_producto', fecha_anyo_venta_producto = '$fecha_anyo_venta_producto' 
WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
}
// ------------------------------------------------------------------------------------------------- //
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_tipo_forma_pago = '$cod_tipo_forma_pago' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
}
// ------------------------------------------------------------------------------------------------- //
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_tipo_pago = '$cod_tipo_pago' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
}
// ------------------------------------------------------------------------------------------------- //
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_tercero = '$cod_tercero' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
if (($campo=='nombre_tipo_cargue_factura') && ($tipo_ajax=='tbl15_info_factura_compra')) {
	...
	$data_sql = ("UPDATE tbl15_factura_compra_producto SET nombre_tipo_cargue_factura = '$nombre_tipo_cargue_factura' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
if (($campo=='nombre_tipo_compra') && ($tipo_ajax=='tbl15_info_factura_compra')) {
	$nombre_tipo_compra                = addslashes($_POST['valor']);
	$cod_info_factura_compra           = intval($_POST['id']);

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET nombre_tipo_compra = '$nombre_tipo_compra' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_factura = '$cod_factura' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_administrador = '$cod_administrador' WHERE cod_info_factura_compra = '$cod_info_factura_compra'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_producto_barra') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$cod_producto_barra           = addslashes($_POST['valor']);
$cod_factura_compra_producto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_producto_barra = '$cod_producto_barra' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_producto') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$nombre_producto              = addslashes(strtoupper($_POST['valor']));
$cod_factura_compra_producto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET nombre_producto = '$nombre_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_frec_duracion') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$nombre_frec_duracion         = addslashes(strtoupper($_POST['valor']));
$cod_factura_compra_producto  = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET nombre_frec_duracion = '$nombre_frec_duracion' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_unidades') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$und_unidades                            = intval($_POST['valor']);
	$cod_factura_compra_producto             = intval($_POST['id']);

	$datos_info_temporal = "SELECT und_compra, und_unidades, und_caja, precio_costo_producto, precio_compra_producto, dto1, dto2, iva_ptj, cod_info_factura_compra, precio_ipc, cod_producto_barra
	...
	$data_sql = ("UPDATE tbl15_producto SET cajas_sobre = '$und_unidades' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET und_unidades = '$und_unidades', und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
	WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_caja') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET und_caja = '$und_caja', und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_compra') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	...
	$data_sql = ("UPDATE tbl15_producto SET und_producto = '$und_compra' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET und_compra = '$und_compra', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto', precio_ipc_total = '$precio_ipc_total' 
	WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_compra_producto') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_compra_producto = '$precio_compra_producto' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
	precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='iva_ptj') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	...
	$data_sql = ("UPDATE tbl15_producto SET iva_ptj = '$iva_ptj' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET iva_ptj = '$iva_ptj', valor_iva = '$valor_iva', precio_costo_producto = '$precio_costo_producto', 
	total_costo_producto = '$total_costo_producto', total_compra_producto = '$total_compra_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='dto1') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET dto1 = '$dto1', descuento = '$descuento', total_dto = '$total_dto', precio_costo_producto = '$precio_costo_producto', 
total_costo_producto = '$total_costo_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='dto2') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET dto2 = '$dto2', descuento = '$descuento', total_dto = '$total_dto', precio_costo_producto = '$precio_costo_producto', 
total_costo_producto = '$total_costo_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_ipc') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_ipc = '$precio_ipc', precio_ipc_total = '$precio_ipc_total', total_compra_producto = '$total_compra_producto' 
WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='comision_ptj') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$comision_ptj                 = addslashes(($_POST['valor']));
	$cod_factura_compra_producto = intval($_POST['id']);

	$sql_factura_compra_producto = "SELECT cod_producto, cod_producto_barra FROM tbl15_factura_compra_producto WHERE (cod_factura_compra_producto = '$cod_factura_compra_producto')";
	$consulta_factura_compra_producto = mysqli_query($conectar, $sql_factura_compra_producto) or die(mysqli_error($conectar));
	$info_factura_compra_producto = mysqli_fetch_assoc($consulta_factura_compra_producto);

	$cod_producto                             = $info_factura_compra_producto['cod_producto'];
	$cod_producto_barra                       = $info_factura_compra_producto['cod_producto_barra'];

	$data_sql = ("UPDATE tbl15_producto SET comision_ptj = '$comision_ptj' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET comision_ptj = '$comision_ptj' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='fecha_vencimiento') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	...
	$data_sql = ("UPDATE tbl15_producto SET fecha_vencimiento1 = '$fecha_vencimiento' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='lote_vencimiento') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$lote_vencimiento             = addslashes(($_POST['valor']));
$cod_factura_compra_producto = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET lote_vencimiento = '$lote_vencimiento' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_venta_producto = '$precio_venta_producto' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto = '$precio_venta_producto' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto2') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$precio_venta_producto2                    = addslashes($_POST['valor']);
	$cod_factura_compra_producto               = intval($_POST['id']);
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_venta_producto2 = '$precio_venta_producto2' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto2 = '$precio_venta_producto2' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto3') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$precio_venta_producto3                    = addslashes($_POST['valor']);
	$cod_factura_compra_producto               = intval($_POST['id']);
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_venta_producto3 = '$precio_venta_producto3' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto3 = '$precio_venta_producto3' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto4') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$precio_venta_producto4                    = addslashes($_POST['valor']);
	$cod_factura_compra_producto               = intval($_POST['id']);
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_venta_producto4 = '$precio_venta_producto4' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto4 = '$precio_venta_producto4' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='precio_venta_producto5') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
	$precio_venta_producto5                    = addslashes($_POST['valor']);
	$cod_factura_compra_producto               = intval($_POST['id']);
	...
	$data_sql = ("UPDATE tbl15_producto SET precio_venta_producto5 = '$precio_venta_producto5' WHERE cod_producto_barra = '$cod_producto_barra'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto5 = '$precio_venta_producto5' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
// ------------------------------------------------------------------------------------------------- //
if (($campo=='und_venta') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$und_venta                    = intval($_POST['valor']);
$cod_factura_compra_producto  = intval($_POST['id']);

$datos_info_temporal = "SELECT precio_costo_producto, precio_venta_producto FROM tbl15_factura_compra_producto WHERE (cod_factura_compra_producto = '$cod_factura_compra_producto')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET und_venta = '$und_venta', total_costo_producto = '$total_costo_producto', total_venta_producto = '$total_venta_producto', 
cod_estado_permitir_venta = '$cod_estado_permitir_venta' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
$datos_info_temporal = "SELECT und_venta, precio_costo_producto FROM tbl15_factura_compra_producto WHERE (cod_factura_compra_producto = '$cod_factura_compra_producto')";
$consulta_info_temporal = mysqli_query($conectar, $datos_info_temporal) or die(mysqli_error($conectar));
$info_temporal = mysqli_fetch_assoc($consulta_info_temporal);
...
$data_sql = ("UPDATE tbl15_factura_compra_producto SET precio_venta_producto = '$precio_venta_producto', total_venta_producto = '$total_venta_producto',
cod_estado_permitir_venta = '$cod_estado_permitir_venta' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
...
// ------------------------------------------------------------------------------------------------- //
if (($campo=='nombre_cliente') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$nombre_cliente              = addslashes(strtoupper($_POST['valor']));
$cod_factura_compra_producto = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET nombre_cliente = '$nombre_cliente' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='cod_tipo_cobrar') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$cod_tipo_cobrar             = addslashes(($_POST['valor']));
$id                          = addslashes($_POST['id']);
$frag                        = explode("__", $id);
$cod_factura_compra_producto = $frag[1];

$data_sql = ("UPDATE tbl15_factura_compra_producto SET cod_tipo_cobrar = '$cod_tipo_cobrar' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

if (mysqli_affected_rows($conectar) > 0) { echo "AFECTADO SI"; } else { echo "AFECTADO NO"; }
}
// ------------------------------------------------------------------------------------------------- //
if (($campo=='fecha_alerta') && ($tipo_ajax=='tbl15_factura_compra_producto')) {
$fecha_alerta                = addslashes(($_POST['valor']));
$cod_factura_compra_producto = intval($_POST['id']);

$data_sql = ("UPDATE tbl15_factura_compra_producto SET fecha_alerta = '$fecha_alerta' WHERE cod_factura_compra_producto = '$cod_factura_compra_producto'");
$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
