...
<div class="breadcrumbs"><a href="<?php echo $pagina ?>"><h4>Editar Propietario</h4></a></div>
<div class="row-fluid">
...
$ingreso_impuesto_iva          = $info_cliente['ingreso_impuesto_iva'];
?>
<form name="formulario_insersion" accept-charset="utf-8" method="post" action="../admin/edit_propietario_reg.php">
<fieldset>
    	...
    	<tr>
			<td>
				<select id="select_nombre_tipo_tercero" name="nombre_tipo_tercero" class="input-block-level"  style="font-size:15px">
			        ...
			        $consulta2_sql = "SELECT * FROM tbl15_tipo_tercero WHERE ((nombre_tipo_tercero = 'PROPIETARIO') OR (nombre_tipo_tercero = 'AMBOS')) AND (cod_estado = '1') ORDER BY cod_tipo_tercero DESC";
			...
			<td>
				<select id="select_nombre_tipo_identificacion" name="nombre_tipo_identificacion" class="input-block-level"  style="font-size:15px">
			        ...
			        $consulta2_sql = "SELECT * FROM tbl15_tipo_identificacion WHERE (cod_estado = '1')";
			        $consulta2 = mysqli_query($conectar, $consulta2_sql) or die(mysql_error());
			        while ($datos2 = mysqli_fetch_assoc($consulta2)) {
...
<table border="1" class="table table-responsive">
	<tbody>
		<tr>
			<th style="text-align:center">IMPUESTO IVA  (INGRESO)</th>
		</tr>
    	<tr>
			<td><input class="input-block-level" name="ingreso_impuesto_iva" type="number" min="0" value="<?php echo $ingreso_impuesto_iva ?>"/></td>
    	</tr>
    </tbody>
</table>
<!-- ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
...
<hr>
<fieldset><legend>Inmuebles del Propietario</legend>
...
<tr class="headings">
    ...
    <?php if ($cod_estado_prod_editar == '1') { ?><th style="text-align:center" class="column-title">Edit</th><?php } ?>
</tr>
</thead>
<tbody>
<?php
$sql_consulta_producto = "SELECT cod_producto, cod_producto_barra, nombre_producto, precio_venta_producto, nombre_tipo_producto, cod_estado_inmueble, dia_pago_propietario_inmueble 
FROM tbl15_producto WHERE (cod_tercero = '$cod_tercero')";
$consulta_producto = mysqli_query($conectar, $sql_consulta_producto) or die(mysqli_error($conectar));
while ($total_producto = mysqli_fetch_array($consulta_producto)) {
    ...
    $precio_venta_producto                      = $total_producto['precio_venta_producto'];
    $nombre_tipo_producto                       = $total_producto['nombre_tipo_producto'];
    $cod_estado_inmueble                        = $total_producto['cod_estado_inmueble'];
    $dia_pago_propietario_inmueble              = $total_producto['dia_pago_propietario_inmueble'];

	$sql_estado_inmueble = "SELECT nombre_estado_inmueble FROM tbl15_estado_inmueble WHERE (cod_estado_inmueble = '$cod_estado_inmueble')";
...
<tr class="even pointer">
    ...
    <?php if ($cod_estado_prod_editar == '1') { ?><td style="text-align:center"><a href="../admin/edit_producto_inmueble.php?cod_producto=<?php echo $cod_producto?>&cod_tercero=<?php echo $cod_tercero?>&pagina=<?php echo $pagina ?>"><img src="../imagenes/editar.png" class="img-polaroid" alt=""></a></td><?php } ?>
</tr>
<?php } ?>
<tbody>
</table>
