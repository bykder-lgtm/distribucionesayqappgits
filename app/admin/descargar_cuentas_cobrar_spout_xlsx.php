...
$fecha                                                             = date("Y_m_d");
$hora                                                              = date("H_i_s");
$nombre_archivo                                                    = "REPORTE_CUENTAS_POR_COBRAR_".$fecha.'__'.$hora;
...
if (isset($_GET['fecha_hoy'])) { 
    ...
    while ($datos_cuenta_cobrar = mysqli_fetch_assoc($consulta_datos_cuenta_cobrar)) {
        ...
        $writer->addRow(array($cod_cuentas_cobrar, $identificacion_tercero, $cliente, intval($monto_deuda), intval($abonado), intval($subtotal), $direccion_tercero, $telefono1_tercero));
