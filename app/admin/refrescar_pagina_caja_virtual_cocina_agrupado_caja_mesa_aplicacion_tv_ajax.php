...
if (isset($_REQUEST['pagina'])) { $pagina = 'lista_caja_virtual_cocina_agrupado_caja_mesa_aplicacion_tv.php'; } else { $pagina = 'lista_caja_virtual_cocina_agrupado_caja_mesa_aplicacion_tv.php'; }
if (isset($_REQUEST['pagina_redirect'])) { $pagina_redirect = 'lista_caja_virtual_cocina_agrupado_caja_mesa_aplicacion_tv.php'; } else { $pagina_redirect = 'lista_caja_virtual_cocina_agrupado_caja_mesa_aplicacion_tv.php'; }
...
$sql_venta_producto_temporal_agrupado = "SELECT cod_info_factura_venta, cod_caja_virtual, cuenta, cod_administrador, fecha_seg_venta_producto 
FROM tbl15_venta_producto_temporal WHERE ('1'='1') $condic_origen_produccion_user $condic_estado_venta_temp GROUP BY cod_info_factura_venta ORDER BY fecha_seg_venta_producto ASC";
$resul_venta_producto_temporal_agrupado = mysqli_query($conectar, $sql_venta_producto_temporal_agrupado) or die(mysqli_error($conectar));
$existen_reg = mysqli_num_rows($resul_venta_producto_temporal_agrupado);
while ($venta_producto_temporal_agrupado = mysqli_fetch_assoc($resul_venta_producto_temporal_agrupado)) {

    $cod_info_factura_venta                         = $venta_producto_temporal_agrupado['cod_info_factura_venta'];
    $cod_caja_virtual                               = $venta_producto_temporal_agrupado['cod_caja_virtual'];
    $cuenta                                         = $venta_producto_temporal_agrupado['cuenta'];
    $cod_administrador                              = $venta_producto_temporal_agrupado['cod_administrador'];
    $fecha_seg_venta_producto                       = $venta_producto_temporal_agrupado['fecha_seg_venta_producto'];
    $fecha_hora_pedido_inicial                      = date("Y-m-d H:i:s A", $fecha_seg_venta_producto);

    $datos_info_factura_venta_agrupados[$increment] = array("cod_info_factura_venta"=>$cod_info_factura_venta, "cod_caja_virtual"=>$cod_caja_virtual, "cuenta"=>$cuenta, "cod_administrador"=>$cod_administrador, "fecha_hora_pedido_inicial"=>$fecha_hora_pedido_inicial);
    $increment++;
}

if ($existen_reg <> '0') {
    foreach ($datos_info_factura_venta_agrupados as $datos_agrupados) {
        $cod_info_factura_venta                     = $datos_agrupados["cod_info_factura_venta"];
        $fecha_hora_pedido_inicial                  = $datos_agrupados["fecha_hora_pedido_inicial"];

        $nombre_producto_concat             = '';
        $nombre_producto_con                = '';
        $salida_tra                         = '';

        $sql_venta_producto_temporal = "SELECT * FROM tbl15_venta_producto_temporal WHERE (cod_info_factura_venta = '$cod_info_factura_venta') $condic_origen_produccion_user $condic_estado_venta_temp ORDER BY cod_venta_producto_temporal ASC";
        ...
        while ($venta_producto_temporal = mysqli_fetch_assoc($resul_venta_producto_temporal)) {
            ...
            $cod_estado_revisado_prod                       = $venta_producto_temporal['cod_estado_revisado'];
            $cod_estado_revisado_cocina_prod                = $venta_producto_temporal['cod_estado_revisado_cocina'];
            $cod_estado_revisado_bartender_prod             = $venta_producto_temporal['cod_estado_revisado_bartender'];
            $cod_estado_revisado_jugueria_prod              = $venta_producto_temporal['cod_estado_revisado_jugueria'];
            $fecha_seg_venta_producto                       = $venta_producto_temporal['fecha_seg_venta_producto'];
            $fecha_hora_cargue                              = date("Y-m-d H:i:s A", $fecha_seg_venta_producto);
            $fecha_pedido_inicial                           = date("Y-m-d", $fecha_seg_venta_producto);
            $hora_pedido_inicial                            = date("h:i:s A", $fecha_seg_venta_producto);
            ...
            if ($cod_estado_marcado_revisado_caja_mesa_venta_temporal_global == '1') { $nombre_producto_concat .= "<mark>".intval($und_venta)." | ".$nombre_producto.' | '.$comentario_producto.' | '.$fecha_pedido_inicial.' '.$hora_pedido_inicial.'<a href=../admin/entregar_servicio_comida_caja_mesa_virtual_venta_producto_temporal_reg.php?cod_venta_producto_temporal='.$cod_venta_producto_temporal.'&cod_info_factura_venta='.$cod_info_factura_venta.'&pagina='.$pagina_local.'><img src=../imagenes/desactivar_timbre_aviso.png alt="entregar_servicio_comida"></a></mark><br>'; } else { $nombre_producto_concat .= "".intval($und_venta)." | ".$nombre_producto.' | '.$comentario_producto.' | '.$fecha_pedido_inicial.' '.$hora_pedido_inicial.'<a href=../admin/entregar_servicio_comida_caja_mesa_virtual_venta_producto_temporal_reg.php?cod_venta_producto_temporal='.$cod_venta_producto_temporal.'&cod_info_factura_venta='.$cod_info_factura_venta.'&pagina='.$pagina_local.'</mark><br>'; }
        }
    ...
    $tabla_caja_mesa .= '       <!--<th style="text-align:center;">'.$fecha_hora_pedido_inicial.'</th>-->';
    if ($cod_estado_prioridad_caja_mesa_global == '1') {
        $tabla_caja_mesa .= '		<th style="text-align:center;">'.$cod_prioridad.'</th>';
    }
    $tabla_caja_mesa .= '		<th style="text-align:center;"><a href="../admin/entregar_servicio_comida_agrupado_caja_mesa_virtual_info_venta_producto_reg.php?cod_info_factura_venta='.$cod_info_factura_venta.'&pagina='.$pagina_redirect.'"><img src=../imagenes/entregar_servicio_comida.png alt="entregar_servicio_comida"></a></th>';
