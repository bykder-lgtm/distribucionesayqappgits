...
<table class="table table-striped">
	<tr>
		<th style="text-align:center;"><img src=../imagenes/advertencia.gif alt='Advertencia'><a href="<?php echo $paginar_edirect;?>">PRODUTOS CON PRECIO DE VENTA EN CERO</a>.<img src=../imagenes/advertencia.gif alt='Advertencia'></th>
	</tr>
</table>
...
$sql_verificar_precio_venta_en_cero_venta_temp = "SELECT * FROM tbl15_venta_producto_temporal WHERE (cod_info_factura_venta = '$cod_info_factura_venta') ORDER BY $ordenamiento_venta_temp";
$consulta_verificar_precio_venta_en_cero_venta_temp = mysqli_query($conectar, $sql_verificar_precio_venta_en_cero_venta_temp);
while ($datos_verificar_precio_venta_en_cero_venta_temp = mysqli_fetch_assoc($consulta_verificar_precio_venta_en_cero_venta_temp)) {

    $cod_venta_producto_temporal       = $datos_verificar_precio_venta_en_cero_venta_temp['cod_venta_producto_temporal'];
    $cod_producto                      = $datos_verificar_precio_venta_en_cero_venta_temp['cod_producto'];
    $cod_producto_barra                = $datos_verificar_precio_venta_en_cero_venta_temp['cod_producto_barra'];
    $nombre_producto                   = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_producto'];
    $cedula                            = $datos_verificar_precio_venta_en_cero_venta_temp['cedula'];
    $nombre_cliente                    = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_cliente'];
    $und_venta                         = $datos_verificar_precio_venta_en_cero_venta_temp['und_venta'];
    $precio_costo_producto             = $datos_verificar_precio_venta_en_cero_venta_temp['precio_costo_producto'];
    $precio_compra_producto            = $datos_verificar_precio_venta_en_cero_venta_temp['precio_compra_producto'];
    $total_costo_producto              = $datos_verificar_precio_venta_en_cero_venta_temp['total_costo_producto'];
    $precio_venta_producto             = $datos_verificar_precio_venta_en_cero_venta_temp['precio_venta_producto'];
    $total_venta_producto              = $datos_verificar_precio_venta_en_cero_venta_temp['total_venta_producto'];
    ...
    //$nombre_tipo_producto              = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_producto'];
    $nombre_tipo_unidad_medida         = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_unidad_medida'];
    $posologia_cantidad                = $datos_verificar_precio_venta_en_cero_venta_temp['posologia_cantidad'];
    $posologia_peso                    = $datos_verificar_precio_venta_en_cero_venta_temp['posologia_peso'];
    $nombre_tipo_presentacion          = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_presentacion'];
    $nombre_via_administracion         = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_via_administracion'];
    $nombre_frec_duracion              = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_frec_duracion'];
    $cod_tipo_cobrar                   = $datos_verificar_precio_venta_en_cero_venta_temp['cod_tipo_cobrar'];
    $cod_info_factura_venta            = $datos_verificar_precio_venta_en_cero_venta_temp['cod_info_factura_venta'];
    $nombre_tipo_precio_venta          = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_precio_venta'];
    $cod_estado_permitir_venta         = $datos_verificar_precio_venta_en_cero_venta_temp['cod_estado_permitir_venta'];
    $und_producto                      = $datos_verificar_precio_venta_en_cero_venta_temp['und_producto'];
    $cajas_sobre                       = $datos_verificar_precio_venta_en_cero_venta_temp['cajas_sobre'];
    $und_sobre                         = $datos_verificar_precio_venta_en_cero_venta_temp['und_sobre'];
    $fecha_seg_venta_producto          = $datos_verificar_precio_venta_en_cero_venta_temp['fecha_seg_venta_producto'];
    $hora_cargue                       = date("H:i:s", $fecha_seg_venta_producto);

    $comentario_producto               = $datos_verificar_precio_venta_en_cero_venta_temp['comentario_producto'];
    $placa_producto                    = $datos_verificar_precio_venta_en_cero_venta_temp['placa_producto'];
    $fecha_ymd_parqueo_ini             = $datos_verificar_precio_venta_en_cero_venta_temp['fecha_ymd_parqueo_ini'];
    $fecha_hora_parqueo_ini            = $datos_verificar_precio_venta_en_cero_venta_temp['fecha_hora_parqueo_ini'];
    $fecha_ymd_parqueo_fin             = $datos_verificar_precio_venta_en_cero_venta_temp['fecha_ymd_parqueo_fin'];
    $fecha_hora_parqueo_fin            = $datos_verificar_precio_venta_en_cero_venta_temp['fecha_hora_parqueo_fin'];
    $cod_estado_componente_und_venta   = $datos_verificar_precio_venta_en_cero_venta_temp['cod_estado_componente_und_venta'];
    $cod_estado_revisado               = $datos_verificar_precio_venta_en_cero_venta_temp['cod_estado_revisado'];
    $peso_producto                     = $datos_verificar_precio_venta_en_cero_venta_temp['peso_producto'];
    $unidad_medida_peso                = $datos_verificar_precio_venta_en_cero_venta_temp['unidad_medida_peso'];
    $cod_origen_produccion             = $datos_verificar_precio_venta_en_cero_venta_temp['cod_origen_produccion'];
    $und_caja_sobre                    = $datos_verificar_precio_venta_en_cero_venta_temp['und_caja_sobre'];
    $nombre_tipo_und_caja_sobre        = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_und_caja_sobre'];
    $total_dias                        = $datos_verificar_precio_venta_en_cero_venta_temp['total_dias'];
    $total_horas                       = $datos_verificar_precio_venta_en_cero_venta_temp['total_horas'];
    $nombre_tipo_cobro                 = $datos_verificar_precio_venta_en_cero_venta_temp['nombre_tipo_cobro'];
    $comision_ptj                      = $datos_verificar_precio_venta_en_cero_venta_temp['comision_ptj'];
    
    $total_cajas_disponibles           = $datos_verificar_precio_venta_en_cero_venta_temp['total_cajas_disponibles'];
    $total_sobres_disponibles          = $datos_verificar_precio_venta_en_cero_venta_temp['total_sobres_disponibles'];
...
?>
		<tr id="tr<?php echo $cod_venta_producto_temporal;?>">
	        ...
	        <td style="text-align:center; background-color:<?php echo $color_fondo_celda ?>;" id="und_venta_<?php echo $incre;?>"><span style="color:<?php echo $color_letra_celda ?>;"><?php echo $und_venta;?></span></td>
...
$(document).ready(function(){
    $("input").on('change', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../admin/guardar_producto_preio_venta_cero_info_factura_y_venta_producto_temporal_ajax.php",
            data: datos_url_ajax,
            //dataType: 'json',
            ...
            success:function(respuesta){
                ...
                var existe_precio_venta_en_cero_venta_temp = respuesta.existe_precio_venta_en_cero_venta_temp;

                if ((afectado == 'SI')) {
                	$('#'+campo_respuesta+''+id).html('');
	            	$('#'+campo_respuesta+''+id).html('<img src="../imagenes/spam_reg.png">');
                    if (existe_precio_venta_en_cero_venta_temp == '0') {
