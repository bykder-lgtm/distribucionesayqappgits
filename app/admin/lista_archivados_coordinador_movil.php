...
<style>
/* ============================================ */
/* LISTA ARCHIVADOS COORDINADOR - INDIGO/BLUE   */
/* ============================================ */
...
.tab-btn.active { background: #6366f1; color: white; box-shadow: 0 4px 15px rgba(99, 102, 241, 0.3); }
...
.archived-card:hover { opacity: 1; transform: translateY(-2px); background: rgba(255, 255, 255, 0.05); border-color: rgba(99, 102, 241, 0.3); }
...
.pagination-btn.active { background: #6366f1; }
...
<main class="page-container">
    <div class="page-header">
        ...
        <p>Registros archivados bajo tu coordinación</p>
    </div>
    ...
    if ($tab == 'personas') {
        ...
        $where = "WHERE (a.cod_estado = 0 OR a.cod_estado_activacion_usuario = 3) AND (a.cod_coordinador = '$cod_administrador')";
        ...
        $where = "WHERE t.cod_estado = 0 AND t.cod_aliado_estrategico IN (SELECT a.cod_administrador FROM tbl15_administrador a WHERE a.cod_coordinador = '$cod_administrador')";
...
function filtrar(val) {
    if (val.length > 2 || val.length == 0) {
        window.location.href = 'lista_archivados_coordinador_movil.php?tab=<?php echo $tab; ?>&busqueda=' + val;
    }
}
