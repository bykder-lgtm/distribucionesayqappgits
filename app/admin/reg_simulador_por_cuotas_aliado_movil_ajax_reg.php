...
if (isset($_POST['nombre_tipo_origen_simulacion'])) {
    ...
    // --- VERIFICACIÓN DE LÍMITE DE CRÉDITOS PARA ALIADOS DE PRUEBA ---
    $sql_check_prueba = "SELECT cod_estado_usuario_prueba FROM tbl15_administrador WHERE cod_administrador = '$cod_administrador'";
    ...
    if ($data_prueba['cod_estado_usuario_prueba'] == '1') {
        ...
        $sql_count_creditos = "SELECT COUNT(*) as total FROM tbl15_info_factura_venta WHERE cod_administrador_aliado_estrategico = '$cod_administrador'";
        ...
        if ($total_creditos >= 3) {
            ...
            $respuesta_ajax['mensaje'] = "Has alcanzado el límite de 3 créditos permitido para usuarios de prueba. Para continuar, por favor completa tu información y carga los documentos requeridos.";
	...
	if (isset($_POST['cod_estado_acepta_tratamiento_datos'])) { $cod_estado_acepta_tratamiento_datos = addslashes($_POST['cod_estado_acepta_tratamiento_datos']); } else { $cod_estado_acepta_tratamiento_datos = ''; }
	if (isset($_POST['cod_estado_acepta_terminos_condiciones'])) { $cod_estado_acepta_terminos_condiciones = addslashes($_POST['cod_estado_acepta_terminos_condiciones']); } else { $cod_estado_acepta_terminos_condiciones = ''; }
	if (isset($_POST['cod_tipo_simulacion_credito'])) { $cod_tipo_simulacion_credito = addslashes($_POST['cod_tipo_simulacion_credito']); } else { $cod_tipo_simulacion_credito = '1'; }
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	//---------------------------------------------------------------------------------------------------------------------------------//
	$obtener_datos_lider_coord_asesor = "SELECT cod_lider, cod_coordinador, cod_asesor, cod_aliado_estrategico FROM tbl15_administrador WHERE cod_administrador = '$cod_administrador'";
	$resultado_datos_lider_coord_asesor = mysqli_query($conectar, $obtener_datos_lider_coord_asesor) or die(mysqli_error($conectar));
	$info_datos_lider_coord_asesor = mysqli_fetch_assoc($resultado_datos_lider_coord_asesor);

	$cod_lider                                                      = $info_datos_lider_coord_asesor['cod_lider'];
	$cod_coordinador                                                = $info_datos_lider_coord_asesor['cod_coordinador'];
	$cod_asesor                                                     = $info_datos_lider_coord_asesor['cod_asesor'];
    ...
    // $total_interes ya fue calculado arriba según el tipo de simulación
    $monto_deuda                                                    = $total_pagar;
    // Para monto_deuda_sin_interes: si es precio contado, es el precio_venta_producto; si es precio crédito, usar el valor_contado calculado
    if ($cod_tipo_simulacion_credito == 1 || $cod_tipo_simulacion_credito == 0) {
        $monto_deuda_sin_interes                                    = $precio_venta_producto;
    } else {
        $monto_deuda_sin_interes                                    = $valor_contado;
    }
...
/* ----------------------------------------------------------------------------------------------------------/ */
    ...
    if($existe_dato_tercero > 0) {
		$sql_info_factura_venta = sprintf("UPDATE tbl15_tercero SET identificacion_tercero = '$identificacion_tercero', nombre1_tercero = UPPER('$nombre1_tercero'), 
		nombre2_tercero = UPPER('$nombre2_tercero'), apellido1_tercero = UPPER('$apellido1_tercero'), apellido2_tercero = UPPER('$apellido2_tercero'), 
		telefono1_tercero = '$telefono1_tercero', correo_tercero = '$correo_tercero', direccion_tercero = '$direccion_tercero', nombre_estado_civil = '$nombre_estado_civil', 
		nombres_apellidos_tercero = UPPER('$nombres_apellidos_tercero') WHERE (cod_tercero = '$cod_tercero')");
	...
	cod_administrador_lider, cod_administrador_coordinador, cod_administrador_asesor, cod_administrador_aliado_estrategico, nombres_apellidos_tercero, 
	fecha_modificacion, cod_estado_acepta_tratamiento_datos, cod_estado_acepta_terminos_condiciones, cod_intermediario_credito) 
	...
	'$correo_tercero', '$direccion_tercero', '$total_precio_compra', '$total_precio_venta', '$cod_entidad_crediticia', '$cod_tienda', '$cod_administrador_revisor', 
	'$cod_administrador_lider', '$cod_administrador_coordinador', '$cod_administrador_asesor', '$cod_administrador_aliado_estrategico', UPPER('$nombres_apellidos_tercero'),
	'$fecha_modificacion', '$cod_estado_acepta_tratamiento_datos', '$cod_estado_acepta_terminos_condiciones', '1')";
	$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	...
	VALUES ('$cod_info_factura_venta', '$cod_tercero', '$cod_producto', '$cod_producto_barra', UPPER('$nombre_producto'), '$und_venta', 
	'$precio_compra_producto', '$total_compra_producto', '$precio_costo_producto', '$total_costo_producto', '$precio_venta_producto', '$total_venta_producto',	'$precio_venta_producto_orig', 
	'$nombre_tipo_unidad_medida', '$fecha_ymd_venta_producto', '$fecha_mes_venta_producto', '$fecha_anyo_venta_producto', '$fecha_seg_venta_producto', '$cuenta', 
	'$cod_administrador', '$cod_caja_virtual', '$nombre_tipo_precio_venta', '$und_producto', '$cod_base_caja', '$iva_ptj', '$cod_categoria')";
	$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	...
	$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	//---------------------------------------------------------------------------------------------------------------------------------//
	// INSERTAR CUOTAS EN tbl15_cuentas_cobrar_alerta
	//---------------------------------------------------------------------------------------------------------------------------------//
	$monto_cuota_individual = round($total_pagar / $numero_cuotas, 0);
	$monto_cuota_sin_interes_individual = round($monto_deuda_sin_interes / $numero_cuotas, 0);
	
	for ($i = 1; $i <= $numero_cuotas; $i++) {
		// Calcular la fecha de pago de cada cuota según el tipo de cobro
		if ($nombre_tipo_cobro == 'QUINCENAL') {
			// Cada 15 días
			$dias_adicionar = ($i * 15);
		} else {
			// Mensual - cada mes
		...
		$fecha_pago_cuota = date("Y-m-d", strtotime($fecha_creacion . " + $dias_adicionar days"));
		$fecha_mes_cuota = date("Y-m", strtotime($fecha_pago_cuota));
		$anyo_cuota = date("Y", strtotime($fecha_pago_cuota));
		$fecha_seg_cuota = strtotime($fecha_pago_cuota);
		
		// Total pendiente para esta cuota (monto restante después de esta cuota)
		$total_pendiente_cuota = $total_pagar - ($monto_cuota_individual * $i);
		if ($total_pendiente_cuota < 0) { $total_pendiente_cuota = 0; }
		
		// Total recibido acumulado hasta esta cuota (0 porque aún no se ha pagado)
		$total_recibido_cuota = 0;
		$abonado_cuota = 0;
		$cod_estado_pago_cuota = 0; // No pagado
		$cod_estado_cuota = 1; // Activo
		
		$sql_cuota = "INSERT INTO tbl15_cuentas_cobrar_alerta (cod_info_factura_venta, cod_tercero, cod_producto, cod_producto_barra, nombre_producto, monto_deuda, 
		monto_deuda_sin_interes, subtotal, subtotal_sin_interes, numero_cuota, monto_cuota, monto_cuota_sin_interes, interes_ptj,
		monto_deuda_mas_interes, monto_cuota_interes, total_recibido, total_pendiente, nombre_tipo_cobro, abonado, total_pagar, vendedor, cuenta,
		fecha_pago, fecha, fecha_mes, anyo, fecha_invert, fecha_seg, fecha_pago_reg, hora_pago_reg, fecha_creacion, cod_administrador,
		cod_estado, cod_estado_pago, numero_alerta) 
		VALUES ('$cod_info_factura_venta', '$cod_tercero', '$cod_producto', '$cod_producto_barra', '$nombre_producto', '$total_pagar', 
		'$monto_deuda_sin_interes', '$subtotal', '$subtotal_sin_interes', '$i', '$monto_cuota_individual', '$monto_cuota_sin_interes_individual', '$interes_ptj',
		'$total_pagar', '$total_interes', '$total_recibido_cuota', '$total_pendiente_cuota', '$nombre_tipo_cobro', '$abonado_cuota', '$monto_cuota_individual', '$vendedor', '$cuenta',
		'$fecha_pago_cuota', '$fecha_creacion', '$fecha_mes_cuota', '$anyo_cuota', '$fecha_pago_cuota', '$fecha_seg_cuota', '', '', '$fecha_creacion', '$cod_administrador',
		'$cod_estado_cuota', '$cod_estado_pago_cuota', '$i')";
		$exec_cuota = mysqli_query($conectar, $sql_cuota) or die(mysqli_error($conectar));
	}
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	while ($matriz_consulta = mysqli_fetch_assoc($consulta)) {
	    ...
	    $cod_estado_obligatorio                                     = $matriz_consulta['cod_estado_obligatorio'];
		$cod_estado_obligatorio2                                    = $matriz_consulta['cod_estado_obligatorio2'];
    	$cod_estado_obligatorio_revisor                             = $matriz_consulta['cod_estado_obligatorio_revisor'];
		...
		nombre_tipo_recorte_foto, cod_documento_requisito_entidad_crediticia, cod_estado_obligatorio, cod_estado_obligatorio2, cod_estado_obligatorio_revisor, cod_posicion) 
	...
	$respuesta_ajax['nombre_tipo_origen_simulacion']        = $nombre_tipo_origen_simulacion;
	$respuesta_ajax['url_redir']                            = $url_redir;
