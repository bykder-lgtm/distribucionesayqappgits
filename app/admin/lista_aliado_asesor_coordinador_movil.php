...
$cod_asesor = isset($_GET['cod_asesor']) ? intval($_GET['cod_asesor']) : '';
...
if (!empty($cod_asesor)) {
    ...
    if ($res_info_asesor && mysqli_num_rows($res_info_asesor) > 0) {
        ...
        $nombre_asesor = $info_asesor['nombres_apellidos_tercero'] ?: trim($info_asesor['nombres'] . ' ' . $info_asesor['apellidos']);
    }
}
        ...
        <div class="modal-body">
            <form id="formRegistro">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="cod_tipo_sector" name="cod_tipo_sector" required>
                            ...
                            ?>
                            <option value="<?php echo $tipo_sector['cod_tipo_sector']; ?>" title="<?php echo $tipo_sector['descripcion_tipo_sector']; ?>"><?php echo $tipo_sector['nombre_tipo_sector']; ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group" id="container_nit_razon_social" style="display: none;">
                    ...
                    <input type="text" class="form-input" id="nit_razon_social" name="nit_razon_social" placeholder="Ingrese el NIT de la empresa">
                </div>
        ...
        <div class="modal-body">
            <form id="formEditar">
                ...
                <div class="form-group" id="edit_container_nit_razon_social" style="display: none;">
                    <label class="form-label">NIT Razón Social *</label>
                    <input type="text" class="form-input" id="edit_nit_razon_social" name="nit_razon_social" placeholder="Ingrese el NIT de la empresa">
                </div>
...
function cambiarTipoCliente() {
    ...
    var nitInput = document.getElementById('nit_razon_social');
    var labelNombreComercial = document.getElementById('label_nombre_comercial');
    
    // Guardar el nombre del tipo de cliente en el input hidden
    document.getElementById('nombre_tipo_cliente').value = nombreTipoCliente;
    
    // Normalizar para comparación (mayúsculas y quitar acentos básicos si es necesario)
    var nombreNormalizado = nombreTipoCliente ? nombreTipoCliente.toUpperCase() : '';
    
    // Mostrar/Ocultar y cambiar requerimientos según el tipo de cliente
    if (nombreNormalizado === 'EMPRESA' || nombreNormalizado === 'PERSONA JURIDICA' || nombreNormalizado === 'PERSONA JURÍDICA' || nombreNormalizado === 'PERSONA_JURIDICA') {
...
function cambiarTipoClienteEdit() {
    ...
    if (selectedOption) {
        nombreTipoCliente = selectedOption.getAttribute('data-nombre');
    }
    
    var containerNit = document.getElementById('edit_container_nit_razon_social');
    var nitInput = document.getElementById('edit_nit_razon_social');
    var labelNombreComercial = document.getElementById('edit_label_nombre_comercial');
    
    // Guardar el nombre del tipo de cliente en el input hidden
    document.getElementById('edit_nombre_tipo_cliente').value = nombreTipoCliente;
...
function abrirModal() { 
    ...
    // Reiniciar visibilidad de campos dinámicos
    cambiarTipoCliente();
...
function abrirModalEditar(data) {
    ...
    // Trigger para actualizar visibilidad
...
function eliminarEntidadAliado(cod_entidad_crediticia) {
    ...
    Swal.fire({
        ...
        html: '<div style="text-align: left; padding: 1rem;"><p style="margin-bottom: 0.5rem;">Se eliminará la parametrización de:</p><strong style="color: #6366f1;">' + nombre_entidad + '</strong><p style="margin-top: 0.5rem; color: #ef4444; font-size: 0.85rem;">Esta acción no se puede deshacer.</p></div>',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#6b7280',
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        Swal.fire({
                            icon: 'success',
                            title: '¡Enviado!',
                            html: '<p>' + response.mensaje + '</p><p style="margin-top: 0.5rem; font-size: 0.85rem; color: rgba(255,255,255,0.7);">Nueva contraseña temporal: <strong style="color: #6366f1;">' + response.password_temporal + '</strong></p>',
                            confirmButtonColor: '#6366f1',
                            background: '#1a1f2e',
                            color: 'white',
                            customClass: { container: 'swal-high-zindex' }
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.existe) {
                ...
                inputField.css('border-color', '#6366f1');
                mensajeDiv.hide();
