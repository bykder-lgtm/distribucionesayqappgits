...
$sql = "SELECT p.*, t.nombre_tienda, c.nombre_categoria, e.nombre_estado FROM tbl15_producto p LEFT JOIN tbl15_tienda t ON t.cod_tienda = p.cod_tienda
LEFT JOIN tbl15_categoria c ON c.cod_categoria = p.cod_categoria LEFT JOIN tbl15_estado e ON e.cod_estado = p.cod_estado WHERE p.cod_producto = '".intval($cod_producto)."' LIMIT 1";
$q = mysqli_query($conectar, $sql);
if (!$q) { $respuesta['afectado'] = 'NO'; $respuesta['mensaje'] = 'Error en consulta'; echo json_encode($respuesta); exit; }
if (mysqli_num_rows($q) == 0) { $respuesta['afectado'] = 'NO'; $respuesta['mensaje'] = 'Producto no encontrado'; echo json_encode($respuesta); exit; }
