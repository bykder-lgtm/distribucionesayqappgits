...
$datos_factura = "SELECT cod_transferencia_producto_temporal FROM tbl15_transferencia_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
xmlhttp.open("POST","../admin/busqueda_inmediata_transferencia_producto_php.php",true);
...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody><tr>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/lista_info_factura_trasnferencia.php">LISTA DE TRANSFRENCIAS</a></strong></td>
    </tr></tbody>
</table>
<br>
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
...
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_factura_transferencia_pos.php'); ?>
<!-- ***************************************************************************************************************************** -->
<!-- ***************************************************************************************************************************** -->
...
<tbody>
<?php
$sql_transferencia_producto_temporal = "SELECT * FROM tbl15_transferencia_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') 
...
$und_venta                         = $datos_transferencia_producto_temporal['und_venta'];
...
$cod_info_factura_transferencia    = $datos_transferencia_producto_temporal['cod_info_factura_transferencia'];
...
?>
    <tr style="text-align:center;" id="tr<?php echo $cod_transferencia_producto_temporal;?>">
        ...
        <!--<td style="text-align:center"><input name="cod_tipo_cobrar" class="cod_tipo_cobrar__<?php echo $cod_transferencia_producto_temporal;?>" id="cod_tipo_cobrar_<?php echo $cod_transferencia_producto_temporal;?>" type="checkbox" value="1" <?php if($cod_tipo_cobrar=='1'){ echo "checked"; } ?>></td>-->
        <!--<td style="text-align:center;" id="cod_transferencia_producto_temporal_<?php echo $cod_transferencia_producto_temporal ?>" class="service_list" data="<?php echo $cod_transferencia_producto_temporal ?>"><a class="eliminar" id="cod_transferencia_producto_temporal<?php echo $cod_transferencia_producto_temporal ?>"><img src="../imagenes/eliminar.png" class="img-polaroid" alt=""></a></td>-->
        ...
        <td style="text-align:center;" id="und_venta_<?php echo $incre;?>"><input name="und_venta" type="number" id="und_venta<?php echo $incre;?>" class="<?php echo $cod_transferencia_producto_temporal;?>" onChange="calc_total_venta();" value="<?php echo $und_venta;?>" style="width: 70px;" /></td>

        <td style="text-align:center;">
        <select name="nombre_tipo_unidad_medida" id="<?php echo $cod_transferencia_producto_temporal;?>" class="<?php echo $cod_transferencia_producto_temporal;?>" style="width: 70px;">
...
$(document).ready(function() {
    ...
    $('.eliminar').click(function(){
        ...
        var cod_transferencia_producto_temporal = $(this).parent().attr('data');
        var dataString = 'llave='+cod_transferencia_producto_temporal+'&'+'tab='+'<?php echo $tab ?>'+'&'+'campo='+'<?php echo $campo ?>'+'&'+'tipo='+'<?php echo $tipo ?>';
        ...
        $.ajax({
            ...
            success: function() {           
                $('#eliminar-ok').empty();
                $('#eliminar-ok').append('<div align="center" class="correcto">Se ha eliminado correctamente el codigo = '+cod_transferencia_producto_temporal+'.</div>').fadeIn("slow");
                $('#'+parent).fadeOut("slow");
                $('#cod_transferencia_producto_temporal'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#cod_producto_barra_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#nombre_cliente_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#nombre_producto_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#und_venta_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#precio_venta_producto_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#mensaje_alerta_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#total_venta_producto_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#nombre_tipo_unidad_medida_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#nombre_frec_duracion_'+cod_transferencia_producto_temporal).fadeOut("slow");
                $('#tr'+cod_transferencia_producto_temporal).fadeOut("slow");
            }
        });
 ...
 $(document).ready(function(){  
  ...
  let id = this.id;
  var tipo_ajax = "tbl15_transferencia_producto_temporal";
    $.ajax({ url:"guardar_info_factura_transferencia_temporal_producto_ajax.php", method:"GET", data:{valor:nombre_tipo_unidad_medida, campo:"nombre_tipo_unidad_medida", tipo_ajax:tipo_ajax, id:id }, success:function(data){ $('#result').html(data); }  
    });  
  });
 });  
 </script>
