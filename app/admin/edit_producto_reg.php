...
if ((isset($_POST["ins_edit"])) && ($_POST["ins_edit"] == "formulario_insert_edit")) {
	...
	if (isset($_POST['iva_ptj']) <> '') { $iva_ptj = mysqli_real_escape_string($conectar, ($_POST['iva_ptj'])); } else { $iva_ptj = '0'; }
	...
	if (isset($_POST['cod_tipo_habitacion_hotel']) <> '') { $cod_tipo_habitacion_hotel = intval($_POST['cod_tipo_habitacion_hotel']); } else { $cod_tipo_habitacion_hotel = '0'; }
	if (isset($_POST['cod_estado_habitacion_hotel']) <> '') { $cod_estado_habitacion_hotel = intval($_POST['cod_estado_habitacion_hotel']); } else { $cod_estado_habitacion_hotel = '0'; }

	if (isset($_POST['iva_saludable_ptj']) <> '') { $iva_saludable_ptj = addslashes($_POST['iva_saludable_ptj']); } else { $iva_saludable_ptj = '0'; }
	if (isset($_POST['cod_producto_barra2']) <> '') { $cod_producto_barra2 = addslashes($_POST['cod_producto_barra2']); } else { $cod_producto_barra2 = ''; }
	if (isset($_POST['cod_opcion_descontable_inv']) <> '') { $cod_opcion_descontable_inv = addslashes($_POST['cod_opcion_descontable_inv']); } else { $cod_opcion_descontable_inv = '0'; }
	if (isset($_POST['cod_promocion']) <> '') { $cod_promocion = intval($_POST['cod_promocion']); } else { $cod_promocion = '0'; }
	if (isset($_POST['cod_estado']) <> '') { $codigo_estado = intval($_POST['cod_estado']); } else { $codigo_estado = '1'; }

	if (isset($_POST['cod_estado_promocion']) <> '') { $cod_estado_promocion = intval($_POST['cod_estado_promocion']); } else { $cod_estado_promocion = ''; }
	if (isset($_POST['posicion_promocion']) <> '') { $posicion_promocion = intval($_POST['posicion_promocion']); } else { $posicion_promocion = ''; }
	if (isset($_POST['cod_estado_destacado']) <> '') { $cod_estado_destacado = intval($_POST['cod_estado_destacado']); } else { $cod_estado_destacado = ''; }
	if (isset($_POST['posicion_destacado']) <> '') { $posicion_destacado = intval($_POST['posicion_destacado']); } else { $posicion_destacado = ''; }
	if (isset($_POST['cod_tienda']) <> '') { $cod_tienda = intval($_POST['cod_tienda']); } else { $cod_tienda = ''; }

	$sql_promocion = "SELECT * FROM tbl15_promocion WHERE cod_promocion = '$cod_promocion'";
	...
	$nombre_promocion                = $matriz_promocion['nombre_promocion'];
	$nombre_promocion_ing            = $matriz_promocion['nombre_promocion_ing'];

	$sql_promocion = "SELECT * FROM tbl15_estado WHERE codigo_estado = '$codigo_estado'";
	$consulta_promocion = mysqli_query($conectar, $sql_promocion) or die(mysqli_error($conectar));
	$matriz_promocion = mysqli_fetch_assoc($consulta_promocion);

	$cod_estado                      = $matriz_promocion['codigo_estado'];
	$nombre_estado                   = $matriz_promocion['nombre_estado'];

	$sql_data = 'UPDATE tbl15_producto SET 
	...
	precio_venta_producto2 = "'.$precio_venta_producto2.'", precio_venta_producto3 = "'.$precio_venta_producto3.'", 
	precio_venta_producto4 = "'.$precio_venta_producto4.'", precio_venta_producto5 = "'.$precio_venta_producto5.'", 
	...
	genital_semen = "'.$genital_semen.'", observacion_animal = "'.$observacion_animal.'", 
	...
	cod_tipo_habitacion_hotel = "'.$cod_tipo_habitacion_hotel.'", cod_estado_habitacion_hotel = "'.$cod_estado_habitacion_hotel.'", iva_saludable_ptj = "'.$iva_saludable_ptj.'", 
	cod_producto_barra2 = "'.$cod_producto_barra2.'", cod_opcion_descontable_inv = "'.$cod_opcion_descontable_inv.'", 
	cod_promocion = "'.$cod_promocion.'", cod_estado = "'.$cod_estado.'", nombre_estado = "'.$nombre_estado.'", 
	cod_estado_promocion = "'.$cod_estado_promocion.'", posicion_promocion = "'.$posicion_promocion.'", 
	cod_estado_destacado = "'.$cod_estado_destacado.'", posicion_destacado = "'.$posicion_destacado.'", cod_tienda = "'.$cod_tienda.'"
