<?php 
$nombre_pagina          = "Cargar Soportes Revisor";
$cod_seguridad_pag      = "1";
...
if ($nombre_estado_factura == 'ABIERTA') {
    $pagina_redirect_edit_factura = '../admin/checkout_info_factura_venta_abierta_siscredito_visitante_intern_revisor.php';
    $pagina_redirect_regresar = '../admin/lista_info_factura_venta_abierta_siscredito_visitante_intern_revisor.php';
} else {
    $pagina_redirect_edit_factura = '../admin/checkout_info_factura_venta_cerrada_siscredito_visitante_intern_revisor.php';
    $pagina_redirect_regresar = '../admin/lista_info_factura_venta_cerrada_siscredito_visitante_intern_revisor.php';
}
*/

if ($nombre_estado_factura == 'ABIERTA') {
    $pagina_redirect_ver_factura = '../admin/ver_info_factura_venta_abierta_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
    $pagina_redirect_edit_factura = '../admin/checkout_info_factura_venta_abierta_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
    $pagina_redirect_regresar = '../admin/lista_info_factura_venta_abierta_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
    $pagina_redirect_imprimir_factura = '#';
} else {
    $pagina_redirect_ver_factura = '../admin/ver_info_factura_venta_cerrada_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
    $pagina_redirect_edit_factura = '../admin/checkout_info_factura_venta_cerrada_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
    $pagina_redirect_regresar = '../admin/lista_info_factura_venta_cerrada_siscredito_visitante_intern_revisor.php?cod_info_factura_venta='.$cod_info_factura_venta.'&cuenta='.$cuenta.'&cod_caja_virtual='.$cod_caja_virtual.'&modo_venta_por_defecto='.$modo_venta_por_defecto.'&pagina='.$pagina;
                    ...
                    <div class="contact-form-right">
                        <div class="row title-left">
                            ...
                            <div class="col-9 col-sm-9 col-md-9 col-lg-9 col-xl-9 col-xxl-9">
                                <div class=""><h3><a href="<?php echo $pagina_redirect_ver_factura ?>"><?php echo $nombre_cliente ?> | <?php echo $nombre_entidad_crediticia ?> | <?php echo $nombre_producto ?> | <?php echo number_format($monto_deuda, 0, ",", ".") ?></a></h3></div>
                            </div>
...
<input type='hidden' value='<?php echo $nombre_nota_observacion;?>' id='nombre_nota_observacion<?php echo $cod_nota_observacion;?>'>
                            ...
                            -->
                                ...
                                <?php if ($codigo_estado_revision == '0') { // 0 POR CARGAR ?>
                                    ...
                                    <?php } else { ?>
                                    <div class="col-md-12">
                                        <div style="text-align:center; <?php echo $color_fondo_celda_estado ?>; <?php echo $color_letra_celda_estado ?>" id="estado_revision_por_cargar<?php echo $cod_nota_observacion ?>" class=""><?php echo $nombre_nota_observacion ?> | ESTADO: 
                                            <select style="width: 150px;" name="codigo_estado_revision" id="<?php echo $cod_nota_observacion ?>" class="form-control-sm">
                                                <?php if (isset($codigo_estado_revision)) { echo "<option value='' ></option>"; } else { echo "<option value='' ></option>"; }
                                            ...
                                            <span id="cargador_check<?php echo $cod_nota_observacion ?>"><img src="../imagenes/eliminar_vacio.png" class="img-polaroid"></span>
                                        </div>
                                    </div>
                                <?php } ?>
...
$(document).ready(function(){
    ...
    //$("input").on('change', function () {
        ...
        var foco = "focotext"+cod_nota_observacion;
        var pagina_refrescar = pagina+"?cod_info_factura_venta="+cod_info_factura_venta+"&pagina=<?php echo $pagina ?>";

        var datos_url_ajax = 'id='+cod_nota_observacion+'&'+'valor='+codigo_estado_revision+'&'+'cod_info_factura_venta='+cod_info_factura_venta+'&'+'tipo_ajax='+tipo_ajax+'&'+'pagina='+pagina;
        ...
        $.ajax({
            ...
            success:function(respuesta){
                ...
                var cod_nota_observacion = respuesta.cod_nota_observacion;
                var codigo_estado_revision = respuesta.codigo_estado_revision;
                var nombre_estado_revision = respuesta.nombre_estado_revision;
                var color_fondo_celda = respuesta.color_fondo_celda;
                var color_letra_celda = respuesta.color_letra_celda;
                var accion_soportes = respuesta.accion_soportes;
                var mensaje = respuesta.mensaje;

                if (afectado == 'SI') {
                    $('#cargador_check'+cod_nota_observacion).html('');
                    //$('#cargador_check'+cod_nota_observacion).html('<img src="../imagenes/bien.png">');
                }
                if (accion_soportes == 'REFRESCAR_PAGINA') {
                    window.location.href = pagina_refrescar;
                }
                estado_revision_por_cargar.style.backgroundColor = color_fondo_celda;
            }
        });
    });
});
