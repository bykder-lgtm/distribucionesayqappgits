...
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_factura_compra_temporal_precio_compra_por_caja_pos.php'); ?>
<!-- ***************************************************************************************************************************** -->
...
<th style="text-align:center;">PRESENTACION (U.PQ)</th>
...
<th style="text-align:center;">P.COMPRA + IVA (POR CAJA)</th>
<th style="text-align:center;">..</th>
<th style="text-align:center;">P.COMPRA POR UND</th>
...
<th style="text-align:center">P.VENTA POR CAJA</th>
<th style="text-align:center;">..</th>
<th style="text-align:center">P.VENTA POR UND</th>
...
<tbody>
<?php
$sql_compra_producto_temporal = "SELECT * FROM tbl15_compra_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') 
...
while ($datos_compra_producto_temporal = mysqli_fetch_assoc($consulta_compra_producto_temporal)) {
    ...
    $precio_compra_producto_promedio               = $datos_compra_producto_temporal['precio_compra_producto_promedio'];
    ...
    $precio_compra_producto_por_caja               = $datos_compra_producto_temporal['precio_compra_producto_por_caja'];
    $precio_costo_producto_por_caja                = $datos_compra_producto_temporal['precio_costo_producto_por_caja'];
    $precio_venta_producto_por_caja                = $datos_compra_producto_temporal['precio_venta_producto_por_caja'];
    $precio_venta_producto2_por_caja               = $datos_compra_producto_temporal['precio_venta_producto2_por_caja'];
    $precio_venta_producto3_por_caja               = $datos_compra_producto_temporal['precio_venta_producto3_por_caja'];

    $cod_estado_precio_compra_producto_por_caja    = $datos_compra_producto_temporal['cod_estado_precio_compra_producto_por_caja'];
    $cod_estado_precio_venta_producto_por_caja     = $datos_compra_producto_temporal['cod_estado_precio_venta_producto_por_caja'];

    if ($cod_tipo_sistema_numeracion_und_compra == '2') { $und_compra = intval($und_compra); } else { $und_compra = $und_compra; }
    if ($cod_tipo_sistema_numeracion_precio_compra == '2') { $precio_compra_producto = intval($precio_compra_producto); $precio_compra_producto_por_caja = intval($precio_compra_producto_por_caja); } else { $precio_compra_producto = $precio_compra_producto; $precio_compra_producto_por_caja = $precio_compra_producto_por_caja; }
    if ($cod_tipo_sistema_numeracion_precio_venta == '2') { $precio_venta_producto = intval($precio_venta_producto); $precio_venta_producto_por_caja = intval($precio_venta_producto_por_caja); } else { $precio_venta_producto = $precio_venta_producto; $precio_venta_producto_por_caja = $precio_venta_producto_por_caja; }
    ...
    if ($cod_tipo_sistema_numeracion_und_unidades_presentacion == '2') { $und_unidades = intval($und_unidades); $cajas_sobre = intval($cajas_sobre); } else { $und_unidades = $und_unidades; $cajas_sobre = $cajas_sobre; } 
    //if ($cod_tipo_sistema_numeracion_und_caja_presentacion == '2') { $aaaaaa = intval($aaaaaa); } else { $aaaaaa = $aaaaaa; }
    if ($cod_estado_precio_compra_producto_por_caja == 1) { $imagen_precio_compra_producto_por_caja = '<img src=../imagenes/correctok.png alt="Listo">'; } else { $imagen_precio_compra_producto_por_caja = '<img src=../imagenes/incorrecto.png alt="Listo">'; }
    if ($cod_estado_precio_venta_producto_por_caja == 1) { $imagen_precio_venta_producto_por_caja = '<img src=../imagenes/correctok.png alt="Listo">'; } else { $imagen_precio_venta_producto_por_caja = '<img src=../imagenes/incorrecto.png alt="Listo">'; }
    ...
    $incre++;
    $pagina_refrescar = $pagina_local.'?'.'cod_compra_producto_temporal='.$cod_compra_producto_temporal.'&'.'cuenta='.$cuenta_actual.'&'.'cod_caja_virtual='.$cod_caja_virtual.'&'.'cod_info_factura_compra='.$cod_info_factura_compra.'&'.'foco='.'btn_listo'.$incre;
?>
...
<td style="text-align:center;"><input name="precio_compra_producto_por_caja" type="text" id="precio_compra_producto_por_caja<?php echo $incre;?>" class="<?php echo $cod_compra_producto_temporal;?>" onChange="calc_total_compra();" value="<?php echo $precio_compra_producto_por_caja;?>" style="width: 70px;" /></td>
<td style="text-align:center;"><a id="calcular_valor_unitario_precio_compra_producto_por_caja<?php echo $incre;?>" href="../admin/calcular_valor_unitario_precio_compra_venta_producto_por_caja_factura_compra.php?cod_compra_producto_temporal=<?php echo $cod_compra_producto_temporal;?>&cuenta=<?php echo $cuenta_actual;?>&cod_caja_virtual=<?php echo $cod_caja_virtual;?>&cod_info_factura_compra=<?php echo $cod_info_factura_compra;?>&campo=PRECIO_COMPRA_POR_CAJA&foco=calcular_valor_unitario_precio_compra_producto_por_caja<?php echo $incre;?>&pagina=<?php echo $pagina_local;?>"><?php echo $imagen_precio_compra_producto_por_caja;?></a></td>
<td style="text-align:center;"><input name="precio_compra_producto" type="text" id="precio_compra_producto<?php echo $incre;?>" class="<?php echo $cod_compra_producto_temporal;?>" value="<?php echo $precio_compra_producto;?>" style="width: 70px;" /></td>
...
<td style="text-align:center;"><input name="precio_venta_producto_por_caja" type="text" id="precio_venta_producto_por_caja<?php echo $incre;?>" class="<?php echo $cod_compra_producto_temporal;?>" value="<?php echo $precio_venta_producto_por_caja;?>" style="width: 55px;" /></td>
<td style="text-align:center;"><a id="calcular_valor_unitario_precio_venta_producto_por_caja<?php echo $incre;?>" href="../admin/calcular_valor_unitario_precio_compra_venta_producto_por_caja_factura_compra.php?cod_compra_producto_temporal=<?php echo $cod_compra_producto_temporal;?>&cuenta=<?php echo $cuenta_actual;?>&cod_caja_virtual=<?php echo $cod_caja_virtual;?>&cod_info_factura_compra=<?php echo $cod_info_factura_compra;?>&campo=PRECIO_VENTA_POR_CAJA&foco=calcular_valor_unitario_precio_venta_producto_por_caja<?php echo $incre;?>&pagina=<?php echo $pagina_local;?>"><?php echo $imagen_precio_venta_producto_por_caja;?></a></td>
<td style="text-align:center;"><input name="precio_venta_producto" type="text" id="precio_venta_producto<?php echo $incre;?>" class="<?php echo $cod_compra_producto_temporal;?>" value="<?php echo $precio_venta_producto;?>" style="width: 55px;" /></td>
...
<?php if ($cod_estado_fecha_vencimiento_global == '1') { ?>
<td style="text-align:center;"><input name="fecha_vencimiento" type="date" id="fecha_vencimiento<?php echo $incre;?>" class="<?php echo $cod_compra_producto_temporal;?>" value="<?php echo $fecha_vencimiento;?>" style="width: 120px;" /></td>
<?php } ?>

<td style="text-align:right;" id="total_compra_producto<?php echo $incre;?>"><?php echo number_format($total_compra_producto, 0, ",", ".");?></td>
<td style="text-align:center;"><a id="btn_listo<?php echo $incre;?>" href="<?php echo $pagina_refrescar?>"><?php echo $imagen;?></a></td>
<th style="text-align:center;"></th>
...
$(document).ready(function(){
    $('select[name="nombre_tipo_unidad_medida"]').change(function(){ 
        ...
        var valor = nombre_tipo_unidad_medida;
        var campo = "nombre_tipo_unidad_medida";
...
$(document).ready(function(){
    $('select[name="cod_dependencia"]').change(function(){ 
        ...
        var valor = cod_dependencia;
        var campo = "cod_dependencia";
        var tipo_ajax = "tbl15_compra_producto_temporal";
        var campo_incre = campo;
        var pagina = "<?php echo $pagina; ?>";
        var datos_url_ajax = 'id='+id+'&'+'valor='+valor+'&'+'campo='+campo+'&'+'tipo_ajax='+tipo_ajax+'&'+'campo_incre='+campo_incre+'&'+'pagina='+pagina;
...
$(document).ready(function(){
    $('textarea[name="nombre_producto"]').change(function(){ 
        ...
        var valor = nombre_producto;
        var campo = "nombre_producto";
        var tipo_ajax = "tbl15_compra_producto_temporal";
        var campo_incre = campo;
        var pagina = "<?php echo $pagina; ?>";
        var datos_url_ajax = 'id='+id+'&'+'valor='+valor+'&'+'campo='+campo+'&'+'tipo_ajax='+tipo_ajax+'&'+'campo_incre='+campo_incre+'&'+'pagina='+pagina;
