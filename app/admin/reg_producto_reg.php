...
if ((isset($_POST["insersion"])) && ($_POST["insersion"] == "formulario_de_insersion")) {
	...
	if (isset($_POST['cod_tienda']) <> '') { $cod_tienda = intval($_POST['cod_tienda']); } else { $cod_tienda = '0'; }
	...
	genital_potencia, genital_semen, observacion_animal, nombre_tipo_movimiento, 
	...
	cod_tipo_habitacion_hotel, cod_estado_habitacion_hotel, iva_saludable_ptj, cod_producto_barra2, nombre_promocion, nombre_promocion_ing,  
	cod_promocion, cod_estado, nombre_estado, cod_tienda) 
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($archivo_adjunto <> '') { 
		...
		$sql_data = "INSERT INTO tbl15_archivo_adjunto (cod_producto, cod_producto_barra, nombre_archivo_adjunto, url_archivo_adjunto, fecha_creacion, fecha_hora, cod_administrador, formato) 
		VALUES ('$cod_producto', '$cod_producto_barra', '$nombre_archivo_adjunto', '$url_archivo_adjunto', '$fecha_creacion', '$fecha_hora', '$cod_administrador', '$formato')";
	...
	/* ----------------------------------------------------------------------------------------------------------/ */
		...
		if ($imagen_foto_miniatura->uploaded) {
			...
			$sql_data = sprintf("UPDATE tbl15_producto SET url_img_orig_producto = '$url_img_orig_producto', url_img_min_producto = '$url_img_min_producto', 
			url_img_producto_orig = '$url_img_producto_orig', url_img_producto_min = '$url_img_producto_min' WHERE cod_producto = '$cod_producto'");
			$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));

			$sql_data = "INSERT INTO tbl15_producto_imagen (cod_producto, cod_producto_barra, nombre_producto, url_img_orig_producto, url_img_min_producto, url_img_producto_orig, url_img_producto_min, 
			fecha_ymd, fecha_hora, active, cod_posicion) 
			VALUES ('$cod_producto', '$cod_producto_barra', '$nombre_producto', '$url_img_orig_producto', '$url_img_min_producto', '$url_img_producto_orig', '$url_img_producto_min', 
			'$fecha_ymd', '$fecha_hora', '$active', '$cod_posicion')";
			$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
		} else { echo 'error : ' . $imagen_foto_miniatura->error; }
	}
?>
<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/lista_producto.php">
<?php } } ?>
<!-- ***************************************************************************************************************************** -->
