...
<style>
/* ============================================ */
/* DASHBOARD VENDEDOR - TEMA NARANJA/AMBER     */
/* ============================================ */
...
.qa-creditos i { color: #f59e0b; }
.qa-registrar i { color: #10b981; }
.qa-perfil i { color: #3b82f6; }
...
// === Obtener info de la tienda y aliado asociados al vendedor ===
$sql_info_vendedor = "SELECT v.cod_vendedor, v.cod_aliado_estrategico, v.nombres_apellidos_tercero as nombre_vendedor,
    t.nombre_tienda, t.identificacion_tienda, t.telefono_tienda, t.direccion_tienda,
    a.nombres_apellidos_tercero as nombre_aliado
    FROM tbl15_administrador v 
    LEFT JOIN tbl15_tienda t ON v.cod_vendedor = t.cod_tienda 
    LEFT JOIN tbl15_administrador a ON v.cod_aliado_estrategico = a.cod_administrador
    WHERE v.cod_administrador = '$cod_administrador'";
$resultado_info = mysqli_query($conectar, $sql_info_vendedor);
$info_vendedor = $resultado_info ? mysqli_fetch_assoc($resultado_info) : null;

$nombre_tienda = isset($info_vendedor['nombre_tienda']) ? $info_vendedor['nombre_tienda'] : 'Sin tienda asignada';
$nombre_aliado = isset($info_vendedor['nombre_aliado']) ? $info_vendedor['nombre_aliado'] : 'Sin aliado';
$telefono_tienda = isset($info_vendedor['telefono_tienda']) ? $info_vendedor['telefono_tienda'] : '';
$direccion_tienda = isset($info_vendedor['direccion_tienda']) ? $info_vendedor['direccion_tienda'] : '';

// === CONSULTAS PARA KPIs ===
// Total Créditos Activos (Estado ABIERTA) - del vendedor
...
$sql_creditos_cerrados = "SELECT COUNT(*) as total, COALESCE(SUM(monto_deuda), 0) as valor_total FROM tbl15_info_factura_venta WHERE cod_vendedor = '$cod_administrador' AND nombre_estado_factura = 'CERRADA'";
...
WHERE ifv.cod_vendedor = '$cod_administrador' AND nar.cod_estado = '0'";
$resultado_alertas = mysqli_query($conectar, $sql_alertas);
$datos_alertas = $resultado_alertas ? mysqli_fetch_assoc($resultado_alertas) : null;
...
<main class="dashboard-container">
    ...
    <div class="dashboard-header animate-in">
        <h1><i class="fa-solid fa-user-tie"></i> Dashboard Vendedor</h1>
        <p>Resumen de tu gestión de créditos</p>
        <div class="dashboard-date"><i class="fa-regular fa-calendar"></i><?php echo "$mes_nombre $anio"; ?></div>
    </div>

    <!-- Info de Tienda y Aliado -->
    <div class="info-card animate-in delay-1">
        <div class="info-card-header">
            <div class="info-card-icon store"><i class="fa-solid fa-store"></i></div>
            <div>
                <div class="info-card-title">Mi Tienda</div>
                <div class="info-card-value"><?php echo ucwords(strtolower($nombre_tienda)); ?></div>
            </div>
        </div>
        <div class="info-card-details">
            <div class="info-detail"><i class="fa-solid fa-handshake"></i><span><?php echo ucwords(strtolower($nombre_aliado)); ?></span></div>
            <?php if(!empty($telefono_tienda)): ?><div class="info-detail"><i class="fa-solid fa-phone"></i><span><?php echo $telefono_tienda; ?></span></div><?php endif; ?>
            <?php if(!empty($direccion_tienda)): ?><div class="info-detail"><i class="fa-solid fa-map-marker-alt"></i><span><?php echo $direccion_tienda; ?></span></div><?php endif; ?>
        </div>
    </div>

    <!-- Acciones Rápidas -->
    <div class="quick-actions animate-in delay-1">
        <a href="../admin/lista_info_factura_venta_siscredito_visitante_intern_vendedor_movil.php" class="quick-action-btn qa-creditos"><i class="fa-solid fa-credit-card"></i><span>Créditos</span></a>
        <a href="../admin/confirmacion_registro_solicitud_siscredito_visitante_intern_vendedor_movil.php" class="quick-action-btn qa-registrar"><i class="fa-solid fa-plus-circle"></i><span>Registrar</span></a>
        <a href="../admin/perfil_vendedor_movil.php" class="quick-action-btn qa-perfil"><i class="fa-solid fa-user"></i><span>Mi Perfil</span></a>
    </div>
    ...
    <div class="kpi-grid">
        <div class="kpi-card animate-in delay-1">
            ...
            <div class="kpi-change positive"><i class="fa-solid fa-check-circle"></i> En curso</div>
        </div>

        <div class="kpi-card animate-in delay-2">
            <div class="kpi-icon success"><i class="fa-solid fa-dollar-sign"></i></div>
        ...
        <div class="kpi-card animate-in delay-3">
            <div class="kpi-icon warning"><i class="fa-solid fa-calendar-plus"></i></div>
    ...
    <div class="alert-card animate-in">
        <div class="alert-icon-wrap"><i class="fa-solid fa-bell"></i></div>
    ...
    <div class="chart-card animate-in">
        <div class="chart-header"><div class="chart-title"><i class="fa-solid fa-chart-line"></i>Tendencia de Créditos</div><span class="chart-badge">Últimos 6 meses</span></div>
        <div class="chart-container"><canvas id="trendChart"></canvas></div>
    </div>

    <!-- Gráfico por Estado -->
    <?php if (count($estados_labels) > 0): ?>
    <div class="chart-card animate-in">
        <div class="chart-header"><div class="chart-title"><i class="fa-solid fa-chart-pie"></i>Por Estado</div></div>
    ...
    <div class="activity-card animate-in">
        <div class="chart-header"><div class="chart-title"><i class="fa-solid fa-clock"></i>Actividad Reciente</div></div>
        <?php 
        if ($resultado_ultimas && mysqli_num_rows($resultado_ultimas) > 0):
        ...
        <div class="activity-item">
            <div class="activity-icon <?php echo $estado_act == 'CERRADA' ? 'completed' : 'pending'; ?>">
                <i class="fa-solid fa-<?php echo $estado_act == 'CERRADA' ? 'check' : 'clock'; ?>"></i>
            </div>
        ...
        <div style="text-align: center; padding: 2rem;">
            ...
            <p style="color: rgba(255,255,255,0.5); margin-top: 0.5rem;">No hay actividad reciente</p>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="chart-card animate-in">
        <div class="chart-header">
            <div class="chart-title"><i class="fa-solid fa-chart-bar"></i>Resumen General</div>
        </div>
        <div class="stats-grid">
            <div class="stat-item">
                <div class="stat-value"><?php echo number_format($total_creditos_activos); ?></div>
...
// Colores del tema vendedor
const themeColors = { primary: '#f59e0b', secondary: '#d97706', accent: '#fbbf24', success: '#10b981', warning: '#ef4444', danger: '#dc2626', blue: '#3b82f6', purple: '#8b5cf6' };
...
new Chart(trendCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Créditos', data: <?php echo json_encode($tendencia_valores); ?>, borderColor: themeColors.primary, backgroundColor: trendGradient, borderWidth: 3, fill: true, tension: 0.4, pointBackgroundColor: themeColors.primary, pointBorderColor: '#fff', pointBorderWidth: 2, pointRadius: 5, pointHoverRadius: 8
        }]
    },
    options: {
        ...
        scales: {
            y: { beginAtZero: true, grid: { color: 'rgba(245, 158, 11, 0.1)' }, ticks: { stepSize: 1 } },
...
new Chart(statusCtx, {
    ...
    data: {
        ...
        datasets: [{ data: <?php echo json_encode($estados_valores); ?>, backgroundColor: gradientColors, borderColor: '#0d1117', borderWidth: 3, hoverOffset: 10 }]
    },
