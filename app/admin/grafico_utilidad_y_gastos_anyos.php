...
$(function () {
    $('#container_grafico_estadistico').highcharts({
        ...
        xAxis: [{
            categories: [
<?php
$sql = "SELECT anyo FROM tbl15_venta_producto GROUP BY anyo ORDER BY fecha_ymd_venta_producto ASC";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(total_venta_producto-total_compra_producto) AS total_venta_producto FROM tbl15_venta_producto GROUP BY anyo ORDER BY fecha_ymd_venta_producto ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
...
?>
            ...
            data: [
<?php
$sql = "SELECT SUM(costo) AS costo FROM tbl15_egreso GROUP BY anyo ORDER BY fecha_dmy ASC";
$result = mysqli_query($conectar, $sql) or die(mysqli_error($conectar));
while ($registros = mysqli_fetch_array($result)) {
