...
if (isset($_POST['cod_tienda']) && !empty($_POST['cod_tienda'])) {
    ...
    if (!empty($url_img_orig_tienda)) { $campos_update[] = "url_img_orig_tienda = '$url_img_orig_tienda'"; $campos_update[] = "url_img_min_tienda = '$url_img_min_tienda'"; }
    if (!empty($url_documentacion_rut_tienda)) { $campos_update[] = "url_documentacion_rut_tienda = '$url_documentacion_rut_tienda'"; }
    if (!empty($url_documentacion_camaracomercio_tienda)) { $campos_update[] = "url_documentacion_camaracomercio_tienda = '$url_documentacion_camaracomercio_tienda'"; }
    if (!empty($url_img_fachada_tienda)) { $campos_update[] = "url_img_fachada_tienda = '$url_img_fachada_tienda'"; }
    if (!empty($url_img_interna_tienda)) { $campos_update[] = "url_img_interna_tienda = '$url_img_interna_tienda'"; }
    if (!empty($url_img_selfieadmin_tienda)) { $campos_update[] = "url_img_selfieadmin_tienda = '$url_img_selfieadmin_tienda'"; }
    ...
    if ($exec_update && mysqli_affected_rows($conectar) >= 0) { echo json_encode(['success' => true, 'mensaje' => 'Tienda actualizada correctamente', 'cod_tienda' => $cod_tienda, 'nombre_tienda' => $nombre_tienda]); } else { echo json_encode(['success' => false, 'mensaje' => 'Error al actualizar la tienda: ' . mysqli_error($conectar)]); }
} else {
    echo json_encode(['success' => false, 'mensaje' => 'Datos incompletos. El código de tienda es requerido.']);
}
?>
