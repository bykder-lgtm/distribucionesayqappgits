...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody>
    	<tr>
       		<td style="text-align:center;"><strong><a href="../admin/lista_info_factura_auditoria.php">REGRESAR</a></strong></td>
    	</tr>
    	<tr>
       		<td style="text-align:center;">.</td>
    	</tr>
    	<tr>
       		<td style="text-align:center;"><strong><a href="#">LISTA DE AUDITORIAS MASIVAS</a></strong></td>
    	</tr>
</tbody>
</table>
<!-- ***************************************************************************************************************************** -->
...
<!-- ***************************************************************************************************************************** -->
<?php
$sql_info_factura = "SELECT cod_info_factura_auditoria, fecha_anyo, cuenta FROM tbl15_info_factura_auditoria WHERE (nombre_estado_factura = 'CERRADA') ORDER BY cod_info_factura_auditoria DESC";
...
<?php
	...
	while ($datos_factura_auditoria_producto = mysqli_fetch_assoc($consulta_factura_auditoria_producto)) {
		...
		$cod_estado_correccion                         = $datos_factura_auditoria_producto['cod_estado_correccion'];
...
if ($cod_estado_correccion == '1') { $nombre_estado_correccion = 'SI'; } else { $nombre_estado_correccion = 'NO'; }
...
?>
		...
		<td style="text-align:center;"></td>
		<?php if ($cod_estado_prod_auditoria_eliminar == '1') { ?>
		<td style="text-align:center;"><a href="../admin/eliminar_auditoria_por_registro.php?llave=<?php echo $cod_factura_auditoria_producto?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/eliminar.png" class="img-polaroid" alt=""></a></td>
		<?php } ?>
		<td style="text-align:left;" ><?php echo $cod_producto_barra ?></td>
		<td style="text-align:left;"><?php echo $nombre_producto ?></td>
		<?php if ($cod_estado_prod_auditoria_editar == '1') { ?>
		<td style="text-align:center;"><input type="text" onFocus="Focus(this.id, this.value)" onBlur="Blur(this.id, this.value, 'comentario', <?php echo $cod_factura_auditoria_producto;?>)" id="<?php echo $cod_factura_auditoria_producto;?>" class="input-block-level" value="<?php echo $comentario;?>" size="30"></td>
		<td style="text-align:center;"><input type="number" onFocus="Focus(this.id, this.value)" onBlur="Blur(this.id, this.value, 'corregir_inventario', <?php echo $cod_factura_auditoria_producto;?>)" id="<?php echo $cod_factura_auditoria_producto;?>" class="input-block-level" value="<?php echo $corregir_inventario;?>" step="any" size="4"></td>
		<?php } ?>
		<td style="text-align:center;"><?php echo $und_producto ?></td>
		<td style="text-align:center;"><?php echo $und_compra ?></td>
		<td style="text-align:right;"><?php echo $dato_mostrar ?></td>
		<td style="text-align:left;"><?php echo $mensaje?></td>
		<?php if ($cod_estado_prod_auditoria_editar == '1') { ?>
		<?php if ($cod_estado_correccion == '0') { ?><td style="text-align:center;"><?php echo $url_redirect ?></td><?php } else { ?><td  style="text-align:center;"><img src="<?php echo $imagen ?>"></td><?php } ?>
		<?php } ?>
		<td  style="text-align:center;"><?php echo $nombre_estado_correccion ?></td>
		<td style="text-align:center;"></td>
		</tr>
	<?php } ?>
	</tbody>
