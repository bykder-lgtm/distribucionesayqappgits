...
try {
    ...
    // Actualizar ubicación GPS en la tabla
    $sql = "UPDATE tbl15_tienda SET ubicacion_gps_tienda = '$ubicacion_gps' WHERE cod_tienda = '$cod_tienda'";
    if (mysqli_query($conectar, $sql)) {
        if (mysqli_affected_rows($conectar) > 0) {
            $response['success'] = true;
            $response['message'] = 'Ubicación GPS guardada correctamente';
        } else {
            throw new Exception('No se pudo actualizar la ubicación. Verifica que la tienda exista.');
        }
    } else {
