...
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
$cod_cuentas_cobrar_abonos                         = intval($_REQUEST['cod_cuentas_cobrar_abonos']);
...
if ($tamano_papel_impresora == '58') {
	...
	$barra      = $estandar_b.str_pad($cod_cuentas_cobrar_abonos, 5, "0", STR_PAD_LEFT);
	$printer -> setJustification(Printer::JUSTIFY_CENTER);
	...
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	$printer->text("<== ".$fecha_impr.$hora_impr."-".$cod_factura."-".$cod_cuentas_cobrar_abonos."_imposabnccdriv58 ==>");
	$printer->feed(3);
	...
	$printer->setEmphasis(false);
	$printer->text(str_pad("FECHA: ".$fecha_pago.' - '.$hora, 30));
	$printer->text(str_pad("ID ABONO: ".$cod_cuentas_cobrar_abonos, 25));
	$printer->text("\n");
	...
	$printer->setJustification(Printer::JUSTIFY_CENTER);
	$printer->text("<== ".$fecha_impr.$hora_impr."-".$cod_factura."-".$cod_cuentas_cobrar_abonos."_imposabnccdriv80 ==>");
	$printer->feed(2);
