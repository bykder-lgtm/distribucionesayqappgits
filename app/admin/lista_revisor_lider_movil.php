...
<style>
/* ============================================ */
/* LISTA REVISORES LIDER - TEMA PÚRPURA/VIOLETA   */
/* ============================================ */
...
?>
    ...
    <div class="page-header animate-in">
        <h1><i class="fa-solid fa-user-shield"></i> Mis Revisores</h1>
        <p>Gestiona el equipo de revisores de documentos</p>
        <div class="header-stats">
            <div class="header-stat">
                <div class="header-stat-value"><?php echo $total_registros_global; ?></div>
                <div class="header-stat-label">Total Revisores</div>
            </div>
        </div>
    </div>
    ...
    <div class="search-bar animate-in delay-1">
        ...
        <input type="text" id="searchInput" placeholder="Buscar revisor..." value="<?php echo htmlspecialchars($busqueda); ?>" onkeyup="filtrar(this.value)">
    </div>

    <!-- Add Button -->
    <button class="add-btn animate-in delay-2" onclick="abrirModalRegistro()"><i class="fa-solid fa-user-shield"></i> Nuevo Revisor</button>
    ...
    <div class="revisores-list">
        <?php if ($total_registros_pagina > 0): ?>
            <?php while($row = mysqli_fetch_assoc($resultado)): 
                ...
                if (empty($iniciales)) { $iniciales = 'RE'; }
            ...
            ?>
            <div class="revisor-card animate-in">
                <div class="revisor-header">
                    <div style="display: flex; align-items: flex-start; flex: 1;">
                        <div class="revisor-avatar"><?php echo $iniciales; ?></div>
                        <div class="revisor-info">
                            <div class="revisor-name"><?php echo $row['nombres_apellidos_tercero']; ?></div>
                            <span class="revisor-role"><i class="fa-solid fa-user-shield"></i> Revisor</span>
                        </div>
                    </div>
                    <span class="revisor-status <?php echo $estado_class; ?>"><?php echo $estado_texto; ?></span>
                </div>

                <div class="revisor-details">
                ...
                <!-- No stats for Revisors yet -->

                <div class="revisor-actions">
                    <button class="action-btn edit" onclick='abrirModalEditar(<?php echo json_encode($row); ?>)'>
                        <i class="fa-solid fa-edit"></i> Editar
                    </button>
                    <button class="action-btn archive" onclick="archivarEntidad(<?php echo $row['cod_administrador']; ?>, '<?php echo addslashes($row['nombres_apellidos_tercero']); ?>', 'Revisor')">
...
<?php endwhile; ?>
    ...
    <div class="pagination-container animate-in delay-3">
        ...
        <?php if ($total_paginas > 1): ?>
            ...
            $base_url = "lista_revisor_lider_movil.php?" . ($query_string ? $query_string . "&" : "");
            ?>
...
<div class="modal-overlay" id="modalRegistroRevisor" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(8px); z-index: 5000; align-items: center; justify-content: center; padding: 1rem; overflow-y: auto;">
    <div class="modal-content" style="background: linear-gradient(135deg, #1a1f2e 0%, #0d1117 100%); border: 1px solid rgba(139, 92, 246, 0.3); border-radius: 20px; width: 100%; max-width: 600px; max-height: 90vh; overflow-y: auto; position: relative; box-shadow: 0 20px 60px rgba(0,0,0,0.5);">
        <div class="modal-header" style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); padding: 1.5rem; border-radius: 20px 20px 0 0; position: relative; display: flex; justify-content: space-between; align-items: center;">
            <h2 style="color: white; font-size: 1.25rem; font-weight: 700; margin: 0; display: flex; align-items: center; gap: 0.5rem;"><i class="fa-solid fa-user-shield"></i> Nuevo Revisor</h2>
        ...
        <div class="modal-body" style="padding: 1.5rem;">
            <form id="formRegistroRevisor" onsubmit="registrarRevisor(event)">
                ...
                <div style="margin-top: 1.5rem;">
                    <button type="submit" class="submit-btn" style="width: 100%; background: #8b5cf6; color: white; border: none; padding: 1rem; border-radius: 12px; cursor: pointer; font-weight: 700; font-size: 0.95rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem; transition: all 0.3s ease;">
                        <i class="fa-solid fa-save"></i> Guardar Revisor
                    </button>
                </div>
            </form>
        </div>
