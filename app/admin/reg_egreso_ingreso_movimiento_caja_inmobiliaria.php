...
if (isset($_GET['nombre_tipo_puc'])) { $nombre_tipo_puc = addslashes($_GET['nombre_tipo_puc']); } else { $nombre_tipo_puc = 'EGRESO'; }
...
<table class="table table-striped">
    <tr>
        ...
        <th style="text-align:right"><a href="../admin/lista_egreso_ingreso_movimiento_caja_inmobiliaria.php"><font size='+2'>VER REPORTE</font></a></th>
    </tr>
</table>

<div class="table-responsive">
<form name="formulario_de_actualizacion" method="post" enctype="multipart/form-data" action="../admin/reg_egreso_ingreso_movimiento_caja_inmobiliaria_reg.php">
<table class="table table-striped">
<thead>
<tr>
...
<td style="text-align:left">
    ...
    $consulta2_sql = ("SELECT cod_tipo_puc, nombre_tipo_puc FROM tbl15_tipo_puc WHERE (nombre_tipo_puc = '$nombre_tipo_puc') AND (cod_estado = '1') ORDER BY nombre_tipo_puc ASC");
...
<td style="text-align:left">
    ...
    $consulta2_sql = ("SELECT cod_concepto_movimiento_caja, nombre_concepto_movimiento_caja, nombre_tipo_puc FROM tbl15_concepto_movimiento_caja WHERE (nombre_tipo_puc = '$nombre_tipo_puc') AND
    (cod_estado = '1') ORDER BY cod_concepto_movimiento_caja ASC");
    $consulta2 = mysqli_query($conectar, $consulta2_sql);
    while ($datos2 = mysqli_fetch_assoc($consulta2)) {
...
<td style="text-align:center"><input class="input-block-level" style="width: 200px;" name="costo" id="costo" type="number" min="1" value="" required/></td>
...
<td style="text-align:center">
    <select name="cod_dependencia" class="selectpicker" data-show-subtext="true" data-live-search="true" style="width: 120px;" required>
        ...
        $consulta2_sql = ("SELECT cod_dependencia, nombre_dependencia FROM tbl15_dependencia WHERE (nombre_tipo_puc = '$nombre_tipo_puc') OR (nombre_tipo_puc = 'AMBOS') ORDER BY cod_dependencia ASC");
        $consulta2 = mysqli_query($conectar, $consulta2_sql);
        while ($datos2 = mysqli_fetch_assoc($consulta2)) {
