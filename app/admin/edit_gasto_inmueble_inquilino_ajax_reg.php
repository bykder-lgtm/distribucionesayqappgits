...
$cod_gasto_inmueble_inquilino_venta                   = intval($_GET['id']);
$valor_intro                                        = addslashes($_GET['valor']);
$campo                                              = addslashes($_GET['campo']);
$respuesta_ajax                                     = array();

$sql_productos = "SELECT * FROM tbl15_gasto_inmueble_inquilino_venta WHERE cod_gasto_inmueble_inquilino_venta = '$cod_gasto_inmueble_inquilino_venta'";
...
if ($campo == 'descripcion_gasto_inmueble_detalle') {
	...
	$data_sql = ("UPDATE tbl15_gasto_inmueble_inquilino_venta SET descripcion_gasto_inmueble_detalle = UPPER('$descripcion_gasto_inmueble_detalle') WHERE cod_gasto_inmueble_inquilino_venta = '$cod_gasto_inmueble_inquilino_venta'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { $afectado = "SI"; } else { $afectado = "NO"; }

	$sql_productos = "SELECT SUM(precio_compra_producto) AS total_precio_compra, SUM(precio_venta_producto) AS total_precio_venta FROM tbl15_gasto_inmueble_inquilino_venta WHERE cod_factura = '$cod_factura'";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
...
elseif ($campo == 'precio_venta_producto') {
	...
	$data_sql = ("UPDATE tbl15_gasto_inmueble_inquilino_venta SET precio_venta_producto = '$precio_venta_producto' WHERE cod_gasto_inmueble_inquilino_venta = '$cod_gasto_inmueble_inquilino_venta'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));

	if (mysqli_affected_rows($conectar) > 0) { $afectado = "SI"; } else { $afectado = "NO"; }

	$sql_productos = "SELECT SUM(precio_compra_producto) AS total_precio_compra, SUM(precio_venta_producto) AS total_precio_venta FROM tbl15_gasto_inmueble_inquilino_venta WHERE (cod_factura = '$cod_factura') AND (cod_tipo_estado_incluido = '0')";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
...
elseif ($campo == 'precio_compra_producto') {
	...
	$data_sql = ("UPDATE tbl15_gasto_inmueble_inquilino_venta SET precio_compra_producto = '$precio_compra_producto' WHERE cod_gasto_inmueble_inquilino_venta = '$cod_gasto_inmueble_inquilino_venta'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	...
	$data_sql = ("UPDATE tbl15_gasto_inmueble_inquilino_venta SET $campo = '$valor_intro' WHERE cod_gasto_inmueble_inquilino_venta = '$cod_gasto_inmueble_inquilino_venta'");
	$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
