...
if ($campo == 'codigo_puc[]') {
	...
	$cod_movimiento_contable_temporal_concepto         = $id;
	...
	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable_temporal_concepto SET cod_puc = '$cod_puc', codigo_puc = '$codigo_puc', nombre_puc = '$nombre_puc', 
	tipo_puc = '$tipo_puc' WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
...
if ($campo == 'cod_tipo_nota_observacion') {
	if (isset($_REQUEST['id'])) { $id = intval($_REQUEST['id']); } 
	if (isset($_REQUEST['valor'])) { $cod_tipo_nota_observacion = intval($_REQUEST['valor']); } 
	$cod_movimiento_contable         = $id;
	$doc_modifica                    = "";
	$cod_factura                     = "0";
	$fecha_factura                   = "";
	$cod_info_factura_venta          = "0";
	$cod_info_factura_compra         = "0";

	if ($cod_tipo_nota_observacion ==  '0') {
		$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET doc_modifica = '$doc_modifica', cod_factura = '$cod_factura', fecha_factura = '$fecha_factura', 
		cod_info_factura_venta = '$cod_info_factura_venta', cod_info_factura_compra = '$cod_info_factura_compra' 
		WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
		$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
	}
	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET cod_tipo_nota_observacion = '$cod_tipo_nota_observacion' WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'doc_modifica') {
	...
	$cod_movimiento_contable         = $id;
	if (isset($_REQUEST['cod_tipo_nota_observacion'])) { $cod_tipo_nota_observacion = intval($_REQUEST['cod_tipo_nota_observacion']); } else { $cod_tipo_nota_observacion = 0; }
	if (isset($_REQUEST['cod_factura'])) { $cod_factura = addslashes($_REQUEST['cod_factura']); } else { $cod_factura = ""; }
	if (isset($_REQUEST['llave'])) { $llave = addslashes($_REQUEST['llave']); } else { $llave = 0; }
	if (isset($_REQUEST['fecha_factura'])) { $fecha_factura = addslashes($_REQUEST['fecha_factura']); } else { $fecha_factura = 0; }
	if (isset($_REQUEST['doc_modifica'])) { $doc_modifica = addslashes($_REQUEST['doc_modifica']); } else { $doc_modifica = $doc_modifica; }
	if ($cod_tipo_nota_observacion == '2') { $cod_info_factura_venta = $llave; $cod_info_factura_compra = 0; } elseif ($cod_tipo_nota_observacion == '3') { $cod_info_factura_venta = 0; $cod_info_factura_compra = $llave; } else { $cod_info_factura_venta = 0; $cod_info_factura_compra = 0; }
	
	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET doc_modifica = '$doc_modifica', cod_info_factura_venta = '$cod_info_factura_venta', cod_info_factura_compra = '$cod_info_factura_compra', 
	cod_tipo_nota_observacion = '$cod_tipo_nota_observacion', cod_factura = '$cod_factura', fecha_factura = '$fecha_factura' WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'fecha_ymd') {
	...
	$fecha_mes                               = date("m-Y", $fecha_seg);
	$anyo                                    = date("Y", $fecha_seg);

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET fecha_anyo = '$fecha_anyo', fecha_ymd = '$fecha_ymd', fecha_mes = '$fecha_mes', 
	anyo = '$anyo', fecha_seg = '$fecha_seg' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'nombre_movimiento_contable_temporal_concepto[]') {
	$nombre_movimiento_contable_temporal_concepto      = addslashes($valor_intro);
	$cod_movimiento_contable_temporal_concepto         = $id;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable_temporal_concepto SET nombre_movimiento_contable_temporal_concepto = '$nombre_movimiento_contable_temporal_concepto' WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'und_vendida[]') {
	...
	$obtener_info_cod_nota_credit = "SELECT costo_movimiento_contable_temporal_concepto, cod_movimiento_contable 
	FROM tbl15_movimiento_contable_temporal_concepto WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	...
	$costo_movimiento_contable_temporal_concepto      = $info_cod_nota_credit['costo_movimiento_contable_temporal_concepto'];
	$total_movimiento_contable_temporal_concepto      = $und_vendida * $costo_movimiento_contable_temporal_concepto;
	$cod_movimiento_contable                          = $info_cod_nota_credit['cod_movimiento_contable'];

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable_temporal_concepto SET und_vendida = '$und_vendida', total_movimiento_contable_temporal_concepto = '$total_movimiento_contable_temporal_concepto'
	WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));

	$obtener_info_ingre_operacional = "SELECT SUM(total_movimiento_contable_temporal_concepto) AS total_movimiento_contable_temporal 
	FROM tbl15_movimiento_contable_temporal_concepto WHERE cod_movimiento_contable_temporal = '$cod_movimiento_contable_temporal'";
	...
	$total_movimiento_contable_temporal    = $info_ingre_operacional['total_movimiento_contable_temporal'];

	$agregar_total_nota_credit = "UPDATE tbl15_movimiento_contable SET total_movimiento_contable_temporal = '$total_movimiento_contable_temporal' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
...
if ($campo == 'costo_movimiento_contable[]') {
	...
	$obtener_info_cod_nota_credit = "SELECT und_vendida, cod_movimiento_contable 
	FROM tbl15_movimiento_contable_temporal_concepto WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	$resultado_info_cod_nota_credit = mysqli_query($conectar, $obtener_info_cod_nota_credit) or die(mysqli_error($conectar));
	$info_cod_nota_credit = mysqli_fetch_assoc($resultado_info_cod_nota_credit);
	...
	$cod_movimiento_contable                        = $info_cod_nota_credit['cod_movimiento_contable'];
	$total_costo_movimiento_contables               = $und_vendida * $costo_movimiento_contable;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable_temporal_concepto SET costo_movimiento_contable = '$costo_movimiento_contable', 
	total_costo_movimiento_contable = '$total_costo_movimiento_contables' WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));

	$obtener_info_ingre_operacional = "SELECT SUM(total_costo_movimiento_contable) AS total_costo_movimiento_contable 
	FROM tbl15_movimiento_contable_temporal_concepto 
...
if ($campo == 'comentario[]') {
	...
	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable_temporal_concepto SET comentario = '$comentario' WHERE cod_movimiento_contable_temporal_concepto = '$cod_movimiento_contable_temporal_concepto'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'elaborada') {
	$elaborada_movimiento_contable_temporal      = addslashes($valor_intro);
	$cod_movimiento_contable                     = $id;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET elaborada_movimiento_contable_temporal = '$elaborada_movimiento_contable_temporal' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'revisada') {
	$revisada_movimiento_contable_temporal      = addslashes($valor_intro);
	$cod_movimiento_contable_temporal           = $id;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET revisada_movimiento_contable_temporal = '$revisada_movimiento_contable_temporal' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'autorizada') {
	$autorizada_movimiento_contable_temporal      = addslashes($valor_intro);
	$cod_movimiento_contable                      = $id;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET autorizada_movimiento_contable_temporal = '$autorizada_movimiento_contable_temporal' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
...
if ($campo == 'contabilizada') {
	$contabilizada_movimiento_contable_temporal      = addslashes($valor_intro);
	$cod_movimiento_contable_temporal                = $id;

	$agregar_reg_pyg = "UPDATE tbl15_movimiento_contable SET contabilizada_movimiento_contable_temporal = '$contabilizada_movimiento_contable_temporal' 
	WHERE cod_movimiento_contable = '$cod_movimiento_contable'";
	$resultado_pyg = mysqli_query($conectar, $agregar_reg_pyg) or die(mysqli_error($conectar));
