...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_GASTOS_POR_DEPENDENCIA_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_GASTOS_POR_DEPENDENCIA";
...
// Then a foreach
	...
	while ($info_egreso_inmobil = mysqli_fetch_assoc($resultado_egreso_inmobil)) {
		...
		$writer->addRow(array($nombre_dependencia, $egresos_dependencia));
	}
	$writer->addRow(array('TOTAL', $total_egresos_dependencia,));
