...
// ------------------------------------------------------------------------------------------------- //
$sql_infos_empresas = "SELECT cod_estado_bascula_balanza_electronica_pesar_producto_global, cod_estado_bascula_balanza_cod_barras_pesar_producto_global, 
cod_estado_limite_venta_pos_factura_electronica_global, limite_venta_pos_factura_electronica, 
cod_estado_pvar_calculo_automatico_pcompra_global, ptj_pvar_calculo_automatico_pcompra, 
cod_estado_mostrar_venta_por_caja_global, cod_estado_modificar_und_venta_una_sola_vez_global 
...
if (($campo=='und_venta') && ($tipo_ajax=='tbl15_venta_producto_temporal')) {
	...
	$datos_info_temporal = "SELECT und_venta, precio_costo_producto, cod_producto_barra, cod_info_factura_venta, nombre_tipo_precio_venta, precio_venta_producto FROM tbl15_venta_producto_temporal WHERE (cod_venta_producto_temporal = '$cod_venta_producto_temporal')";
	...
	$total_costo_producto         = $info_temporal['precio_costo_producto'] * $info_temporal['precio_venta_producto'];
	...
	if (($cod_estado_pvar_calculo_automatico_pcompra_global == '1') && ($nombre_tipo_precio_venta == 'PVAR')) {
		...
		$data_sql = ("UPDATE tbl15_venta_producto_temporal SET precio_compra_producto = '$precio_compra_producto', total_compra_producto = '$total_compra_producto', 
		precio_costo_producto = '$precio_costo_producto', total_costo_producto = '$total_costo_producto' 
		WHERE cod_venta_producto_temporal = '$cod_venta_producto_temporal'");
		$exec_data = mysqli_query($conectar, $data_sql) or die(mysqli_error($conectar));
	}
	$data_sql = ("UPDATE tbl15_venta_producto_temporal SET und_venta = '$und_venta', total_venta_producto = '$total_venta_producto',
	...
	$sql_verificar_precio_venta_en_cero_venta_temp = "SELECT * FROM tbl15_venta_producto_temporal WHERE (cod_info_factura_venta = '$cod_info_factura_venta') AND (und_venta <= '0')";
	$consulta_verificar_precio_venta_en_cero_venta_temp = mysqli_query($conectar, $sql_verificar_precio_venta_en_cero_venta_temp);
	$existe_unidad_venta_en_cero_venta_temp = mysqli_num_rows($consulta_verificar_precio_venta_en_cero_venta_temp);
	...
	$respuesta_ajax['existe_unidad_venta_en_cero_venta_temp']      = $existe_unidad_venta_en_cero_venta_temp;
...
if (($campo=='und_caja_sobre') && ($tipo_ajax=='tbl15_venta_producto_temporal')) {
	$und_caja_sobre               = addslashes($_POST['valor']);
	$cod_venta_producto_temporal  = intval($_POST['id']);
	...
	$datos_info_temporal = "SELECT precio_compra_producto, precio_venta_producto, cajas_sobre, und_sobre, nombre_tipo_und_caja_sobre, und_venta, 
	cod_info_factura_venta, precio_costo_producto, cod_producto_barra, nombre_tipo_precio_venta   
