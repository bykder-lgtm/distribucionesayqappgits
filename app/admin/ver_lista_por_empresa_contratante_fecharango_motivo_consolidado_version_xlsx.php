...
$nombre_archivo                    = 'LISTA_CONSOLIDADO_'.$cod_factura.'_'.$nombre_empresa_contratante.'_INI_'.$fecha_ini.'_FIN_'.$fecha_fin;
//-----------------------------------------------------------------------------------------------------//
//-----------------------------------------------------------------------------------------------------//
...
cod_estado_facturacion, nombre_empresa_contratante, motivo FROM tbl15_historia_clinica 
...
$increment                        = 2;
$increment_estilo                 = 2;
$inicio_celda_formula             = 3;
// Set document properties
$objPHPExcel->getProperties()
->setCreator($cabecera_emp)
->setLastModifiedBy($cabecera_emp)
->setTitle("CONSOLIDADO - ".$cabecera_emp)
->setSubject("CONSOLIDADO - ".$cabecera_emp)
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D'.$increment, '= (A'.$increment.' * C'.$increment.')');

$objPHPExcel->getActiveSheet()->getStyle('D'.$increment)->getNumberFormat()->setFormatCode('0');
$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn('A'.$increment)->setAutoSize(true); 
$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn('B'.$increment)->setAutoSize(true); 
$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn('C'.$increment)->setAutoSize(true); 
$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn('D'.$increment)->setAutoSize(true); 
}

$nombre_celda_suma                = $increment+1;
$celda_inicial                    = $inicio_celda_formula;
