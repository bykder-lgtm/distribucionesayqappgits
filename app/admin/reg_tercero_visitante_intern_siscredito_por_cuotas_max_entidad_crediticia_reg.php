...
if (isset($_POST['cod_producto_codifcryp'])) {
    ...
    $cuota_credito                                                  = $total_pagar / $codigo_meses_credito;
	...
	fecha_anyo, anyo, fecha_hora, cod_tipo_pago, cod_tipo_forma_pago, cod_administrador, 
	nombre_tipo_factura, nombre_tipo_moneda, cod_tercero, cod_base_caja, cod_resolucion_facturacion, 
	monto_deuda_sin_interes, subtotal, subtotal_sin_interes, numero_cuota, monto_cuota, interes_ptj, monto_deuda_mas_interes, monto_cuota_interes, nombre_tipo_cobro, cod_intermediario_credito) 
	...
	'$fecha_anyo', '$anyo', '$fecha_hora', '$cod_tipo_pago', '$cod_tipo_forma_pago', '$cod_administrador', 
	'$nombre_tipo_factura', '$nombre_tipo_moneda', '$cod_tercero', '$cod_base_caja', '$fecha_creacion', '$cod_resolucion_facturacion', 
	'$monto_deuda_sin_interes', '$subtotal', '$subtotal_sin_interes', '$numero_cuota', '$monto_cuota', '$interes_ptj', '$monto_deuda_mas_interes', '$monto_cuota_interes', '$nombre_tipo_cobro', '1')";
	$exec_data = mysqli_query($conectar, $sql_data) or die(mysqli_error($conectar));
	...
	precio_compra_producto, total_compra_producto, precio_costo_producto, total_costo_producto, precio_venta_producto, total_venta_producto, 
	...
	cod_administrador, cod_caja_virtual, nombre_tipo_precio_venta, precio_venta_producto_orig, und_producto, cod_base_caja, iva_ptj) 
    ...
    if ($cod_seguridad == '25') { // CLIENTE
    	...
    	$pagina_redirect = "../admin/lista_info_factura_venta_siscredito_visitante_intern.php";
    } else {
    	$pagina_redirect = "../admin/lista_cliente_siscredito_visitante_intern.php";
    }
	//---------------------------------------------------------------------------------------------------------------------------------//
	$url_redir = $pagina_redirect."?cod_info_factura_venta=".$cod_info_factura_venta."&cod_entidad_crediticia=".$cod_entidad_crediticia."&cod_tercero_codifcryp=".$cod_tercero_codifcryp;
