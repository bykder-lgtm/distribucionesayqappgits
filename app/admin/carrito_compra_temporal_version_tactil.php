...
$tab1                                     = "tbl15_venta_producto_temporal";
...
$origen1                                  = "tbl15_venta_producto_temporal";
...
FROM tbl15_info_factura_venta WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
FROM tbl15_venta_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') ORDER BY cod_venta_producto_temporal DESC";
...
?>
                                <tr id="tr-<?php echo $cod_venta_producto_temporal;?>">
                                    <!--<td id="num-<?php //echo $cod_venta_producto_temporal;?>"><?php //echo $incre;?></td>-->
                                    <td id="cod_venta_producto_temporal-<?php echo $cod_venta_producto_temporal;?>" data="<?php echo $cod_venta_producto_temporal ?>" class="remove-pr"><a class="eliminar" id="cod_venta_producto_temporal<?php echo $cod_venta_producto_temporal ?>"><i id="eliminar_load<?php echo $cod_venta_producto_temporal ?>" class="fas fa-times"></i></a></td>
                                    <td id="url_img_min_producto-<?php echo $cod_venta_producto_temporal;?>" class="thumbnail-img"><a href="#"><img class="img-fluid" src="<?php echo $url_img_min_producto ?>" alt="" /></a></td>
                                    <td id="nombre_producto-<?php echo $cod_venta_producto_temporal;?>" class="name-pr"><a href="#"><?php echo $nombre_producto ?></a></td>
                                    <td id="precio_venta_producto-<?php echo $cod_venta_producto_temporal;?>" class="price-pr"><p>$ <?php echo number_format($precio_venta_producto, 0, ",", ".") ?></p></td>
                                    <td id="und_venta-<?php echo $cod_venta_producto_temporal;?>" class="quantity-box"><input type="number" name="<?php echo $campo_codifcryp;?>" id="und_venta<?php echo $incre;?>" min="1" max="5" data_codifcryp="<?php echo $cod_venta_producto_temporal;?>" size="4" min="0" step="1" class="c-input-text qty text" value="<?php echo $und_venta;?>">
                                    <td id="total_venta-<?php echo $cod_venta_producto_temporal;?>" class="total-pr"><p id="total_venta_producto<?php echo $incre;?>">$ <?php echo number_format($total_venta_producto, 0, ",", ".") ?></p></td>
                                    <input type="hidden" id="precio_venta_producto<?php echo $incre;?>" class="precio_venta_producto-<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $precio_venta_producto;?>">
                                    <input type="hidden" id="iva_ptj<?php echo $incre;?>" class="iva_ptj-<?php echo $cod_venta_producto_temporal;?>" value="<?php echo $iva_ptj;?>">
                                </tr id="tr-<?php echo $cod_venta_producto_temporal;?>">
<?php
} 
...
-->
                ...
                <?php if ($total_reg <> '0') { ?>
                <div class="col-12 d-flex shopping-box"><a href="../admin/checkout_visitante_version_tactil.php" class="ml-auto btn hvr-hover">Revisar Compra</a> </div>
                <?php } ?>
            </div>
...
$(document).ready(function() {
    $('.eliminar').click(function(){
        ...
        var datos_url_ajax = 'cod_venta_producto_temporal='+cod_venta_producto_temporal+'&'+'tipo_accion='+tipo_accion+'&'+'tab='+tab+'&'+'campo='+campo+'&'+'id='+id+'&'+'cuenta='+cuenta+'&'+'cod_caja_virtual='+cod_caja_virtual+'&'+'cod_base_caja='+cod_base_caja+'&'+'nombre_tipo_moneda='+nombre_tipo_moneda+'&'+'nombre_tipo_factura='+nombre_tipo_factura+'&'+'foco='+foco+'&'+'cod_estado_vacuna='+cod_estado_vacuna+'&'+'buscar_por='+buscar_por+'&'+'pagina='+pagina;

        $.ajax({
            type: "POST",
            url: "../admin/eliminar_venta_producto_temporal_tactil_ajax.php",
            data: datos_url_ajax,
            beforeSend: function(objeto){
                $('#eliminar_load'+cod_venta_producto_temporal).empty();
                $('#eliminar_load'+cod_venta_producto_temporal).html('<img src="../imagenes/loader.gif">');
            },
            success:function(respuesta){
                $('#eliminar_ok').empty();
                //$('#eliminar_ok').append('<div align="center" class="correcto">Se ha eliminado correctamente el codigo = '+cod_venta_producto_temporal+'.</div>').fadeIn("slow");
                $('#'+parent).fadeOut("slow");
                ...
                $("#salida_info_actualizada_carrito_compra_menu_total_reg_ajax").html(venta_producto_temporal_total_reg).fadeIn('slow');
                $("#salida_info_actualizada_carrito_compra_menu_ajax").html(venta_producto_temporal).fadeIn('slow');
                $("#salida_info_actualizada_carrito_compra_menu_total_venta_reg_ajax").html(total_carrito);
                ...
                $('#num-'+cod_venta_producto_temporal).fadeOut("slow");
                $('#url_img_min_producto-'+cod_venta_producto_temporal).fadeOut("slow");
                $('#nombre_producto-'+cod_venta_producto_temporal).fadeOut("slow");
                $('#precio_venta_producto-'+cod_venta_producto_temporal).fadeOut("slow");
                $('#und_venta-'+cod_venta_producto_temporal).fadeOut("slow");
                $('#total_venta-'+cod_venta_producto_temporal).fadeOut("slow");
                $('.precio_venta_producto-'+cod_venta_producto_temporal).fadeOut("slow");
                $('.iva_ptj-'+cod_venta_producto_temporal).fadeOut("slow");
...
<script type="text/javascript">
    $("input").change(function(){  
        var cod_venta_producto_temporal = $(this).attr('data_codifcryp');
        var valor = $(this).val();
        ...
        var foco = '';
        var pagina = '';
        var datos_url_ajax = 'id='+cod_venta_producto_temporal+'&'+'valor='+valor+'&'+'campo_codifcryp='+campo_codifcryp+'&'+'accion_codifcryp='+accion_codifcryp+'origen_codifcryp='+origen_codifcryp+'&'+'vendedor_codifcryp='+vendedor_codifcryp;

        $.ajax({
            type: "POST",
            url: "../admin/actualizar_und_precio_vend_carrito_compra_version_tactil_ajax.php",
            data: datos_url_ajax,
            //dataType: 'json',
            beforeSend: function(objeto){
