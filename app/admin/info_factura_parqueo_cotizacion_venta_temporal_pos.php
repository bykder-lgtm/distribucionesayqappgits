...
<?php
$suma_temporal = "SELECT Sum(total_venta_producto) As total_venta, Sum(total_costo_producto) As total_compra FROM tbl15_cotizacion_venta_producto_temporal 
...
$datos_data_info_factura = "SELECT * FROM tbl15_info_parqueo_cotizacion_factura_venta WHERE (nombre_estado_factura = 'ABIERTA') AND (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
$cod_tipo_cotizacion         = $data_info_factura['cod_tipo_cotizacion'];
...
<script language="javascript">
$(document).ready(function(){
    $("#cod_tipo_cotizacion").on('change', function () {
            var valor = $(this).val();
            var campo = "cod_tipo_cotizacion";
            var tipo_ajax = "tbl15_info_parqueo_cotizacion_factura_venta";
            $.post("guardar_info_factura_y_parqueo_cotizacion_venta_producto_temporal_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_parqueo_cotizacion_factura_venta; ?> }, function(data){
                $("#modelo").html(data);
        });
...
$(document).ready(function(){
    $("input").on('change', function () {
            ...
            var tipo_ajax = "tbl15_parqueo_cotizacion_venta_producto_temporal";
...
<form method="post" name="formulario" action="../admin/parqueo_cotizacion_venta_producto_reg.php">
...
-->
    ...
    <th style="text-align:center;">PARQUEAR</th>
    <td style="text-align:center;"></td>
  </tr>
  <tr>
    <td style="text-align:center;"></td>
...
<!--
    <td style="text-align:center;">
        <select name="cod_tipo_cotizacion" id="cod_tipo_cotizacion" class="selectpicker" data-show-subtext="true" data-live-search="true" required>
            <?php if (isset($cod_tipo_forma_pago)) { echo ""; } else { echo  ""; }
            $consulta2_sql = "SELECT cod_tipo_cotizacion, nombre_tipo_cotizacion FROM tbl15_tipo_cotizacion ORDER BY cod_tipo_cotizacion ASC";
            $consulta2 = mysqli_query($conectar, $consulta2_sql);
            while ($datos2 = mysqli_fetch_assoc($consulta2)) {
            if(isset($cod_tipo_cotizacion) AND $cod_tipo_cotizacion == $datos2['cod_tipo_cotizacion']) {
            $seleccionado = "selected"; } else { $seleccionado = ""; }
            $codigo = $datos2['cod_tipo_cotizacion'];
            $nombre = $datos2['nombre_tipo_cotizacion'];
...
-->
    ...
    <td style="text-align:left;">
        ...
        <br><a href="#" id="modal_abrir"><img src="../imagenes/boton_mas_blanco.png"></a>
    </td>
    ...
    <input type="hidden" name="vlr_cancelado" id="vlr_cancelado" value="<?php echo $total_venta ?>">

    <td style="text-align:center;"><input type="image" src="../imagenes/parquear_reg_grande.png" tabindex=3 name="vender" class="img-polaroid" value="Guardar" /></td>
    <td style="text-align:center;"></td>

<?php 
$sql_temporal = "SELECT cod_parqueo_cotizacion_venta_producto_temporal FROM tbl15_parqueo_cotizacion_venta_producto_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
<input type="hidden" name="cod_parqueo_cotizacion_venta_producto_temporal[]" value="<?php echo $datos_temporal['cod_parqueo_cotizacion_venta_producto_temporal']; ?>" size="4">
<?php } ?>  
<?php $pagina ='facturacion_parqueo_cotizacion_venta_temporal_producto_manual_pos.php'; ?>
<input type="hidden" name="cod_info_parqueo_cotizacion_factura_venta" value="<?php echo $cod_info_parqueo_cotizacion_factura_venta ?>" size="10">
