...
.form-card-title i { color: #f59e0b; }
...
.info-tip i { color: #f59e0b; }

/* Responsive */
@media (max-width: 480px) {
    .gestor-table { min-width: 400px; }
    .ptj-input { width: 65px; }
}
</style>
</head>
<body>
...
<main class="page-container">
    ...
    <div class="form-card animate-in delay-1">
        <div class="form-card-title">
            ...
            <span style="background: rgba(245,158,11,0.2); color: #fcd34d; font-size: 0.7rem; font-weight: 700; padding: 0.2rem 0.6rem; border-radius: 10px; margin-left: 0.5rem;"><?php echo $total_gestores; ?></span>
        </div>
            ...
            <table class="gestor-table">
                ...
                <tbody>
                    ...
                    <tr id="fila_<?php echo (int)$gestor['cod_gestor_operador_credito']; ?>">
                        <td style="padding-left:1rem; font-weight:700; color: rgba(255,255,255,0.5);"><?php echo $fila++; ?></td>
                        <td class="gestor-name"><?php echo htmlspecialchars($gestor['nombre_gestor_operador_credito']); ?></td>
                        <td class="gestor-desc"><?php echo htmlspecialchars($gestor['descripcion_gestor_operador_credito']); ?></td>
                        <td>
...
function guardarPorcentaje(codGestor) {
    ...
    if (isNaN(nuevoPtj) || nuevoPtj < 0 || nuevoPtj > 99) {
        Swal.fire({ icon: 'warning', title: 'Valor inválido', text: 'El porcentaje debe ser un número entero entre 0 y 99.', background: '#1a1f2e', color: 'white', confirmButtonColor: '#f59e0b' });
        return;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                Swal.fire({ icon: 'success', title: '¡Guardado!', text: 'El porcentaje se actualizó correctamente.', background: '#1a1f2e', color: 'white', confirmButtonColor: '#f59e0b', timer: 2500, timerProgressBar: true, showConfirmButton: false });
            } else {
                Swal.fire({ icon: 'error', title: 'Error', text: response.message || 'No se pudo actualizar el porcentaje. Intente nuevamente.', background: '#1a1f2e', color: 'white', confirmButtonColor: '#f59e0b' });
            }
        },
        error: function() {
            btnEl.disabled = false;
            btnEl.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Guardar';
            Swal.fire({ icon: 'error', title: 'Error de conexión', text: 'No se pudo conectar con el servidor. Verifique su conexión.', background: '#1a1f2e', color: 'white', confirmButtonColor: '#f59e0b' });
        }
    });
}
</script>
