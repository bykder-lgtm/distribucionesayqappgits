...
if ($cod_tipo_aplicacion == '0') {
	$pagina_redirect_imprimir = '../admin/venta_productos_opcion_imprimir_hotel.php'; 
...
if (($condicion_zapateria) && ($vlr_cancelado <= 99999999) && ($cod_tipo_pago == '1')) {

	//for ($i=0; $i < $total_datos; $i++) {
	//$cod_venta_producto_temporal       = $_POST['cod_venta_producto_temporal'][$i];

	$sql_mconsulta = "SELECT * FROM tbl15_venta_producto_temporal WHERE (cod_info_factura_venta = '$cod_info_factura_venta')";
...
//----------------------------------------------------------------------- ---------------------------------------------------------//
		if ($cod_estado_subproducto_global == '1') {
			...
			nombre_tipo_compra, cod_tipo_metodo_envio, peso_producto, unidad_medida_peso, cod_base_caja)
			VALUES ('$cod_info_factura_venta', '$cod_factura', '$cod_producto', '$cod_producto_barra', 
...
url_img_orig_producto = '$url_img_orig_producto', url_img_min_producto = '$url_img_min_producto'
...
if (($fecha_ymd_movimiento_caja == $fecha_anyo)) {
	...
	$total_compra_producto                  = $datos_movimiento_caja['total_compra_producto'] + $total_precio_compra;
	$total_venta_producto                   = $datos_movimiento_caja['total_venta_producto'] + $total_precio_venta;
	$total_saldo                            = $total_precio_venta + $datos_movimiento_caja['total_saldo'];
	...
	WHERE cod_movimiento_caja = '1'");
	$resultado_regis = mysqli_query($conectar, $agregar_regis) or die(mysqli_error($conectar));
} else {

	$total_compra_producto                  = $total_precio_compra;
	$total_venta_producto                   = $total_precio_venta;
	$total_saldo                            = $total_precio_venta + $datos_movimiento_caja['total_saldo'];
...
//----------------------------------------------------------------------- ---------------------------------------------------------//
	$url_redir = $pagina_redirect_imprimir."?cod_info_factura_venta=".$cod_info_factura_venta."&cod_tipo_pago=".$cod_tipo_pago."&pagina=".$pagina;
