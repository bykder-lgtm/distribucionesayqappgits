...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_PAGO_DETALLADO_A_PROPIETARIOS_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_PAGO_DETALLADO_A_PROPIETARIOS";
...
//-----------------------------------------------------------------------------------------------------//
// Headers
	$writer->addRow(array('CONTRATO', 'PROPIETARIO | INMUEBLE', 'VALOR PAGADO', 'COMISION ADMINISTRACION', 'FECHA REG PAGO', 'ID'));
// Then a foreach
    ...
    while ($datos_cuentas_cobrar_factura_comision_propietario = mysqli_fetch_assoc($consulta_cuentas_cobrar_factura_comision_propietario)) {
		...
		$writer->addRow(array($cod_factura, $nombre_tercero.' | '.$nombre_producto, $total_ingreso, $deduccion_comision, date("d-m-Y", strtotime($fecha_pago_reg)), $cod_cuentas_cobrar_factura_comision_propietario));
	}
	$writer->addRow(array('','TOTAL' , $total_valor_pagado_a_propietario, $total_ingreso_por_comision_administarcion, '', ''));
