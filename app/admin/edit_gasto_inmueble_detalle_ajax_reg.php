...
if ($campo == 'descripcion_gasto_inmueble_detalle') {
	...
	$sql_productos = "SELECT SUM(precio_compra_producto) AS total_precio_compra, SUM(precio_venta_producto) AS total_precio_venta FROM tbl15_gasto_inmueble_detalle_venta WHERE cod_factura = '$cod_factura'";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
...
elseif ($campo == 'precio_venta_producto') {
	...
	$sql_productos = "SELECT SUM(precio_compra_producto) AS total_precio_compra, SUM(precio_venta_producto) AS total_precio_venta FROM tbl15_gasto_inmueble_detalle_venta WHERE (cod_factura = '$cod_factura') AND (cod_tipo_estado_incluido = '0')";
	$productos_consulta = mysqli_query($conectar, $sql_productos) or die(mysqli_error($conectar));
	$datos_producto = mysqli_fetch_assoc($productos_consulta);
