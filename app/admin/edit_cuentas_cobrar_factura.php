...
if (last != valor)
myajax.Link('guardar_cuentas_cobrar_factura_editable_ajax.php?valor='+valor+'&campo='+campo+'&id='+id);
}
</script>
...
<table class="table table-striped">
  ...
  <?php
  $sql = "SELECT * FROM tbl15_cuentas_cobrar WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar') AND (cod_estado_archivado = '0')";
  ...
  <tr>
    ...
    <?php if ($cod_seguridad== '1') { ?><td style="text-align: center;"><input style="text-align: center;" type="date" onFocus="Focus(this.id, this.value)" onBlur="Blur(this.id, this.value, 'fecha_pago', <?php echo $cod_cuentas_cobrar;?>)" id="<?php echo $cod_cuentas_cobrar;?>" value="<?php echo $fecha_pago;?>"></td><?php } ?>
...
?>
<tr>
<?php if ($cod_seguridad== '1') { ?><td style="text-align: center;"><input class="input-block-level" name="abonado" id="<?php echo $cod_cuentas_cobrar_abonos;?>" type="text" value="<?php echo $abonado ?>" style="width: 150px;"/></td><?php } else { ?><td style="text-align: center;"><font size="4px"><?php echo number_format($abonado, 0, ",", "."); ?></font></td><?php } ?>
...
<script>  
 ...
 <script>  
 $(document).ready(function(){  
  $('input[name="abonado"]').focusout(function(){  
    var valor = $(this).val();
    var campo = $(this).attr("name");
    var id = $(this).attr("id");
    var tab = "tbl15_cuentas_cobrar_abonos";
    $.ajax({  
        url:"guardar_cuentas_cobrar_abonos_editable.php",  
        method:"GET",  
        data:{valor:valor, campo:campo, id:id, tab:tab},  
