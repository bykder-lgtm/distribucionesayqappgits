<?php
// Registro de nuevo producto desde el catálogo
ob_start();
...
try {
    ...
    if ($cod_categoria <= 0) { throw new Exception('Debe seleccionar una categoría'); }
    // Verificar si el código de producto ya existe
    $sql_verificar = "SELECT cod_producto FROM tbl15_producto WHERE cod_producto_barra = '" . mysqli_real_escape_string($conectar, $cod_producto_barra) . "'";
    $resultado_verificar = mysqli_query($conectar, $sql_verificar);
    if (mysqli_num_rows($resultado_verificar) > 0) { throw new Exception('Ya existe un producto con ese código'); }
    ...
    if (empty($url_img_min_producto)) { $url_img_min_producto = '../archivador/img_producto/orig/sin_imagen.jpg'; $url_img_orig_producto = '../archivador/img_producto/orig/sin_imagen.jpg'; }
    // Fecha actual
    $fecha_registro = date('Y-m-d H:i:s');
    // Insertar producto
    $sql_insertar = "INSERT INTO tbl15_producto (cod_producto_barra, nombre_producto, cod_tienda, precio_compra_producto, precio_venta_producto, 
    cod_categoria, nombre_categoria, iva_ptj, descripcion_producto, cod_estado, nombre_estado, url_img_orig_producto, url_img_min_producto, und_producto) 
    VALUES ('".mysqli_real_escape_string($conectar, $cod_producto_barra)."', '".mysqli_real_escape_string($conectar, $nombre_producto)."', '$cod_tienda', '$precio_compra_producto', '$precio_venta_producto',
    '$cod_categoria', '".mysqli_real_escape_string($conectar, $nombre_categoria)."', '$iva_ptj', '".mysqli_real_escape_string($conectar, $descripcion_producto)."',
    '$cod_estado', '".mysqli_real_escape_string($conectar, $nombre_estado)."', '".mysqli_real_escape_string($conectar, $url_img_orig_producto)."', '".mysqli_real_escape_string($conectar, $url_img_min_producto)."', '$und_producto')";
    $resultado = mysqli_query($conectar, $sql_insertar);
    if (!$resultado) { throw new Exception('Error al registrar el producto: ' . mysqli_error($conectar)); }
    
    $cod_producto_nuevo = mysqli_insert_id($conectar);
    // Respuesta exitosa
    $respuesta = ['success' => true, 'mensaje' => 'Producto registrado correctamente', 'cod_producto' => $cod_producto_nuevo, 'timestamp' => date('Y-m-d H:i:s')];
    echo json_encode($respuesta);
} catch (Exception $e) {
    error_log("Error en registrar_producto_catalogo_ajax.php: " . $e->getMessage());
    http_response_code(400);
    $respuesta = ['success' => false, 'mensaje' => $e->getMessage(), 'timestamp' => date('Y-m-d H:i:s')];
    echo json_encode($respuesta);
} catch (Error $e) {
    error_log("Error fatal en registrar_producto_catalogo_ajax.php: " . $e->getMessage());
