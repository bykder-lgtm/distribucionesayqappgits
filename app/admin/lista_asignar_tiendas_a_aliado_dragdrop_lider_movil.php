...
// Consulta de Tiendas activas que pertenecen a los aliados del líder o que están huérfanas
$sql_tiendas = "SELECT cod_tienda, nombre_tienda, cod_aliado_estrategico FROM tbl15_tienda WHERE cod_estado != '0' AND (cod_aliado_estrategico IN ($subquery_aliados) OR cod_aliado_estrategico = '0' OR cod_aliado_estrategico = '' OR cod_aliado_estrategico IS NULL) ORDER BY nombre_tienda ASC";
$res_tiendas = mysqli_query($conectar, $sql_tiendas);

$tiendas_por_aliado = [];
$tiendas_no_asignadas = [];

while($tienda = mysqli_fetch_assoc($res_tiendas)) {
    // Generamos iniciales basadas en el nombre de la tienda
    $iniciales = strtoupper(substr(trim($tienda['nombre_tienda']), 0, 2));
    if (empty($iniciales)) { $iniciales = 'TD'; }
    $tienda['iniciales'] = $iniciales;

    $aliado_id = $tienda['cod_aliado_estrategico'];
    if(empty($aliado_id) || $aliado_id == 0 || $aliado_id == '') {
        $tiendas_no_asignadas[] = $tienda;
    } else {
        if(!isset($tiendas_por_aliado[$aliado_id])) { $tiendas_por_aliado[$aliado_id] = []; }
        $tiendas_por_aliado[$aliado_id][] = $tienda;
    }
}
...
<main class="page-container">
    <div class="page-header animate-in">
        <h1><i class="fa-solid fa-arrows-up-down-left-right"></i> Asignación Interactiva de Tiendas a Aliados</h1>
        <p>Arrastra y suelta a las tiendas para asignarlas a un aliado determinado.</p>
    </div>

    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;" class="animate-in">
        <h3 style="color: #ef4444; font-size: 1.3rem; margin: 0;"><i class="fa-solid fa-store-slash"></i> Tiendas Sin Aliado Asignado</h3>
        <span class="badge-count" id="count-0" style="background: rgba(239, 68, 68, 0.2); color: #ef4444; font-weight: bold; font-size: 1rem; padding: 0.3rem 1rem;"><?php echo count($tiendas_no_asignadas); ?></span>
    </div>
    
    <div class="franja-sin-asignar animate-in">
        <div class="zona-drop zona-drop-horizontal" data-id="0" id="lista-0">
            <?php foreach($tiendas_no_asignadas as $td): ?>
            <div class="item-dragg" data-user="<?php echo $td['cod_tienda']; ?>">
                <div class="item-avatar" style="background: #ef4444;"><?php echo $td['iniciales']; ?></div>
                <div class="item-info">
                    <span class="item-name">
                        <?php echo utf8_encode($td['nombre_tienda']); ?>
                    </span>
                    <span class="item-role">ID: <?php echo $td['cod_tienda']; ?></span>
                </div>
                <i class="fa-solid fa-grip-vertical" style="color: rgba(255,255,255,0.3);"></i>
            </div>
            <?php endforeach; ?>
            
            <div class="empty-msg" style="width: 100%; text-align: center; color: rgba(255,255,255,0.4); font-style: italic; padding: 1rem; <?php echo (count($tiendas_no_asignadas) > 0) ? 'display:none;' : ''; ?>">Todas las tiendas están asignadas. Arrastra aquí para quitar asignación y dejarlas sueltas.</div>
        </div>
    </div>
    ...
    <div style="margin-bottom: 1.5rem; display: flex; flex-direction: column; gap: 0.5rem;" class="animate-in delay-1">
        <div>
            ...
            <p style="color: rgba(255,255,255,0.5); font-size: 0.9rem; margin-bottom: 0.5rem;">Arrastra aquí a las tiendas para asignarlas a la propiedad de un aliado.</p>
        </div>
    ...
    <div class="franja-coordinadores animate-in delay-1">
        <?php while($aliado = mysqli_fetch_assoc($res_aliados_lista)): 
            ...
            $tiendas_esta_lista = isset($tiendas_por_aliado[$al_id]) ? $tiendas_por_aliado[$al_id] : [];
        ?>
        <div class="card-lista">
            <div class="card-header-lista header-asesor">
                ...
                <span class="badge-count" id="count-<?php echo $al_id; ?>" style="background: rgba(16, 185, 129, 0.2); color: #10b981; font-weight: bold;"><?php echo count($tiendas_esta_lista); ?></span>
            </div>
            <div style="padding: 0.5rem 1rem; border-bottom: 1px solid rgba(139, 92, 246, 0.2); background: rgba(0,0,0,0.1);">
                <input type="text" class="buscador-asesor" data-target="lista-<?php echo $al_id; ?>" placeholder="Buscar tienda asignada..." style="width: 100%; padding: 0.5rem; border-radius: 6px; border: 1px solid rgba(139, 92, 246, 0.3); background: rgba(255,255,255,0.05); color: white; outline: none; font-size: 0.85rem;">
            </div>
            <div class="zona-drop" data-id="<?php echo $al_id; ?>" id="lista-<?php echo $al_id; ?>">
                <?php foreach($tiendas_esta_lista as $td): ?>
                <div class="item-dragg" data-user="<?php echo $td['cod_tienda']; ?>">
                    <div class="item-avatar"><?php echo $td['iniciales']; ?></div>
                    <div class="item-info">
                        <span class="item-name">
                            <?php echo utf8_encode($td['nombre_tienda']); ?>
                        </span>
        ...
        zonasDrop.forEach(zona => {
            new Sortable(zona, {
                ...
                onEnd: function (evt) {
                    ...
                    if (fromList !== toList) {
                        let id_tienda = itemEl.getAttribute('data-user');
                        ...
                        $.ajax({
                            url: 'procesar_asignar_tiendas_a_aliado_dragdrop_lider_ajax.php', type: 'POST', data: { accion: 'asignar_tienda_aliado', id_tienda: id_tienda, id_aliado_nuevo: id_aliado_nuevo }, dataType: 'json',
