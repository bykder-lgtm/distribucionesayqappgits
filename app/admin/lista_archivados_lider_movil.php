...
<main class="page-container">
    ...
    <div class="search-bar">
        ...
        <input type="text" id="searchInput" placeholder="Buscar por nombre o cédula..." onkeyup="filtrar(this.value)">
    </div>
    ...
    // Filtrar por los que este líder puede ver
    // En el proyecto, el líder ve todo lo que le pertenece
    ...
    <div class="archivados-list">
        <?php if (mysqli_num_rows($resultado) > 0): ?>
            ...
            ?>
            <div class="archived-card role-<?php echo $row['cod_seguridad']; ?>">
                <div class="card-header">
                    <div class="card-info">
                        <div class="card-name"><?php echo $row['nombres_apellidos_tercero'] ?: ($row['nombres'].' '.$row['apellidos']); ?></div>
                        <span class="card-role"><?php echo $role_name; ?></span>
                    </div>
                    <span class="card-status">Archivado</span>
                </div>
                
                <div class="card-details">
                    <div class="detail-item">Cédula: <span><?php echo $row['cedula']; ?></span></div>
                    ...
                    <div class="detail-item">Fecha Archivo: <span><?php echo !empty($row['fecha_hora_update']) ? date('d/m/Y', strtotime($row['fecha_hora_update'])) : 'Reciente'; ?></span></div>
                </div>

                <div class="card-actions">
                    <button class="action-btn restore" onclick="recuperarEntidad(<?php echo $row['cod_administrador']; ?>, '<?php echo addslashes($row['nombres_apellidos_tercero']); ?>', '<?php echo $role_name; ?>')">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <h3>No hay registros archivados</h3>
    ...
    <div class="pagination-container">
        <?php for($i=1; $i<=$total_paginas; $i++): ?>
            <a href="?pagina=<?php echo $i; ?>&busqueda=<?php echo $busqueda; ?>" class="pagination-btn <?php echo ($i == $pagina) ? 'active' : ''; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>
    </div>
    <?php endif; ?>
</main>
...
function filtrar(val) {
    if (val.length > 2 || val.length == 0) {
        window.location.href = 'lista_archivados_lider_movil.php?busqueda=' + val;
    }
}
