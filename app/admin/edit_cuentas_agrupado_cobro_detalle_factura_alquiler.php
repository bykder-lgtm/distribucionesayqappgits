...
<table border="1" class="table table-responsive">
    <thead>
        ...
        <tr>
            ...
            <td style="text-align:center"><a href="../admin/cuentas_cobrar_agrupado_detalle_factura_alquiler.php?cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar ?>&cod_tercero=<?php echo $cod_tercero ?>&cod_factura=<?php echo $cod_factura ?>&pagina=<?php echo $pagina_local ?>">VER</a></td>
    ...
    <thead>
        <tr>
            ...
            <td style="text-align:left">
                <select name="cod_estado" id="cod_estado" class="<?php echo $cod_cuentas_cobrar_alerta;?>" data-show-subtext="true" data-live-search="true" required>
            ...
            <td style="text-align:left">
                <select name="cod_estado_envio_correo_cuenta_cobro" id="cod_estado_envio_correo_cuenta_cobro" class="<?php echo $cod_cuentas_cobrar_alerta;?>" data-show-subtext="true" data-live-search="true" required>
                    <?php if (isset($cod_estado_envio_correo_cuenta_cobro)) { echo ""; } else { echo  ""; }
                    $consulta2_sql = "SELECT cod_estado_envio_correo_cuenta_cobro, nombre_estado_envio_correo FROM tbl15_estado_envio_correo ORDER BY cod_estado_envio_correo_cuenta_cobro ASC";
                    $consulta2 = mysqli_query($conectar, $consulta2_sql);
                    while ($datos2 = mysqli_fetch_assoc($consulta2)) {
                    if(isset($cod_estado_envio_correo_cuenta_cobro) AND $cod_estado_envio_correo_cuenta_cobro == $datos2['cod_estado_envio_correo_cuenta_cobro']) {
                    $seleccionado = "selected"; } else { $seleccionado = ""; }
                    $codigo = $datos2['cod_estado_envio_correo_cuenta_cobro'];
...
<table border="1" class="table table-responsive">
    <thead>
      ...
      <tr>
        ...
        <td style="text-align:left; font-size:24px"><br><?php echo number_format($monto_deuda_alerta, 0, ",", ".") ?></td>
...
<table border="1" class="table table-responsive">
    <thead>
        ...
        <tr>
            ...
            <td style="text-align:center; font-size:28px"><?php echo $fecha_pago_limite ?></td>
            <input style="font-size:24px" class="input-block-level" type="hidden" name="subtotal_abonado" id="subtotal_abonado" value="<?php echo $monto_deuda_alerta ?>" min="0" size="5">
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------//
        ...
        var total_ingreso = monto_cuota_interes + ingreso_gasto_juridica + ingreso_administracion_incluida + subtotal_abonado + ingreso_deudas_anteriores + ingreso_otro_concepto;
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------//
    $('#fecha_pago').change(function(){
        ...
        if (dias_atraso > 0) {
            var monto_cuota_interes = (subtotal_abonado * ((ptj_interes_inmobiliaria/30)/100)) * dias_atraso;
...
<script>  
 $(document).ready(function(){  
  ...
  $('select[name="cod_estado_envio_correo_cuenta_cobro"]').change(function(){ 
  var cod_estado_envio_correo_cuenta_cobro = $(this).val();  
  var cod_cuentas_cobrar_alerta = $(this).attr('class');

  var tab = "tbl15_cuentas_cobrar_alerta";
  var tipo = "editar";
  var campo = "cod_estado_envio_correo_cuenta_cobro";

  let ids = this.id;
    $.ajax({ url:"edit_cuentas_cobro_estado_factura_alquiler_ajax_reg.php", method:"GET", data:{valor:cod_estado_envio_correo_cuenta_cobro, campo:"cod_estado_envio_correo_cuenta_cobro", tab:tab, tipo:tipo, campo:campo, id:cod_cuentas_cobrar_alerta }, success:function(data){ $('#result').html(data); }  
    });  
  });
 });  
 </script>
