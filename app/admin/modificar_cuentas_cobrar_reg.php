...
/* ----------------------------------------------------------------------------------------------------------/ */
	$cliente                                                  = "";
	$cod_factura_post                                         = $cod_factura;
	$comentario                                               = 'SOPORTES ABONO CUENTA COBRAR POR FACTURA | '.$mensaje;
	$nombre_nota_observacion                                  = 'SOPORTES ABONO CUENTA COBRAR POR FACTURA | '.$mensaje;
/* ----------------------------------------------------------------------------------------------------------/ */
	$time                                                     = time();
...
//-------------------------------------- REGISTRAR DATOS --------------------------------------//
	...
	anyo, fecha_invert, fecha_seg, hora, mensaje, cod_administrador, cod_tipo_forma_pago, cod_dependencia, cod_cuentas_cobrar, cod_puc) 
...
//-------------------------------------- REGISTRAR DATOS --------------------------------------//
	$sql_cuenta_cobrar_factura = "SELECT monto_deuda, abonado, cod_info_factura_venta, total_precio_compra, total_precio_venta, total_utilidad_bruta, total_utilidad_neta 
	FROM tbl15_cuentas_cobrar WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar')";
	...
	$cod_info_factura_venta                                = intval($dato_cuenta_cobrar_factura['cod_info_factura_venta']);
	...
	//-------------------------------------- -----------------------------------------------------------------//
	$sql_total_cuenta_cobrar_factura = "SELECT monto_deuda, abonado, subtotal FROM tbl15_cuentas_cobrar WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar')";
	$consulta_total_cuenta_cobrar_factura = mysqli_query($conectar, $sql_total_cuenta_cobrar_factura) or die(mysqli_error($conectar));
	$dato_total_cuenta_cobrar_factura = mysqli_fetch_assoc($consulta_total_cuenta_cobrar_factura);

	$subtotal_final                                        = $dato_total_cuenta_cobrar_factura['subtotal'];

	if ($subtotal_final <= 0) {
		$cod_estado_pago = '1';
		$actualizar_sql1 = sprintf("UPDATE tbl15_cuentas_cobrar SET cod_estado_pago = '$cod_estado_pago' WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar')");
		$resultado_actualizacion1 = mysqli_query($conectar, $actualizar_sql1) or die(mysqli_error($conectar));
	}
	//-------------------------------------- -----------------------------------------------------------------//
	$sql_info_factura_venta = "SELECT total_precio_compra, total_precio_venta FROM tbl15_info_factura_venta WHERE (cod_info_factura_venta = '$cod_info_factura_venta')";
	$consulta_info_factura_venta = mysqli_query($conectar, $sql_info_factura_venta) or die(mysqli_error($conectar));
	$dato_info_factura_venta = mysqli_fetch_assoc($consulta_info_factura_venta);

	$total_precio_compra                                = intval($dato_info_factura_venta['total_precio_compra']);
	$total_precio_venta                                 = intval($dato_info_factura_venta['total_precio_venta']);

	$sql_cuenta_cobrar_numero_abono_factura = "SELECT COUNT(cod_cuentas_cobrar_abonos) numero_de_abonos FROM tbl15_cuentas_cobrar_abonos WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar') AND (cod_estado_archivado = '0')";
	$consulta_cuenta_cobrar_numero_abono_factura = mysqli_query($conectar, $sql_cuenta_cobrar_numero_abono_factura) or die(mysqli_error($conectar));
	$dato_cuenta_cobrar_numero_abono_factura = mysqli_fetch_assoc($consulta_cuenta_cobrar_numero_abono_factura);

	$numero_de_abonos                                    = $dato_cuenta_cobrar_numero_abono_factura['numero_de_abonos'];

	$sql_cuenta_cobrar_total_factura = "SELECT subtotal, monto_deuda, cod_info_factura_venta, total_precio_compra, total_precio_venta, total_utilidad_bruta, total_utilidad_neta 
	FROM tbl15_cuentas_cobrar WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar')";
	$consulta_cuenta_cobrar_total_factura = mysqli_query($conectar, $sql_cuenta_cobrar_total_factura) or die(mysqli_error($conectar));
	$dato_cuenta_cobrar_total_factura = mysqli_fetch_assoc($consulta_cuenta_cobrar_total_factura);

	$subtotal                                              = $dato_cuenta_cobrar_total_factura['subtotal'];
	$total_utilidad_bruta                                  = $dato_cuenta_cobrar_total_factura['total_utilidad_bruta'];
	$abonado_total_cuenta                                  = $dato_cuenta_cobrar_factura['abonado'];
	$total_utilidad_neta_anterior                          = $dato_cuenta_cobrar_factura['total_utilidad_neta'];
	$total_abonos                                          = $abonado_total_cuenta + $abonado;

	if ($total_abonos > $total_precio_compra) {
		$total_utilidad_neta                               = $total_abonos - $total_precio_compra;
		$total_utilidad_neta_abono                         = ($total_abonos - $total_precio_compra) - $total_utilidad_neta_anterior;
	} else {
		$total_utilidad_neta                               = 0;
		$total_utilidad_neta_abono                         = 0;
	}
	$sql_cuenta_cobrar_utilidad_neta = sprintf("UPDATE tbl15_cuentas_cobrar SET total_utilidad_neta = '$total_utilidad_neta' WHERE (cod_cuentas_cobrar = '$cod_cuentas_cobrar') AND (cod_estado_archivado = '0')");
	$resultado_cuenta_cobrar_utilidad_neta = mysqli_query($conectar, $sql_cuenta_cobrar_utilidad_neta) or die(mysqli_error($conectar));

	$sql_cuenta_cobrar_abono_utilidad_neta = sprintf("UPDATE tbl15_cuentas_cobrar_abonos SET total_utilidad_neta = '$total_utilidad_neta_abono' WHERE (cod_cuentas_cobrar_abonos = '$cod_cuentas_cobrar_abonos')");
	$resultado_cuenta_cobrar_abono_utilidad_neta = mysqli_query($conectar, $sql_cuenta_cobrar_abono_utilidad_neta) or die(mysqli_error($conectar));
	//-------------------------------------- -----------------------------------------------------------------//
...
/* ----------------------------------------------------------------------------------------------------------/ */
	...
	url_img_orig_producto, url_img_min_producto, cod_cuentas_cobrar_abonos, cod_cuentas_cobrar) 
...
/* ----------------------------------------------------------------------------------------------------------/ */
	...
	WHERE (nombre_modulo_puc = 'ABONOS CUENTAS POR COBRAR')";
	...
	$cod_puc                                               = $info_parametrizacion_puc_cuentas_estaticas['cod_puc'];//238
	$codigo_puc                                            = $info_parametrizacion_puc_cuentas_estaticas['codigo_puc'];//138020
	$nombre_puc                                            = $info_parametrizacion_puc_cuentas_estaticas['nombre_puc'];//CUENTAS POR COBRAR DE TERCEROS
...
//-------------------------------------- -----------------------------------------------------------------//
?>
	<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/cuentas_cobrar_opcion_imprimir.php?cod_cuentas_cobrar_abonos=<?php echo $cod_cuentas_cobrar_abonos ?>&cod_cuentas_cobrar=<?php echo $cod_cuentas_cobrar ?>&cod_factura=<?php echo $cod_factura_post ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>&pagina=<?php echo $pagina ?>">
<?php } ?>