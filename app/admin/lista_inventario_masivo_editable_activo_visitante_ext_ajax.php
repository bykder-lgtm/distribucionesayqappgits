...
<a class="btn btn-primary" href="../admin/lista_inventario_masivo_editable_ajax.php"><h6>Inventario Masivo Estado</h6></a>
</div>
<div class="row-fluid">
...
$nombre_estados                       = addslashes($_POST['nombre_estados']);
}
?>
...
<td style="text-align:left;">
    <select name="nombre_estados" class="selectpicker" data-show-subtext="true" data-live-search="true">
        <?php if (isset($nombre_estados)) { echo "<option value='' $seleccionado >TODOS</option>"; } else { echo  "<option value='' $seleccionado >TODOS</option>"; }
        $consulta2_sql = "SELECT cod_estado, nombre_estado FROM tbl15_estado ORDER BY nombre_estado ASC";
        $consulta2 = mysqli_query($conectar, $consulta2_sql);
        while ($datos2 = mysqli_fetch_assoc($consulta2)) {
        if(isset($nombre_estados) AND $nombre_estados == $datos2['cod_dependencia']) {
...
$filtro_consulta_estado = "WHERE (nombre_estado = '$nombre_estados')";
$filtro_consulta_estado_and = "AND (nombre_estado = '$nombre_estados')";
$filtro_consulta_estado_rel = "WHERE (tbl15_producto.nombre_estado = '$nombre_estados')";
...
$sql_dependencia = "SELECT nombre_estado FROM tbl15_estado WHERE nombre_estado = '$nombre_estados'";
...
$nombre_dependencia_get                             = $datos_dependencia['nombre_estado'];
}
...
<th style="text-align:center">TIPO PROMO</th>
...
und_sobre, cod_opcion_descontable_inv, cod_categoria, cod_tipo_producto_cocina, nombre_promocion, nombre_promocion_ing
...
<select name="nombre_estado" id="nombre_estado-<?php echo $cod_producto;?>" class="<?php echo $cod_producto;?>" style="width: 120px;">
<?php if (isset($nombre_estado)) { echo "<option value='' >Selecione</option>";
} else { echo  "<option value='' selected >Selecione</option>"; }
$sql_consulta2 = "SELECT cod_estado, nombre_estado FROM tbl15_estado ORDER BY nombre_estado ASC";
...
<td style="text-align:center">
<select name="cod_promocion" id="cod_promocion-<?php echo $cod_producto;?>" class="<?php echo $cod_producto;?>" style="width: 120px;">
<?php if (isset($nombre_promocion)) { echo "<option value='' >Selecione</option>";
} else { echo  "<option value='' selected >Selecione</option>"; }
$sql_consulta2 = "SELECT cod_promocion, nombre_promocion FROM tbl15_promocion ORDER BY nombre_promocion ASC";
$consulta2 = mysqli_query($conectar, $sql_consulta2);
while ($datos2 = mysqli_fetch_assoc($consulta2)) {
