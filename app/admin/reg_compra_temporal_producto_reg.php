...
if (isset($_GET['cod_producto_barra'])) {
	...
	if (isset($_GET['cod_info_factura_compra'])) { $cod_info_factura_compra = intval($_GET['cod_info_factura_compra']); } else { $cod_info_factura_compra = ""; }
	if (isset($_GET['cuenta'])) { $cuenta = addslashes($_GET['cuenta']); } else { $cuenta = $cuenta_actual; }
	...
	$nombre_tipo_factura                                = 'FACTURA_COMPRA';
	$cod_movimiento_contable_cuenta_personal            = 1;
	$cod_movimiento_caja                                = 1;
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	$sql_info_factura_abierta = "SELECT cod_info_factura_compra, cod_tercero FROM tbl15_info_factura_compra WHERE (nombre_estado_factura = '$nombre_estado_factura') AND (cuenta = '$cuenta') AND (cod_caja_virtual = '$cod_caja_virtual')";
	$consulta_info_factura_abierta = mysqli_query($conectar, $sql_info_factura_abierta) or die(mysqli_error($conectar));
	$datos_info_factura_abierta = mysqli_fetch_assoc($consulta_info_factura_abierta);

	$cod_tercero                                        = $datos_info_factura_abierta['cod_tercero'];
	//---------------------------------------------------------------------------------------------------------------------------------//
	//---------------------------------------------------------------------------------------------------------------------------------//
	...
	$precio_compra_producto_por_caja                    = $datos_producto['precio_compra_producto_por_caja'];
	$precio_venta_producto_por_caja                     = $datos_producto['precio_venta_producto_por_caja'];
	$precio_venta_producto2_por_caja                    = $datos_producto['precio_venta_producto2_por_caja'];
	$precio_venta_producto3_por_caja                    = $datos_producto['precio_venta_producto3_por_caja'];
	...
	$precio_compra_producto_ant_desc                    = $datos_producto['precio_compra_producto'];
	$total_compra_producto_ant_desc                     = $precio_compra_producto_ant_desc * $und_compra;

	$precio_compra_producto                             = $precio_compra_producto - ($precio_compra_producto * ($dto1/100));
	$total_compra_producto                              = $precio_compra_producto * $und_compra;
	$precio_compra_producto_promedio                    = $precio_compra_producto;
	$precio_venta_producto_promedio                     = $precio_venta_producto;

	if ($total_costo_producto > $total_venta_producto) { $cod_estado_permitir_venta = '1'; } else { $cod_estado_permitir_venta = '0'; }

	//$datos_info_temporal = "SELECT AVG(precio_compra_producto) AS precio_compra_producto_promedio, AVG(precio_venta_producto) AS precio_venta_producto_promedio 
	//FROM tbl15_factura_compra_producto WHERE (cod_producto_barra = '$cod_producto_barra') ORDER BY fecha_ymd_venta_producto DESC LIMIT 0, 3";
...
//---------------------------------------------------------------------------------------------------------------------------------------------//
	$datos_info = "SELECT * FROM tbl15_info_factura_compra WHERE (nombre_estado_factura = '$nombre_estado_factura') AND (cuenta = '$cuenta') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
//---------------------------------------------------------------------FACTURA NUEVA INICIO----------------------------------------------------------------//
		if ($factura_abierta == '0') {
			...
			cod_tipo_producto_consumo, cod_resolucion_facturacion, cod_movimiento_contable_cuenta_personal, cod_movimiento_caja) 
			...
			$sql_data = "INSERT INTO tbl15_compra_producto_temporal (cod_info_factura_compra, cod_tercero, cod_producto, cod_producto_barra, nombre_producto, und_compra, 
			...
			und_unidades, und_caja, nombre_tipo_cargue_factura, comision_ptj, peso_producto, unidad_medida_peso, cajas_sobre, und_sobre, 
			cod_dependencia, dto1, dto2, precio_compra_producto_ant_desc, total_compra_producto_ant_desc, und_producto, precio_compra_producto_promedio, precio_venta_producto_promedio, 
...
//---------------------------------------------------------------------FACTURA ABIERTA INICIO----------------------------------------------------------------//
			...
			$sql_info_factura = "SELECT cod_info_factura_compra, cod_tercero FROM tbl15_info_factura_compra WHERE (nombre_estado_factura = '$nombre_estado_factura') AND (cuenta = '$cuenta') AND (cod_caja_virtual = '$cod_caja_virtual')";
			...
			$cod_info_factura_compra                        = $datos_info_factura['cod_info_factura_compra'];
...
//---------------------------------------------------------------------FACTURA ABIERTA FIN----------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------------------//
	$pagina_redirect                                    = $pagina."?&cuenta=".$cuenta."&cod_caja_virtual=".$cod_caja_virtual."&cod_info_factura_compra=".$cod_info_factura_compra."&foco=".$foco."&buscar_por=".$buscar_por."&pagina=facturacion_compra_iva_inc_temporal_producto_manual_pos.php";
?>
...
?>
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; <?php echo $pagina_redirect?>">
	<?php } ?>
	<META HTTP-EQUIV="REFRESH" CONTENT="0; <?php echo $pagina_redirect?>">
<?php } ?>
<!-- ***************************************************************************************************************************** -->
