...
//----------------------------------------------------------------------------------------------------------------------------------//
    ...
    $total_saldo_recarga                          = $info_administrador['total_saldo_recarga'];
//----------------------------------------------------------------------------------------------------------------------------------//
    $enterbr                                      = '%0A';
    $negrita_abre                                 = "%0A*";
    $negrita_cierre                               = "*%0A";
    ...
    $vengo_de                                     = "Hola la compra ha sido cargada correctamente.";
    ...
    $subtotal_cliente                             = 'Subtotal'.'%3A '.number_format($total_venta, 0, ",", ".");
    $entrega_cliente                              = 'Entrega'.'%3A '.'0';
    $total_cliente                                = $negrita_abre.'Total'.'%3A '.number_format($total_venta, 0, ",", ".").$negrita_cierre;
    ...
    $mensaje_final                                = $enterbr."🤚 Gracias Por Preferirinos.";

    $mensaje_whatsapp = $vengo_de.''.$enterbr.$tipo_solicitud_cliente.$enterbr.$fecha_hora_registro.$enterbr.$nombre_cliente.$enterbr.$telefono_cliente.$enterbr.$productos_cliente.$forma_pago_cliente.$enterbr.$total_saldo_recarga_cliente.$enterbr.$mensaje_final.$factura_cliente.$enterbr.$id_cod_info_factura_venta;
	$url_redir = "https://api.whatsapp.com/send?phone=57$telefono1_tercero&text=".$mensaje_whatsapp;
	header("Location: $url_redir");
} 
elseif ($accion == 'redirecionar_telefono' && $origen == 'carrito') {

	$url_redir = "tel:57$telefono1_tercero";
	header("Location: $url_redir");
}
//----------------------------------------------------------------------------------------------------------------------------------//
//----------------------------------------------------------------------------------------------------------------------------------//
