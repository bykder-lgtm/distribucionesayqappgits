...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formulario_de_actualizacion")) {
	...
	$tipo_soporte                                                 = "ABONO_CUENTA_PAGAR";
	$nombre_nota_observacion                                      = 'SOPORTES ABONO CUENTA PAGAR | '.$mensaje;
	$cod_tipo_nota_observacion                                    = 13;
	$comentario                                                   = $mensaje;
/* ----------------------------------------------------------------------------------------------------------/ */
...
/* ----------------------------------------------------------------------------------------------------------/ */
	...
	$fecha_seg                                                = strtotime($fecha_pago.' '.date("H:i:s"));
	$hora                                                     = date("H:i:s");
...
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($url_img1 <> '') { 
		...
		$nombre_normal2                                     = $tipo_soporte.'_'.$fecha_ymdHis.'_'.$cod_movimiento_contable.'_'.$cod_cuentas_pagar_abonos.'_'.$cod_tercero.'_'.$cod_administrador.'.'.$formato_img2;
	...
	anyo, fecha_invert, fecha_seg, hora, mensaje, cod_administrador, cod_tipo_forma_pago, cod_cuentas_pagar, cod_puc, url_img_orig_producto, url_img_min_producto) 
	...
	if ($subtotal <= '0') { $cod_estado_pago = 1; } else { $cod_estado_pago = 0; }

	$actualizar_sql1 = sprintf("UPDATE tbl15_cuentas_pagar SET abonado = '$abonado_total', subtotal = '$subtotal', cod_estado_pago = '$cod_estado_pago' WHERE cod_cuentas_pagar = '$cod_cuentas_pagar'");
	...
	$nombre_tipo_puc                                       = 'EGRESOS';

	if ($nombre_tipo_puc == 'INGRESOS') { $total_saldo_final = $total_saldo + $abonado; } else { $total_saldo_final = $total_saldo - $abonado; }
...
/* ----------------------------------------------------------------------------------------------------------/ */
	...
	url_img_orig_producto, url_img_min_producto, cod_cuentas_pagar_abonos) 
...
/* ----------------------------------------------------------------------------------------------------------/ */
	...
	WHERE (codigo_parametrizacion_puc_cuentas_estaticas = '14')";
	...
	$cod_puc                                               = $info_parametrizacion_puc_cuentas_estaticas['cod_puc'];//1011
	$codigo_puc                                            = $info_parametrizacion_puc_cuentas_estaticas['codigo_puc'];//2710
	$nombre_puc                                            = $info_parametrizacion_puc_cuentas_estaticas['nombre_puc'];//ABONOS DIFERIDOS
...
/* ----------------------------------------------------------------------------------------------------------/ */
		$doc_modifica                                      = "";
		$descripcion_movimiento                            = "Abono cuenta por pagar: ".$nombres_clientes.' | '.$fecha_anyo.' | '.$comentario.' | FACTURA: '.$cod_factura.' | ID: '.$cod_cuentas_pagar;
...
/* ----------------------------------------------------------------------------------------------------------/ */
		...
		cod_tercero, nombres_clientes, nit_cliente, fecha_anyo, fecha_ymd, fecha_mes, anyo, fecha_seg, fecha_factura, cod_tipo_forma_pago, ip, cuenta, cod_guia, cod_tipo_nota_observacion)
...
//---------------------------------------------------------------------------------------------------------------------------------//
	if ($cod_estado_movimiento_contable_cuenta_personal_global == '1') {
		...
		$costo_movimiento_contable                             = $abonado;
		$total_costo_movimiento_contable                       = $abonado;
		$comentario                                            = "ABONO CUENTA POR PAGAR | "."FACTURA: ".$cod_factura." | PROVEEDOR: ".$nombres_clientes." | ".$mensaje;
		...
		cuenta, comentario, cod_tercero, cod_tipo_forma_pago, cod_cuentas_pagar_abonos, simbolo_tipo_operacion, total_saldo, cod_tipo_pago)
		VALUES ('$cod_movimiento_contable_cuenta_personal', '$cod_puc',  '$nombre_tipo_movimiento', '$nombre_tipo_documento', '$codigo_puc', 
		'$nombre_puc', '$tipo_puc', '$und_vendida', '$costo_movimiento_contable', '$total_costo_movimiento_contable', '$fecha_anyo', '$fecha_mes', '$fecha_seg', '$fecha_ymd', '$anyo', '$ip', 
		'$cuenta', '$comentario', '$cod_tercero', '$cod_tipo_forma_pago', '$cod_cuentas_pagar_abonos', '$simbolo_tipo_operacion', '$total_saldo_movimiento_contable_cuenta_personal', '$cod_tipo_pago')";
		$resultado_mov_credito = mysqli_query($conectar, $agreg_mov_credito_reg) or die(mysqli_error($conectar));
	}
/* ----------------------------------------------------------------------------------------------------------/ */
	if ($url_img1 <> '') { copy($_FILES['url_img1']['tmp_name'], $url_img_orig_producto); }
?>
<META HTTP-EQUIV="REFRESH" CONTENT="0; ../admin/cuentas_pagar_opcion_imprimir.php?cod_cuentas_pagar_abonos=<?php echo $cod_cuentas_pagar_abonos ?>&cod_cuentas_pagar=<?php echo $cod_cuentas_pagar ?>&cod_factura=<?php echo $cod_factura_post ?>&cod_tercero=<?php echo $cod_tercero ?>&cliente=<?php echo $cliente ?>">
<?php } ?>