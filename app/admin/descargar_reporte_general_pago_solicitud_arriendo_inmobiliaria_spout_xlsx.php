...
if (isset($_GET['fecha_pago_reg_ini'])) { 
	...
	$fecha                         = date("Y_m_d");
	$hora                          = date("H_i_s");
	$nombre_archivo                = "REPORTE_GENERAL_PAGO_SOLICITUD_ARRIENDO_".$fecha_pago_reg_ini.'_A_'.$fecha_pago_reg_fin;
	$cabecera_emp                  = "REPORTE_GENERAL_PAGO_SOLICITUD_ARRIENDO";
...
//-----------------------------------------------------------------------------------------------------//
// Headers
	$writer->addRow(array('DOCUMENTO', 'ARRENDATARIO', 'VALOR RECIBIDO', 'FECHA', 'COD'));
// Then a foreach
    ...
    while ($datos_solicitud_arreiendo = mysqli_fetch_assoc($consulta_solicitud_arreiendo)) {
		...
		$writer->addRow(array($identificacion_tercero, $nombre1_tercero, $valor_solicitud_arriendo, $fecha_solicitud_arriendo, $cod_solicitud_arriendo));
	}
	$writer->addRow(array('','TOTAL' , $total_ingreso_por_solicitud_arriendo_inquilino));
